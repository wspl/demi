...
| A single conversation's context menu copies its ID to the clipboard and confirms with a toast; clipboard failures show an error | SidebarSelectionMenu and shared `copyConversationId`, also used by AgentTabBar | Sidebar live specimen and `web` AppSidebar |
| Assistant copy, fork and timestamp footers appear only on final text before the next visible user message, or on the idle transcript's final text; intermediate updates have no footer | `assistantFooterIds` through AgentMessageList and AssistantTextBlock | Session Blocks includes intermediate and final replies; `web` and child panels use AgentMessageList |
...
| Subagent roster and inspect (live count with a breathing accent dot; no chip when all children finish) | AgentsChip, SubagentPanel over SessionOverlay, SubagentHistoryMenu | Session Windows view and dock chip; `web` ChatPage over conversation.subagents |
...
| Session and list load: first opening uses the loading pane until history arrives; model discovery and connection establishment keep restored history readable; cached navigation reuses state and connection without reloading; an open session's dropped connection shows Connecting in the activity slot. Failed restore never reads as an empty conversation or first-run list. Retry reloads: loading, then ready | SessionStatus, `ConversationCache`, ActivitySlot, `SessionLoad` / `ListLoad`, AgentMessageList, AppSidebar `listStatus` | Session States view (history ready · models loading, cached switching uses ConversationCache and AgentMessageList; failed and missing are live; reconnecting pins ActivitySlot); Sidebar States loading / failed · live / first run, timed by the gallery's `RestoreSweep` fixture; `web` ChatPage and AppSidebar |
| Chrome row entrance: a thinking, tool or abort row that joins a live transcript, and the activity slot when it appears, slide in from the left while fading in; history and scroll remounts stay still. Blocks delivered during history loading, including the batch that marks it ready, form the settled baseline | `useChromeEntrance` observes AgentMessageList's load state and conversation ID; `chrome-enter` on AgentMessageVirtualBlock and ActivitySlot | Motion page Entrance specimen; Session States restores history and appends live blocks; Session Turns and Session views live; `web` ChatPage supplies conversation load state |
