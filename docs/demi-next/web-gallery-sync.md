...
| Session and list load: history arriving, reconnect as the Connecting tail row (same as Requesting), failed restore; never an empty conversation or first-run list while still loading. Retry sweeps loading then ready | SessionStatus, `RestoreSweep`, `LoadingBlock`, `SessionLoad` / `ListLoad`, AgentMessageList, AppSidebar `listStatus` | Session States view (failed and missing are live; reconnecting pins `LoadingBlock`); Sidebar States loading / failed · live / first run; `web` ChatPage and AppSidebar |
