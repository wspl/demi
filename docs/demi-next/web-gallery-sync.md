...
| Composer attachment upload: determinate donut from 0 to 1, then ready; a failed tile offers Retry | AttachmentTile, `ComposerAttachment` phase / progress / destination, SessionComposer send block | Session Composer specimens pin ready and a mid-upload ring; gallery uses timed fixtures; `web` uses actual byte-transfer progress and retry handlers |
| Session and list load: opening a conversation uses the loading pane through history and initial connection, including cached history; an open session's dropped connection uses the Connecting tail row (same as Requesting). Failed restore never reads as an empty conversation or first-run list. Retry sweeps loading then ready | SessionStatus, `RestoreSweep`, `LoadingBlock`, `SessionLoad` / `ListLoad`, AgentMessageList, AppSidebar `listStatus` | Session States view (cached opening uses AgentMessageList; failed and missing are live; reconnecting pins `LoadingBlock`); Sidebar States loading / failed · live / first run; `web` ChatPage and AppSidebar |
