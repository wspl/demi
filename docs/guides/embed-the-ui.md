...
```ts
import { AgentClient, AgentServer, createStdioClientTransport } from '@demi/agent'
...
[docs/tool-rendering-spec.md](../tool-rendering-spec.md). `@demi/web-ui` ships
...
- `@demi/repl` — a terminal renderer over the in-process/stdio client.
- `@demi/web-ui` — a Vue UI that consumes an **injected** `AgentClient`, so the same
