...
A `Host` is the sandbox boundary for `@demicodes/shell`. The bash engine never touches
...
`@demicodes/host-local` is the Node reference implementation; this guide describes the
...
`decodeUtf8` from `@demicodes/utils`.
...
- Reuse `@demicodes/utils` (`normalizePath`, `isAbsolutePath`, byte helpers) rather than
