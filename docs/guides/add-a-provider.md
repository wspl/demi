...
`@demi/provider`. The runtime never imports your SDK directly — it only sees the
contract — so a provider is the single place that knows about one backend.

## The contract

```ts
import { defineProvider, type AgentProvider, type InferenceRequest, type ProviderRun } from '@demi/provider'
```
...
Don't re-derive what `@demi/provider` already ships — the boundary test forbids
...
- `zeroUsage` (from `@demi/core`) — a zeroed `TokenUsage`.
- `normalizeBaseUrl`, `parseJsonObject`, `numberOrZero` (from `@demi/utils`).
