...
static void fail(int status) { fprintf(stderr, "%s\n", uv_strerror(status)); exit(1); }
static void check(int status) { if (status < 0) fail(status); }
static void read_input(void);
static void sent(uv_write_t *req, int status) { (void)req; check(status); read_input(); }
static void shut(uv_shutdown_t *req, int status) { (void)req; check(status); }
static void input_read(uv_fs_t *req) {
    ssize_t n = req->result;
    uv_fs_req_cleanup(req);
    if (n < 0) fail((int)n);
    if (!n) { check(uv_shutdown(&shutdown_req, (uv_stream_t *)&peer, shut)); return; }
...
static void received(uv_stream_t *stream, ssize_t n, const uv_buf_t *buf) {
    (void)stream;
    if (n == UV_EOF) { free(buf->base); exit(0); }
    if (n < 0) fail((int)n);
    size_t offset = 0;
    while (offset < (size_t)n) {
        uv_fs_t req;
        uv_buf_t out = uv_buf_init(buf->base + offset, (unsigned)((size_t)n - offset));
        int written = uv_fs_write(&loop, &req, 1, &out, 1, -1, NULL);
        uv_fs_req_cleanup(&req);
        if (written <= 0) fail(written ? written : UV_EIO);
...
int main(int argc, char **argv) {
    if (argc != 2) { fprintf(stderr, "Usage: ipc-client endpoint\n"); return 2; }
