...
bun packages/runner/runtime/build.ts "$source/tjs.mjs" "$out/tjs-linux-arm64" aarch64-linux-musl
...
cc -Os -flto=thin -Ivendor/txiki.js/deps/libuv/include -c "$source/uv.c" -o "$out/uv.o"
cc -Os -flto=thin -Wl,-dead_strip "$out/uv.o" .cache/txiki/host/deps/libuv/libuv.a -o "$out/uv-macos-arm64"
...
"$ZIG" cc -target aarch64-linux-musl -Os -flto=thin -s -Wl,--gc-sections "$source/posix.c" -o "$out/posix-linux-arm64"
"$ZIG" cc -target aarch64-linux-musl -Os -flto=thin -s -Wl,--gc-sections -Ivendor/txiki.js/deps/libuv/include "$source/uv.c" .cache/txiki/aarch64-linux-musl/deps/libuv/libuv.a -o "$out/uv-linux-arm64"
...
TOOLCHAIN
"$CMAKE" -S "$source" -B "$out/windows" -DCMAKE_TOOLCHAIN_FILE="$out/windows.cmake" -DCMAKE_BUILD_TYPE=MinSizeRel
"$CMAKE" --build "$out/windows" --target uv_a -j 8
# Direct linking avoids Zig 0.16/CMake's whole-archive Windows CRT link failure.
"$ZIG" cc -target x86_64-windows-gnu -Os -s -Ivendor/txiki.js/deps/libuv/include "$source/uv.c" "$out/windows/libuv/libuv.a" -lpsapi -luser32 -ladvapi32 -liphlpapi -luserenv -lws2_32 -ldbghelp -lole32 -lshell32 -o "$out/uv-windows-x64.exe"
python3 "$source/verify.py" "$out/posix-macos-arm64" "$out/uv-macos-arm64" "$out/tjs-macos-arm64"
