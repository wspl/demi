...
| Date | 2026-06-19 |
| Status | Passed |
| Scope | Real REPL + real Claude Code provider + abort/resume/auto compact/manual compact interleaving |
...
| Acceptance target | Abort a long tool turn, resume from the transcript, compact, and keep using tools without provider replay or REPL rendering corruption |

## Scenario Design

Run the REPL in a fresh temporary workspace and send this fuzzy long-task prompt:
...
- `/abort` returned to idle and emitted exactly one `state> turn aborted` line in the final passing log.
- `/resume` after abort started a new running turn and did not produce `API Error`, `thinking.signature`, or `error> agent`.
...
- Usage-based auto compact occurred after the long resumed turn, then the REPL returned to running and the model successfully called `shell_exec`.
- Manual `/compact` returned `state> compacting` then `state> idle`.
- A follow-up after manual compact used `shell_exec` and reported 12 checkpoint files plus the summary first line.
- The final passing REPL process closed normally with `/exit`.
...
- REPL transcript patches do not repeatedly re-render the same abort or error block.
...
- Compact leaves the REPL idle but later tool calls fail.
- REPL repeatedly prints the same abort/error block from transcript patches.
...
- Date: 2026-06-19
- Workspace: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-compact-abort-resume-7r1ixv`
- Log path: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-compact-abort-resume-7r1ixv/repl-compact-abort-resume.log`
...
- Date: 2026-06-19
- Workspace: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-compact-abort-resume-9840pS`
- Log path: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-compact-abort-resume-9840pS/repl-compact-abort-resume.log`
- Prompt: checkpoint recovery prompt above.
...
- UI failure: the REPL repeatedly rendered the same `state> turn aborted` block from transcript patch replay.
- Product gap found: REPL renderer de-duped streamed text/tool/usage deltas but not stable `error` and `abort` blocks.
- Fix added: renderer now tracks seen abort/error block ids and prints each only once.
- Regression test added: `renderer.test.ts` now replays the same abort/error blocks and asserts the patch delta does not contain duplicate `state> turn aborted` or `error> agent`.

### Run 3

- Date: 2026-06-19
- Workspace: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-compact-abort-resume-9A65iN`
- Log path: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-compact-abort-resume-9A65iN/repl-compact-abort-resume.log`
...
- `/abort`: line 82 `abort requested`, line 84 `state> turn aborted`, line 85 `state> idle`.
- `/resume`: line 88 `state> running`.
- Auto compact: line 882 `usage> in=300 out=11431 cache_read=271431 cache_write=20537`, line 883 `state> compacting`, line 884 `state> running`.
- Post-auto-compact tool use: lines 901/928 and 936/981 show `shell_exec` calls executing and completing after auto compact.
- Manual compact: line 1032 `state> compacting`, line 1033 `state> idle`.
...
- Final status: line 1059 `state> idle`, line 1061 `closed`.
...
- `error> agent`: absent.
- `state> turn aborted`: exactly one occurrence.
...
- Transcript patch replay can resend stable abort/error blocks. The REPL must render these terminal blocks by block id rather than printing them on every patch.

Both bugs were only visible when a real REPL session interleaved abort, resume, provider replay, compact, and post-compact tool calls.
...
- `packages/repl/src/__tests__/renderer.test.ts`: repeated transcript patches for the same abort/error block do not duplicate user-visible output.
