...
- Cache fields are preserved through REPL rendering.
...
- Cache usage fields disappear from REPL output.
...
- Date: 2026-06-19
- Workspace: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-context-cache-myL1kk`
- Log path: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-context-cache-myL1kk/repl-context-cache-stability-marathon.log`
- Prompt sequence:
...
The original short-turn assumption was too weak as a provider-cache oracle: three stable, one-tool user turns reported `cache_read=0 cache_write=0`. That is still useful evidence because the REPL preserved the provider-reported fields instead of inventing cache state.

The meaningful real cache signal appeared when the same Claude Code turn performed multiple tool continuations. That path matches the long-task behavior we care about: the provider keeps a stable request prefix across tool result continuations, the REPL preserves cache usage fields, and after manual compact the next multi-tool turn again reports non-zero cache usage.
...
- Existing REPL renderer tests cover `usage>` line rendering.
- Existing agent context-cache tests cover stable provider request shape and compacted replay. This real REPL record covers the provider-dependent cache-hit signal that deterministic tests cannot prove.
