...
- REPL shows real tool output and returns to idle.
...
- Date: 2026-06-19
- Workspace: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-real-project-gbawof`
- Log path: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-repl-real-project-gbawof/repl-real-project-build.log`
...
- REPL verification: `npm run build` exited 0 inside the REPL run; `node test-store.js` exited 0 with 9 assertions passed.
...
- REPL usage: first response `in=10190 out=23668 cache_read=595002 cache_write=68498`, then auto compact, then resume response `in=10 out=628 cache_read=0 cache_write=15036`.
- Phase evidence: `state> running`, `state> compacting`, resumed `state> running`, final `state> idle`.
- Verdict: Passed.
...
- Independent rerun with Homebrew `npm` exited 137 in this workspace, while the REPL's Node/npm runtime and direct `vue-tsc`/`vite build` commands passed. The verdict uses the REPL runtime and direct build tools as authoritative for this run.

## Follow-Up Deterministic Tests

Failures should become `coding-agent` scenario tests, shell foreground-control tests, or REPL process tests depending on the broken layer.
