...
`AgentProvider.clone()` returns an independently disposable runtime with the
...
- `packages/agent/src/__tests__/session.test.ts` covers snapshot isolation,
  provider runtime ownership, and disposal.
