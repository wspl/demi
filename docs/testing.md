...
| Compaction 可支撑长任务 | 有效 / Gated；真实 TUI 长上下文未通过 | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore、context overflow 裁剪重试、action 交错有效；真实 provider 下 medium thinking + budget 组合已有 gated 验证。但 2026-06-19 的 Haiku 4.5 TUI 长上下文验收只观察到第一次 compact，compact 后真实模型认为 shell tool 不可用，未达到 3 次 post-compact continuation；记录见 `docs/tui-compact-haiku-acceptance.md`。 |
...
| 真实 TUI 长上下文 compact 连续工作 | Gated；当前未通过 | 2026-06-19 手动 harness 使用真实 TUI、`claude-haiku-4-5`、200k context selection、`--no-thinking` 和本地大 stdout 压力测试。有限 benchmark cycle 中第 2 轮发送前观察到 `status: compacting`，但 compact 后真实模型回复 shell execution tool 不可用，未继续执行后续工具，因此没有达到 3 次 compact continuation。详细记录见 `docs/tui-compact-haiku-acceptance.md`。 | 手动真实 TUI harness，尚未落成默认或 gated 自动测试 | 防止只凭 deterministic compact 测试或 `status: compacting` 判定真实长任务能力可用；该验收能暴露 compact summary / provider replay / MCP tool 可用性在真实模型路径下的组合问题。 |
