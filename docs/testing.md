...
验证：`bun test packages/provider-codex/src --timeout 120000`，27 pass / 4 gated skip。
...
| provider-stream steer 不需要 native provider hook | 有效 | `session.test.ts` 用无 `ProviderRun.steer` 的 gated provider，在 active provider stream 期间提交 queued send 与 steer，断言 steer 写入当前 turn、触发同 turn provider continuation，queued send 保持等待；另用 slow snapshot store 证明 continuation counter 在持久化完成前已生效；`server.test.ts` 断言同一路径经 AgentClient 收到 accepted `steer_result` 和 transcript patch。验证：`bun test packages/agent/src/__tests__/session.test.ts packages/agent/src/__tests__/server.test.ts`，56 pass。 | `session.test.ts`、`server.test.ts` | 防止 active steer 被误拒、被自动转成 queue、accepted ack 早于 transcript 但没有 same-turn provider continuation，或慢持久化导致 continuation race。 |
