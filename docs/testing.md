...
| Compaction 可支撑长任务 | 有效 / Gated | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore、context overflow 裁剪重试、action 交错有效；真实 provider 下 medium thinking + budget 组合已有 gated 验证。Claude Code CLI 当前只支持 thinking effort 和 cost budget，不支持 per-request max output token flag；这不是当前默认测试缺口。 |
