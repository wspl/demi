...
| `ProviderRegistry` model catalog dispatch | 有效 | `registry.test.ts` 注册带 `listModels` 的 definition，断言 registry 会把 config 原样交给 provider catalog 并返回 full model id；同时缺 catalog 或缺 provider 会抛错。验证：`bun test packages/provider-claude-code/src/__tests__/models.test.ts packages/provider-codex/src/__tests__/models.test.ts packages/repl/src/__tests__/renderer.test.ts packages/repl/src/__tests__/process.test.ts`，26 pass / 1 skip。 | `packages/provider/src/__tests__/registry.test.ts` | 防止模型列表被绕到 REPL 硬编码或错误 provider 上，导致 UI 展示的模型和实际能力不一致。 |
