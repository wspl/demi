...
| 范围 | demi agent、shell、provider、AgentServer/transport、coding agent、TUI |
...
- 核心链路是 AgentSession 状态机、Transcript replay、Provider 可见上下文、Tool call/result、Shell 控制面、Compaction、Context cache/usage、AgentClient/TUI 事件呈现。
...
| 壳子路径能呈现真实模型行为 | TUI/AgentClient 能显示真实 text/thinking/tool output，真实 Claude Code provider 确实产出模型回复 | transport tests，TUI 自动化或 gated smoke |
...
| Compaction 可支撑长任务 | 有效 / Gated | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore、context overflow 裁剪重试、action 交错有效；真实 provider 下 medium thinking + budget 组合已有 gated 验证。2026-06-19 的 Haiku 4.5 TUI 长上下文验收最初发现 post-compact shell tool 不可用、重复 tool loop、MCP request id 复用污染 tool result 配对；对应修复已由 provider/agent/shell 测试锁住。真实 TUI 已分别复测 preflight/tool-heavy compact continuation 和 provider-usage auto compact：前者观察到 4 次 compact、至少 3 次 post-compact shell tool continuation 和 1 次 no-tool continuation；后者在同一会话中观察到 3 次 usage-based auto compact。Compact + Abort + Resume 真实验收又覆盖 abort 后 resume、usage-based auto compact、manual compact、post-compact shell follow-up 的同会话交错，并暴露 unsigned thinking replay 与 abort/error 重复渲染问题；记录见 `docs/tui-compact-haiku-acceptance.md` 和 `docs/tui-acceptance/compact-abort-resume.md`。 |
| Context cache baseline | 有效 / Gated | `5.3`、`5.7`、`5.16`、`5.17` 覆盖 usage 字段、stable prefix、cache usage 作为 context pressure 触发 auto compact、cache 字段不进入模型上下文、AgentClient transcript usage 传播和 TUI usage 呈现；真实 provider cache hit 已有 gated 验证，真实 TUI auto compact 已验证 cache-backed usage 超过阈值后触发 compact 并恢复。`docs/tui-acceptance/context-cache-stability-marathon.md` 又验证真实 TUI 中短 one-tool turns 可能报告 zero cache，但 multi-tool provider continuation 在 manual compact 前后都能报告非零 cache usage。 |
| Shell 控制面支撑真实长命令 | 有效 / Gated | `5.10` 和 `5.15` 覆盖 wait/input/abort、busy-default auxiliary shell、raw stdin/newline、hard-stop timeout 和 agent prompt guard；真实 TUI/model fuzzy shell-control 记录见 `docs/tui-acceptance/interactive-shell-control.md`，默认测试负责稳定契约，gated smoke 负责真实模型策略漂移。`docs/tui-acceptance/runaway-tool-loop-guard.md` 验证 repeated command guard 在真实 provider 下可见、可收敛，且后续用户 turn 不会被旧 control_request 吞掉。 |
| Coding workflow 能发现真实问题 | 有效 | `5.12` 到 `5.15` 覆盖真实文件、todo、测试失败到修复、长命令控制、shell cwd/env 复用、file reference Host 边界和 editor 写入失败事务。 |
| 壳子路径能呈现真实模型行为 | Gated | `5.16` AgentServer/transport 层有效，`5.17` TUI renderer/command/process 层已有 deterministic 测试；真实 Claude Code provider 回复、thinking、tool output 显示已有 gated smoke 入口。Compact + Abort + Resume 验收确认真实 TUI 在 abort/resume/compact 后仍能呈现模型输出和 shell 工具结果，并补上 abort/error patch 去重测试；Runaway Tool Loop Guard 又补上 errored tool output 的真实 TUI 可见性和去重测试。 |

### 3.2.1 长程 TUI Gated 验收设计

长程真实 TUI 验收的设计清单和每个测试项的过程记录模板见 `docs/tui-long-run-acceptance.md`。这些记录用于补充默认 deterministic 测试，重点发现真实模型、真实 provider、长上下文、shell 控制和 TUI/AgentClient action 收敛中的问题。
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| `ProviderRegistry` 注册、查找、创建 provider | 有效 | `registry.test.ts` 注册带 `state` 和 `createProvider` 的 definition，断言 `get/state/createProvider` 返回同一 definition 并实际运行 provider 输出配置值；能发现 type lookup、config 传递或 provider 创建错位。验证：`bun test packages/core/src/__tests__/platform-entrypoints.test.ts packages/provider/src/__tests__/registry.test.ts packages/provider/src/__tests__/stub.test.ts`，15 pass。 | `packages/provider/src/__tests__/registry.test.ts` | 防止 provider id/type 解析错位，导致 AgentServer/transport 打开的 provider 不是用户配置的 provider。 |
...
| `ProviderRegistry` model catalog dispatch | 有效 | `registry.test.ts` 注册带 `listModels` 的 definition，断言 registry 会把 config 原样交给 provider catalog 并返回 full model id；同时缺 catalog 或缺 provider 会抛错。验证：`bun test packages/provider-claude-code/src/__tests__/models.test.ts packages/provider-codex/src/__tests__/models.test.ts packages/tui/src/__tests__/renderer.test.ts packages/tui/src/__tests__/process.test.ts`，22 pass / 1 skip。 | `packages/provider/src/__tests__/registry.test.ts` | 防止模型列表被绕到 TUI 硬编码或错误 provider 上，导致 UI 展示的模型和实际 provider 能力不一致。 |
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| Claude CLI path/version 检测 | 有效 | `cli.test.ts` 注入 shell runner，断言 `which claude` 后使用解析出的 `/bin/claude --version`，并返回修剪后的 path/version；能发现未按实际 CLI path 查询版本或 version 解析异常。验证：5.3 targeted command，32 pass / 3 skip。 | `packages/provider-claude-code/src/__tests__/cli.test.ts` | 防止 provider 在没有可用 CLI 或版本信息异常时进入半可用状态。 |
...
| Claude Code 模型目录来自 `models.dev` | 有效 / Gated | `models.test.ts` 用 `models.dev` fixture 覆盖 Claude full id 解析、`minimumModelVersion = "4.6"` 过滤、unknown 5.x family 保留、缺失 metadata 映射为 `null`、ETag/Last-Modified 缓存、stale fallback 和无硬编码 fallback；真实 smoke 已用 `listClaudeCodeModels()` 请求 `https://models.dev/api.json`，本机返回 `claude-opus-4-7`、`claude-opus-4-8`、`claude-fable-5`、`claude-opus-4-6`、`claude-sonnet-4-6`。验证：同模型目录 targeted command，22 pass / 1 skip；真实 smoke 为手动命令。 | `packages/provider-claude-code/src/__tests__/models.test.ts`、`docs/provider-model-catalog-design.md` | 防止仓库维护过期 Claude 模型表、别名映射或按名字臆测能力；也防止网络失败时伪造可选模型。 |
...
| Codex 模型目录来自官方 backend | 有效 / Gated | `models.test.ts` 覆盖 `auth.json` ChatGPT/PAT/agentIdentity header、`client_version` query、401 force refresh 后重试、API key auth 不请求 ChatGPT catalog、backend `slug` 映射为 full model id、capability metadata 映射和 stale cache；真实 smoke 已用本机 `~/.codex/auth.json` 请求 `https://chatgpt.com/backend-api/codex/models`，本机返回 `gpt-5.5`、`gpt-5.4`、`gpt-5.4-mini`、`gpt-5.3-codex-spark`、`codex-auto-review`。验证：同模型目录 targeted command，22 pass / 1 skip；真实 smoke 为手动命令。 | `packages/provider-codex/src/__tests__/models.test.ts`、`docs/provider-model-catalog-design.md` | 防止 Codex provider 自建鉴权、用错模型 id 字段、把 ChatGPT catalog 套到 API key auth，或在 backend 不可用时返回仓库硬编码模型。 |
...
| thinking 模型下 summary request 的 token/budget 设置有效 | 有效 / Gated | preflight 测试断言 thinking 模型的 summary request 携带 `{ type: 'effort', effort: 'medium' }`；Claude Code CLI contract 测试断言该 request thinking effort 和 provider max budget 会映射到 `--effort medium`、`--max-budget-usd`；真实 provider gated smoke 已验证 `claude-opus-4-8` medium thinking + budget summary-shaped request 能返回 thinking 信号、text 和 usage。本机 `claude --help` 显示当前 CLI 没有 per-request max output token flag，因此输出 token hard cap 不作为当前 Claude Code provider 测试缺口。 | `compaction.test.ts`、`cli.test.ts`、`real-cli.e2e.test.ts` gated smoke | 防止 summary 请求因为 thinking effort 或 provider budget 丢失而使用错误模型配置；真实 smoke 能发现 CLI 参数组合或 thinking event 映射在本机真实路径退化。 |
...
| cache usage 被 AgentSession 记录并对外暴露 | 有效 | `context-cache.test.ts` 断言 response blocks 记录 cacheRead/cacheWrite tokens，`server.test.ts` 断言同一 usage 经 transcript patch frame 到达 `AgentClient.transcript()`；`packages/tui/src/__tests__/renderer.test.ts` 断言 TUI stdout 显示 `cache_read` 和 `cache_write` 字段，`process.test.ts` 断言 TUI 会话进程路径显示同一 usage 字段。验证：5.7 targeted command 和 `bun test packages/tui/src`，TUI 8 pass。 | `context-cache.test.ts`、`server.test.ts`、`renderer.test.ts`、`process.test.ts` | 需要发现 usage 在 provider 到 session 到 UI/AgentClient/TUI 呈现链路中丢字段。 |
| cache usage 作为 context pressure 参与 auto compact，但不进入模型上下文 | 有效 / Gated | `auto compaction counts cache usage as context pressure` 证明 `cacheWriteTokens` 足以触发 usage-based auto compact，并断言 summary request 与 post-compact resume request 形状；`cache usage is recorded without leaking into model context or breaking tool loop` 证明 cache usage 被记录、tool loop 可继续、没有可压缩窗口时不会凭空写 boundary。真实 TUI Haiku 4.5 验收在同一会话中 3 次观察到 cache-backed usage 超过 160k 后进入 `status: compacting`，resume 后 usage 回到约 16k。验证同上，真实记录见 `docs/tui-compact-haiku-acceptance.md`。 | `compaction.test.ts`、`context-cache.test.ts`、真实 TUI gated smoke | 防止真实 provider 把大上下文记到 cache read/write 后 agent 不 compact；同时防止 cache 字段作为内部 usage 元数据泄漏进模型可见上下文或破坏 tool loop。 |
...
| shell output、bash audit、generic tool progress frame 映射 | 有效 | 测试 shell tool progress 映射为 shell_output/audit，任意 progress 包括 undefined、BigInt、cyclic object 映射成 text output，并过滤非良构 audit；能发现 TUI/GUI 实时事件缺失或格式破坏。验证同上。 | `server.test.ts` | 防止 TUI/GUI 看不到实时 shell 输出、审计事件或工具进度。 |
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| 基本渲染、输入、scroll | 有效 / Gated | `renderer.test.ts` 通过注入 output 断言 TUI renderer 会输出 phase、queue、shell stdout/stderr、audit、progress、error、rejected、closed；断言 `/help`、`/abort`、`/retry`、`/resume`、`/compact`、`/input`、unknown、`/exit` 的命令分发和 usage 输出；`runInputLoop` 测试断言普通消息异步发送时仍能继续处理 `/abort`，并断言 send 异步失败会打印 `send failed`；`process.test.ts` 真实启动 TUI `--help` 进程并断言不会打开 provider session，还通过 `--claude-path` 指向 Claude Code 协议 fixture 启动真实 TUI 会话进程，经 stdin prompt 触发 ClaudeCodeProvider、shell tool、AgentServer/transport、renderer，再断言 tool、shell stdout、thinking、assistant text、usage、`/input` 写入 running shell 后的 stdout 和 `/exit` close；high-volume shell output 进程测试断言超过常见 pipe buffer 的 stdout 首尾 sentinel、末尾行、assistant text 和 usage 都能出现；`DEMI_TUI_TTY_E2E=1` gated 用例通过 BSD `script` pseudo-terminal 跑 fixture session，断言 ANSI TTY 输出、tool、shell stdout、thinking、assistant text、usage 和 close。当前 TUI 没有独立 scroll 产品面；若未来引入独立滚屏控件，再按第 6 章补交互滚屏测试。验证：`bun test packages/tui/src`，10 pass / 3 skip；gated TTY 验证：`DEMI_TUI_TTY_E2E=1 bun test packages/tui/src/__tests__/process.test.ts --timeout 30000`，5 pass。 | `packages/tui/src/__tests__/renderer.test.ts`、`packages/tui/src/__tests__/process.test.ts` | 防止核心 AgentServer/transport 事件到了 TUI 后用户仍看不到状态/输出，或输入循环因为等待 send 结果而卡住 `/abort`、`/input` 等控制命令；同时防止真实 TUI 进程入口、readline pipe、provider/AgentClient/session 组装后才暴露链路断裂；高体量输出测试能发现 shell output 写入和 renderer 分行在较大 stdout 下漏掉尾部输出或卡住退出；TTY gated 测试能发现真实终端模式下 readline、ANSI/color 输出或 prompt 交错破坏会话。 |
| TUI 模型选择来自 provider catalog | 有效 / Gated | `renderer.test.ts` 通过 fake provider catalog 断言无 `--model` 时选择 `defaultModelId`、映射 context window / thinking efforts / warning；另测显式 full id 不做 catalog rewrite，`opus` 等旧 alias 直接拒绝。真实 smoke 分别以 Claude Code 和 Codex provider 启动 TUI 后 `/exit`，banner 显示 catalog 默认模型 `claude-opus-4-7` 和 `gpt-5.5`。验证：同模型目录 targeted command，22 pass / 1 skip；真实 smoke 为手动命令。 | `packages/tui/src/__tests__/renderer.test.ts`、`packages/tui/src/__tests__/process.test.ts` | 防止 TUI 继续持有 provider-specific 默认模型、历史 alias 或手写 capability，导致 compact/context limit、thinking 控制和真实 provider 支持范围不一致。 |
| thinking/text/tool output 显示 | 有效 | `renderer.test.ts` 构造 thinking、assistant text、tool_call、response blocks，断言 stdout 包含 `thinking>`、`assistant>`、`tool:` 和 usage；随后用同 id 更新 block，断言只输出新增 delta、不重复完整 text、不重复 usage；同一测试还覆盖 error/abort block id 去重，防止 transcript patch replay 重复打印 `agent error` 或 `turn aborted`。新增 errored `tool_call` 输出测试断言 repeated-shell guard 的 `tool error>` 内容会显示且不会在 patch replay 中重复。真实模型是否产出这些 blocks 仍由真实 provider/TUI smoke 覆盖。验证同上。 | `renderer.test.ts` | 防止真实模型已经产生 thinking/text/tool/error blocks，但 TUI renderer 分流、重复或漏显示；同时防止稳定 abort/error/tool-error block 在长会话 patch 中反复刷屏。 |
| 通过 AgentClient open/send/receive phase/transcript/shell frames | 有效 | AgentServer/transport 5.16 已覆盖 open/send/phase/transcript/shell frames；TUI 层新增 `renderEvent` stdout snapshot 覆盖 phase、queue、transcript、shell_output、audit、tool_progress、error/rejected/closed 的呈现和去重；并用真实 `AgentClient`、`AgentServer`、in-process transport、StubProvider 断言 `attachRenderer(client, renderer)` 能收到 open/send/phase/transcript/closed 事件并渲染 assistant text 和 usage。验证同上。 | `server.test.ts`、`renderer.test.ts` | 需要发现 TUI 自己订阅、状态合并、刷新节奏的问题。 |
| 真实 Claude Code provider 输出真实模型回复 | Gated | Provider 层有 `DEMI_CLAUDE_CODE_E2E=1` 的 real-cli gated 测试；TUI 层新增 `real-tui.e2e.test.ts`，只有 `DEMI_TUI_REAL_E2E=1` 时运行，会以 full id `--model claude-opus-4-8`、`--thinking medium` 启动真实 TUI，要求模型触发 `shell_exec`，每次 run 断言 tool、tool output、assistant text、usage，默认两次 run 中至少一次断言 `thinking>`。默认测试中它是 1 skip，不能替代真实运行记录。本机验证命令：`DEMI_TUI_REAL_E2E=1 bun test packages/tui/src/__tests__/real-tui.e2e.test.ts --timeout 360000`，1 pass。 | `real-tui.e2e.test.ts` gated smoke | 防止验收只跑协议 fixture 或 stub provider，没有确认真实 provider/模型路径。 |
| 真实 TUI 长上下文 compact 连续工作 | Gated；手动验收通过 | 2026-06-19 手动 harness 使用真实 TUI、`claude-haiku-4-5`、200k context selection、`--no-thinking` 和本地大 stdout 压力测试。最初失败暴露 compact 后真实模型认为 shell tool 不可用；修复 provider replay 后又暴露重复 tool loop 和 MCP request id 复用导致历史 tool block 被改写。当前修复后复测通过：同一会话观察到 `status: compacting` 4 次，前三次 compact 后都继续执行真实 `shell_exec` 并完成压力回合，第 4 次 compact 后完成 no-tool continuation；没有 tool unavailable 或重复调用抑制。详细记录见 `docs/tui-compact-haiku-acceptance.md`。 | 手动真实 TUI harness，尚未落成默认或 gated 自动测试；`provider.test.ts` / `jsonl-output.test.ts` 锁住 provider replay 修复；`provider.test.ts` 覆盖重复 MCP request id；`transcript.test.ts` 覆盖重复 tool id 只完成 pending block；`tools.test.ts` 覆盖 repeated-shell guard 返回非 terminal tool result 和 suppression 后 AgentSession 可继续 | 防止只凭 deterministic compact 测试或 `status: compacting` 判定真实长任务能力可用；该验收能暴露 compact summary / provider replay / MCP tool 可用性 / tool result 配对 / 真实模型 tool-loop 在真实模型路径下的组合问题。 |
| 真实 TUI abort/resume/compact 交错 | Gated；手动验收通过 | 2026-06-19 手动 harness 使用真实 TUI、`claude-haiku-4-5`、`--no-thinking`、checkpoint recovery 模糊任务和真实 Claude Code provider。三轮运行中，第一轮暴露 `/resume` 后 unsigned thinking 被 replay 成 Claude invalid input，第二轮暴露 TUI 重复渲染同一 abort block，第三轮通过：`/abort` 回 idle、`/resume` 完成 12 个 checkpoint、usage-based auto compact 后继续 shell tool、manual `/compact` 后 follow-up shell 验证仍可用。详细记录见 `docs/tui-acceptance/compact-abort-resume.md`。 | `docs/tui-acceptance/compact-abort-resume.md`；`jsonl-output.test.ts` 锁住 unsigned thinking replay；`renderer.test.ts` 锁住 abort/error patch 去重；`session.test.ts` 和 `compaction.test.ts` 覆盖 abort/resume/compact runtime 收敛 | 防止 abort/resume 只在 deterministic session 层通过，真实 Claude Code replay 却因 thinking payload 失败；也防止长程 TUI patch replay 把同一个终止块重复呈现成大量噪声。 |
| 真实 TUI context cache stability | Gated；手动验收通过 | `docs/tui-acceptance/context-cache-stability-marathon.md` 使用真实 TUI 和 `claude-haiku-4-5` 跑同一 workspace 中的稳定追加任务。前三个 one-tool turns 按真实 provider 报告 cache 为 0；随后 multi-tool continuation 在 compact 前报告 `cache_read=4112 cache_write=5124`，manual `/compact` 回 idle，compact 后 multi-tool continuation 报告 `cache_read=10686 cache_write=6449`，最终文件从 `CACHE_STEP_01` 连续到 `CACHE_STEP_15`。 | `docs/tui-acceptance/context-cache-stability-marathon.md`；provider/AgentClient/TUI usage tests；`context-cache.test.ts` | 防止只在 deterministic 层验证 cache 字段存在，却不知道真实 provider 何时上报 cache hit；同时验证 compact 后不会让 provider continuation 永久退化成 cold context。 |
| 真实 TUI runaway tool loop guard | Gated；手动验收通过 | `docs/tui-acceptance/runaway-tool-loop-guard.md` 用真实 TUI 和 `claude-haiku-4-5` 跑 sentinel polling 模糊任务。前两轮分别暴露 errored tool output 未显示、suppression 被错误当成 terminal result 后吞掉后续输入；最终通过：第 7 次 identical `cat sentinel.txt` 被 suppression，TUI 显示 `tool error>` 细节，turn 回 idle，后续 `wc -c sentinel.txt` 直接执行并返回 `8`。 | `docs/tui-acceptance/runaway-tool-loop-guard.md`；`renderer.test.ts` 锁住 `tool error>` 呈现；`tools.test.ts` 锁住 repeated-shell suppression 非 terminal 和 AgentSession continuation | 防止真实模型在等待型任务里无界重复工具，也防止 guard 虽然触发但 operator 看不到原因，或 guard 破坏 provider continuation 导致下一条用户消息被旧循环吞掉。 |
| 交互式 shell 操作在 TUI 中顺畅 | 有效 / Gated | `process.test.ts` 用 Claude Code 协议 fixture 发起一个等待 stdin 的 `shell_exec`，TUI 进程从 stdout 解析 shellId 后发送 `/input <shellId> typed-from-tui`，断言同一 TUI stdout 出现 `fixture-input:typed-from-tui`；证明最终用户命令路径能写入 active shell 并渲染结果。`real-tui.e2e.test.ts` 的 `DEMI_TUI_FUZZY_SHELL_E2E=1` 真实模型 smoke 已覆盖模糊任务下 wait/input/abort 选择，3 次 attempt 通过。默认验证：`bun test packages/tui/src`，10 pass / 3 skip；gated 验证：`DEMI_TUI_FUZZY_SHELL_E2E=1 bun test packages/tui/src/__tests__/real-tui.e2e.test.ts --timeout 600000`，1 pass / 1 skip。 | `packages/tui/src/__tests__/process.test.ts`、`packages/tui/src/__tests__/real-tui.e2e.test.ts` gated smoke | 用来发现 TUI `/input` 虽然命令分发存在，但真实进程/AgentServer/transport/active shell 写入链路断裂；同时发现真实模型虽然能看到工具说明，但仍可能误用控制面的策略问题。 |
...
1. 如果后续引入独立 TUI scroll UI，再补交互滚屏测试。
