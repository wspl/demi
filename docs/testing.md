...
| Compaction 可支撑长任务 | 有效 / Gated；真实 TUI 三轮长上下文仍未通过 | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore、context overflow 裁剪重试、action 交错有效；真实 provider 下 medium thinking + budget 组合已有 gated 验证。2026-06-19 的 Haiku 4.5 TUI 长上下文验收发现 post-compact shell tool 不可用；根因已修在 Claude Code provider replay 层，真实 TUI 已验证 compact 后能再次调用 shell tool。但三次连续真实 Haiku compact continuation 仍未通过，当前失败点是模型重复 tool-call loop 和 abort/history 交错；记录见 `docs/tui-compact-haiku-acceptance.md`。 |
| Context cache baseline | 有效 / Gated | `5.3`、`5.7`、`5.16`、`5.17` 覆盖 usage 字段、stable prefix、cache 不影响 agent 行为、RPC client transcript usage 传播和 TUI usage 呈现；真实 provider cache hit 已有 gated 验证。 |
...
| 真实 TUI 长上下文 compact 连续工作 | Gated；当前部分修复、三轮未通过 | 2026-06-19 手动 harness 使用真实 TUI、`claude-haiku-4-5`、200k context selection、`--no-thinking` 和本地大 stdout 压力测试。有限 benchmark cycle 中第 2 轮发送前观察到 `status: compacting`，最初失败为 compact 后真实模型回复 shell execution tool 不可用；该根因已修在 provider replay 层，真实 TUI 复测已观察到 compact 后 shell tool marker 输出。三次 compact continuation 仍未达到，当前失败点是 Haiku 在 synthetic probe 中重复调用工具，受控 `/abort` 又会污染后续模型理解。详细记录见 `docs/tui-compact-haiku-acceptance.md`。 | 手动真实 TUI harness，尚未落成默认或 gated 自动测试；`provider.test.ts` / `jsonl-output.test.ts` 锁住 provider replay 修复 | 防止只凭 deterministic compact 测试或 `status: compacting` 判定真实长任务能力可用；该验收能暴露 compact summary / provider replay / MCP tool 可用性在真实模型路径下的组合问题，也暴露真实模型 tool-loop 对验收 harness 的影响。 |
