...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| `shell_exec` running/yield 后可用 `shell_status` 继续观察状态 | 待更新 | 现有测试覆盖长命令先返回 running、后续 `status(commandId)` 收敛；最终契约需要改为断言 `shell_status` 只返回 status/runningMs/idleMs/bytes/artifact paths，不返回 stdout/stderr 正文；coding-agent 集成测试应通过标准工具组拿到 `commandId` 并继续控制。 | `environment.test.ts`、`packages/coding-agent/src/__tests__/coding-marathon.test.ts` | 防止长命令超出初始观察窗口后失联，同时防止 `shell_status` 继续承担输出分页职责。 |
...
| shell tool result 格式保留 metadata 且模型可读 | 待更新 | 标准工具 result 文本应包含 status/shellId/commandId/exitCode、stdout/stderr artifact 路径、bytes、自动预算 preview 和 next action，metadata 保留 `ShellCommandSnapshot`；不再包含 stdout/stderr offset 分页协议。 | `packages/agent/src/tools.ts` via provider/coding integration tests | 防止模型必须解析不稳定 JSON 或看不到 commandId/status/next action，同时防止 tool result 继续泄漏大段正文。 |
