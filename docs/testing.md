...
| `ProviderRegistry` model catalog dispatch | 有效 | `registry.test.ts` 注册带 `listModels` 的 definition，断言 registry 会把 config 原样交给 provider catalog 并返回 full model id；同时缺 catalog 或缺 provider 会抛错。验证：`bun test packages/provider-claude-code/src/__tests__/models.test.ts packages/provider-codex/src/__tests__/models.test.ts packages/tui/src/__tests__/renderer.test.ts packages/tui/src/__tests__/process.test.ts`，24 pass / 1 skip。 | `packages/provider/src/__tests__/registry.test.ts` | 防止模型列表被绕到 TUI 硬编码或错误 provider 上，导致 UI 展示的模型和实际 provider 能力不一致。 |
...
| Claude Code 模型目录来自 `models.dev` | 有效 / Gated | `models.test.ts` 用 `models.dev` fixture 覆盖 Claude full id 解析、`minimumModelVersion = "4.6"` 过滤、unknown 5.x family 保留、缺失 metadata 映射为 `null`、ETag/Last-Modified 缓存、stale fallback 和无硬编码 fallback；真实 smoke 已用 `listClaudeCodeModels()` 请求 `https://models.dev/api.json`，本机返回 `claude-opus-4-7`、`claude-opus-4-8`、`claude-fable-5`、`claude-opus-4-6`、`claude-sonnet-4-6`。验证：同模型目录 targeted command，24 pass / 1 skip；真实 smoke 为手动命令。 | `packages/provider-claude-code/src/__tests__/models.test.ts`、`docs/provider-model-catalog-design.md` | 防止仓库维护过期 Claude 模型表、别名映射或按名字臆测能力；也防止网络失败时伪造可选模型。 |
...
| Codex 模型目录来自官方 backend | 有效 / Gated | `models.test.ts` 覆盖 `auth.json` ChatGPT/PAT/agentIdentity header、默认静态 `client_version=0.130.0`、显式 `clientVersion` override、401 force refresh 后重试、API key auth 不请求 ChatGPT catalog、backend `slug` 映射为 full model id、capability metadata 映射和 stale cache；真实 smoke 已用本机 `~/.codex/auth.json` 请求 `https://chatgpt.com/backend-api/codex/models`，本机返回 `gpt-5.5`、`gpt-5.4`、`gpt-5.4-mini`、`gpt-5.3-codex-spark`、`codex-auto-review`。 | `packages/provider-codex/src/__tests__/models.test.ts`、`docs/provider-model-catalog-design.md` | 防止 Codex provider 自建鉴权、用错模型 id 字段、把 ChatGPT catalog 套到 API key auth、运行时依赖 `codex --version`，或在 backend 不可用时返回仓库硬编码模型。 |
