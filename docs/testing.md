...
| Compaction 可支撑长任务 | 部分有效 | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore 多数有效；preflight retry/resume 交错、context overflow 裁剪/重试策略、thinking budget 仍需补测。 |
...
| compact/preflight 期间 queued send、abort、retry、resume 收敛 | P0 | `5.4`、`5.6`：部分有效；preflight abort/queued send 已覆盖 | 防止用户在 summary 阶段继续操作时消息丢失、乱序或 session 卡在 compacting。 |
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| 手动 compact 插入 boundary 和 marker | 有效 | `session.test.ts` 手动 compact 后精确断言 block 序列包含 `compaction_boundary` 和 `compaction_marker`，并检查 collectInferenceItems 的 summary user message；能发现 compact 成功但没有边界标记。验证：5.6 targeted command，58 pass。 | `session.test.ts` | 防止 compact 完成后 transcript 无法标识历史摘要边界。 |
...
| compact/preflight 期间 abort、retry、resume 的 action 收敛 | 部分有效 | 测试覆盖 manual compact summary 卡住时 abort 收敛、preflight summary 卡住时 abort 在普通 model request 前停止且无 boundary/marker、preflight queued send 按序 drain、以及 compact 期间 queued retry/resume 在 summary commit 后运行；仍未覆盖 preflight retry/resume 的完整交错矩阵。验证同上。 | `session.test.ts`、`compaction.test.ts` | 防止停止、重试或继续操作与 summary 写入交错，留下 busy phase、重复 action 或半截 transcript。 |
| compact 过程遇到 context-window 错误与普通 provider 错误分流 | 部分有效 | `compaction.test.ts` 覆盖 summary context overflow code 保留、transcript atomic、后续 send 可恢复；普通 summary error 也 atomic。`context-cache.test.ts` 覆盖 provider context overflow 明确写 error 并可继续发送。未验证自动裁剪/重试策略或普通错误避免无限重试。验证同上。 | `compaction.test.ts`、`context-cache.test.ts` | 防止可通过裁剪/重试恢复的超限错误被当成普通失败，或普通 provider 错误无限重试。 |
...
1. 先补 `5.6 Compaction` 的剩余 P0 部分有效项：preflight summary 卡住时 retry/resume 交错、context overflow 裁剪/重试策略与 fault injection、thinking budget 与 summary 请求约束。
