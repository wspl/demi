...
| 标准工具 schema 都允许 UI `description` | 有效 | `coding-harness.test.ts` 断言五个标准工具的 input schema 都包含可选 `description`；渲染规范见 `docs/tool-rendering-spec.md`。 | `coding-harness.test.ts` | 防止模型按规范传用户可见动作标题时被 schema 拒绝，或只有 `shell_exec` 能显示自然语言说明。 |
...
| 标准工具按具体 toolName 渲染 | 有效 | Web `ToolCallBlock` 显式分发 `shell_exec/shell_status/shell_write/shell_abort/yield`，未知工具才进入 generic；`tool-rendering.test.ts` 锁住标准工具 title/rows/partial parse helper，`renderer.test.ts` 构造五个标准 `tool_call` 断言 REPL 摘要优先使用 `description` 或 per-tool fallback。 | `packages/web-ui/src/agent/__tests__/tool-rendering.test.ts`、`packages/repl/src/__tests__/renderer.test.ts` | 防止 UI 只看到泛化 tool_call，用户无法区分执行、状态检查、stdin 写入、停止和 yield 唤醒。 |
