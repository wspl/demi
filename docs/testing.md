...
| 模型在真实长进程场景中稳定选择 status/write/abort/yield | Gated | `real-repl.e2e.test.ts` gated prompt 要求真实模型 `shell_exec → yield → shell_status` 观察、`shell_write` 注入 stdin、`shell_abort` 停止长命令；历史记录保留在 `docs/repl-acceptance/interactive-shell-control.md`。 | `packages/repl/src/__tests__/real-repl.e2e.test.ts`、`docs/repl-acceptance/interactive-shell-control.md` | 用来发现 prompt/tool result 虽然单测正确，但真实模型仍然误用控制面的问题。 |
...
| 长命令 running/yield -> status/write/abort 的 agent 级流程 | 有效 | 测试启动长 foreground 命令并 yield 为 running，随后通过 `commandId` 执行 status/write/abort 控制；`session.test.ts` 额外覆盖 yield terminal result、idle wakeup、active steer 和 abort 分层。 | `coding-marathon.test.ts`、`session.test.ts` | 用来发现 shell 控制面在模型多轮工具调用中是否仍保持正确上下文。 |
...
| 标准工具按具体 toolName 渲染 | 待补 | 规范要求 Web `ToolCallBlock` 对 `shell_exec/shell_status/shell_write/shell_abort/yield` 都有专门 renderer，REPL 摘要同样按 toolName 使用 `description` 或 fallback；generic 只用于未知工具。 | 待新增 Web component tests；`renderer.test.ts` 待补五工具摘要断言 | 防止 UI 只看到泛化 tool_call，用户无法区分执行、状态检查、stdin 写入、停止和 yield 唤醒。 |
