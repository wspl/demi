...
| Compaction 可支撑长任务 | 部分有效 | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore 多数有效；action 交错、context overflow 裁剪/重试策略、thinking budget 仍需补测。 |
...
| compact/preflight 期间 queued send、abort、retry、resume 收敛 | P0 | `5.4`、`5.6`：部分有效；preflight abort 已覆盖 | 防止用户在 summary 阶段继续操作时消息丢失、乱序或 session 卡在 compacting。 |
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| 手动 compact 插入 boundary 和 marker | 有效 | `session.test.ts` 手动 compact 后精确断言 block 序列包含 `compaction_boundary` 和 `compaction_marker`，并检查 collectInferenceItems 的 summary user message；能发现 compact 成功但没有边界标记。验证：5.6 targeted command，57 pass。 | `session.test.ts` | 防止 compact 完成后 transcript 无法标识历史摘要边界。 |
...
1. 先补 `5.6 Compaction` 的剩余 P0 部分有效项：preflight summary 卡住时 retry/resume/queued send 交错、context overflow 裁剪/重试策略与 fault injection、thinking budget 与 summary 请求约束。
