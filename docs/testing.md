...
| 交互式 shell 操作在 TUI 中顺畅 | 部分有效 | `process.test.ts` 用 Claude Code 协议 fixture 发起一个等待 stdin 的 `shell_exec`，TUI 进程从 stdout 解析 shellId 后发送 `/input <shellId> typed-from-tui`，断言同一 TUI stdout 出现 `fixture-input:typed-from-tui`；证明最终用户命令路径能写入 active shell 并渲染结果。真实模型在模糊任务下是否稳定选择 wait/input/abort 仍需 gated 多次 smoke。验证：`bun test packages/tui/src/__tests__/process.test.ts --timeout 15000`，3 pass。 | `packages/tui/src/__tests__/process.test.ts`；真实模型行为仍需 gated smoke | 用来发现 TUI `/input` 虽然命令分发存在，但真实进程/RPC/active shell 写入链路断裂；同时保留模型随机策略的真实验收。 |
...
1. 继续运行并记录剩余真实 provider/TUI gated smoke：模糊任务下 shell wait/input/abort 策略；gated smoke 只补充 deterministic 测试，不替代契约测试。
2. 如果后续引入独立 TUI scroll UI，再补交互滚屏测试。
