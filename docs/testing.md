...
1. 继续补 `5.17 TUI` 自动化：真实 TTY 会话模式；如果后续引入独立 scroll UI，再补交互滚屏测试。真实 Claude 端到端会话放入 gated smoke。
2. 继续运行并记录剩余真实 provider/TUI gated smoke：模糊任务下 shell wait/input/abort 策略；gated smoke 只补充 deterministic 测试，不替代契约测试。
