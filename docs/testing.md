...
| production source 依赖图与 `docs/dependency-boundaries.md` 一致 | 有效 | 测试解析所有非测试源码的 workspace imports，构造包级有向图，断言不存在越过文档允许边的依赖和循环；能发现 provider 反向依赖 agent/shell、coding-agent 组装 runtime、或中立包依赖 TUI。验证同上。 | `platform-entrypoints.test.ts` | 防止包职责重新混乱，尤其是 provider、shell、agent、TUI 之间出现双向或越层依赖。 |
...
| `ProviderRegistry` model catalog dispatch | 有效 | `registry.test.ts` 注册带 `listModels` 的 definition，断言 registry 会把 config 原样交给 provider catalog 并返回 full model id；同时缺 catalog 或缺 provider 会抛错。验证：`bun test packages/provider-claude-code/src/__tests__/models.test.ts packages/provider-codex/src/__tests__/models.test.ts packages/tui/src/__tests__/renderer.test.ts packages/tui/src/__tests__/process.test.ts`，26 pass / 1 skip。 | `packages/provider/src/__tests__/registry.test.ts` | 防止模型列表被绕到 TUI 硬编码或错误 provider 上，导致 UI 展示的模型和实际能力不一致。 |
...
验证：`bun test packages/provider-codex/src --timeout 120000`，26 pass / 4 gated skip。
