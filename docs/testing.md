...
| Context cache baseline | 有效 / Gated | `5.3`、`5.7`、`5.16`、`5.17` 覆盖 usage 字段、stable prefix、cache usage 作为 context pressure 触发 auto compact、cache 字段不进入模型上下文、RPC client transcript usage 传播和 TUI usage 呈现；真实 provider cache hit 已有 gated 验证，真实 TUI auto compact 已验证 cache-backed usage 超过阈值后触发 compact 并恢复。 |
| Shell 控制面支撑真实长命令 | 有效 / Gated | `5.10` 和 `5.15` 覆盖 wait/input/abort、busy-default auxiliary shell、raw stdin/newline、hard-stop timeout 和 agent prompt guard；真实 TUI/model fuzzy shell-control 记录见 `docs/tui-acceptance/interactive-shell-control.md`，默认测试负责稳定契约，gated smoke 负责真实模型策略漂移。 |
| Coding workflow 能发现真实问题 | 有效 | `5.12` 到 `5.15` 覆盖真实文件、todo、测试失败到修复、长命令控制、shell cwd/env 复用、file reference Host 边界和 editor 写入失败事务。 |
| 壳子路径能呈现真实模型行为 | Gated | `5.16` RPC 层有效，`5.17` TUI renderer/command/process 层已有 deterministic 测试；真实 Claude Code provider 回复、thinking、tool output 显示已有 gated smoke 入口。Compact + Abort + Resume 验收确认真实 TUI 在 abort/resume/compact 后仍能呈现模型输出和 shell 工具结果，并补上 abort/error patch 去重测试。 |
...
| thinking/text/tool output 显示 | 有效 | `renderer.test.ts` 构造 thinking、assistant text、tool_call、response blocks，断言 stdout 包含 `thinking>`、`assistant>`、`tool:` 和 usage；随后用同 id 更新 block，断言只输出新增 delta、不重复完整 text、不重复 usage；同一测试还覆盖 error/abort block id 去重，防止 transcript patch replay 重复打印 `agent error` 或 `turn aborted`。真实模型是否产出这些 blocks 仍由真实 provider/TUI smoke 覆盖。验证同上。 | `renderer.test.ts` | 防止真实模型已经产生 thinking/text/tool blocks，但 TUI renderer 分流、重复或漏显示；同时防止稳定 abort/error block 在长会话 patch 中反复刷屏。 |
