...
| Compaction 可支撑长任务 | 有效 / Gated | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore、context overflow 裁剪重试、action 交错有效；真实 provider 下 medium thinking + budget 组合已有 gated 验证。2026-06-19 的 Haiku 4.5 TUI 长上下文验收最初发现 post-compact shell tool 不可用、重复 tool loop、MCP request id 复用污染 tool result 配对；对应修复已由 provider/base-agent/shell 测试锁住，真实 TUI 复测观察到 4 次 compact、至少 3 次 post-compact shell tool continuation 和 1 次 no-tool continuation。记录见 `docs/tui-compact-haiku-acceptance.md`。 |
