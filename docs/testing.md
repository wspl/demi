...
| Compaction 可支撑长任务 | 有效 / Gated | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore、context overflow 裁剪重试、action 交错有效；真实 provider 下 medium thinking + budget 组合已有 gated 验证。2026-06-19 的 Haiku 4.5 TUI 长上下文验收最初发现 post-compact shell tool 不可用、重复 tool loop、MCP request id 复用污染 tool result 配对；对应修复已由 provider/base-agent/shell 测试锁住。真实 TUI 已分别复测 preflight/tool-heavy compact continuation 和 provider-usage auto compact：前者观察到 4 次 compact、至少 3 次 post-compact shell tool continuation 和 1 次 no-tool continuation；后者在同一会话中观察到 3 次 usage-based auto compact。Compact + Abort + Resume 真实验收又覆盖 abort 后 resume、usage-based auto compact、manual compact、post-compact shell follow-up 的同会话交错，并暴露 unsigned thinking replay 与 abort/error 重复渲染问题；记录见 `docs/tui-compact-haiku-acceptance.md` 和 `docs/tui-acceptance/compact-abort-resume.md`。 |
| Context cache baseline | 有效 / Gated | `5.3`、`5.7`、`5.16`、`5.17` 覆盖 usage 字段、stable prefix、cache usage 作为 context pressure 触发 auto compact、cache 字段不进入模型上下文、RPC client transcript usage 传播和 TUI usage 呈现；真实 provider cache hit 已有 gated 验证，真实 TUI auto compact 已验证 cache-backed usage 超过阈值后触发 compact 并恢复。`docs/tui-acceptance/context-cache-stability-marathon.md` 又验证真实 TUI 中短 one-tool turns 可能报告 zero cache，但 multi-tool provider continuation 在 manual compact 前后都能报告非零 cache usage。 |
...
| cache read/write usage 从 provider 传到 session/RPC，但不改变 agent 行为 | 基线 | `5.3`、`5.7`：有效；真实 provider hit gated | 防止 cache 指标丢失，或把 provider cache 当成会影响 transcript/tool loop 的状态。 |
...
| 真实 TUI context cache stability | Gated；手动验收通过 | `docs/tui-acceptance/context-cache-stability-marathon.md` 使用真实 TUI 和 `claude-haiku-4-5` 跑同一 workspace 中的稳定追加任务。前三个 one-tool turns 按真实 provider 报告 cache 为 0；随后 multi-tool continuation 在 compact 前报告 `cache_read=4112 cache_write=5124`，manual `/compact` 回 idle，compact 后 multi-tool continuation 报告 `cache_read=10686 cache_write=6449`，最终文件从 `CACHE_STEP_01` 连续到 `CACHE_STEP_15`。 | `docs/tui-acceptance/context-cache-stability-marathon.md`；provider/RPC/TUI usage tests；`context-cache.test.ts` | 防止只在 deterministic 层验证 cache 字段存在，却不知道真实 provider 何时上报 cache hit；同时验证 compact 后不会让 provider continuation 永久退化成 cold context。 |
