...
| production source 依赖图与 `docs/package-boundaries.md` 一致 | 有效 | 测试解析所有非测试源码的 workspace imports，构造包级有向图，断言不存在越过文档允许边的依赖和循环；能发现 provider 反向依赖 agent/shell、coding-agent 组装 runtime、或中立包依赖 TUI。验证同上。 | `platform-entrypoints.test.ts` | 防止包职责重新混乱，尤其是 provider、shell、agent、TUI 之间出现双向或越层依赖。 |
