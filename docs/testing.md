...
| 标准工具按具体 toolName 渲染 | 有效 | Web `ToolCallBlock` 显式分发 `shell_exec/shell_status/shell_write/shell_abort/yield`，未知工具才进入 generic；`tool-rendering.test.ts` 锁住标准工具 title/partial parse helper，`renderer.test.ts` 构造五个标准 `tool_call` 断言 REPL 摘要优先使用 `description` 或 per-tool fallback。 | `packages/web-ui/src/agent/__tests__/tool-rendering.test.ts`、`packages/repl/src/__tests__/renderer.test.ts` | 防止 UI 只看到泛化 tool_call，用户无法区分执行、状态检查、stdin 写入、停止和 yield 唤醒。 |
