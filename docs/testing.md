...
| 输出内容通过 `/@` artifact 读取，tool preview 按模型窗口预算 | 待补 | 最终测试应断言大 stdout/stderr 完整保存在 `/@/commands/<commandId>/stdout.txt`、`stderr.txt`、`output.txt`，`tail`/`grep`/`sed`/`awk` 经 just-bash portable commands 可读取；`shell_status` 不返回正文；`createStandardAgentTools` 根据 `Model.contextWindow` 选择 preview 预算：300k 及以下约 1k tokens，1M 及以下约 10k tokens，更大窗口 hard cap。 | `environment.test.ts`、`tools.ts` integration、`session.test.ts` | 防止大输出撑爆 tool result 或 transcript，同时防止模型继续靠 `shell_status`/offset 分页读输出。 |
