...
| Shell 控制面支撑真实长命令 | 有效 / Gated | `5.10` 和 `5.15` 覆盖 wait/input/abort 的 deterministic 流程；真实 TUI/model fuzzy shell-control smoke 已 3 次通过，默认测试负责稳定契约，gated smoke 负责真实模型策略漂移。 |
...
| 模型在真实长进程场景中稳定选择 wait/input/abort | Gated | `real-tui.e2e.test.ts` 新增 `DEMI_TUI_FUZZY_SHELL_E2E=1` gated smoke：用真实 TUI + Claude Code opus/medium 跑模糊 shell-control 任务，shell 命令由模型自选，但要求用独立 `shell_wait` 观察等待 stdin 的前台进程、`shell_input` 写入非空 stdin、再用 `shell_wait` 观察长进程并用 `shell_abort` 停止；断言 TUI stdout 出现 `tool: shell_exec`、`tool: shell_wait`、`tool: shell_input`、`tool: shell_abort`、`DEMI_FUZZY_INPUT:`、`DEMI_FUZZY_SHELL_OK` 和 usage。真实运行先暴露模型会误用 session script builtin `read`，因此已把 `shell_input` tool description 和 coding agent prompt 明确为 foreground system process stdin；修正后验证通过：`DEMI_TUI_FUZZY_SHELL_E2E=1 bun test packages/tui/src/__tests__/real-tui.e2e.test.ts --timeout 600000`，3 次 attempt 全部通过，1 pass / 1 skip。默认测试仍只证明 deterministic contract。 | `real-tui.e2e.test.ts` gated smoke，不进默认测试；`tools.test.ts` 和 `coding-definition.test.ts` 锁住 stdin 说明 | 用来发现 prompt/tool result 虽然单测正确，但真实模型仍然误用控制面的问题；本次真实 smoke 已发现并规避 session script builtin `read` 跨 turn 不适合作为 `shell_input` 目标的误用。 |
