...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| `shell_exec` running/yield 后可用 `shell_status` 继续观察 | 有效 | `environment.test.ts` 断言长命令先返回 running，后续 `status(commandId)` 得到 exited/stdout/audit/last status；coding-agent 集成测试通过标准工具组拿到 `commandId` 并继续控制。验证：targeted shell/agent/coding tests。 | `environment.test.ts`、`packages/coding-agent/src/__tests__/coding-marathon.test.ts` | 防止长命令超出初始观察窗口后失联，模型无法继续观察输出。 |
...
| bounded output delta 使用 `maxOutputBytes` | 有效 | 测试设置小 `maxOutputBytes`，断言首次 exec/status 只返回 bounded delta 且标记 truncated，后续 status 能继续读剩余输出。 | `environment.test.ts` | 防止大输出撑爆 tool result 或 transcript。 |
...
| shell tool result 格式保留 metadata 且模型可读 | 有效 | 标准工具 result 文本包含 status/shellId/commandId/exitCode/stdout/stderr artifact 与 offset，metadata 保留 `ShellCommandSnapshot`；provider/coding 集成测试从文本解析 command 状态继续执行。 | `packages/agent/src/tools.ts` via provider/coding integration tests | 防止模型必须解析不稳定 JSON 或看不到 commandId/status/next action。 |
...
| 标准工具按具体 toolName 渲染 | 有效 | Web `ToolCallBlock` 显式分发 `shell_exec/shell_status/shell_write/shell_abort/yield`，未知工具才进入 generic；`tool-rendering.test.ts` 锁住标准工具 title/partial parse helper，`block-helpers.test.ts` 锁住 terminal block 展开内容优先渲染 `output.chunks` 的 stdout/stderr 交错顺序，并在旧记录缺少 chunks 时回退为 stdout 后 stderr；`environment.test.ts` 断言 BashEnvironment 对可见 stdout/stderr 记录到达顺序；`renderer.test.ts` 构造五个标准 `tool_call` 断言 REPL 摘要优先使用 `description` 或 per-tool fallback。 | `packages/web-ui/src/agent/__tests__/tool-rendering.test.ts`、`packages/web-ui/src/agent/__tests__/block-helpers.test.ts`、`packages/shell/src/__tests__/environment.test.ts`、`packages/repl/src/__tests__/renderer.test.ts` | 防止 UI 只看到泛化 tool_call，用户无法区分执行、状态检查、stdin 写入、停止和 yield 唤醒；同时防止 Web terminal 展开内容重新泄漏完整 tool result 协议字段，或再次退化成只显示 stderr / 丢失 stdout。 |
