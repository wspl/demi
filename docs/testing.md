...
| Shell 控制面支撑真实长命令 | 长进程走 foreground，status/write/abort/yield 语义稳定；不恢复空 input 轮询；不误报 needs_input | `shell` 单测、`coding-agent` 长命令 scenario、真实 provider/REPL gated smoke |
...
| Shell 控制面支撑真实长命令 | 有效 / Gated | `5.10` 和 `5.15` 覆盖 status/write/abort/yield、busy-default auxiliary shell、raw stdin/newline、无隐式 timeout kill 和 agent prompt guard；真实 REPL/model fuzzy shell-control 记录见 `docs/repl-acceptance/interactive-shell-control.md`，默认测试负责稳定契约，gated smoke 负责真实模型策略漂移。`docs/repl-acceptance/runaway-tool-loop-guard.md` 验证 repeated command guard 在真实 provider 下可见、可收敛，且后续用户 turn 不会被旧 control_request 吞掉。 |
...
- `packages/web-ui/src/agent/__tests__/visible-blocks.test.ts`：覆盖 web 消息列表的可见
  ...
  在工具之间制造额外 virtual gap；同时过滤当前没有行渲染器的 `resume` block，避免
  内部 yield wakeup 后出现 0 高度 virtual item。
...
| 真实 OpenAI API e2e | Gated；已手动验证 DeepSeek compatible | 预留 `DEMI_OPENAI_API_E2E=1`：最小文本、standard shell tool roundtrip、active steer fallback 三类真实验收。默认 deterministic 测试不依赖网络和账号。2026-06-25 手动用 DeepSeek `deepseek-v4-pro`、`baseUrl=https://api.deepseek.com`、`wireApi="chat-completions"` 验证 provider runtime 最小文本返回 `demi-provider-openai-ok`、工具流返回 `record_value({ value: "openai-tool-ok" })`，通过 REPL 真实路径返回 `repl-openai-deepseek-ok`，并验证 DeepSeek Chat Completions SSE 暴露的 `reasoning_content` 会映射为 Demi thinking 事件。 | 待新增 `real-openai-api.e2e.test.ts` | 用来发现 fake SSE 无法覆盖的真实账号、模型、API event shape、thinking delta 扩展、tool_call 分片和限流/错误分类变化。 |
...
Owner：`packages/agent`、`packages/shell`

| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| `shell_exec` running/yield 后可用 `shell_status` 继续观察状态 | 待更新 | 测试覆盖长命令先返回 running、后续 `status(commandId)` 收敛；普通短命令完成且 preview 完整时，tool result 不暴露 `commandId` / artifact path，并释放 command record；完成态 status/write/abort 观察到完整结果后也释放 handle。 | `environment.test.ts`、`packages/agent/src/__tests__/tools.test.ts`、`packages/coding-agent/src/__tests__/coding-marathon.test.ts` | 防止长命令超出初始观察窗口后失联，同时防止模型拿到完整结果后继续 status/write/cat artifact。 |
| 标准模型工具面由 `@demi/agent` 创建 | 有效 | `coding-harness.test.ts` 断言 runtime tools 精确为 `shell_exec/shell_status/shell_write/shell_abort/yield`，harness 自身不暴露 `tools()`；provider 测试用 `createStandardAgentTools` 驱动真实 shell tool call。 | `coding-harness.test.ts`、provider tests | 防止模型工具面重新散落到 shell/coding 包，或恢复旧 `shell_wait/shell_input`。 |
| 标准工具 schema 都允许 UI `description` | 有效 | `coding-harness.test.ts` 断言五个标准工具的 input schema 都包含可选 `description`；渲染规范见 `docs/tool-rendering-spec.md`。 | `coding-harness.test.ts` | 防止模型按规范传具体用户可见状态/结果标题时被 schema 拒绝，或只有 `shell_exec` 能显示自然语言说明。 |
| 默认 shell 忙时，省略 `shellId` 的 `shell_exec` 使用辅助 shell | 有效 | 测试先在 agent 默认 shell 启动前台长进程，再调用不带 `shellId` 的 `shell_exec` 执行 probe，断言 probe 在新 shell 成功返回；随后 abort 原前台进程，再断言默认 shell 可复用。 | `environment.test.ts` | 防止模型验证 dev server 时无法在同一 agent session 里发起 curl probe，只能背景化服务或误杀进程。 |
| 输出内容通过 preview 优先，`/@` artifact 只用于截断、长跑和检索 | 有效 | 测试断言普通短命令完整输出直接进入预算 preview 且不暴露 command handle；stdout/stderr 对需要保留的 command 完整分开保存在 `/@/commands/<commandId>/stdout.txt` 和 `stderr.txt`，不存在交错合并的 `output.txt`；`tail`/`grep` 经 just-bash portable commands 可读取；artifact JSON 持久化到 Host.store；`createStandardAgentTools` 不暴露 `maxOutputBytes`/offset，并根据 `Model.contextWindow` 选择 preview 预算：未知或 800k 以下约 1k tokens，800k 及以上约 10k tokens。 | `environment.test.ts`、`host-fs.test.ts`、`tools.test.ts` | 防止大输出撑爆 tool result 或 transcript，同时防止模型把简单命令也转成 artifact 读取流程。 |
| `shell_write` 可向 foreground command 写非空 stdin | 有效 | `environment.test.ts` 启动等待 stdin 的 foreground command，再 `write({ commandId, stdin: "typed\n" })` 并断言后续 status exited/stdout；AgentServer/Client 测试覆盖 `shell_write` frame 和 `shell_write_result`。 | `environment.test.ts`、`server.test.ts` | 防止交互式命令无法继续，或输入写入错误 command。 |
| `shell_write` 精确写入 bytes，行式 reader 必须收到 newline 才继续 | 有效 | 测试启动 `read -r line`，先写入 `typed` 并断言进程仍 running 且无 stdout，再写入 `\n` 并断言输出 `typed` 后退出；coding prompt 断言 line-oriented prompt 要包含 newline。 | `environment.test.ts`、`coding-harness.test.ts` | 防止模型以为发送非空 stdin 就等于按下回车，导致真实 readline/read 进程卡住。 |
| idle foreground command 默认保持 running，不误报 needs_input | 有效 | 测试启动等待 stdin 的安静 foreground command，`shell_status` 立即返回 running，随后 write 可正常完成；不再存在等待窗口推断 needs_input。 | `environment.test.ts` | 防止安静安装、构建、dev server 被误判为需要用户输入。 |
| `shell_status` 非阻塞 | 有效 | 测试长命令 running 后调用 `status(commandId)`，断言调用很快返回 running，不等待命令完成。 | `environment.test.ts` | 防止 status 退化成 wait，造成模型空轮询或 UI 卡住。 |
| `shell_abort` 终止 foreground command，且是 control action | 有效 | `environment.test.ts` 断言 abort running command 返回 status aborted；标准工具 result 标记 `isError=false`。 | `environment.test.ts`、`tools.ts` via integration tests | 防止停止 dev server 被当作任务失败，或进程继续占端口。 |
| shell abort flush redirected foreground output | 有效 | 测试 abort 正在写入重定向文件的 foreground command，断言 tool stdout/stderr 不泄漏内容且目标文件包含 aborted 文本；能发现 flush 丢失或输出错路由。 | `environment.test.ts` | 防止本应写入文件的输出泄漏到 tool output，或文件内容丢失。 |
...
| `yield` 是 terminal tool result，idle 时启动内部唤醒 turn | 有效 | `session.test.ts` 断言 yield tool result 写入 transcript 后当前 provider continuation 停止，duration 到点后 session idle 时 push resume turn 并启动普通新 provider turn。 | `session.test.ts` | 防止 yield 被当成同轮 wait，或 tool result 后继续采样造成轮次边界错误。 |
| pending `yield` 到点时 active session 走 steer，不进普通 queue | 有效 | `session.test.ts` 在已有 active provider turn 时触发 yield wakeup，断言 provider 收到 native steer、message queue 为空。 | `session.test.ts` | 防止长命令复查被用户新话题无限期排队，违背 yield 的唤醒目的。 |
| 会话 `abort` 分层且 pending yield 最后清理 | 有效 | `session.test.ts` 断言无 active work 时 abort 清理 pending yield；有 active provider stream + pending yield 时，第一次 abort 只取消 active stream 且 `canAbortAgain=true`，第二次才清理 pending yield。 | `session.test.ts` | 防止一次 abort 顺手取消低优先级 wakeup，也防止 UI 不知道还能继续 abort。 |
| shell tool result 格式保留 metadata 且模型可读 | 待更新 | 标准工具 result 文本应在普通完成结果里只包含 status/exitCode/preview；running 或 preview 截断时才包含 shellId/commandId/artifact 路径和 next action；metadata 保留 `ShellCommandSnapshot` 供 UI/审计使用；不再包含 stdout/stderr offset 分页协议。 | `packages/agent/src/tools.ts` via provider/coding integration tests | 防止模型必须解析不稳定 JSON，也防止简单命令暴露无意义 commandId 后诱导 status/write/artifact 误用。 |
| `shell_write` 拒绝空 stdin，不承担 polling | 有效 | `createStandardAgentTools` 校验空 stdin 抛错，错误文本提示用 `shell_status` polling；server/client 只暴露 `shell_write(commandId, stdin)`。 | `tools.ts` via integration tests、`server.test.ts` | 防止空 input 被滥用成 wait，重新引入无意义控制动作。 |
| 模型在真实长进程场景中稳定选择 status/write/abort/yield | Gated | `real-repl.e2e.test.ts` gated prompt 要求真实模型 `shell_exec → yield → shell_status` 观察、`shell_write` 注入 stdin、`shell_abort` 停止长命令，并断言 REPL 输出包含 `tool> shell_exec`、`tool> yield`、`tool> shell_status`、`tool> shell_write`、`tool> shell_abort`；历史记录保留在 `docs/repl-acceptance/interactive-shell-control.md`。 | `packages/repl/src/__tests__/real-repl.e2e.test.ts`、`docs/repl-acceptance/interactive-shell-control.md` | 用来发现 prompt/tool result 虽然单测正确，但真实模型仍然误用控制面的问题。 |
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| coding harness 暴露 Host/commands 并由组装层创建标准 agent tools | 有效 | 测试 `createCodingAgentHarness({ host })` 后断言默认 commands 为 `editor/todo`；测试局部按 AgentServer 的标准组装创建 `BashEnvironment + createStandardAgentTools`，断言 runtime tools 为 `shell_exec/shell_status/shell_write/shell_abort/yield`，并通过环境执行默认 todo command；同时断言 harness 自身不暴露 `tools()`/`dispose`。 | `packages/coding-agent/src/__tests__/coding-harness.test.ts` | 防止 coding harness 回退到 public `BashEnvironment` 注入或自定义 tool 替换，同时防止模型打开后缺少标准 shell 控制面或默认命令未注册。 |
| registered command prompt 注入 system prompt | 有效 | 测试 systemPrompt 包含 editor/todo prompt、effects/success/failure output、todo example、workspace-cwd rule、foreground long-process rules、`yield + shell_status`、auxiliary shell guidance、line-oriented stdin newline、avoid pkill/killall 和 no redundant restart 等文本，并断言 `editor prompt` 输出同源内容；能发现专属命令或 shell-control 说明未注入。验证同上。 | `coding-harness.test.ts` | 防止模型不知道 `editor`、`todo` 等专属命令的正确调用方式，也防止真实 REPL 里反复出现错误工作目录、误用 timeout 或重复启动已验证服务。 |
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| StubProvider 通过 shell tools 驱动 editor/todo，真实写文件 | 有效 | 测试用 StubProvider 依次发起 `shell_exec`，通过 `editor create`、`todo add`、`editor edit` 驱动真实 BashEnvironment/LocalHost，并断言工具结果；能发现 AgentSession 到 registered command 的集成断裂。验证：5.15 targeted command，6 pass。 | `packages/coding-agent/src/__tests__/coding-marathon.test.ts` | 能发现 AgentSession、shell tools、registered command、Host 写文件之间的集成断裂。 |
...
| 五个标准工具的 agent 级流程 | 有效 | 新增 deterministic 测试用 StubProvider + 真实 BashEnvironment 在同一任务里执行 `shell_exec -> yield -> shell_status -> shell_write -> yield -> shell_status -> shell_exec -> shell_status -> shell_abort`，断言 stdin 输出、长命令 running 和 abort 结果，并断言 transcript 中五个标准工具都出现且无 pending tool；`session.test.ts` 额外覆盖 yield terminal result、idle wakeup、active steer 和 abort 分层。 | `coding-marathon.test.ts`、`session.test.ts` | 用来发现 shell 控制面在模型多轮工具调用中是否仍保持正确上下文，并防止某个标准工具只存在于 schema 但没有集成发挥作用。 |
...
| shell_write frames 桥接到 active foreground command | 有效 | 测试 shell foreground 等待 stdin，`client.shellWrite(commandId, stdin)` 后看到 typed stdout 和 `shell_write_result`，且不重复发 generic tool_progress；能发现输入没有写入 active command。验证同上。 | `server.test.ts` | 防止用户在壳子里输入内容但没有写入正在等待的进程。 |
| client `shellWrite` 等待 result，未 open 时 reject | 有效 | 测试未 open 时 reject `No session is open`，并用 gate 延迟 shell_write invoke，断言 promise 在 result 前不 settle、result 后 resolve；能发现 UI 过早认为输入成功。验证同上。 | `server.test.ts` | 防止 UI 认为输入成功但实际上没有 active session。 |
...
| 标准工具按具体 toolName 渲染 | 待更新 | Web `ToolCallBlock` 显式分发 `shell_exec/shell_status/shell_write/shell_abort/yield`，未知工具才进入 generic；`tool-rendering.test.ts` 锁住标准工具 title/partial parse helper；`block-helpers.test.ts` 应锁住 `shell_exec` 展开内容只展示命令和交错 terminal preview，`shell_status` 只展示状态摘要且不展示 stdout/stderr 正文；`environment.test.ts` 断言 BashEnvironment 对可见 stdout/stderr 记录到达顺序；`renderer.test.ts` 构造五个标准 `tool_call` 断言 REPL 摘要优先使用 `description` 或 per-tool fallback。 | `packages/web-ui/src/agent/__tests__/tool-rendering.test.ts`、`packages/web-ui/src/agent/__tests__/block-helpers.test.ts`、`packages/shell/src/__tests__/environment.test.ts`、`packages/repl/src/__tests__/renderer.test.ts` | 防止 UI 只看到泛化 tool_call，用户无法区分执行、状态检查、stdin 写入、停止和 yield 唤醒；同时防止 `shell_status` 被渲染成输出块，或 Web terminal 展开内容重新泄漏完整 tool result 协议字段。 |
...
| 交互式 shell 操作在 REPL 中顺畅 | 有效 / Gated | `process.test.ts` 用 Claude Code 协议 fixture 发起一个等待 stdin 的 `shell_exec`，REPL 进程从 stdout 解析 commandId 后发送 `/input <commandId> typed-from-repl`，断言同一 REPL stdout 出现 `fixture-input:typed-from-repl`；证明最终用户命令路径能写入 active command 并渲染结果。`real-repl.e2e.test.ts` 的 `DEMI_REPL_FUZZY_SHELL_E2E=1` 真实模型 smoke 覆盖模糊任务下 status/write/abort/yield 选择。 | `packages/repl/src/__tests__/process.test.ts`、`packages/repl/src/__tests__/real-repl.e2e.test.ts` gated smoke | 用来发现 REPL `/input` 虽然命令分发存在，但真实进程/AgentServer/transport/active command 写入链路断裂；同时发现真实模型虽然能看到工具说明，但仍可能误用控制面的策略问题。 |
