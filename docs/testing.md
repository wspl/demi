...
| Compaction 可支撑长任务 | 部分有效 | `5.6` 是当前最强覆盖面，preflight/manual/auto、tool pair、multi-compact、snapshot restore、context overflow 裁剪重试、action 交错多数有效；真实 provider 下 medium thinking + budget 组合已有 gated 验证，默认测试仍只覆盖 deterministic contract。 |
...
| Shell 控制面支撑真实长命令 | 部分有效 | `5.10` 和 `5.15` 覆盖 wait/input/abort 的 deterministic 流程；真实 TUI/model fuzzy shell-control smoke 已 3 次通过，默认测试仍只覆盖 deterministic contract。 |
...
| 测试点 | 审查结论 | 审查记录 | 候选覆盖 / 待核对 | 能发现或规避的问题 |
|---|---|---|---|---|
| Claude CLI path/version 检测 | 有效 | `cli.test.ts` 注入 shell runner，断言 `which claude` 后使用解析出的 `/bin/claude --version`，并返回修剪后的 path/version；能发现未按实际 CLI path 查询版本或 version 解析异常。验证：5.3 targeted command，31 pass / 3 skip。 | `packages/provider-claude-code/src/__tests__/cli.test.ts` | 防止 provider 在没有可用 CLI 或版本信息异常时进入半可用状态。 |
...
| provider mock 行为贴近真实 provider 事件顺序 | 部分有效 | `RecordingProvider`、`GateProvider`、`HangingProvider`、`ThrowingProvider`、`CompactGateProvider` 覆盖 tool_call+response 后二次 roundtrip、async stall、provider iterator throw、abort event、summary gate、auto compact after tool_result 等非单步路径；但它们仍是 deterministic fake，不能证明真实 Claude stream/control_request 的全部时序。验证同上。 | `session.test.ts`、`session-marathon.test.ts`、`compaction.test.ts` 覆盖 pending tool、response、iterator exception、auto compact、abort event；真实 Claude 随机路径仍靠 gated smoke | 防止 deterministic 测试用一个过于理想的 provider，真实 Claude Code 路径才暴露 tool/response 时序问题。 |
...
| thinking 模型下 summary request 的 token/budget 设置有效 | 部分有效 | preflight 测试断言 thinking 模型的 summary request 携带 `{ type: 'effort', effort: 'medium' }`；Claude Code CLI contract 测试断言该 request thinking effort 和 provider max budget 会映射到 `--effort medium`、`--max-budget-usd`；真实 provider gated smoke 已验证 `claude-opus-4-8` medium thinking + budget summary-shaped request 能返回 thinking 信号、text 和 usage。当前 request 类型仍没有 per-request max output token 字段，因此默认测试只证明现有契约。 | `compaction.test.ts`、`cli.test.ts`、`real-cli.e2e.test.ts` gated smoke | 防止 summary 请求因为 thinking effort 或 provider budget 丢失而使用错误模型配置；真实 smoke 能发现 CLI 参数组合或 thinking event 映射在本机真实路径退化。 |
