...
| 标准工具 schema 都允许 UI `description` | 有效 | `coding-harness.test.ts` 断言五个标准工具的 input schema 都包含可选 `description`；渲染规范见 `docs/tool-rendering-spec.md`。 | `coding-harness.test.ts` | 防止模型按规范传用户可见意图标题时被 schema 拒绝，或只有 `shell_exec` 能显示自然语言说明。 |
