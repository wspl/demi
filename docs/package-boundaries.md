...
- Public boundary: provider contract, direct `Provider[]` composition types, quota helpers (`createProviderQuota`, `ensureQuota`), credential public types, schema parsing with payload-free diagnostics (`parseProviderData`, `parseProviderJson`, `ProviderDataError`), provider test helpers only from `@demicodes/provider/testing`, and pool IO only from `@demicodes/provider/credentials-pool`. Concrete providers own their wire schemas; the shared parser owns JSON decoding and validation error classification.
