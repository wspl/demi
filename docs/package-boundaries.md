...
- Status: implemented on txiki.js (`src/entry.ts` is the bundle entry — command mode for any root name, runner mode for `demi-runner`; native linking in `runtime/build.ts`).
