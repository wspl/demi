...
- Layout (directories mirror the design record's backend modules):
  ...
  - `llm/` — the provider runtime assembled per configured provider (provider-type factories, catalog, the Test button) and the metering wrap at the inference entry.
