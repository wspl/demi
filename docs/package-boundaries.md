...
- Layout (directories mirror the design record's backend modules):
  ...
  - `vault/` — instance secret, credential crypto, the typed connection vault over the control plane, and the connection scope (whose connections a caller works with under the instance mode).
