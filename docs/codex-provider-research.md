...
- `listCodexModels`
- `codexAuthStatus`
- public config/status/model option types

`CodexProvider` class、auth store、transport、Responses request/event conversion helpers 不是公共 root API。包内测试可以直接 import 对应实现文件断言这些边界，但其他包只能通过 provider definition、auth status 和 model catalog 能力组合。
