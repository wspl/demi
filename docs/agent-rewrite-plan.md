...
```text
shell_exec(script, yieldAfterMs?, timeoutMs?)
  → 命令结束：返回 exited + exitCode + stdout/stderr
  → 命令仍在跑：返回 running + shellId + stdout/stderr delta

shell_wait(shellId, yieldAfterMs?, timeoutMs?)
  → 轮询/等待前台进程；不写 stdin
  → 每次调用的 yieldAfterMs 都从本次调用开始计时

shell_input(shellId, stdin, yieldAfterMs?)
  → stdin 必须是非空字符串；写入前台进程
  → 轮询必须使用 shell_wait，不允许空 input 兼容路径
  → 每次调用的 yieldAfterMs 都从本次调用开始计时
...
`shell_wait` 是唯一轮询工具；模型不应因为重复看到 `running` 就盲目连续 wait。工具结果必须直接可读，包含 status、shellId、stdout/stderr delta、exitCode 或 next action，不要求模型从一坨 JSON 里猜状态。

runner 不提供由 idle timeout 触发的 shell 输入需求状态。`yieldAfterMs` 到期时，如果进程还活着，就返回 `running + shellId`；模型用 `shell_wait` 继续等待，或在明确知道命令正在等待具体输入时用 `shell_input` 写入非空 stdin。demi 不保留空 input 轮询入口，避免把一个无效果的 stdin 写入伪装成控制动作，也避免把 `npm install --silent` 这类安静长命令误判成交互输入。
...
`CommandStorage` 是 `DemiStore`（见 §8.2）的 agent-session-scoped 视图：Bash Environment 创建 `CommandRunContext` 时用 `DEMI_SESSION_ID` 包一层，命令看到的 key 自动带 agent session 前缀。命令作者只写 `storage.writeJson('todos.json', ...)`，不拼前缀。需要跨 agent session 共享的状态第一版不开放。
...
四个工具里的 `shellId` 指此 shell session。session 同时只跑一条前台命令；前台命令 running 时 `shell_exec` 返回 `running + shellId`，后续 `shell_wait` / `shell_input` / `shell_abort` 操作同一 session 直到其退出。

shell session 通常对一个 AgentSession 长期存活（跨多个 user turn），销毁时其 cwd / env / 后台进程一并释放。默认 `shell_exec` 不传 `shellId` 时使用该 AgentSession 的默认 shell；需要等待/输入/中断长命令，或显式复用某个非默认 shell 时，才把工具返回的 `shellId` 传给 `shell_wait` / `shell_input` / `shell_abort` / 后续 `shell_exec`。
...
shell_exec(script)
  ...
  → 命令未结束：返回 running + shellId
  → 后续 shell_wait / shell_input / shell_abort 操作同一 session
```

Agent Loop 只在边界点恢复：exit / timeout / abort / yieldAfterMs / output_limit。每次恢复给模型的是程序化 output snapshot，不是模型生成的摘要。模型不应每个输出 chunk 都被唤醒。

`yieldAfterMs` 到期且进程仍在运行时，默认状态是 `running`，即使 stdout/stderr 没有新增内容。安静长命令由 `runningMs` / `idleMs` 暴露观测信息，是否继续等、改用非静默命令、或主动 abort 由模型决定；runner 不能把 idle timeout 自动升级成输入需求状态。
...
type ShellToolResult =
  | { status: 'exited'; shellId: string; exitCode: number; output: OutputSnapshot; audit: BashAuditEvent[] }
  | { status: 'running'; shellId: string; reason: 'yield' | 'output_limit'; output: OutputSnapshot; runningMs: number; idleMs: number }
...
Demi 第一版只暴露 shell session tools。Agent Loop 对 shell continuation 只保存 opaque token，不理解其背后是不是本地进程或远端进程：

```ts
type ToolContinuation = {
  toolCallId: string
  // 对 Agent Loop 是 opaque token。Bash Environment 内部即 shellId。
  // Agent Loop 不解释，只透传回 wait/input/abort。
  shellId: string
  status: 'running'
}
```

真正的 pid、stdout buffer、idleMs、stdin、abort 都留在 Bash Environment 内部。术语统一：shell 工具入参/出参的 `shellId`、`ToolContinuation.shellId` 是同一个值，不另起 `continuationId`。
...
interface AgentSessionParams<State> {
  ...
  runtime: AgentHarnessRuntime<State> // RPC host 从 AgentHarness 解析出的内部 hooks
}
...
interface AgentHarnessRuntime<State> {
  ...
  tools(ctx: AgentToolContext<State>): AgentTool[] // 固定 shell tools，由 RPC host 创建
...
`AgentHarness` 依赖 `Host` / `CommandSpec`，因此它属于 RPC host / agent assembly 边界，不要求 `base-agent` 直接依赖 `@demi/shell`。RPC host 查到 harness 后创建 Bash Environment 和固定 shell tools，再把解析后的内部 `AgentHarnessRuntime` hooks 传给 `AgentSession`。
...
- `lifecycle()` 做 harness 自己的清理；Bash Environment 的 shell session 清理由 RPC host 统一处理。

Harness 不暴露这些可替换点：

- 不提供 `tools()`；四个 shell session 工具固定由 demi 创建并暴露给模型。
- 不接收 `BashEnvironment`；RPC host 按 harness 的 `Host + commands` 创建标准 Bash Environment。
- 不允许替换 shell tool result 格式、wait/input/abort 语义、audit 事件或 command registry 调度。

RPC host 打开 session 时的装配顺序：

```text
open(harnessName, provider, cwd)
  → 查找 AgentHarness
  → harness.host(...) + harness.commands(...)
  → 创建 BashEnvironment(host, commands, store, sessionId)
  → 创建固定 shell tools: shell_exec / shell_wait / shell_input / shell_abort
...
`shell_wait(shellId)` 续接：取出 `session.pendingExec`，继续 race `executeScript` 和新边界点。`shell_abort` / `shell_input` 操作 `session.foreground` 后继续 race。
...
`BashEnvironment`/`ShellToolResult`/`OutputSnapshot`/`BashAuditEvent`/`CommandMetadataRecord` 公开接口不变。`tools.ts`/`command.ts`/`coding-harness.ts`/`rpc/host.ts`/测试随 harness API 迁移，但 shell 控制面语义不变。
...
`DEMI_SESSION_ID` 在 AgentSession 创建时分配并记入 session metadata，Bash Environment 创建 shell session 时注入 env；`DEMI_SHELL_ID` 仅表示当前 shell 控制句柄。注册命令经 `CommandStorage`（DemiStore 的 agent-session-scoped 视图，自动按 `DEMI_SESSION_ID` 隔离 key，见 §4.6）访问 DemiStore。
...
- Coding harness 注册少量 agent 专属命令，并让 RPC host 用这些命令创建标准 Bash Environment。
...
**一连接一 session。** 一个 RPC 连接对应一个 agent session。`open` 在握手时完成（传 harness + provider 配置 + cwd），之后普通 agent 动作都作用在这个 session 上，帧里不带 agent session id；shell 输入/输出帧用 `shellId` 关联具体 shell。多 agent session 场景由多连接解决。
...
type ClientFrame =
  | { type: 'open'; harness: string; provider: ProviderConfig; cwd: string }  // harness 是 host 侧预注册的 harness 名
...
type ServerFrame =
  ...
  | { type: 'tool_progress'; toolUseId: string; output: ToolResultContentBlock[] }
  | { type: 'shell_output'; shellId: string; snapshot: OutputSnapshot }
  | { type: 'shell_input_result'; shellId: string; output: ToolResultContentBlock[] } // shell_input 完成确认，按 shellId 关联
  | { type: 'audit'; events: BashAuditEvent[] }
  | { type: 'error'; message: string; code?: string }
  | { type: 'closed' }
```
...
`AgentHarness` 含闭包（`systemPrompt` / `host` / `commands` / `resolveReferences` / `lifecycle`），不可序列化。因此 host 侧维护一个 harness 注册表：宿主在启动时用 `registerHarness(name, harness)` 注册（如 `'coding'` → coding agent harness），`open` 帧的 `harness: string` 是这个注册名。host 查表拿到 harness 对象后，用 harness 提供的 `Host + commands` 创建标准 Bash Environment，再创建 AgentSession。harness 不跨传输序列化。
...
`AgentHarness` 类型作为协议注册概念保留在 agent assembly 边界；实现上可以先放在 `rpc` 或一个位于 `base-agent + shell` 之上的小包，跑通后再按依赖关系拆分。重要的是对外只注册 harness，不暴露可替换 Bash Environment，也不让 `base-agent` 直接依赖 `@demi/shell` 形成循环。
...
`AgentHarness` 的类型先放进 runtime 边界，跑通后再按包依赖拆。demi 是纯库，不做 module-agent / frontend 实现，但提供协议层（§11）供壳子接入。
...
**3.6 wait / input / abort / timeout / dispose**：`shell_wait`/`shell_input`/`shell_abort`/`timeoutMs`/`disposeShell`/`disposeAllShells`；`shell_wait` 负责轮询，`shell_input` 要求非空 stdin；主动 `shell_abort` 是控制动作；AgentSession abort 时终止前台进程；RPC host 关闭 session 时统一 dispose Bash Environment，并调用 harness lifecycle 清理。**验收**：各路径单测通过。
...
**5.5 验收**：InProcessTransport 端到端；shell_output 帧正确带 shellId；busy 时 send 触发 queue 帧并按序 resolve；retry 的 transcript_patch 包含删除的 block；stdio transport 端到端；本地直连和跨进程走完全相同的 RpcClient/帧协议代码。
...
Step 0-6 的本地包和集成测试已落地：core / provider / base-agent / just-bash / shell / 现有 agent-coding / rpc / provider-claude-code。目标命名和 API 已调整为 `coding-agent` + `AgentHarness`；现有 `agent-coding` 包需要迁移。当前门禁已通过：`bun run typecheck`、`bun run test`、`bun run test:just-bash-core`、`packages/just-bash/packages/just-bash` 下的 `pnpm typecheck`、`./node_modules/.bin/vitest run src/interpreter/`。
...
- 现有 `@demi/agent-coding` 运行时入口去除 Node-only 依赖：editor/reference resolver 使用 Host contract、Web UTF-8、平台无关路径归一化。该包后续迁移为 `@demi/coding-agent`，并把主入口改为 `createCodingAgentHarness`。
- 平台默认入口静态闭包测试覆盖所有平台无关包；just-bash 边界静态扫描；RPC 边界静态扫描；package manifest 分层扫描。
- shell session 生命周期、状态连续性、OutputSnapshot、长命令 yield、wait/input/abort/timeout、DEMI_SESSION_ID/DEMI_SHELL_ID、DemiStore/CommandStorage、audit、注册命令 prompt/`--json`/commandMetadata、shell 工具接入 AgentSession abort signal——均已落地并有回归测试。
...
1. 旧 `AgentDefinition` API 已能通过 `commands()` 暴露 editor/todo 命令，命令 prompt 仍由 `CommandRegistry.renderPrompt()` 同源渲染；后续按 §6 迁移为 `AgentHarness`，并移除 public BashEnvironment 注入。
2. RPC host `shell_input` 不再伪造 `toolCallId`；server 增加 `shell_input_result` 帧，用 `shellId` 作为完成确认与客户端 Promise 收敛的关联键。
...
- shell session tool 形状（exec/wait/input/abort，shellId 统一）
