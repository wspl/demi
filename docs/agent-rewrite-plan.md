...
**一连接一 session。** 一个 RPC 连接对应一个 agent session。`open` 在握手时完成（传 definition + provider 配置 + cwd），之后普通 agent 动作都作用在这个 session 上，帧里不带 agent session id；shell 输入/输出帧用 `shellId` 关联具体 shell。多 agent session 场景由多连接解决。
...
**3.6 wait / input / abort / timeout / dispose**：`shell_wait`/`shell_input`/`shell_abort`/`timeoutMs`/`disposeShell`/`disposeAllShells`；`shell_wait` 负责轮询，`shell_input` 要求非空 stdin；主动 `shell_abort` 是控制动作；AgentSession abort 时终止前台进程；coding definition dispose 清理 BashEnvironment；RPC `close` 等待 definition dispose。**验收**：各路径单测通过。
...
Step 0-6 的本地包和集成测试已落地：core / provider / base-agent / just-bash / shell / agent-coding / rpc / provider-claude-code。当前门禁已通过：`bun run typecheck`、`bun run test`、`bun run test:just-bash-core`、`packages/just-bash/packages/just-bash` 下的 `pnpm typecheck`、`./node_modules/.bin/vitest run src/interpreter/`。
