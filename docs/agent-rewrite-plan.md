...
Step 0-6 的本地包和集成测试已落地：core / provider / base-agent / just-bash / shell / coding-agent / rpc / provider-claude-code。目标命名和 API 已调整为 `coding-agent` + `AgentHarness`；RPC host 固定组装 Bash Environment 和 shell tools，coding harness 只定义 Host、commands、prompt、reference resolver 和 lifecycle。当前门禁已通过：`bun run typecheck`、`bun run test`（305 pass / 6 skip）、`bun run test:just-bash-core`、`packages/just-bash/packages/just-bash` 下的 `pnpm typecheck`、`./node_modules/.bin/vitest run src/interpreter/`。
