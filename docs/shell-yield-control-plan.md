...
automatically by `@demicodes/agent` from the current model's `contextWindow`; it affects only the
...
`AgentServerOptions.tools.shellPreviewBudgetTokens` pins a fixed budget for every session in the
tree (root and subagents) instead of the context-window heuristic.
...
- `@demicodes/core` owns the shared `Block` types, including the `hidden` flag on user / steer.
- `@demicodes/agent` owns the model-visible tool surface: the names, schemas, and tool-call/result
  ...
  `yield`'s wakeup payload is constructed by `@demicodes/agent` and does not depend on the shell.
- `@demicodes/shell` owns the shell runtime services: BashEnvironment, shell sessions, command
  ...
  wakeups**.
- `@demicodes/coding-agent` only explains the usage policy for these five tools in the prompt.
- `@demicodes/web-ui` and `@demicodes/repl` consume the unified protocol events to display state, and skip
  `hidden` user/steer blocks per §5.

So the final layering is: `@demicodes/agent` owns the complete tool surface and the yield wakeup
semantics, `@demicodes/shell` provides only shell execution, and the two are orthogonal.
...
- `packages/agent/src/__tests__/session.test.ts`
  ...
  - The tool-result preview budget is chosen automatically from the current `Model.contextWindow`:
    10k tokens when unknown or below 800k, 100k tokens at 800k and above; a fixed
    `AgentServerOptions.tools.shellPreviewBudgetTokens` replaces the heuristic for root and children.
