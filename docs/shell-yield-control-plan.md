...
这些文件不写进任务 cwd，也不污染用户 workspace。它们是完整输出的持久审计落点：command settle、
session snapshot 保存或会话恢复时，runtime 必须能从持久 command artifact 重建 `/@/commands/<commandId>/...`。
transcript 的模型可见 tool result 只保存 artifact 引用和自动预算 preview，不内联完整大输出；UI/runtime
...
- artifact 生命周期跟随 AgentSession 持久历史；关闭 live session 时释放内存态 overlay，但已保存的 session
  snapshot 必须保留 artifact 内容或可恢复引用，使恢复后的 `/@` 读取和审计历史一致。
...
- `packages/shell/src/__tests__/environment.test.ts`
  ...
  - 完整 stdout/stderr 进入 command artifact；tool result preview 截断不影响 artifact 完整性。
