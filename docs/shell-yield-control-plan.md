...
- 返回 `runningMs`、`idleMs`、stdout/stderr/output 字节计数和 artifact 路径，方便模型判断是否有新输出或是否异常安静。
...
每个 command 都有三个只读 append-only artifact：
...
`output.txt` 是 stdout/stderr 按到达顺序交错后的 terminal transcript；`stdout.txt` 和 `stderr.txt`
分别保留对应 stream；`meta.json` 暴露 status、exitCode、runningMs、idleMs、bytes 和 timestamps。
这些文件不写进任务 cwd，也不污染用户 workspace。
...
- visible stdout/stderr chunk 实时 append 到 command artifact 和 `output.txt`。
...
```bash
tail -n 80 /@/commands/<commandId>/output.txt
grep -n "ERROR" /@/commands/<commandId>/stderr.txt
sed -n '200,260p' /@/commands/<commandId>/stdout.txt
awk '/failed|error/i { print NR ":" $0 }' /@/commands/<commandId>/output.txt
```
...
tail -n 40 /@/commands/<commandId>/output.txt
```
...
- `packages/agent/src/tools.ts`（由 agent/coding/provider integration tests 覆盖）
  ...
  - tool result 包含 `shellId`、`commandId`、stdout/stderr/output artifact 路径、状态字段和 next action。
...
- `packages/shell/src/__tests__/environment.test.ts`
  ...
  - command artifact 暴露 `/@/commands/<commandId>/stdout.txt`、`stderr.txt`、`output.txt` 和 `meta.json`。
