...
- 命令在 `yieldAfterMs` 内退出且 preview 完整，返回 `status: "exited"`、`exitCode` 和 preview，不暴露 `commandId` 给模型，并释放 command record。
- 命令在 `yieldAfterMs` 内退出但 preview 截断，返回 `status: "exited"`、`exitCode`、preview 和 artifact 路径，保留 artifact 读取入口。
- 命令在 `yieldAfterMs` 后仍在运行，返回 `status: "running"`，命令继续运行。
- `shell_exec` 不接受 `timeoutMs`，也不会隐式终止进程。

`shell_exec` tool result 包含一段自动预算控制的输出 preview，用于让模型快速判断普通输出、明显失败或已完成状态。普通短命令的结果必须直接从 preview 判断，不应该再读取 `/@`。只有 preview 截断、命令仍在长时间运行并需要看历史输出，或需要文本检索时，才走 `/@` artifact 文件。

### 3.2 `shell_status`

读取一条运行中 command handle 的当前状态。它不等待、不写 stdin；返回新的预算 preview 只是为了让模型在命令完成或产出少量新增输出时直接判断结果，不是输出分页接口。
...
- 如果命令已退出，返回 `status: "exited"`、`exitCode` 和预算 preview；preview 完整时返回后释放 command record。
- 如果命令已被中止，返回 `status: "aborted"`；preview 完整时返回后释放 command record。
...
- 写入目标 command 的 stdin 后，立即返回一次 status snapshot 和新的预算 preview；如果命令因此完成且 preview 完整，返回后释放 command record。
...
保留的 command artifact 中，`stdout.txt` 和 `stderr.txt` 分别保留对应 stream；`meta.json` 暴露 status、exitCode、runningMs、idleMs、bytes 和 timestamps。
这些文件不写进任务 cwd，也不污染用户 workspace。普通短命令完成且 preview 完整时不保留这些文件；running、preview 截断或需要检索的 command 才把完整输出作为持久审计落点。session snapshot 保存或会话恢复时，runtime 必须能从持久 command artifact 重建仍需读取的 `/@/commands/<commandId>/...`。
transcript 的模型可见 tool result 优先保存自动预算 preview，只在需要保留 artifact 时保存引用；UI/runtime
可以保存或接收交错输出事件用于展示，但 stdout/stderr 交错后的 terminal transcript 不作为 `/@` 文件保存。
...
- 只有被保留的 command artifact 生命周期跟随 AgentSession 持久历史；普通短命令完成且 preview 完整时会释放 command record，不再暴露 `/@`。关闭 live session 时释放内存态 overlay，但已保存的 session snapshot 必须保留需要继续读取的 artifact 内容或可恢复引用，使恢复后的 `/@` 读取和审计历史一致。
...
- tool result 直接返回状态和自动预算控制的 preview；仍在运行或 preview 截断时才返回 artifact 路径。

模型只有在 preview 截断、命令长时间运行且需要看历史输出，或需要文本检索时，才使用普通 shell 文本命令读取 artifact：
...
`budgetTokens` 转换成字符上限。preview 截断时必须带 `truncated: true` 标记，并明确提示模型用
`/@/commands/<commandId>/...` 文件读取需要的部分；preview 完整时不能暴露 artifact 路径诱导模型二次读取。
...
`shell_status` 判断命令是否仍在跑、是否有新 bytes、是否 idle 过久；普通新增输出直接看预算 preview；只有 preview 截断或需要文本检索时再用
...
- `packages/agent/src/tools.ts`（由 agent/coding/provider integration tests 覆盖）
  ...
  - tool result 对普通完成结果只包含状态、exitCode 和 preview；running 或 preview 截断时才包含 `shellId`、`commandId`、stdout/stderr artifact 路径和 next action。
...
- `packages/shell/src/__tests__/environment.test.ts`
  ...
  - 需要保留的完整 stdout/stderr 进入 command artifact；tool result preview 截断不影响 artifact 完整性。
...
- `packages/agent/src/__tests__/session.test.ts`
  ...
  - tool result preview 预算按当前 `Model.contextWindow` 自动选择：未知或 800k 以下上下文 1k tokens，800k 及以上上下文 10k tokens。
  - transcript/session snapshot 持久化 artifact 引用和恢复所需内容，但 provider replay 只包含预算 preview，不包含完整大输出。
...
- 未知或 800k 以下 context window 的模型，单次 tool result preview 不超过约 1k tokens；800k 及以上窗口不超过约 10k tokens。
