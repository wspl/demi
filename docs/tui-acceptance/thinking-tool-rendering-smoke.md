...
| TUI command | `/usr/bin/script -q <log> bun run packages/tui/src/index.ts --cwd <tmp> --model claude-opus-4-8 --thinking medium --budget 1.00 --yield-after-ms 1000 --timeout-ms 180000` |
...
- The TUI header shows `model: claude-opus-4-8`.
- The TUI header shows `thinking: medium`.
- The TUI log contains `thinking> [signed]` from the real provider.
- The TUI log contains `tool: shell_exec executing` and `tool: shell_exec completed`.
...
- The TUI log contains `usage: in=53 out=252 cache_read=3755 cache_write=4070`.
- The session closed normally with `/exit`.
- Negative checks found no `API Error`, `agent error`, `tool: shell_exec error`, or `turn aborted`.
...
- Date: 2026-06-19
- Workspace: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-tui-thinking-smoke-XXXXXX.XiUiC9ql5B`
- Log path: `/var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-tui-thinking-smoke-XXXXXX.XiUiC9ql5B/tui-thinking-tool-rendering-smoke.log`
...
- Workspace evidence: `wc -c smoke.txt` returned `116`, and the first line was `DEMI_THINKING_TOOL_SMOKE /var/folders/bj/xcm3f3zx2z710fbv_jt6p3zr0000gn/T/demi-tui-thinking-smoke-XXXXXX.XiUiC9ql5B`.
- Verdict: Passed.

## Failure Analysis

No product bug was found in this run. It confirms the real TUI path can display a real Claude Code provider response with medium thinking, shell tool execution/output, assistant text, usage fields, and normal close behavior.
