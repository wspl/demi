...
| B20 | 代码确认 | 设计示例与可调用能力不一致 | key 示例参数形式、无目标 type/key、wait --load 等需要逐项核对；不能把未实现说成已支持 |
...
- 不能仅靠普通等待或 Codex expectNavigation 的名字，认定它完整替代了
  Demi `--wait-url` 的预期契约。先明确需要的是“当前 URL 满足条件”还是
  “本次动作触发的导航达到条件”，再决定保留哪种接口。
...
- **B11、B12、B13：错误需要保留可行动信息。** P 使用异常类型、定位错误和
  带阶段信息的 call log，没有与 Demi 完全相同的四种动作进展协议。
...
- 选择复用层次，核实依赖、许可和所有 Host 平台的交付代价；不再以原始 CDP
  调用成功作为动作实现完整的依据。
- 明确普通动作、关联导航、显式 URL 等待和附带 metadata 的完成边界；
  尚未决定保留或移除 `--wait-url`，上一轮的移除建议只是提案。
- 选定输入语义：fill、逐字符输入、定向按键、当前焦点按键分别承诺什么；
  原生控件遵循已验证的浏览器语义，不自行纠正非法值。
- 设计更新后，按 B01–B20 对照实现和回归用例；B21 保留单独复现，
