...
- [Execution](#execution): browser distribution, command dispatch, conversation-scoped state, and the owners inside the service.
...
- [Command reference](#command-reference): the catalog, commands, example output, and result fields.
...
[Live browser view](live-view.md) owns how the user watches and
...
this design or Playwright behavior differs, follow the corresponding
...
The registry retains the public ID of each top-level page for the browser
...
bounded buffer. When that buffer overflows, the registry's owner reconciles the
registry from the browser's current target list before serving the next command; a lost
event costs one reconciliation, never a failed `tabs` call.

The registry's owner publishes a snapshot of the registry after every change.
A command finds its tab in the latest snapshot, without a call to Chrome and
without waiting for a command on another tab. Opening and closing a tab are
...
Its lifecycle uses one state, kept by the owner of the conversation's browser
([Owners inside the service](#owners-inside-the-service)):
...
on that page finishing its navigation before disappearing. The tab registry's
owner decides that the final tab has closed: it counts the tab creations still
in flight and the `content fetch` batches still collecting results, and
retires the environment only when no tab remains and none of those is in
flight. Once retirement begins, the environment admits no new tab; the next
`open` starts a fresh one.

After a failed startup or a lost browser has completed cleanup, the next explicit
`open` may create a fresh environment. Operations that failed are not
...
location. Screenshots already persisted into the transcript follow the
transcript's media storage contract.
...
Service shutdown releases every conversation the service holds
([Conversation-scoped state](../execution/native-runtime.md#conversation-scoped-state)),
and the service retires all of those browsers at the same time, not one after
another, within the runner's shutdown deadline
([Invoke and retire a service](../execution/native-runtime.md#invoke-and-retire-a-service)).
For example, when the runner loses its backend connection while four
conversations have browsers open, the four retirements run together, so
shutting down takes as long as the slowest of them rather than their sum. A
service that ends before its browsers are retired, because it crashed, was
killed, or overran that deadline, leaves them to the rules in
...
Brush on Host -> Declared-command dispatcher
               ...
               stdout / stderr / exit status
                         |
                         v
               Shell result and media handling
```

Declaration, argument conversion, help, and `--json` follow
[Commands](../execution/commands.md), which also defines
[help](../execution/commands.md#parse-input-and-render-help). Brush builtins call
...
The browser is [conversation-scoped state](../execution/native-runtime.md#conversation-scoped-state)
...
The service answers the runner's `status` request from the state each
conversation's browser publishes, without waiting for a startup or a
retirement in progress. A browser that takes seconds to retire therefore
never delays the answer.

### Owners inside the service

One service holds the browsers of every conversation on the Host, and they
progress independently: one conversation's slow retirement or busy page never
holds up another conversation's commands. Each piece of browser state has one
owner, a task that alone changes it; other tasks send the owner requests or
read a snapshot it publishes. The service runs these tasks on a multi-threaded
runtime ([Programs and threads](../architecture/concurrency.md#programs-and-threads)).

```text
Browser service
  conversations ............. finds each invocation's conversation browser;
   |                          forgets it after the conversation's release
   `-- conversation browser . one per conversation: absent | starting | ready | closing;
         |                    admits commands, runs the release, publishes its state
         `-- Chrome environment  one per running Chrome: process tree, profile,
               |                 CDP event pump
...
- The conversation browser keeps the lifecycle state of [Lifetime](#lifetime).
  Commands, live view invocations and the release reach the environment
  through it, and it publishes its state for the runner's `status`.
- The tab registry serves lookups from its snapshot, so commands on different
  tabs run in parallel ([One tab registry](#one-tab-registry)). Creating and
  closing a tab are requests to it, and it decides when the final tab has
  closed.
- Commands drive Chrome through the environment's one browser connection,
  which they share without taking turns; only retirement closes it. The
  debugging connections of `cdp` commands are separate
  ([CDP commands and events](#cdp-commands-and-events)).
- The capture channel and the live hub serve the
  [live view](live-view.md#parts).
- A tab's references, asset inventories and WebMCP tool sets belong to the
  ...
  connections.
- Every task started for an environment, including those that serve live view
  viewers, belongs to that environment, and retirement joins all of them
  before it removes the profile. No browser work runs detached or outlives the
  environment it served.
...
`content fetch`, and help operate without an existing tab. Actions
do not depend on a mutable globally selected tab.

Ordinary arguments use the command system's scalar and repeated-array
fields ([Commands](../execution/commands.md)). Multiple files use repeated `--file`, selected values use repeated
...
Clipboard write uses the command system's raw byte stdin channel so PNG bytes remain
...
page-provided tool schemas/results at entry: each is decoded into its type and
...
[Edit tracking](../execution/edit-tracking.md), not another recorder.
...
Each structured leaf declares `output.json`. The command system's `--json` path returns
...
| Cancellation | No success result | Cancellation or the shell's termination information | 130 when reportable |

Browser handlers own the error object `{error: {code, message, details?}}`.
Other Demi commands need not use that error format. Dispatcher-level usage
errors keep the common command diagnostic. The dispatcher validates successful
JSON stdout as it does for every command.
...
The browser contract defines these defaults and limits once:
...
`inspect` returns a full bounded snapshot each time. It has no
...
([Pixel ratio](live-view.md#pixel-ratio)); scaling keeps the image the
...
Waits do not use network-idle as a business completion signal.
Fixed delays use the shell's `sleep` command.
...
A tab's viewport has a mode ([Modes](live-view.md#modes)). In Web
...
not another Host access channel. Its supported version is pinned together with
...
Help is rendered from the declarations; the example is an excerpt. Do not maintain
...
and isolated storage. Never run tests that call real models. Acceptance runs
the suite that starts real Chrome, after installing the pinned release; it is
part of every acceptance that touches the browser, not an optional run.
...
5. Concurrent calls on one tab report busy; separate tabs progress independently,
   including while one of them waits for its deadline.
