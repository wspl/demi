...
uses belong to [Native runtime](../execution/native-runtime.md#user-streams),
[Runner](../execution/runner.md#service-streams),
[Sessions and targets](../execution/sessions-and-targets.md#host-operations) and
[Conversation idle](../execution/resource-lifecycle.md#activity); the routes belong to
[Web API](../product/web-api.md#user-streams). The gallery shows what the view looks like.
...
+--------------------+      | - - - - - - - - - - - |      |     live view module:           |
                            ...
                            | its pipes             |      |     pages and their observers   |
                            +-----------------------+      +---------------------------------+
```
...
- The runner and the backend carry bytes between the module and the page
  through a [user stream](../execution/native-runtime.md#user-streams). In the
  backend, the [user's shard](../architecture/concurrency.md#the-user-shard)
...
4. The runner invokes the declared operation on the resident `demi-commands`
   ...
   pipes.
5. Once the runner has opened the stream, the edge completes the WebSocket
   upgrade and relays bytes between the socket and the two pipes until the
   stream ends. A refusal before this step is an HTTP answer
   ([User streams](../product/web-api.md#user-streams)).
...
([Backend communication](../product/web-application.md#backend-communication)): pictures,
...
message: a four-byte length, a one-byte kind, and the payload. The edge
forwards bytes as they arrive; the page and the module split frames. The
module sends control messages and video frames; the page sends control
messages and file frames, which carry the bytes of files the user chose
...
A frame the protocol refuses is therefore a defect of its sender, and it ends
the view. A frame is refused when:

- its length is zero or above the frame limit;
- its kind is one its sender does not send;
- a video or file frame is shorter than its header;
- a control message is not JSON or does not match its schema.

The module ends a view whose page sends such a frame. The page reports one from
the module and ends the view like any other end. Each view reads its own
stream from the first byte, so a frame an ended stream left unfinished never
...
```text
page reads slowly
  -> the edge's WebSocket send waits, and the edge stops reading the output pipe
...
The edge honors its WebSocket's send backpressure: it waits for each send to
complete before it reads more from the output pipe, and the socket's write
buffer is bounded. A relay that ignores backpressure lets pictures pile up in
backend memory instead of dropping stale ones on the Host.
...
and the captures on the connection fail as they do when the extension
disconnects. The module never drops such a message and carries on.
...
- Watching costs Host CPU. A small Host shares its CPU between Chrome,
  ...
  The adaptation above measures delivery, not an encoder or Host CPU budget,
  and the design promises no capacity for a particular page or workload.
...
9. Retirement leaves no Chrome process, helper, profile or capture socket
   behind.
10. Failures reach the page. A frame the protocol refuses ends the view on the
    side that receives it, and the page's next view reads its stream from the
    first byte. A screen or viewport the module cannot apply arrives as a
...
| --- | --- |
| `builtin-protocol` | The live protocol: its message types, the frame kinds, the frame header and the frame limit; the capture extension's messages. The page receives the live protocol as generated TypeScript in `@demicodes/protocol`. |
