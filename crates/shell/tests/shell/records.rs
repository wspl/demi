...
use demi_shell::{CommandRecord, CommandState, Ending, Reader, TAIL_CHARS, final_stdout_boundary};
...
async fn views_deliver_each_stream_once_within_the_budget_between_characters() {
    ...
    let first = record.status(Reader::Model, 2, None);
    ...
    let rest = record.status(Reader::Model, 0, Some("Working".into()));
    ...
    let empty = record.status(Reader::Model, 0, None);
...
async fn an_exit_appends_what_it_adds_and_a_binary_stdout_is_presented_anew() {
    ...
    let seen = record.status(Reader::Model, 0, None);
    ...
    let exited = record.status(Reader::Model, 0, None);
    ...
    binary.status(Reader::Model, 0, None);
    ...
    let status = binary.status(Reader::Model, 0, None);
...
fn a_stop_after_the_streams_end_is_aborted_and_the_merged_view_follows_the_streams() {
    ...
    let status = record.status(Reader::Model, 0, None);
    ...
    record.mark_aborted();
    let status = record.status(Reader::Model, 1, None);
    assert!(matches!(status.state, CommandState::Aborted));
