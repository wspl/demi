...
fn new_record() -> CommandRecord {
    CommandRecord::new(ShellId::try_from("shell").unwrap(), CommandId::try_from("command").unwrap())
}
...
async fn views_deliver_each_stream_once_within_the_budget_between_characters() {
    ...
    let chunks: Vec<_> = rest.output.chunks.iter().map(|chunk| (chunk.stream, chunk.text.as_str())).collect();
    assert_eq!(chunks, [
        (StreamKind::Stdout, "éllo "),
        (StreamKind::Stderr, "warn\n"),
        (StreamKind::Stdout, "wörld\n"),
    ]);
    assert!(matches!(rest.state, CommandState::Running { hint: Some(ref hint) } if hint == "Working"));
...
async fn an_exit_appends_what_it_adds_and_a_binary_stdout_is_presented_anew() {
    ...
    record.settle(Ending::Exited(3), "head\n[gap]\ntail\n".into(), "oops\n".into(), None);
    let exited = record.status(0, None);
    assert_eq!(exited.output.text, "[gap]\ntail\noops\n");
    assert_eq!(exited.stdout.delta, "[gap]\ntail\n");
    assert!(matches!(exited.state, CommandState::Exited { exit_code: 3, binary_stdout: None }));

    let mut binary = new_record();
    binary.append_output(StreamKind::Stdout, "\u{fffd}PNG");
    binary.status(0, None);
    let (text, bytes) = final_stdout_boundary(Bytes::from_static(b"\x89PNG\r\n"), 4, Some("/out/stdout.txt"));
    assert_eq!(text, "<binary stdout: 6 bytes, exceeds the 4-byte binary limit; raw bytes at /out/stdout.txt>\n");
    ...
    let CommandState::Exited { binary_stdout: Some(stdout), .. } = status.state else {
    ...
    assert_eq!(text, "<binary stdout: 1 bytes; not kept beyond this view>\n");
    assert!(!bytes.unwrap().info.truncated);
    assert_eq!(final_stdout_boundary(Bytes::from_static(b"text"), 16, None), ("text".into(), None));
}
