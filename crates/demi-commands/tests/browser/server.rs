            ...
            loop {
                ...
                let work = async {
                    ...
                    let (status, headers, body) = match path {
                        "/load-replaced" => ("200 OK", "", "<!doctype html><img src='/stall'><script>setTimeout(() => location.href='/', 250)</script>"),
                        "/submitted-state" => ("200 OK", "", submitted.as_str()),
                        "/frame-child" => ("200 OK", "", "<!doctype html><label>Cross frame input<input id='cross-input'></label><button id='cross-button' onclick='this.textContent=\"Cross clicked\";console.info(\"cross-frame console\")'>Cross button</button><iframe id='nested' srcdoc=\"<label>Nested input<input id='nested-input'></label>\"></iframe>"),
