//! Browser clipboard command family.
...
/// Execute clipboard within the invoking conversation's browser ownership.
pub(super) async fn execute(
    ...
    Err(BrowserError::UnsupportedCapability(
        "clipboard implementation is pending the catalog checkpoint".into(),
    ))
}
