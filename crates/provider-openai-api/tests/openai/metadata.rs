...
async fn the_provider_names_its_entry_and_its_wire_and_is_ready_with_its_key() {
    ...
    assert_eq!((responses.id(), responses.display_name()), ("openai-work", "Work"));
    assert!(!responses.capabilities().process_host);
    assert_eq!(responses.auth_status().await, AuthState::Authenticated { account_label: None });
    let message = |state: RuntimeState| match state {
        RuntimeState::Ready { message } => message,
        other => panic!("{other:?}"),
    };
    assert_eq!(message(responses.runtime_state()).as_deref(), Some("Uses the OpenAI Responses API"));
    let chat = provider_at(&vendor, "/v1", WireApi::ChatCompletions, VendorPolicy::default());
    assert_eq!(message(chat.runtime_state()).as_deref(), Some("Uses the OpenAI Chat Completions API"));
...
async fn the_directory_is_built_in_with_the_newest_gpt_first() {
    ...
    assert_eq!(ids, ["gpt-5.5", "gpt-5.4", "gpt-5.4-mini", "gpt-5.3-codex-spark"]);
    ...
    assert_eq!((first.display_name.as_str(), first.context_window, first.output_limit), ("GPT-5.5", Some(272_000), None));
    assert_eq!(first.supported_thinking_efforts.as_deref(), Some(&["low", "medium", "high", "xhigh"].map(String::from)[..]));
    let tier = &first.service_tiers[0];
    assert_eq!((tier.id.as_str(), tier.label.as_str(), tier.fast), ("priority", "Fast", true));
