...
async fn reasoning_follows_the_thinking_setting_and_a_summary_turned_off_leaves_the_field_out() {
    let cases = [
        (
            effort("high", Some(ThinkingSummary::Off)),
            json!({ "effort": "high" }),
        ),
        (
            effort("high", None),
        ...
        (
            effort("low", Some(ThinkingSummary::Concise)),
    ...
    for thinking in [
        ...
        assert!(
            body_of(WireApi::Responses, NO_POLICY, request.clone())
                .await
                .get("reasoning")
                .is_none()
        );
        assert!(
            body_of(WireApi::ChatCompletions, NO_POLICY, request)
                .await
...
async fn assistant_status_is_replayed_only_when_the_vendor_policy_asks() {
    ...
    assert_eq!(
        lean["input"][1],
        json!({ "type": "message", "role": "assistant", "content": [{ "type": "output_text", "text": "prior reply", "annotations": [] }] })
    );
...
async fn user_media_rides_as_images_and_files() {
    ...
    let request = || {
        request_with(vec![InferenceItem::UserMessage {
...
async fn media_a_tool_returned_follows_its_output_in_a_user_message() {
    ...
    let responses = body_of(
        ...
        NO_POLICY,
        request_with(vec![
            tool_use("call-1|fc-1", "shot.png"),
            tool_result("call-1|fc-1", output.clone()),
        ]),
    )
    .await;
    assert_eq!(
    ...
    let chat = body_of(
        ...
        NO_POLICY,
        request_with(vec![
            tool_use("call-1", "shot.png"),
            tool_result("call-1", output),
        ]),
    )
    .await;
    assert_eq!(
...
async fn thinking_is_replayed_as_reasoning_content_only_when_the_vendor_policy_asks() {
    ...
    assert_eq!(
        kept[1],
        json!({ "role": "assistant", "content": null, "tool_calls": call, "reasoning_content": "inspect first" })
    );
