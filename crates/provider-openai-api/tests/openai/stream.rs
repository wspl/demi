...
async fn events_of(wire: WireApi, response: MockResponse) -> Vec<ProviderEvent> {
    ...
    run(runtime(&vendor, wire, VendorPolicy::default()).as_mut(), inference_request()).await
}
...
async fn the_responses_runtime_maps_the_stream_the_endpoint_sends_back() {
    ...
    assert_eq!(events, [ProviderEvent::TextDelta("hi".into()), ProviderEvent::Response(usage)]);
}
...
async fn a_malformed_stream_event_fails_the_run_naming_its_field() {
    let body = sse_body(&[json!({ "type": "response.output_item.done", "item": { "type": "message", "content": 42 } })]);
    let events = events_of(WireApi::Responses, MockResponse::event_stream(body)).await;
    let [ProviderEvent::Error(failure)] = events.as_slice() else {
        panic!("{events:?}");
    };
    assert_eq!(failure.code, None);
    assert!(failure.message.starts_with("OpenAI API stream sent a frame Demi cannot read"), "{}", failure.message);
    assert!(failure.message.contains("content"), "{}", failure.message);
    assert_eq!(failure.diagnostics.as_ref().unwrap().source, FailureSource::Stream);
}
...
async fn a_refused_request_fails_with_the_vendor_record_and_the_wait_it_names() {
    ...
    for wire in [WireApi::Responses, WireApi::ChatCompletions] {
        let events = events_of(wire, MockResponse::status(429).header("retry-after", "20").chunk(body)).await;
        let [ProviderEvent::Error(failure)] = events.as_slice() else {
            panic!("{events:?}");
        };
        assert_eq!(failure.message, format!("OpenAI API request failed with HTTP 429: {body}"));
        assert_eq!((failure.code.clone(), failure.retry_after), (Some(ErrorCode::RateLimit), Some(Duration::from_secs(20))));
