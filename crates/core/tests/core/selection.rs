...
fn accepted_types_come_from_the_catalog_or_from_its_flags() {
    ...
    let both: Vec<FileExtension> = ATTACHMENT_FILE_EXTENSIONS
        .into_iter()
    ...
    assert_eq!(
        with(Some(false), Some(true), None),
        Some(VIDEO_FILE_EXTENSIONS.to_vec())
    );
    assert_eq!(
        with(None, Some(true), None),
        Some(VIDEO_FILE_EXTENSIONS.to_vec())
    );
    // The exact list a catalog states wins over its flags, an empty one too.
    assert_eq!(
        with(Some(true), None, Some(vec![FileExtension::Png])),
        Some(vec![FileExtension::Png])
    );
    assert_eq!(
        with(Some(true), Some(true), Some(Vec::new())),
        Some(Vec::new())
    );
}
