...
async fn the_provider_names_its_entry_and_is_ready_with_its_key() {
    ...
    assert_eq!((provider.id(), provider.display_name()), ("google-work", "Work"));
    assert!(!provider.capabilities().process_host);
    assert_eq!(provider.auth_status().await, AuthState::Authenticated { account_label: None });
    assert_eq!(provider.runtime_state(), RuntimeState::Ready { message: Some("Uses the Gemini generateContent API".into()) });
...
async fn the_directory_is_built_in_and_reads_video() {
    ...
    assert_eq!(ids, ["gemini-3.6-flash", "gemini-3.5-flash", "gemini-3.1-pro-preview", "gemini-2.5-flash"]);
    ...
    assert_eq!((first.context_window, first.output_limit), (Some(1_048_576), Some(65_536)));
    assert_eq!((first.supports_video, first.default_thinking_effort.as_deref()), (Some(true), Some("medium")));
