...
Service tiers name their Fast Mode tier: the web UI's Fast switch only appears for models that carry a Fast tier and only ever writes that tier's id. A model switch clears the tier instead of carrying a foreign id into the next provider.
