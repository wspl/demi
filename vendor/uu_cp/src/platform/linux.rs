...
use uucore::context::io::{self, Read};
...
fn clone(
    ...
    let dest_existed = matches!(fallback, CloneFallback::Error) && dest.context_symlink_metadata().is_ok();
        ...
        return match fallback {
            CloneFallback::Error => {
                ...
                if !dest_existed && path_still_refers_to(dest, &dst_file) {
                    let _ = uucore::context::fs::remove_file(dest);
                }
...
fn path_still_refers_to(path: &Path, file: &File) -> bool {
    let (Ok(current), Ok(opened)) = (path.context_symlink_metadata(), file.metadata()) else {
        return false;
    };
...
fn check_dest_is_fifo(dest: &Path) -> bool {
    ...
    uucore::context::fs::metadata(dest).is_ok_and(|f| f.file_type().is_fifo())
}
