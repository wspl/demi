...
fn cluster_size(dest: &Path) -> uucore::context::io::Result<usize> {
...
fn read_full(reader: &mut impl Read, buf: &mut [u8]) -> uucore::context::io::Result<usize> {
    let mut total = 0;
    while total < buf.len() {
        match reader.read(&mut buf[total..]) {
            Ok(0) => break,
            Ok(n) => total += n,
            Err(e) if e.kind() == uucore::context::io::ErrorKind::Interrupted => {}
            Err(e) => return Err(e),
        }
    }
    Ok(total)
...
fn write_all_at(file: &File, mut buf: &[u8], mut offset: u64) -> uucore::context::io::Result<()> {
    while !buf.is_empty() {
        match file.seek_write(buf, offset) {
            Ok(0) => {
                return Err(uucore::context::io::Error::new(
                    uucore::context::io::ErrorKind::WriteZero,
...
enum SparseCopyError {
    ...
    Unsupported,
    Io(uucore::context::io::Error),
}

impl From<uucore::context::io::Error> for SparseCopyError {
    fn from(e: uucore::context::io::Error) -> Self {
        Self::Io(e)
    }
}
...
fn plain_copy(src_file: &mut File, dest: &Path) -> uucore::context::io::Result<()> {
    ...
    let mut dst_file = OpenOptions::new()
        ...
        .share_mode(FILE_SHARE_READ)
        .open(dest)?;
    uucore::context::io::copy(src_file, &mut dst_file)?;
    Ok(())
}
...
fn sparse_copy_or_plain(src_file: &mut File, dest: &Path) -> uucore::context::io::Result<SparseDebug> {
...
fn is_sparse(file: &File) -> uucore::context::io::Result<bool> {
    ...
    match sparse_mode {
        ...
        SparseMode::Auto => {
            ...
            if is_sparse(&src_file).map_err(context_err)? {
                ...
                uucore::context::fs::copy(source, dest).map_err(context_err)?;
            }
        }
...
mod tests {
    ...
    impl Read for ShortReader<'_> {
        fn read(&mut self, buf: &mut [u8]) -> uucore::context::io::Result<usize> {
    ...
    fn write_all_at_writes_at_offset() {
        ...
        drop(file);
        let contents = uucore::context::fs::read(&path).unwrap();
