...
use uucore::context::fs::{self, exists};
...
impl<'a> Context<'a> {
    fn new(root: &'a Path, target: &'a Path) -> io::Result<Self> {
        ...
        let target_is_file = target.context_is_file();
        let root_parent =
            if target.context_exists() && !root.as_os_str().as_encoded_bytes().ends_with(b"/.") {
                root_path.parent().map(ToOwned::to_owned)
            } else if root == Path::new(".") && target.context_is_dir() {
...
impl Entry {
    fn new<A: AsRef<Path>>(
        ...
        if no_target_dir {
            let source_is_dir = source.context_is_dir();
        ...
        } else if context.root == Path::new(".") && context.target.context_is_dir() {
            // Special case: when copying current directory (.) to an existing directory,
...
fn copy_direntry(
    ...
    let source_is_dir = if source_is_symlink && !options.dereference {
        ...
        entry.source_absolute.context_is_dir()
    } else {
    ...
    let dest_is_symlink = entry.local_to_target.context_is_symlink();

    // If the source is a directory and the destination does not
    // exist, ...
    if source_is_dir && (dest_is_symlink || !entry.local_to_target.context_exists()) {
...
pub(crate) fn copy_directory(
    ...
    if !options.dereference(source_in_command_line) && root.context_is_symlink() {
        return copy_file(
            progress_bar,
            root,
            target,
        ...
        if let Some(parent) = root.parent() {
            ...
            if options.verbose {
                ...
                for (x, y) in aligned_ancestors(root, &target.join(root)) {
                    uucore::context_println!("{} -> {}", x.display(), y.display());
                }
            }
    ...
    for direntry_result in WalkDir::new(uucore::context::resolve(root))
        ...
        match direntry_result {
            Ok(direntry) => {
                ...
                let (entry_is_symlink, entry_is_dir_no_follow) =
                    match direntry_path.context_symlink_metadata() {
                        Ok(metadata) => {
                            let file_type = metadata.file_type();
                            (file_type.context_is_symlink(), file_type.context_is_dir())
                        }
                        Err(_) => (direntry_type.context_is_symlink(), direntry_type.context_is_dir()),
                    };
                ...
                let is_dir_for_permissions =
                    entry_is_dir_no_follow || (options.dereference && direntry_path.context_is_dir());
