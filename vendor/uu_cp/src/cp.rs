...
use std::fmt::Display;
use uucore::context::fs::{self, Metadata, OpenOptions, Permissions};
#[cfg(unix)]
use std::os::unix::fs::{FileTypeExt, MetadataExt, PermissionsExt};
#[cfg(unix)]
use std::os::unix::net::UnixListener;
use std::path::{Path, PathBuf, StripPrefixError};
use {std::fmt, uucore::context::io};
...
impl Options {
    ...
    fn from_matches(matches: &ArgMatches) -> CopyResult<Self> {
        ...
        let backup_mode =
            backup_control::determine_backup_mode(uucore::context::env::var("VERSION_CONTROL").ok(), matches)
...
impl TargetType {
    ...
    fn determine(sources: &[PathBuf], target: &Path) -> Self {
        if sources.len() > 1 || target.context_is_dir() {
            Self::Directory
        } else {
            Self::File
        }
...
pub fn copy(sources: &[PathBuf], target: &Path, options: &Options) -> CopyResult<()> {
    ...
    for source in sources {
        ...
        if options.backup == BackupMode::None && seen_sources.contains(&normalized_source) {
            let file_type = if source.context_symlink_metadata()?.file_type().context_is_dir() {
                "directory"
            } else {
                "file"
            };
            ...
            if FileInformation::from_path(&dest, true).is_ok()
                && !fs::symlink_metadata(&dest).is_ok_and(|m| m.file_type().context_is_symlink())
                // if both `source` and `dest` are symlinks, it should be considered as an overwrite.
                || fs::metadata(source).is_ok()
                    && fs::symlink_metadata(source)?.file_type().context_is_symlink()
                || matches!(options.copy_mode, CopyMode::SymLink)
            {
                ...
                if copied_destinations.contains(&dest) && options.backup != BackupMode::Numbered {
                    ...
                    let dest_is_dir = fs::metadata(&dest).is_ok_and(|m| m.context_is_dir());
                    let source_is_dir = fs::metadata(source).is_ok_and(|m| m.context_is_dir());
...
fn construct_dest_path(
    ...
    if options.no_target_dir && target.context_is_dir() {
        return Err(
    ...
    if options.parents && !target.context_is_dir() {
    ...
    Ok(match target_type {
        TargetType::Directory => {
            let root = if options.parents {
                ...
                if source_path == Path::new(".") && target.context_is_dir() {
...
fn copy_source(
    ...
    if source_path.context_is_dir() && (options.dereference || !source_path.context_is_symlink()) {
        // Copy as directory
        copy_directory(
            progress_bar,
            source,
...
impl OverwriteMode {
    fn verify(self, path: &Path, debug: bool) -> CopyResult<()> {
        match self {
            Self::NoClobber => {
                if debug {
                    uucore::context_println!("{}", translate!("cp-debug-skipped", "path" => path.quote()));
                }
...
fn copy_extended_attrs(source: &Path, dest: &Path, skip_selinux: bool) -> CopyResult<()> {
    ...
    } else if metadata.context_is_file() {
...
fn is_forbidden_to_copy_to_same_file(
    ...
    let source_is_symlink = source.context_is_symlink();
    let dest_is_symlink = dest.context_is_symlink();
...
fn handle_existing_dest(
    ...
    if options.update == UpdateMode::None {
        if options.debug {
            uucore::context_println!("skipped {}", dest.quote());
        }
    ...
    if let Some(backup_path) = backup_path {
        ...
        is_dest_removed = dest.context_is_symlink();
...
fn print_paths(parents: bool, source: &Path, dest: &Path) -> CopyResult<()> {
    ...
    // disk) surfaces as one error instead of panicking inside uucore::context_println!.
...
fn handle_copy_mode(
    ...
    match options.copy_mode {
        CopyMode::Link => {
            if dest.context_exists() {
                let backup_path =
                    backup_control::get_backup_path(options.backup, dest, &options.backup_suffix);
                if let Some(backup_path) = backup_path {
                    backup_dest(dest, &backup_path, dest.context_is_symlink())?;
                    fs::remove_file(dest)?;
                }
            ...
            if options.dereference(source_in_command_line) && source.context_is_symlink() {
                let resolved =
        ...
        CopyMode::SymLink => {
            if dest.context_exists() && options.overwrite == OverwriteMode::Clobber(ClobberMode::Force) {
                fs::remove_file(dest)?;
            }
...
fn copy_file(
    ...
    let source_is_symlink = source.context_is_symlink();
    let initial_dest_metadata = dest.context_symlink_metadata().ok();
    let dest_is_symlink = initial_dest_metadata
        .as_ref()
        .is_some_and(|md| md.file_type().context_is_symlink());
    let dest_target_exists = dest.context_try_exists().unwrap_or(false);
    ...
    if dest_is_symlink {
        ...
        if copy_contents
            ...
            && uucore::context::env::var_os("POSIXLY_CORRECT").is_none()
        {
            return Err(CpError::Error(
    ...
    let dest_metadata = dest.context_symlink_metadata().ok();
    ...
    } else if source_is_stream && !source.context_exists() {
...
fn copy_helper(
    ...
    if path_ends_with_terminator(dest) && !dest.context_is_dir() {
    ...
    if source_metadata.context_is_symlink() {
...
fn copy_fifo(dest: &Path, overwrite: OverwriteMode, debug: bool) -> CopyResult<()> {
    ...
    nix::unistd::mkfifo(&uucore::context::resolve(dest), Mode::from_bits_truncate(0o666))
...
fn copy_link(
    ...
    if dest.context_is_symlink() || dest.context_is_file() {
        delete_path(dest, options)?;
    }
...
pub fn verify_target_type(target: &Path, target_type: TargetType) -> CopyResult<()> {
    match (target_type, target.context_is_dir()) {
...
fn disk_usage(paths: &[PathBuf], recursive: bool) -> io::Result<u64> {
    ...
    for p in paths {
        ...
        if md.file_type().context_is_dir() {
            if recursive {
...
fn disk_usage_directory(p: &Path) -> io::Result<u64> {
    ...
    for entry in fs::read_dir(p)? {
        let entry = entry?;
        if entry.file_type()?.context_is_dir() {
