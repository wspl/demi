...
use {std::char, std::error::Error, std::fmt::Debug, std::fmt::Display, uucore::context::io::BufRead, uucore::context::io::Write, std::ops::Range};
    ...
    W: Write,
{
    ...
    loop {
        let length = match input.read(&mut buf[..]) {
            ...
            Ok(len) => len,
            Err(e) if e.kind() == uucore::context::io::ErrorKind::Interrupted => continue,
...
pub fn flush_output<W: Write>(output: &mut W) -> UResult<()> {
    ...
    match output.flush() {
        Ok(()) => Ok(()),
        Err(err) if err.kind() == uucore::context::io::ErrorKind::BrokenPipe => {
            uucore::context::process::exit(13);
        }
