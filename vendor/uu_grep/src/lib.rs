...
use uucore::context::io::{IsTerminal as _, Read};
use std::path::Path;
...
/// nothing.
fn read_error(label: impl std::fmt::Display, err: &uucore::context::io::Error) -> Box<dyn UError> {
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let grep_color = uucore::context::env::var("GREP_COLOR").unwrap_or_default();
    let grep_colors = uucore::context::env::var("GREP_COLORS").unwrap_or_default();
    ...
    let mut patterns = Vec::new();
    {
        ...
        for path in file_pattern {
            let contents = if *path == "-" {
                ...
                uucore::context::fs::read_to_string(path).map_err(|err| read_error(path, &err))?
            };
    ...
    let exclude_globs = {
        ...
        for path in exclude_from {
            let contents = uucore::context::fs::read_to_string(path).map_err(|err| read_error(path, &err))?;
    ...
    let show_filename = if with_filename {
        ...
        match file_args {
            ...
            [one] if one.to_str() != Some("-") => {
                directory_mode == DirectoryMode::Recurse && Path::new(one).context_is_dir()
            }
            [_] => false,
            _ => true,
        }
    ...
    let use_color = match color {
        ...
        ColorMode::Auto => uucore::context::io::stdout().is_terminal(),
    };
