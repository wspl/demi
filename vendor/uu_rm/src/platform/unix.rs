...
use uucore::context::io::{IsTerminal, stdin};
...
pub fn safe_remove_file(
    ...
    match dir_fd.unlink_at(file_name, false) {
        ...
        Err(e) => {
            if e.kind() == uucore::context::io::ErrorKind::PermissionDenied {
...
fn handle_error_with_force(e: uucore::context::io::Error, path: &Path, options: &Options) -> bool {
...
pub fn safe_remove_dir_recursive(
    ...
    let (initial_mode, root_dev, root_ino) = match fs::symlink_metadata(path) {
        Ok(metadata) if !metadata.context_is_dir() => {
...
pub fn safe_remove_dir_recursive_impl(
    ...
    let entries = match dir_fd.read_dir() {
        Ok(entries) => entries,
        Err(e) if e.kind() == uucore::context::io::ErrorKind::PermissionDenied => {
            if !options.force {
                show_permission_denied_error(path);
            }
