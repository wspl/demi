...
use uucore::context::io::{self, IsTerminal, Write, stdin};
use std::ops::BitOr;
#[cfg(unix)]
...
/// panicking like `uucore::context_println!` would.
...
fn count_files(paths: &[&OsStr], recursive: bool) -> u64 {
    ...
    for p in paths {
        ...
        if let Ok(md) = fs::symlink_metadata(path) {
            if md.context_is_dir() && !is_symlink_dir(&md) {
                if recursive {
    ...
    let entries_count = fs::read_dir(p).map_or(0, |entries| {
        entries
            ...
            .map(|entry| match entry.file_type() {
                Ok(ft) if ft.context_is_dir() => count_files_in_directory(&entry.path()),
                Ok(_) => 1,
                Err(_) => 0,
            })
            .sum()
...
pub fn remove(files: &[&OsStr], options: &Options) -> bool {
    ...
    for filename in files {
        ...
        had_err = match file.context_symlink_metadata() {
            Ok(metadata) => {
...
fn remove_dir_recursive(
    ...
    if !path.context_is_dir() || path.context_is_symlink() {
...
fn is_root_path(path: &Path) -> bool {
    ...
    if let Ok(canonical) = path.context_canonicalize() {
...
fn prompt_file(path: &Path, options: &Options) -> bool {
    ...
    if metadata.context_is_symlink() {
...
fn is_symlink_dir(metadata: &Metadata) -> bool {
    ...
    metadata.file_type().context_is_symlink()
