...
use uucore::context::io::{Error, ErrorKind};
#[cfg(unix)]
...
fn is_first_filename_timestamp(
    ...
    timestamp.is_none()
        ...
        && matches!(uucore::context::env::var("_POSIX2_VERSION").as_deref(), Ok("199209"))
...
fn create_without_truncate(path: &Path) -> uucore::context::io::Result<fs::File> {
    OpenOptions::new()
        .write(true)
        .create(true)
        .truncate(false)
...
fn touch_file(
    ...
    if let Err(e) = metadata_result {
        ...
        if let Err(e) = create_without_truncate(path) {
            ...
            // we cannot use path.context_is_dir() because it calls fs::metadata which we already called
            // when stable, we can change to use e.kind() == uucore::context::io::ErrorKind::IsADirectory
...
fn update_times(
    ...
    if opts.no_deref && !is_stdout {
        return set_symlink_file_times(uucore::context::resolve(path), atime, mtime).map_err_context(
    ...
    #[cfg(unix)]
    {
        if is_stdout {
            ...
            return futimens(uucore::context::io::stdout(), &timestamps)
    ...
    #[cfg(not(unix))]
    {
        set_file_times(uucore::context::resolve(path), atime, mtime).map_err_context(
            || translate!("touch-error-setting-times-of-path", "path" => path.quote()),
        )
    }
}
...
fn set_times_by_path(path: &Path, atime: FileTime, mtime: FileTime) -> UResult<()> {
    set_file_times(uucore::context::resolve(path), atime, mtime)
        .map_err_context(|| translate!("touch-error-setting-times-of-path", "path" => path.quote()))
}
...
fn try_futimens_via_write_fd(path: &Path, atime: FileTime, mtime: FileTime) -> uucore::context::io::Result<()> {
...
fn stat(path: &Path, follow: bool) -> uucore::context::io::Result<(FileTime, FileTime)> {
...
mod tests {
    ...
    fn test_try_futimens_via_write_fd_sets_times() {
        ...
        uucore::context::fs::write(&path, b"data").unwrap();
        ...
        let metadata = uucore::context::fs::metadata(&path).unwrap();
    ...
    fn create_without_truncate_does_not_truncate_existing_file() {
        ...
        uucore::context::fs::write(&path, b"do not truncate me").unwrap();

        super::create_without_truncate(&path).unwrap();

        assert_eq!(uucore::context::fs::read(&path).unwrap(), b"do not truncate me");
    }
    ...
    fn create_without_truncate_creates_missing_file() {
        ...
        assert_eq!(uucore::context::fs::read(&path).unwrap(), b"");
    }
}
