...
use crate::Config;
use std::fmt;
use uucore::context::io::{BufWriter, Stdout, Write};
...
pub fn print_dired_output(
    ...
    uucore::context_println!("//DIRED-OPTIONS// --quoting-style={}", config.quoting_style);
    Ok(())
}

/// Helper function to print positions with a given prefix.
fn print_positions(prefix: &str, positions: &[BytePosition]) {
    uucore::context_print!("{prefix}");
    for c in positions {
        uucore::context_print!(" {c}");
    }
...
pub fn is_dired_arg_present() -> bool {
    uucore::context::env::args_os().any(|x| x == "--dired" || x == "-D")
}
