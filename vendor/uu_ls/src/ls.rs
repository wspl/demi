...
use {std::cell::OnceCell, std::cmp::Reverse, std::ffi::OsStr, std::ffi::OsString, uucore::context::fs, uucore::context::fs::DirEntry, uucore::context::fs::FileType, uucore::context::fs::Metadata, uucore::context::fs::ReadDir, uucore::context::io::BufWriter, uucore::context::io::ErrorKind, uucore::context::io::Stdout, uucore::context::io::Write, uucore::context::io::stdout, std::ops::RangeInclusive, std::path::Path, std::path::PathBuf, std::time::Duration, std::time::SystemTime, std::time::UNIX_EPOCH};
use thiserror::Error;
...
enum LsError {
    ...
    IOError(#[from] uucore::context::io::Error),

    #[error("{}: {}", translate!("common-write-error"), strip_errno(.0))]
    WriteError(uucore::context::io::Error),
    ...
    IOErrorContext(PathBuf, uucore::context::io::Error, bool),
            ...
            .get_or_init(|| {
                ...
                match get_metadata_with_deref_opt(self.path(), self.must_dereference) {
                    Err(err) => {
                        ...
                        let mut out: uucore::context::io::StdoutLock<'static> = stdout().lock();
                        let _ = out.flush();
                        ...
                        if self.must_dereference
                            && errno == 9i32
                            && let Ok(file) = self.path().context_read_link()
                        {
                            return file.context_symlink_metadata().ok();
                        }
...
///     uucore::context_println!("{}: {:?}", entry.display_name.to_string_lossy(), entry.file_type);
/// }
/// ```
pub fn list_with_output<O: LsOutput>(
    ...
    for loc in locs {
        ...
        let show_dir_contents = if let Some(ft) = path_data.file_type() {
            !config.directory && ft.context_is_dir()
        } else {
            set_exit_code(1);
            false
        };
...
fn dotdot_path(parent: &Path) -> PathBuf {
    ...
    #[cfg(target_os = "wasi")]
    if uucore::context::fs::metadata(&dotdot).is_err() {
            ...
            !match ft {
                None => {
                    ...
                    get_metadata_with_deref_opt(&p.p_buf, true)
                        .map_or_else(|_| false, |m| m.context_is_dir())
                }
                Some(ft) => ft.context_is_dir(),
            }
        });
    }
}
