...
pub(crate) fn current_open_fd_count() -> Option<usize> {
    fn count_dir(path: &str) -> Option<usize> {
        let entries = uucore::context::fs::read_dir(path).ok()?;
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    #[cfg(not(target_os = "wasi"))]
    {
        ...
        let pool = rayon::ThreadPoolBuilder::new()
            ...
            .spawn_handler(|thread| {
                uucore::context::thread::spawn(move || thread.run());
                Ok(())
            })
            .build()
            .map_err(|error| USimpleError::new(2, error.to_string()))?;
        settings.pool = Some(std::sync::Arc::new(pool));
    }
    ...
    let mut tmp_dir = TmpDirWrapper::new(matches.get_one::<String>(options::TMP_DIR).map_or_else(
        || {
            // WASI does not support uucore::context::env::temp_dir() — it panics with
    ...
    exec(&mut files, opened_inputs, &settings, output, &mut tmp_dir)
}
