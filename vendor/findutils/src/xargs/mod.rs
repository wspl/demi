...
use {std::collections::HashMap, std::error::Error, std::ffi::OsStr, std::ffi::OsString, std::fmt::Display, uucore::context::fs, uucore::context::io, uucore::context::io::BufRead, uucore::context::io::BufReader, uucore::context::io::Read, uucore::context::process::Command, uucore::context::process::Stdio};
...
impl MaxCharsCommandSizeLimiter {
    ...
    fn new_system(_env: &HashMap<OsString, OsString>) -> MaxCharsCommandSizeLimiter {
        ...
        // uucore::context::process unconditionally surrounds the program name with quotes.
...
impl CommandBuilder<'_> {
    ...
    fn execute(self) -> Result<CommandResult, CommandExecutionError> {
        ...
        if self.options.verbose {
            uucore::context_eprintln!("{command:?}");
        }
...
fn do_xargs(args: &[&str]) -> Result<CommandResult, XargsError> {
    ...
    let matches = match matches {
        ...
        Err(e) => match e.kind() {
            ErrorKind::DisplayHelp | ErrorKind::DisplayVersion => {
                // The help/version text already has a newline, so use `uucore::context_print!` here, not `uucore::context_println!`
                uucore::context_print!("{e}");
    ...
    let env = uucore::context::env::vars_os().collect();
...
pub fn xargs_main(args: &[&str]) -> i32 {
    match do_xargs(args) {
        ...
        Err(e) => {
            uucore::context_eprintln!("Error: {e}");
