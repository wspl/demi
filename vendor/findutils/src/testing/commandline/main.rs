...
use uucore::context::io::{stdin, stdout, Read, Write};
use std::path::PathBuf;

fn usage() -> ! {
    uucore::context_println!("Simple command-line app just used for testing -exec flags!");
    uucore::context::process::exit(2);
}
...
fn main() {
    ...
    match config.exit_with {
        None => uucore::context::process::exit(0),
        Some(ExitWith::Failure) => uucore::context::process::exit(2),
        Some(ExitWith::UrgentFailure) => uucore::context::process::exit(255),
        #[cfg(unix)]
