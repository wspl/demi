...
use uucore::context::fs::{self, Metadata};
use uucore::context::io::{self, ErrorKind};
#[cfg(unix)]
...
impl FileType {
    pub fn context_is_dir(self) -> bool {
        self == Self::Directory
    }

    pub fn context_is_file(self) -> bool {
        self == Self::Regular
    }

    pub fn context_is_symlink(self) -> bool {
        self == Self::Symlink
    }
}
...
impl From<fs::FileType> for FileType {
    fn from(t: fs::FileType) -> Self {
        ...
        if t.context_is_file() {
            return Self::Regular;
        }
        if t.context_is_symlink() {
            return Self::Symlink;
        }
...
impl From<&walkdir::Error> for WalkError {
    fn from(e: &walkdir::Error) -> Self {
        Self {
            ...
            depth: Some(e.depth()),
            raw: e.io_error().and_then(uucore::context::io::Error::raw_os_error),
        }
    }
}
...
impl WalkEntry {
    /// Preserve command-line path spelling while traversal opens absolute paths.
    pub fn with_lexical_root(mut self, root: &Path, display: &Path) -> Self {
        let relative = self.path().strip_prefix(root).expect("walk entry below root");
        let path = if relative.as_os_str().is_empty() { display.to_owned() } else { display.join(relative) };
        self.inner = Entry::Explicit(path, self.depth());
        self
    }
        ...
        match result {
            ...
            Err(e) if e.is_not_found() => {
                ...
                if let (Some(path), Some(depth)) = (e.path(), e.depth()) {
                    if let Ok(meta) = path.context_symlink_metadata() {
                        return Ok(Self {
                            inner: Entry::Explicit(path.into(), depth),
                            follow: Follow::Never,
                            meta: Ok(meta).into(),
            ...
            Entry::Explicit(path, _) => {
                if self.follow() {
                    path.context_symlink_metadata()
                        .is_ok_and(|m| m.file_type().context_is_symlink())
                } else {
                    self.file_type().context_is_symlink()
                }
            }
