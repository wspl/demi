...
use uucore::context::io::{self, stderr, Write};
...
impl DeleteMatcher {
    ...
    fn delete(&self, entry: &WalkEntry) -> io::Result<()> {
        if entry.file_type().context_is_dir() && !entry.path_is_symlink() {
...
impl Matcher for DeleteMatcher {
    fn matches(&self, file_info: &WalkEntry, matcher_io: &mut MatcherIO) -> bool {
        ...
        // POSIX rmdir() not accepting "." (EINVAL). uucore::context::fs::remove_dir()
...
mod tests {
    ...
    fn delete_matcher() {
        ...
        assert!(
            !temp_dir.path().join("test").context_exists(),
        ...
        assert!(
            !temp_dir.path().join("test_dir").context_exists(),
