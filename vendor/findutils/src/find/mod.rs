...
use uucore::context::io::{self, stderr, stdout, BufRead, BufReader, Write};
...
pub struct StandardDependencies {
    ...
    tty: Option<RefCell<BufReader<uucore::context::fs::File>>>,
}
...
impl StandardDependencies {
    ...
    pub fn new() -> Self {
        #[cfg(unix)]
        let tty = if uucore::context::io::stdin().is_terminal() {
            uucore::context::fs::File::open("/dev/tty")
                .ok()
...
impl Dependencies for StandardDependencies {
    ...
    fn confirm(&self, prompt: &str) -> bool {
        ...
        let response = if let Some(tty) = &self.tty {
            ...
            uucore::context::io::stdin().lock().lines().next()
        }
        .and_then(Result::ok)
        .unwrap_or_default();
...
fn process_dir(
    ...
    let root = uucore::context::resolve(dir);
    ...
    while let Some(result) = it.next() {
        match WalkEntry::from_walkdir(result, config.follow).map(|entry| entry.with_lexical_root(&root, std::path::Path::new(dir))) {
            Err(err) => {
                ret = 1;
...
mod tests {
    ...
    fn find_mtime() {
        ...
        // so skip tests that won't pass due to shortcomings in uucore::context::fs.
    ...
    fn test_no_permission_file_error() {
        use {std::path::Path, uucore::context::process::Command};

        let path = Path::new("./test_data/no_permission");
        let _result = fs::create_dir(path);
        // Generate files without permissions.
        // uucore::context::fs cannot change file permissions to 000 in normal user state,
    ...
    fn test_user_predicate() {
        use {std::os::unix::fs::MetadataExt, std::path::Path};
