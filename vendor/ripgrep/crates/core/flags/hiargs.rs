...
impl HiArgs {
    ...
    pub(crate) fn from_low_args(mut low: LowArgs) -> anyhow::Result<HiArgs> {
        ...
        crate::debug!("using {threads} thread(s)");
                ...
                MmapMode::Auto => {
                    if paths.paths.len() <= 10
                        && paths
                            .paths
                            .iter()
                            .all(|p| uucore::context::resolve(p).is_file())
                    {
    ...
    /// stdout.
    pub(crate) fn buffer_writer(&self) -> crate::invocation::BufferWriter {
        crate::invocation::BufferWriter::new(
            self.color.to_termcolor(),
            self.file_separator.clone(),
        )
    }
    ...
    pub(crate) fn haystack_builder(&self) -> anyhow::Result<HaystackBuilder> {
        ...
        I: Iterator<Item = Haystack> + 'a,
    {
        ...
        fn attach_timestamps(
            ...
            haystacks.map(move |s| {
                let time = uucore::context::resolve(s.path())
                    .metadata()
                    .and_then(|m| get(&m))
                    .ok();
                (s, time)
    ...
    pub(crate) fn stdout(&self) -> crate::invocation::Writer {
        let line = matches!(self.buffer, BufferMode::Line)
            || matches!(self.buffer, BufferMode::Auto)
                && self.is_terminal_stdout;
        crate::invocation::stdout(self.color.to_termcolor(), line)
    }
    ...
    pub(crate) fn walk_builder(&self) -> anyhow::Result<ignore::WalkBuilder> {
        let mut builder = ignore::WalkBuilder::new(
            crate::invocation::walk_path(&self.paths.paths[0]),
        );
        for path in self.paths.paths.iter().skip(1) {
            builder.add(crate::invocation::walk_path(path));
        }
        if !self.no_ignore_files {
            for path in self.ignore_file.iter() {
                if let Some(err) =
                    builder.add_ignore(uucore::context::resolve(path))
                {
                    ignore_message!("{err}");
                }
            }
...
impl State {
    ...
    fn new() -> anyhow::Result<State> {
        use uucore::context::io::IsTerminal;

        let cwd = current_dir()?;
        crate::debug!("read CWD from environment: {}", cwd.display());
        Ok(State {
            is_terminal_stdout: uucore::context::io::stdout().is_terminal(),
                ...
                PatternSource::File(path) => {
                    if path == Path::new("-") {
                        ...
                        for pat in grep::cli::patterns_from_reader(
                            uucore::context::io::stdin(),
                        )? {
                            add(pat);
                        }
                        state.stdin_consumed = true;
                    } else {
                        for pat in grep::cli::patterns_from_path(
                            uucore::context::resolve(&path),
                        )? {
                            add(pat);
                        }
                    }
...
impl Paths {
    ...
    fn from_low_args(
        ...
        crate::debug!("number of paths given to search: {}", paths.len());
        if !paths.is_empty() {
            let is_one_file = paths.len() == 1
                // Note that we specifically use `!uucore::context::resolve(&paths[0]).is_dir()` here
                ...
                && (paths[0] == Path::new("-") || !uucore::context::resolve(&paths[0]).is_dir());
            crate::debug!("is_one_file? {is_one_file:?}");
        ...
        let is_readable_stdin = crate::invocation::readable_stdin();
        ...
        let (path, is_one_file) = if use_cwd {
            crate::debug!("heuristic chose to search ./");
            (PathBuf::from("./"), false)
        } else {
            crate::debug!("heuristic chose to search stdin");
            (PathBuf::from("-"), true)
        };
...
fn take_hyperlink_config(
    ...
    if let Some(hostname) = hostname(low.hostname_bin.as_deref()) {
        crate::debug!(
            "found hostname for hyperlink configuration: {hostname}"
        );
    ...
    crate::debug!("hyperlink format: {:?}", fmt.to_string());
...
fn current_dir() -> anyhow::Result<PathBuf> {
    let err = match uucore::context::env::current_dir() {
        Err(err) => err,
        Ok(cwd) => return Ok(cwd),
    };
    if let Some(cwd) = uucore::context::env::var_os("PWD") {
        if !cwd.is_empty() {
...
fn platform_hostname() -> Option<String> {
    let hostname_os = match grep::cli::hostname() {
        Ok(x) => x,
        Err(err) => {
            crate::debug!("could not get hostname: {}", err);
            return None;
        }
    };
...
fn wsl_prefix() -> Option<String> {
    ...
    let distro_os = uucore::context::env::var_os("WSL_DISTRO_NAME")?;
