...
pub(crate) struct HaystackBuilder {
    ...
    stdout: Option<Arc<same_file::Handle>>,
}
...
impl HaystackBuilder {
    ...
    pub(crate) fn new(
        ...
        paths: &[PathBuf],
        skip_stdout: bool,
    ) -> std::io::Result<HaystackBuilder> {
        let stdout = if skip_stdout {
            Some(Arc::new(same_file::Handle::from_file(
                uucore::context::snapshot().stdout.try_clone()?,
            )?))
        } else {
            None
        };
        let roots = paths
            ...
            .map(|path| (cwd.join(path), path.clone()))
            .collect();
        Ok(HaystackBuilder { strip_dot_prefix: false, roots, stdout })
    }
    ...
    fn build(&self, dent: ignore::DirEntry) -> Option<Haystack> {
        if !dent.is_stdin()
            && !dent.file_type().is_some_and(|kind| kind.is_dir())
        {
            if let Some(stdout) = &self.stdout {
                // An unreadable path is diagnosed by the search worker itself.
                if same_file::Handle::from_path(dent.path())
                    .is_ok_and(|file| &file == stdout.as_ref())
                {
                    err_message!(
        ...
        let display = self
            .roots
            .iter()
            .find_map(|(absolute, lexical)| {
                dent.path().strip_prefix(absolute).ok().map(|suffix| {
            ...
            .unwrap_or_else(|| dent.path().to_owned());
        let hay = Haystack {
            dent,
            display,
...
impl Haystack {
    ...
    pub(crate) fn path(&self) -> &Path {
        if self.strip_dot_prefix && self.display.starts_with("./") {
            self.display.strip_prefix("./").unwrap()
        } else {
            &self.display
        }
    }
