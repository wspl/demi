...
pub fn uumain(args: Vec<std::ffi::OsString>) -> i32 {
    messages::with(std::sync::Arc::new(messages::State::default()), || {
        main(args).0
    })
}

fn main(args: Vec<std::ffi::OsString>) -> ExitCode {
    match run(flags::parse(args)) {
        Ok(code) => code,
        Err(err) => {
...
fn search_parallel(args: &HiArgs, mode: SearchMode) -> anyhow::Result<bool> {
    ...
    let invocation = uucore::context::snapshot();
    let message_state = messages::snapshot();
    args.walk_builder()?.build_parallel().run(|| {
        ...
        Box::new(move |result| {
            uucore::context::with(invocation.clone(), || {
                messages::with(message_state.clone(), || {
                    let haystack =
                        match haystack_builder.build_from_result(result) {
...
fn generate(mode: crate::flags::GenerateMode) -> anyhow::Result<ExitCode> {
    ...
    writeln!(uucore::context::io::stdout(), "{}", output.trim_end())?;
    Ok(ExitCode::from(0))
}
