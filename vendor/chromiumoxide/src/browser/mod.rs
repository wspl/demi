...
use crate::listeners::{DEFAULT_EVENT_CAPACITY, EventStream};
use crate::page::Page;
use crate::utils;
...
impl Browser {
    ...
    /// using later events. The capacity is rounded up to a power of two by Tokio.
    pub async fn event_listener_with_capacity<T: IntoEventKind>(
        &self,
        capacity: usize,
    ) -> Result<EventStream<T>> {
        let (request, stream) = EventStream::channel(capacity)?;
        self.sender
            .clone()
            .send(HandlerMessage::AddEventListener(request))
            .await?;
        Ok(stream)
    }
