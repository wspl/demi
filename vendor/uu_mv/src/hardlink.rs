...
use uucore::context::io::{self, Write as _};
...
impl HardlinkTracker {
    ...
    pub fn check_hardlink(
        ...
        let metadata = match source.context_symlink_metadata() {
            Ok(meta) => meta,
            Err(e) => {
...
impl HardlinkGroupScanner {
    ...
    fn scan_single_path(&mut self, path: &Path) -> io::Result<()> {
        ...
        if path.context_is_dir() {
            ...
            let metadata = path.context_symlink_metadata()?;
            if metadata.context_is_file() && metadata.nlink() > 1 {
                let key = (metadata.dev(), metadata.ino());
    ...
    fn scan_directory_recursive(&mut self, dir: &Path) -> io::Result<()> {
        ...
        let entries = uucore::context::fs::read_dir(dir)?;
