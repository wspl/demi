...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    if let Some(ref maybe_dir) = target_dir
        && !Path::new(&maybe_dir).context_is_dir()
    {
...
fn handle_two_paths(source: &Path, target: &Path, opts: &Options) -> UResult<()> {
    ...
    let Ok(source_metadata) = source.context_symlink_metadata() else {
    ...
    // `source.context_is_dir() && !source.context_is_symlink()` without the extra `stat` calls.
    let source_is_dir = source_metadata.context_is_dir();
    let target_is_dir = match target.context_symlink_metadata() {
        Ok(metadata) if metadata.context_is_symlink() => fs::canonicalize(target).is_ok_and(|p| p.context_is_dir()),
        Ok(metadata) => metadata.context_is_dir(),
        Err(_) => false,
    };
    ...
    } else if source_is_dir && target.context_exists() {
...
fn assert_not_same_file(
    ...
    let canonicalized_source = match canonicalize(
        ...
        Ok(source) if source.context_exists() => source,
...
fn move_files_into_dir(files: &[PathBuf], target_dir: &Path, options: &Options) -> UResult<()> {
    ...
    for sourcepath in files {
        if sourcepath.context_symlink_metadata().is_err() {
...
fn rename(
    ...
    // `to.context_exists() && to.context_is_dir() && !to.context_is_symlink()` check
    if to
        .context_symlink_metadata()
        .is_ok_and(|metadata| metadata.context_is_dir())
    {
        // normalize behavior between *nix and windows
        if from.context_is_dir() {
            if is_empty_dir(to) {
                fs::remove_dir(to)?;
            } else {
                return Err(io::Error::new(
    ...
    if opts.verbose {
        ...
        match display_manager {
            Some(pb) => pb.suspend(|| {
                uucore::context_println!("{message}");
            }),
            None => uucore::context_println!("{message}"),
        }
    }
    Ok(())
}
...
fn rename_with_fallback(
    ...
    fs::rename(from, to).or_else(|err| {
        ...
        let should_fallback =
            matches!(err.raw_os_error(), Some(EXDEV)) || (from.context_is_file() && can_delete_file(from));
        if !should_fallback {
            return Err(err);
        }
        // Get metadata without following symlinks
        let metadata = from.context_symlink_metadata()?;
        let file_type = metadata.file_type();
        if file_type.context_is_symlink() {
            rename_symlink_fallback(from, to)
        } else if file_type.context_is_dir() {
            #[cfg(unix)]
            {
                with_optional_hardlink_context(
                    hardlink_tracker,
...
fn rename_fifo_fallback(from: &Path, to: &Path) -> io::Result<()> {
    if to.context_try_exists()? {
        fs::remove_file(to)?;
    }
    // rustix::fs::mkfifoat is linux only
    nix::unistd::mkfifo(&uucore::context::resolve(to), nix::sys::stat::Mode::from_bits_truncate(0o666))?;
    fs::remove_file(from)
}
...
fn rename_symlink_fallback(from: &Path, to: &Path) -> io::Result<()> {
    ...
    match fs::symlink(&path_symlink_points_to, to) {
        ...
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {
            ...
            {
                fs::remove_file(to)?;
                uucore::context::fs::symlink(&path_symlink_points_to, to)?;
            }
        }
        Err(e) => return Err(e),
    }
...
fn rename_symlink_fallback(from: &Path, to: &Path) -> io::Result<()> {
    let path_symlink_points_to = fs::read_link(from)?;
    if path_symlink_points_to.context_exists() {
        if path_symlink_points_to.context_is_dir() {
...
fn copy_dir_contents_recursive(
    ...
    for entry in entries {
        ...
        if from_path.context_is_symlink() {
        ...
        } else if from_path.context_is_dir() {
...
fn rename_file_fallback(
    ...
    #[cfg(unix)]
    {
        use uucore::context::fs::Permissions;
...
fn preserve_ownership(from: &Path, to: &Path) -> io::Result<bool> {
    ...
    let source_meta = from.context_symlink_metadata()?;
    let uid = source_meta.uid();
    let gid = source_meta.gid();

    let dest_meta = to.context_symlink_metadata()?;
...
fn can_delete_file(path: &Path) -> bool {
    use {std::os::windows::ffi::OsStrExt as _, std::ptr::null, std::ptr::null_mut};
