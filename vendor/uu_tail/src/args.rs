...
use uucore::context::io::{IsTerminal, Write};
...
impl Settings {
    ...
    pub fn check_warnings(&self) -> uucore::context::io::Result<()> {
        if self.retry {
        ...
        if self.follow.is_some() && self.has_stdin() {
            #[cfg(unix)]
            let stdin_is_regular = rustix::fs::fstat(uucore::context::io::stdin())
            ...
            if !blocking_stdin && uucore::context::io::stdin().is_terminal() {
