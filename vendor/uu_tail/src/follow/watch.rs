...
impl WatcherRx {
    ...
    fn watch_with_parent(&mut self, path: &Path) -> UResult<()> {
        ...
        if path.is_relative() {
            path = path.context_canonicalize()?;
        }
    ...
    pub fn add_path(
        ...
        if self.follow.is_some() {
            let path = if path.is_relative() {
                uucore::context::env::current_dir()?.join(path)
            } else {
                path.to_owned()
            };
            ...
            for input in inputs {
                match input.kind() {
                    ...
                    InputKind::File(path) => {
                        ...
                        if path.is_relative() {
                            path = uucore::context::env::current_dir()?.join(path);
                        }
    ...
    fn handle_event(
        ...
        match event.kind {
            ...
            EventKind::Create(CreateKind::File | CreateKind::Folder | CreateKind::Any) => {
                ...
                } else if event_path.context_is_symlink() && settings.retry {
                    self.files.reset_reader(event_path);
        ...
        match rx_result {
            Ok(Ok(event)) => {
                ...
                for _ in 0..100 {
                    ...
                    uucore::context::thread::yield_now();
                    std::hint::spin_loop();
                }
            }
            Ok(Err(notify::Error {
                ...
                paths,
            })) if e.kind() == uucore::context::io::ErrorKind::NotFound => {
