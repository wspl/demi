...
use {uucore::context::env, uucore::context::fs};
...
/// because the format specification of uucore::context_print! does not support general
/// fill characters.
fn pad_and_print(result: &str, left: bool, width: usize, padding: Padding) {
    match (left, padding) {
        (false, Padding::Zero) => uucore::context_print!("{result:0>width$}"),
        (false, Padding::Space) => uucore::context_print!("{result:>width$}"),
        (true, Padding::Zero) => uucore::context_print!("{result:0<width$}"),
        (true, Padding::Space) => uucore::context_print!("{result:<width$}"),
    }
}
...
/// writer is genric to be any buffer like: `uucore::context::io::stdout`
...
fn print_it(output: &OutputType, flags: Flags, width: usize, precision: Precision) {
    ...
    match output {
        ...
        OutputType::Unknown => uucore::context_print!("?"),
    }
}
...
fn process_token_filesystem(t: &Token, meta: &StatFs, display_name: &str) {
    match *t {
        ...
        Token::Char(c) => uucore::context_print!("{c}"),
        Token::Directive {
            flag,
            width,
            precision,
...
impl Stater {
    ...
    fn find_mount_point<P: AsRef<Path>>(&self, p: P) -> Option<&OsString> {
        ...
        let path = p.as_ref().context_canonicalize().ok()?;
        mount_list
            .as_ref()?
            .iter()
    ...
    fn do_stat(&self, file: &OsStr, stdin_is_fifo: bool) -> UResult<i32> {
        ...
        let file = if cfg!(unix) && display_name == "-" {
            ...
            if let Ok(p) = Path::new("/dev/stdin").context_canonicalize() {
                p.into_os_string()
            } else {
