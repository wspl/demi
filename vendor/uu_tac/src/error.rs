...
pub enum TacError {
    ...
    /// [`uucore::context::io::Error`] that caused this error.
    #[error("{}", translate!("tac-error-open-error", "filename" => .0.quote(), "error" => strip_errno(.1)))]
    OpenError(OsString, uucore::context::io::Error),
    ...
    ReadError(OsString, uucore::context::io::Error),
    /// An error writing the (reversed) contents of a file or stdin.
    ///
    /// The parameter is the underlying [`uucore::context::io::Error`] that caused
    /// this error.
