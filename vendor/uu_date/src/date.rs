...
use uucore::context::io::{BufRead, BufReader, BufWriter, Read, Write, stderr};
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let dates: Box<dyn Iterator<Item = _>> = match settings.date_source {
        ...
        DateSource::FileMtime(ref path) => {
            let metadata = uucore::context::fs::metadata(path)
    ...
    let mut stdout = BufWriter::new(uucore::context::io::stdout().lock());
...
fn set_system_datetime(date: Zoned) -> UResult<()> {
    ...
    if result == 0 {
        Err(uucore::context::io::Error::last_os_error()
            .map_err_context(|| translate!("date-error-cannot-set-date")))
    } else {
        Ok(())
    }
