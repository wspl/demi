...
enum ChmodError {
    ...
    ChangingPermissions(PathBuf, uucore::context::io::Error),
}
...
impl Chmoder {
    ...
    fn handle_symlink_during_traversal(
        ...
        match fs::metadata(path) {
            Ok(meta) if meta.context_is_dir() => self.walk_dir_with_context(path, false, ancestors),
            Ok(_) => {
    ...
    fn print_neither_changed(file: OsString) -> uucore::context::io::Result<()> {
        use uucore::context::io::{Write as _, stdout};
        writeln!(
            stdout(),
            "{}",
    ...
    fn chmod(&self, files: &[OsString]) -> UResult<()> {
        ...
        for filename in files {
            ...
            // GNU (issue #9789).
            match file.context_try_exists() {
                Ok(false) => {
                    if file.context_is_symlink() {
                ...
                Err(err) if err.kind() == uucore::context::io::ErrorKind::PermissionDenied => {
                    if !self.quiet {
                ...
                Ok(true) | Err(_) => {
                    if !self.dereference && file.context_is_symlink() {
                        // The file is a symlink and we should not follow it
    ...
    fn descends_into_root(&self, path: &Path) -> bool {
        self.preserve_root && path.context_is_symlink() && Self::is_root(path)
    }
    ...
    fn walk_dir_with_context(
        ...
        if (!file_path.context_is_symlink() || should_follow_symlink) && file_path.context_is_dir() {
        ...
        if (!file_path.context_is_symlink() || should_follow_symlink) && file_path.context_is_dir() {
            match DirFd::open(file_path, SymlinkBehavior::Follow) {
                ...
                Err(err) => {
                    ...
                    if err.kind() == uucore::context::io::ErrorKind::PermissionDenied {
    ...
    fn safe_traverse_dir(
        ...
        for entry_name in entries {
            ...
            let Ok(meta) = dir_meta else {
                ...
                let error = if e.kind() == uucore::context::io::ErrorKind::PermissionDenied {
            ...
            if entry_path.context_is_symlink() {
                r = self
                ...
                if meta.context_is_dir() {
                    match dir_fd.open_subdir(&entry_name, should_follow_symlink.into()) {
                        ...
                        Err(err) => {
                            let error = if err.kind() == uucore::context::io::ErrorKind::PermissionDenied {
                                ChmodError::PermissionDenied(entry_path).into()
                            } else {
                                err.into()
                            };
    ...
    fn safe_chmod_file(
        ...
        let (new_mode, _) = self.calculate_new_mode(current_mode, file_path.context_is_dir())?;
    ...
    fn chmod_file_internal(&self, file: &Path, dereference: bool) -> UResult<()> {
        ...
        let fperm = match metadata {
            ...
            Err(err) => {
                ...
                return if file.context_is_symlink() && !dereference {
                    if self.verbose {
                        Self::print_neither_changed(file.into())?;
                    }
                    Ok(()) // Skip dangling symlinks
                } else if err.kind() == uucore::context::io::ErrorKind::PermissionDenied {
        ...
        let (new_mode, naively_expected_new_mode) =
            self.calculate_new_mode(fperm, file.context_is_dir())?;
        ...
        if file.context_is_symlink() && !dereference {
            if self.verbose {
                Self::print_neither_changed(file.into())?;
            }
        } else {
