...
mod tests {
    ...
    fn test_permutations() {
        ...
        let _ = uucore::context::fs::create_dir(target);
        for &a in &[0, 1, 2] {
            for &b in &[0, 1, 2] {
                for &c in &[0, 1, 2] {
                    for &d in &[0, 1, 2] {
                        for &e in &[0, 1, 2] {
                            for &f in &[0, 1, 2] {
                                ...
                                {
                                    use uucore::context::process::Command;
                                    ...
                                    //uucore::context_println!("{}", String::from_utf8_lossy(&output.stdout));
                                    //uucore::context_println!("{}", String::from_utf8_lossy(&output.stderr));
                                    let alef = uucore::context::fs::read(format!("{target}/alef")).unwrap();
                                    assert_eq!(alef, bet);
                                }
                            }
                        }
                        ...
                        for &e in &[0, 1, 2] {
                            for &f in &[0, 1, 2] {
                                ...
                                #[cfg(not(windows))] // there's no ed on windows
                                {
                                    ...
                                    //uucore::context_println!("{}", String::from_utf8_lossy(&output.stdout));
                                    //uucore::context_println!("{}", String::from_utf8_lossy(&output.stderr));
                                    let alef = uucore::context::fs::read(format!("{target}/alef_")).unwrap();
                                    assert_eq!(alef, bet);
                                }
                            }
                        }
                        ...
                        for &e in &[0, 1, 2] {
                            for &f in &[0, 1, 2] {
                                ...
                                #[cfg(not(windows))] // there's no ed on windows
                                {
                                    ...
                                    //uucore::context_println!("{}", String::from_utf8_lossy(&output.stdout));
                                    //uucore::context_println!("{}", String::from_utf8_lossy(&output.stderr));
                                    let alef = uucore::context::fs::read(format!("{target}/alefr")).unwrap();
                                    assert_eq!(alef, bet);
                                }
                            }
                        }
