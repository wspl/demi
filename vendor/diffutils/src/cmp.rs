...
use uucore::context::io::{BufRead, BufReader, BufWriter, Read, Write};
use std::iter::Peekable;

use {std::cmp, uucore::context::fs, uucore::context::io};
...
pub fn parse_params<I: Iterator<Item = OsString>>(mut opts: Peekable<I>) -> Result<Params, String> {
    ...
    while let Some(param) = opts.next() {
        ...
        if param == "--help" {
            uucore::context_println!("{}", usage_string(&executable_str));
