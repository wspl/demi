...
#[cfg(unix)]
fn is_xattr_unsupported(err: &crate::context::io::Error) -> bool {
    matches!(
        err.raw_os_error(),
...
#[cfg(not(unix))]
fn is_xattr_unsupported(_err: &crate::context::io::Error) -> bool {
    false
}
...
#[cfg(unix)]
pub fn copy_xattrs_fd(source: &crate::context::fs::File, dest: &crate::context::fs::File) -> crate::context::io::Result<()> {
    use xattr::FileExt;
...
pub fn copy_xattrs_fd_ignore_unsupported(
    source: &crate::context::fs::File,
    dest: &crate::context::fs::File,
...
#[cfg(unix)]
pub fn retrieve_xattrs_fd(source: &crate::context::fs::File) -> crate::context::io::Result<FxHashMap<OsString, Vec<u8>>> {
    use xattr::FileExt;
    let mut attrs = FxHashMap::default();
    for attr_name in source.list_xattr()? {
        if let Some(value) = source.get_xattr(&attr_name)? {
...
mod tests {
    ...
    fn test_copy_xattrs_fd() {
        ...
        let dst = crate::context::fs::OpenOptions::new()
            .write(true)
            .open(&dest_path)
            .unwrap();
    ...
    fn test_get_perm_bits_from_xattrs() {
        ...
        crate::context::fs::create_dir(&source_path).unwrap();
