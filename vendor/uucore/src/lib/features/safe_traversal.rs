...
impl DirFd {
    ...
    pub fn open(path: &Path, symlink_behavior: SymlinkBehavior) -> io::Result<Self> {
        ...
        let fd = nix::fcntl::open(&crate::context::resolve(path), flags, Mode::empty()).map_err(|e| {
...
impl Metadata {
    ...
    /// Compatibility methods to match crate::context::fs::Metadata interface
...
mod tests {
    ...
    fn test_dirfd_unlink_at_file() {
        ...
        assert!(!file_path.context_exists());
    }
    ...
    fn test_file_type() {
        ...
        assert!(!file_type.context_is_symlink());
    ...
    fn test_mkdir_at_creates_directory() {
        ...
        assert!(temp_dir.path().join("new_subdir").context_is_dir());
    }
    ...
    fn test_create_dir_all_safe_creates_nested_dirs() {
        ...
        assert!(dir_fd.as_raw_fd() >= 0);
        assert!(nested_path.context_is_dir());
    }
    ...
    fn test_create_dir_all_safe_follows_symlink() {
        ...
        assert!(symlink_path.context_is_symlink());
        ...
        assert!(symlink_path.context_is_symlink());
        assert!(symlink_path.context_is_dir()); // still resolves to a directory via the symlink
    }
    ...
    fn test_create_dir_all_safe_nested_symlink_in_path() {
        ...
        assert!(symlink_in_path.context_is_symlink());
        assert!(symlink_in_path.context_is_dir()); // resolves via symlink

        // subdir should have been created inside the real target directory
        assert!(target.join("subdir").context_exists());
    }
