...
impl From<crate::context::io::Error> for LocalizationError {
    fn from(error: crate::context::io::Error) -> Self {
        Self::Io {
            source: error,
...
fn find_uucore_locales_dir(utility_locales_dir: &Path) -> Option<PathBuf> {
    ...
    uucore_locales.context_exists().then_some(uucore_locales)
}
...
/// crate::context_println!("{greeting}");
/// ```
...
fn detect_system_locale() -> Result<LanguageIdentifier, LocalizationError> {
    let locale_str = crate::context::env::var("LANG")
...
///     Ok(_) => crate::context_println!("Localization initialized successfully"),
///     Err(e) => crate::context_eprintln!("Failed to initialize localization: {e}"),
/// }
/// ```
...
";
    ...
    fn test_localization_error_from_io_error() {
        let io_error = crate::context::io::Error::new(crate::context::io::ErrorKind::NotFound, "File not found");
    ...
    fn test_setup_localization_fallback_to_embedded() {
        std::thread::spawn(|| {
            ...
            let result = setup_localization("test");
            if let Err(e) = &result {
                crate::context_eprintln!("Setup localization failed: {e}");
            }
            assert!(result.is_ok());
    ...
    fn test_error_display() {
        let io_error = LocalizationError::Io {
            source: crate::context::io::Error::new(crate::context::io::ErrorKind::NotFound, "File not found"),
...
mod fhs_tests {
    ...
    fn resolves_fhs_share_locales_layout() {
        ...
        crate::context::fs::create_dir_all(&share_dir).unwrap();
        crate::context::fs::create_dir_all(&bin_dir).unwrap();
