...
use crate::context::io::Write as _;
use crate::context::io::stderr;
...
fn get_color_choice() -> clap::ColorChoice {
    if crate::context::env::var("NO_COLOR").is_ok() {
        clap::ColorChoice::Never
    } else if crate::context::env::var("CLICOLOR_FORCE").is_ok() || crate::context::env::var("FORCE_COLOR").is_ok() {
...
fn should_use_color_for_stream<S: crate::context::io::IsTerminal>(stream: &S) -> bool {
    match get_color_choice() {
        clap::ColorChoice::Always => true,
        clap::ColorChoice::Never => false,
        clap::ColorChoice::Auto => {
            stream.is_terminal() && crate::context::env::var("TERM").unwrap_or_default() != "dumb"
        }
    }
}
...
impl<'a> ErrorFormatter<'a> {
        ...
        F: FnOnce(),
    {
        ...
        callback();
        crate::context::process::exit(code);
    }
    ...
    fn handle_display_errors(&self, err: &Error) -> i32 {
        crate::context_print!("{}", err.render());
        0
    }
        ...
        if let (Some(arg), Some(value)) = (invalid_arg, invalid_value) {
            ...
            if value.is_empty() {
                ...
                match err.source() {
                    ...
                    _ => crate::context_eprintln!("{error_msg}"),
                }
            }
    ...
    fn handle_missing_required(&self, err: &Error, exit_code: i32) -> i32 {
        ...
        match lines.first() {
            ...
            _ => crate::context_eprint!("{}", err.render()),
        }
        exit_code
    }
...
/// # Exit Behavior
///
/// This function will call `crate::context::process::exit()` with the specified exit code
...
/// # Panics
///
/// This function never returns - it always calls `crate::context::process::exit()`.
///
/// # Examples
///
/// ```no_run
...
pub fn configure_localized_command(mut cmd: Command) -> Command {
    ...
    let colors_enabled = should_use_color_for_stream(&crate::context::io::stdout());
