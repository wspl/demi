...
impl Gitignore {
    ...
    pub fn global() -> (Gitignore, Option<Error>) {
        match uucore::context::env::current_dir() {
...
fn gitconfig_global_env_contents() -> Option<Vec<u8>> {
    let path = uucore::context::env::var_os("GIT_CONFIG_GLOBAL").map(PathBuf::from)?;
...
fn gitconfig_system_contents() -> Option<Vec<u8>> {
    let path = uucore::context::env::var_os("GIT_CONFIG_SYSTEM")
        .map(PathBuf::from)
...
fn gitconfig_xdg_contents() -> Option<Vec<u8>> {
    let path = uucore::context::env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .filter(|x| !x.as_os_str().is_empty())
...
fn excludes_file_default() -> Option<PathBuf> {
    uucore::context::env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .filter(|x| !x.as_os_str().is_empty())
        .or_else(|| home_dir().map(|p| p.join(".config")))
...
fn home_dir() -> Option<PathBuf> {
    // We're fine with using uucore::context::env::home_dir for now. Its bugs are, IMO,
    // pretty minor corner cases.
    #![allow(deprecated)]
    uucore::context::env::home_dir()
}
