...
fn get_input_type(path: &OsString) -> CatResult<InputType> {
    ...
    match ft {
        ...
        ft if ft.context_is_dir() => Ok(InputType::Directory),
        ft if ft.context_is_file() => Ok(InputType::File),
        ft if ft.context_is_symlink() => Ok(InputType::SymLink),
...
fn print_lines<R: FdReadable>(
    ...
    loop {
        ...
        // on a `uucore::context::io::Read::read` call.
...
#[cfg(test)]
mod tests {
    use uucore::context::io::{BufWriter, stdout};
