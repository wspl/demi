...
Workspaces resolve `@demi/*` from source in dev/test via the `development` export
...
3. **Code reuse is mandatory.** Generic helpers live in `@demi/utils` (helpers
   that return a `core` type, like `zeroUsage`, live in `@demi/core`). Do **not**
...
- **A new provider** — implement the `@demi/provider` contract (a `run()` that
...
- **A new Host** — implement `{ defaultCwd, fs, process, store }`;
  `@demi/host-local` is the Node reference for a remote/container/sandbox backend.
