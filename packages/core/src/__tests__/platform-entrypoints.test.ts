...
for (const [name, pkg] of workspaces) {
  for (const [subpath, file] of pkg.developmentExports) {
    workspaceEntryFiles.set(
      subpath === '.' ? name : `${name}${subpath.slice(1)}`,
      `${pkg.directory}/${file.replace(/^\.\//, '')}`
    )
  }
}
...
function entryFile(specifier: string): string {
  ...
  if (!file)
    throw new Error(
      `${specifier} is not a development export of any workspace package`
    )
  return file
}
...
const neutralPackageLeakPatterns = [
  [
    'concrete provider package reference',
    /@demicodes\/provider-(?:claude-code|codex|openai-api|anthropic-api|grok-build|google)\b|provider-(?:claude-code|codex|openai-api|anthropic-api|grok-build|google)/i
  ],
  [
    'concrete provider implementation class',
    /\b(?:ClaudeCodeProvider|CodexProvider|OpenAIApiProvider|AnthropicApiProvider|GrokBuildProvider|GoogleProvider)\b/
  ],
  ['concrete catalog source label', /\bcodex-backend\b/i],
  [
    'provider backend identifier',
    /\b(?:backend-api|chatgpt\.com|api\.openai\.com|cli-chat-proxy\.grok\.com|generativelanguage\.googleapis\.com|responses_websockets)\b/i
  ],
  [
    'concrete provider product name',
    /\bClaude Code\b|\bOpenAI Codex\b|\bGrok Build\b|\bGoogle Gemini\b/i
  ],
] as const
...
for (const entryName of platformNeutralEntries) {
  test(
    `${entryName} root entry has no Node-only source in its static closure`,
    async () => {
...
test(
  'only AgentServer imports AgentSession as a runtime value outside tests',
  async () => {
    ...
    for (const file of files) {
      ...
      if (!hasRuntimeImportFromAgent(source, 'AgentSession'))
        continue
      if (relativeFile !== 'packages/agent/src/node/assemble.ts')
...
test(
  'runtime source uses the forked bash package without embedded upstream snapshots',
  async () => {
    const forbiddenDirs = [
      'packages/bash',
    ...
    for (const directory of forbiddenDirs) {
      if (await isDirectory(resolveRepoPath(directory)))
        existingForbiddenDirs.push(directory)
    }
...
test('@demicodes/shell does not depend on the agent runtime', async () => {
  ...
  for (const file of files) {
    const source = await readFile(file, 'utf8')
    if (hasRuntimeImportFromPackage(
      source,
      '@demicodes/agent'
...
test('package manifests preserve layering boundaries', () => {
  expect(
    packageDependencyNames(manifests.get('@demicodes/shell')).filter(
      (name) => name === '@demicodes/core'
  ...
  for (const packageName of platformNeutralPackages) {
    ...
    expect(productionDependencyNames(manifests.get(packageName)))
      .not.toContain('@demicodes/host-remote')
    expect(packageDependencyNames(manifests.get(packageName)))
      .not.toContain('@demicodes/provider-claude-code')
    expect(packageDependencyNames(manifests.get(packageName)))
      .not.toContain('@demicodes/provider-codex')
    expect(packageDependencyNames(manifests.get(packageName)))
      .not.toContain('@demicodes/provider-openai-api')
    expect(packageDependencyNames(manifests.get(packageName)))
      .not.toContain('@demicodes/provider-anthropic-api')
    expect(packageDependencyNames(manifests.get(packageName)))
      .not.toContain('@demicodes/provider-grok-build')
    expect(packageDependencyNames(manifests.get(packageName)))
      .not.toContain('@demicodes/provider-google')
  }

  const claudeProviderDependencies = packageDependencyNames(manifests.get('@demicodes/provider-claude-code'))
  expect(
    claudeProviderDependencies.some(
      (name) => name === '@anthropic-ai/claude-agent-sdk'
        || name.includes('claude-agent-sdk')
    )
  ).toBe(false)
...
test(
  '@demicodes/core and @demicodes/provider contain no concrete provider product details',
  async () => {
    const scopes = [
...
test(
  'models.dev client facts belong only to the provider catalog client',
  () => {
    const source = `export const url = 'https://models.dev/api.json';
    export const warning = 'Using stale models.dev catalog';`
    expect(findNeutralPackageLeaks(
      resolveRepoPath('packages/provider/src/models-dev.ts'),
      source
    )).toEqual([])
    for (const file of [
      ...
      expect(findNeutralPackageLeaks(resolveRepoPath(file), source))
        .toContain('models.dev client outside its owning module')
    }
  }
)

test.each(['codex-backend', 'models.dev', 'cache'])(
  'shared catalog types cannot expose the %s source label',
  (
    label
  ) => {
    for (const file of [
      'packages/provider/src/types.ts',
      'packages/provider/src/models-dev.ts',
      'packages/core/src/index.ts'
    ]) {
      for (const source of [
        ...
        expect(findNeutralPackageLeaks(resolveRepoPath(file), source))
          .toContain('concrete catalog source label in metadata')
      }
    }
  }
)
...
test(
  'generic quota cache metadata is independent of model catalog source labels',
  () => {
    const source = `export type ProviderQuotaSource = 'probe' | 'cache';
    export const quota = { source: 'cache' };`
    expect(findNeutralPackageLeaks(
      resolveRepoPath('packages/provider/src/quota.ts'),
      source
    )).toEqual([])
  }
)
...
test(
  ...
  async () => {
    ...
    for (const edge of edges) {
      ...
      if (manifest?.dependencies?.[edge.toPackage])
        continue
      violations.push(
        `${edge.fromPackage} imports ${edge.toPackage} in ${edge.file} but does not declare it in dependencies`
      )
    }
  ...
  async () => {
    ...
    for (const [script, command] of Object.entries(rootManifest.scripts
      ...
      for (const path of command.match(/packages\/[\w./-]+/g) ?? []) {
        if (!(await isFile(resolveRepoPath(path)))
          && !(await isDirectory(resolveRepoPath(path)))) missing.push(`${script}: ${path}`)
      }
    }
    ...
    for (const [name, pkg] of workspaces) {
      if (!tested.has(`${pkg.directory}/src`)
        && (await hasTestFiles(resolveRepoPath(`${pkg.directory}/src`)))) untested.push(name)
    }
...
test(
  'public root exports do not expose provider internals or testing helpers',
  async () => {
    const checks = [
      [
        ...
        [
          [
            ...
            /\bexport\b[\s\S]*?from\s+['"]\.\/(?:stub|testing)['"]/
          ],
        ],
      ],
      [
        ...
        [
          ...
          [
            'internal module export',
            /['"]\.\/(?:cli|jsonl|output|transport)['"]/
          ],
          ['provider class export', /\bClaudeCodeProvider\b/],
          [
            'catalog parser or test helper export',
            /\b(?:ModelCatalogFetch|modelsDevAnthropicCatalogToModelList|parseClaudeModelVersion|resetClaudeCodeModelCatalogCacheForTests)\b/
          ],
        ],
      ],
      [
        ...
        [
          ...
          [
            'auth store or transport helper export',
            /\b(?:FileCodexAuthStore\b|StaticCodexAuthStore\b|CodexAuthStore\b|buildCodexHeaders|responsesUrlForAuth)\b/
          ],
          [
            'catalog parser or test helper export',
            /\b(?:ModelCatalogFetch|codexBackendModelsToModelList|resetCodexModelCatalogCacheForTests)\b/
          ],
        ],
      ],
      [
      ...
      [
        ...
        [
          ...
          [
            'auth store helper export',
            /\b(?:FileGrokAuthStore\b|StaticGrokAuthStore\b|GrokAuthStore\b|buildGrokBuildHeaders)\b/
          ],
        ],
      ],
      [
    ...
    for (const [path, rules] of checks) {
      ...
      for (const [label, pattern] of rules) {
        if (pattern.test(source))
          violations.push(`${path} exposes ${label}`)
      }
    }
...
async function findPlatformViolations(entry: string): Promise<string[]> {
  ...
  while (pending.length > 0) {
    ...
    const relativeFile = formatPath(file)
    if (nodeOnlyFiles.has(file))
      violations.push(`${relativeFile} is an explicit Node-only adapter`)
...
function findNeutralPackageLeaks(file: string, source: string): string[] {
  ...
  if (parsed.errors.length > 0) {
    ...
    throw new Error(
      `Unable to parse ${formatPath(file)} for boundary checks: ${messages}`
    )
  }
  ...
  if (formatPath(file) !== 'packages/provider/src/models-dev.ts'
    && /\bmodels\.dev\b/i.test(semanticSource)) {
  ...
  new Visitor({
    TSLiteralType(node) {
      collectAstStrings(node, metadataStrings)
    },
    TSEnumMember(node) {
      collectAstStrings(node.initializer, metadataStrings)
    },
    TSInterfaceDeclaration(node) {
      if (/Model|Catalog/.test(node.id.name))
        collectAstStrings(
          node,
          catalogTypeStrings
        )
    ...
    Property(node) {
      ...
      if (key === 'source')
        collectAstStrings(node.value, metadataStrings)
    },
  }).visit(parsed.program)
  if (metadataStrings.some((value) => /^(?:codex-backend|models\.dev)$/i.test(value)) ||
    catalogTypeStrings.some(
      (value) => /^(?:codex-backend|models\.dev|cache)$/i.test(value)
    )) {
...
async function resolveImport(
  ...
  if (workspaceEntry)
    return resolveRepoPath(workspaceEntry)
...
async function listProductionSourceFiles(): Promise<string[]> {
  const files = await Promise.all(
    [...productionPackageDirectories.values()].map((
      directory
    ) =>
...
async function readWorkspacePackages(
  ...
  for (const directory of root.workspaces ?? []) {
    ...
    for (const [subpath, target] of Object.entries(manifest.exports ?? {})) {
      ...
      if (file)
        developmentExports.set(subpath, file)
    }
  ...
  for (const [packageName, packageDirectory] of productionPackageDirectories) {
    ...
    for (const file of files) {
      ...
      for (const specifier of findModuleSpecifiers(file, source)) {
        ...
        if (!toPackage || toPackage === packageName)
          continue
        edges.push({
...
async function resolveWorkspacePackageDependency(
  fromFile: string,
  specifier: string
): Promise<string | null> {
...
function workspacePackageNameForFile(path: string): string | null {
  ...
  for (const [packageName, packageDirectory] of productionPackageDirectories) {
    if (relativePath === packageDirectory
      || relativePath.startsWith(`${packageDirectory}/`)) return packageName
  }
  return null
}

function findPackageDependencyCycle(
  edges: ProductionImportEdge[]
): string | null {
  const adjacency = new Map<string, Set<string>>()
  for (const packageName of productionPackageDirectories.keys()) adjacency.set(
    packageName,
    new Set()
  )
...
function hasRuntimeImportFromAgent(source: string, name: string): boolean {
  ...
  while (match) {
    if (importClauseHasRuntimeName(match[1] ?? '', name))
      return true
...
function hasRuntimeImportFromPackage(source: string, specifier: string): boolean {
  ...
  const staticPattern = new RegExp(
    `\\b(?:import|export)\\s+([\\s\\S]*?)\\s+from\\s+['"]${escapedSpecifier}['"]`,
    'g'
  )
  let staticMatch = staticPattern.exec(source)
  while (staticMatch) {
    if (importClauseHasRuntimeBinding(staticMatch[1] ?? ''))
      return true
...
function importClauseHasRuntimeBinding(clause: string): boolean {
  ...
  if (namedStart === -1 || namedEnd === -1)
    return true
  if (trimmed.slice(0, namedStart).trim().length > 0)
    return true
