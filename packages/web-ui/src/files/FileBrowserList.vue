...
const selectedIndex = computed(
  () => props.entries.findIndex((entry) => entry.name === selected.value)
)
...
function moveSelection(delta: number) {
  ...
  nextTick(() => {
    listEl.value?.querySelector<HTMLElement>('[data-selected]')?.scrollIntoView(
      { block: 'nearest' }
    )
  })
}
...
function onKeydown(event: KeyboardEvent) {
  ...
  if (event.key === 'ArrowDown')
    ...
    moveSelection(-props.entries.length)
  else if (event.key === 'End')
    moveSelection(props.entries.length)
  else if (event.key === 'Enter' && selectedIndex.value >= 0)
    activate(props.entries[selectedIndex.value]!)
  else if (event.key === 'Backspace' || (event.key === 'ArrowUp' && modifier))
    emit('up')
  else if (event.key === 'ArrowLeft' && modifier)
    emit('back')
  else if (event.key === 'ArrowRight' && modifier)
    emit('forward')
  else return
...
function onNewNameKeydown(event: KeyboardEvent) {
  if (event.key === 'Enter') {
    event.preventDefault()
    if (isValidEntryName(newName.value))
...
watch(() => props.entries, (next, previous) => {
  if (next !== previous && scroll.value?.el)
...
const failureCopy = computed(() => {
  ...
  if (failure.kind === 'permission')
    return {
    icon: Lock,
    title: 'You do not have permission to open this folder.'
  }
  if (failure.kind === 'offline')
    return {
    icon: WifiOff,
    title: 'The device is offline.'
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          >{{ formatModified(entry.modifiedAt) }}</span>
          <span
            class="truncate px-1 text-right text-[12px] text-fg-subtle"
            :title="entryKind(entry.name, entry.isDirectory)"
          >
        ...
        >
          ...
          <span
            ...
            class="text-[12px] leading-4 text-fg-subtle"
