...
export function dir(children: Record<string, MemoryNode>, extra: Omit<MemoryDirectory, 'kind' | 'children'> = {}): MemoryDirectory {
...
export function createMemoryFileSource(options: MemoryFileSourceOptions): FileBrowserSource & { root: MemoryDirectory } {
  ...
  return {
    ...
    async list(path, signal) {
      await wait(signal)
      if (options.offline) throw new FileBrowserError('offline')
      const node = lookup(path)
      if (!node || node.kind !== 'directory') throw new FileBrowserError('not-found', `No such directory: ${normalizePath(path)}`)
      if (node.failure) throw new FileBrowserError(node.failure.kind, node.failure.message)
      const entries: FileBrowserEntry[] = Object.entries(node.children).map(([name, child]) =>
        child.kind === 'directory'
          ? { name, isDirectory: true, modifiedAt: child.modifiedAt }
          : { name, isDirectory: false, size: child.size, modifiedAt: child.modifiedAt },
      )
      return entries
    },
    async createDirectory(path) {
      ...
      if (!parent || parent.kind !== 'directory') throw new FileBrowserError('not-found', `No such directory: ${parentPath(path)}`)
      if (parent.failure?.kind === 'permission') throw new FileBrowserError('permission', parent.failure.message)
      const name = baseName(path)
      if (parent.children[name]) throw new FileBrowserError('other', `${joinPath(parentPath(path), name)} already exists.`)
      parent.children[name] = { kind: 'directory', children: {}, modifiedAt: new Date().toISOString() }
    },
  }
}
