...
test('pending steer clones and reconciles native video content', () => {
  ...
  expect(
    (pending.content[0] as Extract<UserContentBlock, { type: 'video' }>).source
  ).not.toBe(
    source
  )
  expect(reconcilePendingSteers(
      [
        videoSteerBlock('materialized', new Uint8Array([1, 2, 3]))
      ],
      [pending]
    )).toEqual(
    []
...
test('reconcile removes pending steer after a matching transcript steer materializes', () => {
  ...
  const nextBlocks = [
    ...baseline,
    steerBlock('materialized', 'same-turn guidance')
  ]
...
test('reconcile removes pending steer when reference resolution changes materialized content', () => {
  const pending = createPendingSteerMessage(
    'local-1',
    [
      {
        type: 'reference',
  ...
  expect(reconcilePendingSteers(
      [
        steerBlock('materialized', 'resolved file content')
      ],
      [pending]
    )).toEqual(
    []
...
test('reconcile handles duplicate pending steer messages one materialized block at a time', () => {
  ...
  expect(reconcilePendingSteers(
      [steerBlock('materialized-1', 'duplicate')],
      [first, second]
    )).toEqual(
    [second]
  )
  expect(reconcilePendingSteers(
      [
        steerBlock('materialized-1', 'duplicate'),
        steerBlock('materialized-2', 'duplicate')
      ],
      [first, second]
    )).toEqual(
    []
