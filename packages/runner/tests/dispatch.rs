...
struct Fixture {
    ...
    calls: Arc<Calls>,
    server: Server,
    outgoing: mpsc::Receiver<demi_runner_protocol::Outbound>,
    hash: String,
}
...
impl Fixture {
    async fn new() -> Self {
        ...
        let hash = demi_command_protocol::canonical_digest(&body).unwrap();
        ...
        let server = Server::start(Arc::new(Dispatcher {
    ...
    async fn context(
        ...
        Arc<demi_runner::contexts::ExecutionContext>,
        demi_runner::contexts::Lease,
    ) {
        self.contexts
            .create(
                "job".into(),
...
async fn callback_exit_clears_hint_and_revoked_context_cannot_dispatch() {
    tokio::time::timeout(Duration::from_secs(10), async {
        ...
        fixture
            .calls
            .reply(&demi_runner_protocol::Inbound::RpcExit {
