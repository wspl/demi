...
export async function bundleRuntime(
  ...
  const worker = await Bun.build({
    entrypoints: [resolve(import.meta.dir, '../src/commands/worker.ts')],
    target: 'browser',
    format: 'esm',
  ...
  if (!worker.success)
    throw new Error(worker.logs.join('\n'))
  ...
  const app = await Bun.build({
    ...
    conditions: ['development'],
    external: ['tjs:*'],
    define: { DEMI_COMMAND_WORKER_SOURCE: JSON.stringify(source) },
  })
  if (!app.success)
    throw new Error(app.logs.join('\n'))
...
if (import.meta.main) {
  ...
  if (!entry || !output)
    throw new Error('Usage: bundle.ts entry output')
