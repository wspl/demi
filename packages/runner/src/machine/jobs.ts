...
export interface TeedSpawnHandle {
  ...
  /**
   * The full stdout, when asked for: a pipe end's source (`runner.md` § Pipes);
   * dropping it early stops only this copy.
   */
...
/**
 * Log every byte, retain a bounded preview, and backpressure the optional live
 * copy.
 */
export async function spawnTeed(
  params: TeedSpawnParams
): Promise<TeedSpawnHandle> {
  ...
  try {
    ...
    const kind: HostSpawnError['kind'] = errorCode(error) === 'ENOENT'
  ...
  const stdout = logStream(
    handle.stdout,
    stdoutFile,
    params.tee.viewLimit,
    params.tee.stream === true
  )
  const stderr = logStream(
    handle.stderr,
    stderrFile,
    params.tee.viewLimit,
    false
  ...
  let finished = false
  const wait = Promise.all([handle.wait(), stdout.done, stderr.done]).then((
    [exit, stdoutBytes, stderrBytes]
  ) => {
    finished = true
  ...
  async function copyToOutputs(): Promise<number> {
    ...
    try {
      for await (const chunk of source) {
        ...
        if (previewController && remaining > 0)
          previewController.enqueue(
            chunk.slice(
              0,
              remaining
            )
  ...
  try {
    ...
    while (read < result.length) {
      const count = await file.read(
        result.subarray(read),
        size - result.length + read
      )
      if (count === null)
        break
      read += count
