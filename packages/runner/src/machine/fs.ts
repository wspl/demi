...
export function createRunnerFileSystem(defaultCwd: string): HostFileSystem {
  const resolve = (path: string, cwd?: string): string =>
    isAbsolutePath(path)
      ? normalizePath(path)
      : normalizePath(`${cwd ?? defaultCwd}/${path}`)
  ...
  return {
    ...
    stat: async (path, options) => toHostFileStat(await tjs.stat(resolve(
      path,
      options?.cwd
    ))),
    lstat: async (path, options) => toHostFileStat(await tjs.lstat(resolve(
      path,
      options?.cwd
    ))),
    readdir: (async (
      ...
      return options?.withFileTypes
        ? entries.map(toHostDirent)
        : entries.map((entry) => entry.name)
    }) as HostFileSystem['readdir'],
    mkdir: (path, options) => tjs.makeDir(
      resolve(path, options?.cwd),
    ...
    rm: async (path, options) => {
      ...
      try {
        ...
        if (options?.force && isFileNotFoundError(error))
          return
        throw error
      }
      if (!stat.isDirectory)
        return tjs.remove(target)
      if (!options?.recursive)
        throw errnoError(
          'EISDIR',
          `EISDIR: is a directory, rm '${target}'`,
          { syscall: 'rm', path: target }
        )
    ...
    cp: async (path, destination, options) => {
      ...
      if (stat.isDirectory && !options?.recursive) {
        throw errnoError(
          'EISDIR',
          `EISDIR: is a directory, cp '${source}'`,
          { syscall: 'cp', path: source }
        )
      }
    ...
    chmod: (path, mode, options) => tjs.chmod(
      resolve(path, options?.cwd),
      mode
    ),
    symlink: (target, path, options) => tjs.symlink(
      target,
      resolve(path, options?.cwd)
    ),
    link: (existingPath, path, options) => tjs.link(
      resolve(existingPath, options?.cwd),
      resolve(path, options?.cwd)
    ),
    readlink: (path, options) => tjs.readLink(resolve(path, options?.cwd)),
    realpath: (path, options) => tjs.realPath(resolve(path, options?.cwd)),
    utimes: (path, atime, mtime, options) => tjs.utime(
      resolve(path, options?.cwd),
      atime,
      mtime
    ),
...
async function copyEntry(
  ...
  switch (true) {
    ...
    default:
      throw errnoError(
        'EINVAL',
        `EINVAL: cannot copy a special file, cp '${source}'`,
        { syscall: 'cp', path: source }
      )
  }
}
