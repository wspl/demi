...
export class ExecutionContexts {
  ...
  create(
    ...
    const context: ExecutionContext = {
      ...
      owner,
      ...(jobId ? { jobId } : {}),
      agentSessionId: jobId ? env.DEMI_SESSION_ID ?? '' : '',
      shellId: jobId ? env.DEMI_SHELL_ID ?? '' : '',
      manifest
    }
  ...
  get(id: string): ExecutionContext {
    ...
    if (!context)
      throw new Error('execution context is not live on this runner')
    return context
  }
  remove(owner: string): void {
    for (const [id, context] of this.contexts) if (context.owner === owner)
