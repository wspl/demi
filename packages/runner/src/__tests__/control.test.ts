...
test(
  'blocked job and spawn stdin leave ping, filesystem and kill responsive; stdin order survives EOF',
  async () => {
    ...
    const server = Bun.serve({ port: 0,
      fetch(request, server) {
        return server.upgrade(request) ? undefined : new Response(
          'bad',
          { status: 400 }
        )
      },
    ...
    try {
      ...
      for (const kind of ['job', 'spawn'] as const) {
        ...
        send(
          kind
            === 'job' ? { type: 'job_kill', jobId: kind, signal: 'SIGKILL' } : {
            type: 'spawn_kill',
            spawnId: kind,
            signal: 'SIGKILL'
          }
        ...
        await waitFor(
          () => received.slice(start).some((message) => message.type === 'pong')
            && received.some((message) => message.type === 'fs_ok'
              && message.id === kind),
          () => runner.log.join('\n'),
          { timeoutMs: 750 }
        )
        await waitFor(
          () => received.slice(start)
            .some((message) => message.type === `${kind}_exit`)
        )
        expect(
          received.slice(start)
            .find((message) => message.type === `${kind}_exit`)
        ).toMatchObject(
          {
            signal: 'SIGKILL'
          }
      ...
      await waitFor(
        () => received.some((message) => message.type === 'spawn_exit'
          && message.spawnId === 'ordered')
      )
      const output = received.flatMap(
        (message) => message.type === 'spawn_output'
          && message.spawnId === 'ordered'
          ? [message.bytes]
          : []
      )
