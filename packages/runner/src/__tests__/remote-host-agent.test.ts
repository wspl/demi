...
test(
  'bare AgentServer executes over a live runner; death mid-command is a tool error; reconnect resumes',
  async () => {
    ...
    const active: { socket: Bun.ServerWebSocket<SocketData> | null } = {
      socket: null
    }
    /**
     * Every frame type the runner sent, in order: the wire audit of the
     * acceptance.
     */
    ...
    const server = Bun.serve<SocketData>({
      ...
      fetch(request, bunServer) {
        if (bunServer.upgrade(request, { data: { authed: false } }))
          return
      ...
      websocket: {
        message(ws, data) {
          if (typeof data === 'string')
            throw new Error('runner frames are binary')
      ...
      createRuntime: () =>
        new StubProvider([
          ...
          [events.toolCall(
            't1',
            'shell_exec',
            { script: 'printf hello | tee made.txt | cat', timeoutMs: 10_000 }
          )],
          ...
          [events.toolCall(
            't2',
            'shell_exec',
            { script: 'echo started && sleep 30', timeoutMs: 60_000 }
          )],
    ...
    expect(new Set(inbound.slice(framesBefore))).toEqual(new Set([
      'job_output',
      'job_exit'
    ]))
