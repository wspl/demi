...
async function peer(run: (data: Socket, control: Socket) => void | Promise<void>) {
  const path = await mkdtemp(join(tmpdir(), 'demi-client-wire-')), endpoint = join(path, 'ipc.sock')
    ...
    void (async () => {
      for await (const frame of localFrames(socket)) {
        if (frame.type === LOCAL.frames.invoke) {
          ...
          data = socket; socket.write(localFrame(LOCAL.frames.ready))
        } else {
  ...
  const child = Bun.spawn([commandClientBinary(), 'probe', '你好'], { env: { ...process.env, DEMI_RUNNER_ENDPOINT: endpoint, DEMI_CONTEXT_ID: 'a'.repeat(32) }, stdin: 'pipe', stdout: 'pipe', stderr: 'pipe' })
  return { child, async close() { child.kill(); await child.exited; for (const socket of sockets) socket.destroy(); await new Promise<void>(resolve => server.close(() => resolve())); await rm(path, { recursive: true, force: true }) } }
}
...
test('native decoder accepts fragmented binary frames and preserves final exit status', async () => {
  const fixture = await peer(async data => {
    ...
    for (const byte of bytes) { data.write(new Uint8Array([byte])); await Bun.sleep(1) }
    data.write(localFrame(LOCAL.frames.stderr, new TextEncoder().encode('diagnostic')))
    const exit = new Uint8Array(4); new DataView(exit.buffer).setUint32(0, 23)
  ...
  try {
    ...
    expect(stdout).toEqual(new Uint8Array([0, 255, 10, 128])); expect(stderr).toBe('diagnostic'); expect(code).toBe(23)
  } finally { await fixture.close() }
})
test('native decoder rejects an oversized frame before buffering its body', async () => {
  const fixture = await peer(data => { const frame = new Uint8Array(5); new DataView(frame.buffer).setUint32(0, LOCAL.maxFrame + 1); frame[4] = LOCAL.frames.stdout; data.write(frame) })
  try { expect(await fixture.child.exited).toBe(1); expect(await new Response(fixture.child.stderr).text()).toContain('frame exceeds limit') } finally { await fixture.close() }
})
test('native client fails if runner disconnects without an exit frame', async () => {
  const fixture = await peer(data => { data.end() })
  try { expect(await fixture.child.exited).toBe(1); expect(await new Response(fixture.child.stderr).text()).toContain('before exit') } finally { await fixture.close() }
})
