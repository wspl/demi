    ...
    const handled = (async () => {
      for await (const frame of localFrames(socket)) {
        if (frame.type === LOCAL.frames.invoke) {
          const request = localInvokeSchema.parse(JSON.parse(new TextDecoder()
            .decode(frame.body)))
          ...
          localWatchSchema.parse(JSON.parse(new TextDecoder()
            .decode(frame.body)))
          socket.write(localFrame(LOCAL.frames.ready))
          if (!data)
            throw new Error('control connected before invocation')
...
test(
  'native decoder accepts fragmented binary frames and preserves final exit status',
  async () => {
    const fixture = await peer(async data => {
      const bytes = localFrame(
        LOCAL.frames.stdout,
    ...
    try {
      const [stdout, stderr, code] = await Promise.all([
        new Response(fixture.child.stdout).bytes(),
        new Response(fixture.child.stderr).text(),
...
test(
  'native decoder rejects an oversized frame before buffering its body',
  async () => {
    ...
    try {
      ...
      expect(await new Response(fixture.child.stderr).text())
        .toContain('frame exceeds limit')
    } finally {
      await fixture.close()
    }
  }
...
test(
  'native client fails if runner disconnects without an exit frame',
  async () => {
    const fixture = await peer(data => {
      data.end()
    })
...
test(
  'control disconnect cancels a native client while stdout is backpressured',
  async () => {
    const fixture = await peer((data, control) => {
      const output = localFrame(
        LOCAL.frames.stdout,
        new Uint8Array(LOCAL.chunkSize)
      )
