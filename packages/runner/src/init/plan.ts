...
export interface InitStep {
  ...
  args: string[]
  /**
   * The step's failure is fatal for the boot (default); a tolerated step is
   * logged and skipped.
   */
  tolerated?: boolean
}
...
export interface GuestLayout {
  ...
  newRoot: string
  /**
   * Where the read-only rootfs remains reachable after the pivot, under the new
   * root.
   */
  oldRoot: string
}
...
/**
 * The kernel's own filesystems, first: everything after reads `/proc`. `/dev`
 * is the kernel's (`CONFIG_DEVTMPFS_MOUNT`).
 */
...
export function upperOverlay(layout: GuestLayout): InitStep[] {
  ...
  return [
    ...
    {
      command: 'mkdir',
      args: ['-p', `${upperMount}/upper`, `${upperMount}/work`]
    },
    {
      command: 'mount',
      args: [
        ...
        'overlay',
        '-o',
        `lowerdir=/,upperdir=${upperMount}/upper,workdir=${upperMount}/work`,
        newRoot
      ]
    },
    { command: 'mkdir', args: ['-p', `${newRoot}${oldRoot}`] },
    ...['/proc', '/sys', '/dev'].map((dir) => ({
      command: 'mount',
      args: ['--move', dir, `${newRoot}${dir}`]
    })),
...
export function homeMount(layout: GuestLayout): InitStep[] {
  return [
    ...
    {
      command: 'mount',
      args: ['-t', 'ext4', layout.homeDevice, layout.homeMount]
    },
...
/**
 * The guest's name (`/etc/hosts` in the rootfs resolves it, which sudo insists
 * on).
 */
export const GUEST_HOSTNAME = 'demi'

/**
 * One interface, one address, one default route, from the command line; the
 * nameservers are a file the caller writes.
 */
export function networkSteps(network: GuestNetwork, iface = 'eth0'): InitStep[] {
  return [
    ...
    {
      command: 'ip',
      args: ['route', 'add', 'default', 'via', network.gateway, 'dev', iface]
    },
  ]
}
...
/**
 * The whole boot, in order: kernel filesystems, the upper, the home, the
 * network.
 */
export function initPlan(
  ...
  return [
    ...
    ...upperOverlay(layout),
    ...homeMount(layout),
    ...(network ? networkSteps(network) : [])
  ]
}

/**
 * Runs a spawn and reports its exit and output; what `runInit` needs of a
 * process facet.
 */
...
/**
 * Runs the plan step by step; a failed step that is not tolerated ends the boot
 * with its stderr.
 */
