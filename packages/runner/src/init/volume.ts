...
/**
 * What the runner mode needs of a filesystem; the default over a plain
 * directory syncs and knows nothing else.
 */
...
export function parseDf(output: string): VolumeUsage | null {
  ...
  if (lines.length < 2 || fields.length < 6)
    return null
...
export interface GrowthPolicy {
  ...
  /**
   * … or less than this many bytes, for a filesystem large enough that a tenth
   * is too little …
   */
  reserveBytes: number
  /**
   * … but never more than this fraction, so a small filesystem is not asked to
   * grow at once.
   */
...
/**
 * The reserve: a tenth of the filesystem, raised toward 256 MB but never past a
 * quarter.
 */
export function reserveBytes(
  ...
  return Math.max(
    totalBytes * policy.reserveFraction,
    Math.min(policy.reserveBytes, totalBytes * policy.reserveCapFraction)
  )
}

/** The size to ask for, or null while the reserve holds. */
export function growthWanted(
  usage: VolumeUsage,
  policy: GrowthPolicy = DEFAULT_GROWTH_POLICY
): number | null {
  if (usage.availableBytes >= reserveBytes(
    usage.totalBytes,
    policy
  )) return null
...
export class BlockVolume implements Volume {
  ...
  async sync(): Promise<void> {
    ...
    if (result.code !== 0)
      throw new Error(`sync ${this.mount} failed`)
  }
  ...
  async grown(bytes: number): Promise<void> {
    ...
    if (result.code !== 0)
      throw new Error(
        `resize2fs ${this.device} exited ${result.code ?? 'by signal'} after growth to ${bytes} bytes`
      )
  }
}

/**
 * A connected host with no managed block volumes: `sync` flushes everything and
