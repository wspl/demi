use demi_command_protocol::Invocation;
use demi_runner::{
    command_client::{self, RawCommand, Stdio},
    mode::{self, Options},
    shell::{self, ShellOptions},
...
async fn shell_job(path: &Path) -> io::Result<u8> {
    let bytes = tokio::fs::read(path).await?;
    let job: ShellJob = serde_json::from_slice(&bytes).map_err(io::Error::other)?;
    let mut env: BTreeMap<_, _> = std::env::vars().collect();
    let input_reference = standard_file(0)?;
    env.remove(stdio::LIVE_INPUT_ENV);
    if job.live {
        env.insert(
            stdio::LIVE_INPUT_ENV.into(),
            stdio::live_reference(&input_reference)?,
        );
    }
    let options = ShellOptions {
        ...
        env,
        stdin: standard_file(0)?,
        stdout: standard_file(1)?,
        stderr: standard_file(2)?,
    };
    let result = shell::execute(&job.script, options)
        .await
        .map_err(io::Error::other)?;
    tokio::fs::write(job.cwd_file, result.cwd.to_string_lossy().as_bytes()).await?;
    drop(input_reference);
    Ok(result.code)
}
...
async fn runner(args: Vec<String>) -> io::Result<u8> {
    ...
    let runner = demi_runner_protocol::HelloRunner {
        name: env
...
fn identity(home_dir: String) -> io::Result<demi_runner_protocol::HelloRunnerIdentity> {
    #[cfg(unix)]
    {
        ...
        Ok(demi_runner_protocol::HelloRunnerIdentity {
...
fn main() {
    ...
    } else if args.len() == 3 && args[1] == "shell-job" {
        runtime.block_on(shell_job(Path::new(&args[2])))
    } else {
    ...
    // Completed shell jobs have joined their background tasks. Process termination
    // also releases any blocking work left by a failed or cancelled script.
    runtime.shutdown_background();
    std::process::exit(i32::from(code));
}
