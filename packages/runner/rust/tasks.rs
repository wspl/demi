...
impl TaskTable {
    ...
    async fn run(
        ...
        let mut job = None;
        let (command, args, process_group, stdin, stdout) = match spec.command {
            TaskCommand::Process {
                command,
                args,
                process_group,
            } => (command, args, process_group, None, None),
            TaskCommand::Shell {
                script,
                stdin,
                stdout,
                ...
                let scratch = tempfile::Builder::new()
                    .prefix(".work-")
                    .tempdir_in(&path)?;
                let cwd_file = scratch.path().join("cwd");
                let job_file = scratch.path().join("job.json");
                ...
                job = Some((logs, scratch, cwd_file));
                (
                    self.executable.to_string_lossy().into_owned(),
                    vec!["shell-job".into(), job_file.to_string_lossy().into_owned()],
                    true,
                    stdin,
                    stdout,
                )
        ...
        if cancel.is_cancelled() {
            return match kind {
                TaskKind::Job => wire::job_exit(id, None, Some("SIGKILL".into()), None, None, None),
                TaskKind::Spawn => wire::spawn_exit(id, None, Some("SIGKILL".into()), None),
            }
            .map_err(io::Error::other);
        }
        let mut child = match ChildProcess::spawn(SpawnOptions {
            command,
            args,
            cwd: spec.cwd,
            env,
        ...
        match job {
            Some((logs, scratch, cwd_file)) => {
                let cwd = match tokio::fs::read_to_string(&cwd_file).await {
                    Ok(cwd) => Some(cwd),
