...
/**
 * Canonical catalog order: flagship family first (Opus > Sonnet > Haiku >
 * others), newest version first.
 */
...
export function parseClaudeModelVersion(id: string): ClaudeVersion | null {
  ...
  const isInteger = (value: string | undefined): value is string => value
    !== undefined
    && /^\d+$/.test(value)
  const isDate = (value: string | undefined): boolean => value !== undefined
    && /^\d{8}$/.test(value)
...
function parseMinimumModelVersion(value: string): ClaudeVersion {
  ...
  if (!match)
    throw new Error(`Invalid minimum Claude model version: ${value}`)
