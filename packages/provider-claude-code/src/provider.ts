...
import {
  coldStartInputMessages,
  controlResponse,
  inferenceItemToClaudeMessage,
...
export interface ClaudeCodeProviderOptions {
  ...
  /**
   * `Host.process`-shaped spawn the CLI runs through (remote execution
   * targets).
   */
  spawn?: ClaudeSpawn
  /**
   * Public CLI env overlay applied last (e.g. `ANTHROPIC_BASE_URL`,
...
interface ActiveClaudeRun {
  ...
  hasStreamed: boolean
  /**
   * Session this live process belongs to; a different session forces a cold
   * restart.
   */
  sessionId: string
  ...
  /**
   * Signature of the first user message, used to detect a rewritten transcript
   * (compaction).
   */
...
export class ClaudeCodeProvider implements AgentProvider {
  ...
  private observeQuotaFromMessage(
    message: unknown,
      ...
      while (true) {
        ...
        if (next.done) {
          ...
          if (!wasAborted && exit.exitCode !== 0) {
            yield {
              type: 'error',
              message: active.transport.stderrText()
        ...
        if (mapped.controlRequest) {
          const handled = await this.handleControlRequest(
            active,
            mapped.controlRequest,
            request
          )
          if (handled === 'tool-call') {
            const event = active.pendingControlRequest
              ? controlRequestToToolCall(active.pendingControlRequest)
              : null
          ...
          if (handled === 'sdk-tool-call') {
            const pending = active.pendingSdkControlRequests.get(
              mapped.controlRequest.toolUseId
                ?? ''
            )
  ...
  /**
   * Returns the live process for this request, reusing the one kept alive from
   * the previous
   * turn whenever possible. Reuse delivers only the *new* input (a pending tool
   * result and/or
   * a freshly appended user message); a cold start is taken only when there is
   * no live process,
   * the session changed, or the transcript was rewritten underneath us
   * (compaction).
   */
  private async ensureActiveForRequest(
    ...
    const credentialId = this.getActiveCredentialId
      ? await this.getActiveCredentialId()
      : null
    ...
    if (existing)
      await this.disposeActive(existing)
  ...
  /**
   * Feeds a reused process only the input it has not seen: pending tool
   * results, then any new user turns.
   */
  private async sendContinuation(
    ...
    if (userCount > active.sentUserMessageCount) {
      const userItems = request.items.filter(
        (
          item
        ): item is Extract<InferenceItem, { type: 'user_message' | 'user_steer' }> =>
      ...
      for (const item of userItems.slice(active.sentUserMessageCount)) {
        ...
        if (message)
          await active.transport.writeJson(message)
      }
  ...
  private async writeSdkMcpToolResults(
    ...
    const results = new Map(
      request.items
        .filter((item): item is Extract<InferenceItem, { type: 'tool_result' }> => item.type
          === 'tool_result')
    ...
    if (missing.length > 0) {
      throw new Error(
        `Claude Code provider missing tool_result for SDK MCP tool_use ${missing.join(', ')}`
      )
    }
    ...
    while (remaining.size > 0) {
      ...
      if (mapped.controlRequest) {
        ...
        if (handled === 'tool-call') {
          throw new Error(
            'Claude Code emitted a legacy tool call while awaiting SDK MCP results'
          )
        }
        continue
      }
  ...
  private async writeToolResults(
    request: InferenceRequest,
    controlRequest: ClaudeControlRequest
  ): Promise<void> {
    if (!this.active)
      throw new Error('No active Claude transport')
    const expectedToolUseId = controlRequest.toolUseId
      ?? String(controlRequest.id)
    const results = request.items.filter((
      item
    ): item is Extract<InferenceItem, { type: 'tool_result' }> => {
      return item.type === 'tool_result' && item.toolUseId === expectedToolUseId
    })
    if (results.length === 0) {
      throw new Error(
        `Claude Code provider missing tool_result for control_request ${String(controlRequest.id)}`
      )
    }
  ...
  private async writeToolResultMessages(
    ...
    if (missing.length > 0) {
      throw new Error(
        `Claude Code provider missing tool_result for tool_use ${missing.join(', ')}`
      )
    }
  ...
  private async initializeSdkMcp(
    active: ActiveClaudeRun,
    systemPrompt: string
  ): Promise<void> {
  ...
  private async writeControlError(
    active: ActiveClaudeRun,
    request: ClaudeControlRequest,
    message: string
  ): Promise<void> {
    ...
    await active.transport.writeJson(controlResponse(
      request.id,
      { error: { message } }
    ))
  }
...
export function createClaudeCodeProvider(
  ...
  const pool = !options.authStore
    && enableCredentials ? openClaudeCodeCredentialPool(
    {
...
function countUserMessages(items: InferenceItem[]): number {
  let count = 0
  for (const item of items) if (item.type === 'user_message'
    || item.type === 'user_steer') count += 1
  return count
}

function firstUserSignature(items: InferenceItem[]): string | null {
  const first = items.find((item) => item.type === 'user_message')
  if (!first || first.type !== 'user_message')
    return null
  // Cheap content fingerprint that avoids hashing large base64 image/document payloads.
  return first.content.map((block) => (block.type === 'text'
    ? `t:${block.text}`
    : block.type)).join('|')
}

/**
 * True when the transcript no longer extends what the live process has already
 * consumed —
 * i.e. user turns were removed or the leading user message changed (compaction
 * rewrote the
 * history). In that case the process must be cold-restarted from the rewritten
 * transcript.
 */
function itemsDiverged(active: ActiveClaudeRun, items: InferenceItem[]): boolean {
  if (countUserMessages(items) < active.sentUserMessageCount)
    return true
  if (active.firstUserSig !== null
