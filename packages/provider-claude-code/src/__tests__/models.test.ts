...
test(
  'Provider.listModels forwards refresh and retains the configured Claude model filter',
  async () => {
    resetClaudeCodeModelCatalogCacheForTests()
    const fetch = spyOn(globalThis, 'fetch').mockImplementation(Object.assign(
      async () => Response.json(modelsDevFixture()),
      { preconnect() {} }
    ))
    try {
      const provider = createClaudeCodeProvider({
        credentials: false,
        models: { include: ['claude-opus-4-8'] }
      })
      ...
      expect(refreshed.models.map((model) => model.id))
...
test(
  'parseClaudeModelVersion handles Claude full ids and date snapshots',
  () => {
    expect(parseClaudeModelVersion('claude-opus-4-8')).toEqual({
      major: 4,
      minor: 8
    })
    expect(parseClaudeModelVersion('claude-opus-4-5-20251101')).toEqual({
      major: 4,
      minor: 5
    })
    expect(parseClaudeModelVersion('claude-fable-5')).toEqual({
      major: 5,
      minor: 0
    })
    expect(parseClaudeModelVersion('claude-3-5-sonnet-20241022')).toEqual({
      major: 3,
      minor: 5
    })
    expect(parseClaudeModelVersion('claude-sonnet-4-20250514')).toEqual({
      major: 4,
      minor: 0
    })
...
test(
  'models.dev catalog keeps Claude full ids at or above minimum version without family allowlists',
  () => {
    ...
    expect(list.models.map((model) => model.id))
      .not.toContain('claude-haiku-4-5')
    expect(list.models.map((model) => model.id))
      .not.toContain('claude-3-5-sonnet-20241022')
    expect(
      list.models.every((model) => !model.id.startsWith('anthropic/'))
    ).toBe(true)
  }
)

test(
  'models.dev mapping preserves explicit metadata and leaves missing capabilities null',
  () => {
    const list = modelsDevAnthropicCatalogToModelList(modelsDevFixture(), {
      minimumModelVersion: '4.6',
      sourceFetchedAt: '2026-06-20T00:00:00.000Z',
    ...
    expect(list.models.find((model) => model.id === 'claude-opus-4-8'))
      .toMatchObject(
      {
    ...
    expect(
      list.models.find((model) => model.id === 'claude-newfamily-5')
    ).toMatchObject(
      {
        contextWindow: null,
        outputLimit: null,
...
test(
  'listClaudeCodeModels uses stale cache on network failure and never returns hardcoded fallback models',
  async () => {
    resetClaudeCodeModelCatalogCacheForTests()
...
test(
  'listClaudeCodeModels filters one shared models.dev snapshot per minimum version',
  async () => {
    resetClaudeCodeModelCatalogCacheForTests()
    const now = new Date('2026-06-20T00:00:00.000Z')
    let calls = 0
    const fetchFixture = async () => {
      ...
      return Response.json(
        modelsDevFixture(),
        { headers: { etag: `fixture-etag-${calls}` } }
      )
    }
    ...
    expect(calls).toBe(1)
    expect(defaultList.models.map((model) => model.id))
      .toContain('claude-sonnet-4-6')
    expect(majorFiveList.models.map((model) => model.id)).toEqual([
      'claude-fable-5',
      'claude-newfamily-5'
    ])
    resetClaudeCodeModelCatalogCacheForTests()
