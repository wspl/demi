...
test(
  'requestToInputMessages groups Claude assistant turns and tool results',
  () => {
    const messages = requestToInputMessages({
      sessionId: 'test-session',
      turnId: 'test-turn',
      ...
      items: [
        ...
        },
        {
          ...
          modelId: 'model',
          toolUseId: 'tool-2',
          toolName: 'mcp__main__shell_exec',
          input: { script: 'ls' }
        },
        {
          type: 'tool_result',
...
test(
  'requestToInputMessages skips unsigned assistant thinking for Claude replay',
  () => {
...
test(
  'requestToInputMessages converts binary media content to Claude base64 sources',
  () => {
    const messages = requestToInputMessages({
      sessionId: 'test-session',
      turnId: 'test-turn',
...
test(
  'requestToInputMessages preserves unicode, long fields, and mixed tool results',
  () => {
...
test(
  'mapClaudeStdoutMessage reports the last iteration as response usage, not the turn total',
  () => {
        ...
        usage: {
          ...
          iterations: [
            ...
            },
            {
              input_tokens: 20,
              output_tokens: 200,
              cache_read_input_tokens: 689_277,
