...
test(
  'runClaudeCodeLogin builds a PKCE authorize URL and exchanges the pasted code',
  async () => {
    const calls: Array<{
      url: string;
      body: string
...
test(
  'refreshClaudeCodeSecret renews tokens and keeps the refresh token on rotation gaps',
  async () => {
