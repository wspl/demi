...
function makeRequestWithoutTools(
...
test(
  ...
  async () => {
    ...
    expect(
      (provider as unknown as { transportFactory?: unknown }).transportFactory
    ).not.toBe(injectedFactory)
  }
)
...
test(
  'ClaudeCodeProvider streams text and response events from transport messages',
  async () => {
    ...
    expect(transport.writes.find((write) => isRecord(write)
      && write.type === 'user')).toMatchObject({
      type: 'user'
    })
...
test(
  'ClaudeCodeProvider preserves cache usage fields from result messages',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'assistant',
...
test(
  'ClaudeCodeProvider handles empty successful streams without leaking transport state',
  async () => {
    const first = new FakeClaudeTransport([])
    const second = new FakeClaudeTransport([
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'next run' }] }
      },
...
test(
  'ClaudeCodeProvider forwards context overflow result errors with usage',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'result',
...
test(
  'ClaudeCodeProvider handles control_request tool calls across run calls',
  async () => {
    const transport = new FakeClaudeTransport([
      {
      ...
      },
      {
        ...
        id: 'call-1',
        method: 'tools/call',
        params: { name: 'mcp__main__shell_exec', arguments: { script: 'pwd' } }
      },
      {
        type: 'assistant',
    ...
    const firstToolUseId = firstEvents[0]?.type === 'tool_call_requested'
...
test(
  ...
  async () => {
    const transport = new FakeClaudeTransport([
      ...
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'after sdk tool' }] }
      },
...
test(
  'ClaudeCodeProvider exposes one Claude SDK MCP tool batch before replying to any call',
  async () => {
...
test(
  'ClaudeCodeProvider forwards tool_result images to the SDK MCP response as passthrough base64',
  async () => {
    const transport = new FakeClaudeTransport([
      sdkMcpRequest('list-sdk', 'list-1', 'tools/list'),
      sdkMcpRequest(
        ...
        'call-1',
        'tools/call',
        { name: 'shell_exec', arguments: { script: 'shot' } }
      ),
      {
        type: 'assistant',
    ...
    const toolUseId = firstEvents[0]?.type === 'tool_call_requested'
      ? firstEvents[0].toolUseId
      : ''
...
test(
  'ClaudeCodeProvider renders prior tool calls as text on a cold start, never as structured tool_use',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'assistant',
    ...
    const structuredToolUse = transport.writes.some((write) => {
      return (
        ...
        write.message.content.some((block) => isRecord(block)
          && block.type === 'tool_use')
      )
    })
    ...
    const assistantWrite = transport.writes.find((
      write
    ): write is {
      type: 'assistant';
      message: { content: unknown[] }
    } => {
      return isRecord(write) && write.type === 'assistant'
        && isRecord(write.message)
        && Array.isArray(write.message.content)
    })
    expect(JSON.stringify(assistantWrite?.message.content))
    ...
    const userWrites = transport.writes.filter((
      write
    ): write is {
      type: 'user';
      message: { content: unknown[] }
    } => {
      return isRecord(write) && write.type === 'user' && isRecord(write.message)
        && Array.isArray(write.message.content)
    })
...
test(
  'ClaudeCodeProvider keeps one process alive across turns and sends only the new user message',
  async () => {
    ...
    const transport = new FakeClaudeTransport([
      ...
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'turn one done' }] }
      },
      { type: 'result', usage: { input_tokens: 1 } },
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'turn two done' }] }
      },
      { type: 'result', usage: { input_tokens: 1 } },
    ])
    const provider = new ClaudeCodeProvider({
    ...
    expect(contEvents).toMatchObject([
      { type: 'text_delta', text: 'turn one done' },
      { type: 'response' }
    ])
    ...
    expect(secondEvents).toMatchObject([
      { type: 'text_delta', text: 'turn two done' },
      { type: 'response' }
    ])
    ...
    expect(transport.killed).toBe(false)
    expect(
      transport.writes.filter((write) => isSdkInitializeRequest(write))
    ).toHaveLength(1)
...
test(
  'ClaudeCodeProvider clone owns independent live process state',
  async () => {
    const parentTransport = new FakeClaudeTransport([
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'parent one' }] }
      },
      { type: 'result', usage: { input_tokens: 1, output_tokens: 1 } },
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'parent two' }] }
      },
      { type: 'result', usage: { input_tokens: 1, output_tokens: 1 } },
    ])
    const cloneTransport = new FakeClaudeTransport([
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'clone' }] }
      },
      { type: 'result', usage: { input_tokens: 1, output_tokens: 1 } },
    ])
    const provider = new ClaudeCodeProvider({
    ...
    expect(parentTransport.writes.filter((write) => isRecord(write)
      && write.type === 'user')).toHaveLength(2)
    expect(cloneTransport.writes.filter((write) => isRecord(write)
      && write.type === 'user')).toHaveLength(1)
...
test(
  'ClaudeCodeProvider rejects malformed SDK MCP tools/call without entering pending state',
  async () => {
    const transport = new FakeClaudeTransport([
      sdkMcpRequest(
        'call-sdk',
      ...
      ),
      {
        type: 'assistant',
        message: {
          content: [{ type: 'text', text: 'continued after malformed sdk call' }]
        }
      },
      { type: 'result', usage: { input_tokens: 3, output_tokens: 1 } },
    ])
...
test(
  'ClaudeCodeProvider handles assistant tool_use messages across run calls',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'assistant',
      ...
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'after native tool' }] }
      },
...
test(
  'ClaudeCodeProvider fails fast when an assistant tool_use has no matching tool_result',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'assistant',
      ...
      },
      {
        type: 'assistant',
        message: {
          content: [{ type: 'text', text: 'after missing native tool result' }]
        }
      },
    ])
    const provider = new ClaudeCodeProvider({
    ...
    const secondRun = async () => {
      for await (const _event of provider.run(makeRequestWithoutTools([{
        type: 'user_message',
        content: [{ type: 'text', text: 'missing result' }]
      }]))) {
        // The provider should throw before reading more Claude output.
      }
    }
    await expect(secondRun())
      .rejects.toThrow('missing tool_result for tool_use native-tool-1')
    expect(transport.killed).toBe(true)
    expect(transport.waitCalls).toBe(1)
  }
)
...
test(
  'ClaudeCodeProvider reports malformed assistant tool_use blocks as provider errors',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'assistant',
      ...
      },
      {
        type: 'assistant',
        message: {
          content: [{ type: 'text', text: 'after malformed tool use' }]
        }
      },
    ])
    const provider = new ClaudeCodeProvider({
...
test(
  'ClaudeCodeProvider terminates active transport when consumers stop after a provider error event',
  async () => {
    const transport = new FakeClaudeTransport([
...
test(
  'ClaudeCodeProvider fails fast when a pending control_request has no matching tool_result',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'control_request',
      ...
      },
      {
        type: 'assistant',
        message: {
          content: [{ type: 'text', text: 'after missing tool result' }]
        }
      },
    ])
    const provider = new ClaudeCodeProvider({
    ...
    const secondRun = async () => {
      for await (const _event of provider.run(makeRequest([{
        type: 'user_message',
        content: [{ type: 'text', text: 'missing result' }]
      }]))) {
        // The provider should throw before reading more Claude output.
    ...
    await expect(secondRun())
      .rejects.toThrow('missing tool_result for control_request call-1')
    expect(transport.killed).toBe(true)
    expect(transport.waitCalls).toBe(1)
    expect(transport.writes.some((write) => isRecord(write)
      && write.type === 'control_response'
      && write.id === 'call-1')).toBe(false)
  }
)

test(
  'ClaudeCodeProvider rejects malformed tools/call control requests without entering pending state',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'control_request',
...
test(
  'ClaudeCodeProvider reports nonzero CLI exits that do not produce result messages',
  async () => {
    const transport = new FakeClaudeTransport(
      [],
      { exitCode: 1, stderr: 'auth failed' }
    )
    const provider = new ClaudeCodeProvider({
      transportFactory: fakeFactory(transport)
    })
    ...
    expect(events)
      .toEqual([{ type: 'error', message: 'auth failed', code: null }])
  }
)

test(
  'ClaudeCodeProvider clears and terminates active transport when stdout iteration fails',
  async () => {
    const broken = new FakeClaudeTransport(
      [],
      { throwNext: new Error('bad json') }
    )
    ...
    const provider = new ClaudeCodeProvider({
      transportFactory: sequenceFactory([broken, recovered])
    })
    ...
    expect(recovered.writes.find((write) => isRecord(write)
      && write.type === 'user')).toMatchObject({
      type: 'user'
    })
    expect(events).toEqual([
  ...
  async () => {
    const transport = new FakeClaudeTransport([
      ...
      },
      {
        ...
        params: {
          name: 'mcp__main__shell_exec',
          arguments: { script: 'printf demi-provider', timeoutMs: 1_000 }
        },
      },
      {
        type: 'assistant',
...
test(
  'ClaudeCodeProvider keeps repeated MCP request ids distinct in AgentSession',
  async () => {
    const transport = new FakeClaudeTransport([
      {
        type: 'control_request',
      ...
      },
      {
        ...
        params: {
          name: 'mcp__main__shell_exec',
          arguments: { script: 'printf first-tool', timeoutMs: 1_000 }
        },
      },
      {
        type: 'control_request',
        ...
        params: {
          name: 'mcp__main__shell_exec',
          arguments: { script: 'printf second-tool', timeoutMs: 1_000 }
        },
      },
      {
        type: 'assistant',
    ...
    const toolBlocks = session.transcript()
    ...
    const responses = transport.writes.filter((
      ...
      return isRecord(write) && write.type === 'control_response'
        && write.id === '0'
...
test(
  'ClaudeCodeProvider cold-restarts with a new process when the model changes mid-session',
  async () => {
    ...
    const onModelA = new FakeClaudeTransport([
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'answered on model A' }] }
      },
      { type: 'result', usage: { input_tokens: 1 } },
    ])
    const onModelB = new FakeClaudeTransport([
      {
        type: 'assistant',
        message: { content: [{ type: 'text', text: 'answered on model B' }] }
      },
      { type: 'result', usage: { input_tokens: 1 } },
    ])
    const provider = new ClaudeCodeProvider({
      transportFactory: sequenceFactory([onModelA, onModelB])
    })
    ...
    expect(turn1).toMatchObject([
      { type: 'text_delta', text: 'answered on model A' },
      { type: 'response' }
    ])
    ...
    expect(turn2).toMatchObject([
      { type: 'text_delta', text: 'answered on model B' },
      { type: 'response' }
    ])
    ...
    expect(onModelB.writes.some((write) => isRecord(write)
      && write.type === 'user')).toBe(true)
  }
)

test(
  'ClaudeCodeProvider starts a new process for the next turn after switching accounts',
  async () => {
...
function sequenceFactory(
  ...
  return {
    start: async () => {
      ...
      index += 1
      if (!transport)
        throw new Error('No fake transport left')
      return transport
    },
  }
}
...
function findControlResponse(
  ...
  if (!found)
    throw new Error(`missing control_response ${id}`)
  return found
}

function sdkMcpRequest(
  outerId: string,
  id: string,
  method: string,
  params?: unknown
...
function findSdkMcpResponse(
  ...
  const found = writes.find((
    write
  ): write is { response: { response: { mcp_response: {
    result?: unknown;
    error?: unknown
  } } } } => {
    return (
  ...
  if (!found)
    throw new Error(`missing SDK MCP response ${outerId}`)
