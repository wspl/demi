  ...
  async () => {
    ...
    try {
      const started = await shell.exec({
        script: 'attend first | attend second',
        timeoutMs: 1
      })
      ...
      const hint = (
        ...
        value: string | null,
        jobId = job.jobId
      ) => host.handleMessage(wire.decodeRunnerToBackend(wire.encode({
      ...
      expect(
        'runningHint' in await shell.status({ commandId: started.commandId })
      ).toBe(false)
