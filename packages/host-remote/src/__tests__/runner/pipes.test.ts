...
test(
  'job pipes: stdin is fetched into the job, stdout is streamed out as it is written, both report pipe_done',
  async () => {
    ...
    const server = Bun.serve({
      ...
      async fetch(request, bunServer) {
        ...
        if (url.pathname === '/api/pipes/early')
          return new Response(
            'refused early',
            { status: 404 }
          )
        ...
        if (request.method === 'PUT'
          && url.pathname.startsWith('/api/pipes/out')) {
          ...
          uploads.get(url.pathname)?.resolve(new Uint8Array(await new Blob(chunks)
            .arrayBuffer()))
    ...
    const doneFor = (pipeId: string) => inbound.find((
      ...
      === 'pipe_done'
      && m.pipeId === pipeId)
    const exitOf = (jobId: string) => inbound.find((
      m
    ): m is Extract<RunnerToBackendMessage, { type: 'job_exit' }> => m.type
      === 'job_exit'
      && m.jobId === jobId)
    ...
    send({
      type: 'job_start', context: TEST_COMMAND_CONTEXT,
      jobId: 'j1',
      script: 'tr a-z A-Z',
      cwd: runnerDir,
    ...
    await waitFor(
      () => uploaded !== undefined,
      () => `${runner.log.join('\n')}\n${JSON.stringify(inbound.slice(-8).map((m) => m.type === 'job_output' ? { type: m.type, bytes: m.bytes.length } : m))}`,
      { timeoutMs: 15_000 }
    )
    if (!uploaded)
    ...
    expect(
      inbound.filter((m) => m.type === 'job_output' && m.jobId === 'j1').reduce(
        (n, m) => n + (m.type === 'job_output' ? m.bytes.byteLength : 0),
        0
      )
    ...
    await waitFor(
      () => doneFor('gone') !== undefined && exitOf('j2') !== undefined,
      () => runner.log.join('\n'),
      { timeoutMs: 20_000 }
    )
    expect(doneFor('gone')).toEqual({
      type: 'pipe_done',
      pipeId: 'gone',
      ok: false,
    ...
    await waitFor(
      () => doneFor('missing') !== undefined && exitOf('j3') !== undefined,
      () => runner.log.join('\n'),
      { timeoutMs: 20_000 }
    )
    expect(doneFor('missing')?.ok).toBe(false)
    expect(
      new TextDecoder().decode(exitOf('j3')?.output?.stdoutTail).trim()
    ).toBe('0')

    // Refusal before the first output byte cancels a pending read without stopping the job log.
    send({
      type: 'job_start', context: TEST_COMMAND_CONTEXT, jobId: 'j4', script: 'sleep 0.1; head -c 2000000 /dev/zero',
      cwd: runnerDir, env: {
    ...
    await waitFor(
      () => doneFor('early') !== undefined && exitOf('j4') !== undefined,
      () => runner.log.join('\n'),
      { timeoutMs: 10_000 }
    )
