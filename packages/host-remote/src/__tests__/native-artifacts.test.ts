...
test('artifact requests require the active job and its exact manifest artifact', async () => {
  ...
  await Bun.sleep(0)
  expect(calls).toBe(0)
  expect(messages.at(-1)).toMatchObject({ type: 'artifact_location', error: 'Artifact does not belong to the live work\'s packages' })
  ...
  host.handleMessage(request)
  expect(calls).toBe(1)
  expect(messages.at(-1)).toMatchObject({ error: 'No matching live job or stream' })
  host.detach()
})
