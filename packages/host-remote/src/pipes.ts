...
// writes them itself or attaches them to a job elsewhere. The broker does
// no network IO: the backend's HTTP routes hand it the device ends.
import type { PipeRef } from '@demicodes/runner-protocol'
import {
  ByteChannel,
  createId,
  deferred,
...
export interface Pipe {
  ...
  /** Ends the pipe for both ends: readers and the writer see `reason`. */
...
export class PipeBroker {
  ...
  open(source?: DeviceEnd, sink?: DeviceEnd): Pipe {
    ...
    return {
      ...
      fail: (reason) => this.fail(id, reason),
    }
  }
...
/**
 * The pipes a Host's file contents travel through (`runner.md` § File
 * contents), each with that Host's runner at the far end.
 */
export interface HostPipes {
  /** The runner writes into it; this process reads it. */
  fromRunner(): Pipe
  /** This process writes into it; the runner reads it. */
  toRunner(): Pipe
}

/** The pipes of one device's Host, minted by the broker. */
export function devicePipes(broker: PipeBroker, deviceId: string): HostPipes {
  return {
    fromRunner: () => broker.open({ deviceId }),
    toRunner: () => broker.open(undefined, { deviceId }),
  }
}
