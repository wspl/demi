...
test(
  'AgentSessionCommandStorage prefixes keys by agent session id and exposes session-local keys',
  async () => {
...
test(
  'AgentSessionCommandStorage rejects keys and agent session ids that escape the session prefix',
  async () => {
    const store = new MemoryHostStore()
    const first = new AgentSessionCommandStorage(store, 'session-a')
    const second = new AgentSessionCommandStorage(store, 'session-b')
    ...
    await expect(Promise.resolve().then(() => first.writeJson(
      '../session-b/todos.json',
      [{ text: 'hijack' }]
    ))).rejects.toThrow(
      'path traversal',
    )
    ...
    expect(() => new AgentSessionCommandStorage(store, '../session-b'))
      .toThrow('Invalid command storage agent session id')
