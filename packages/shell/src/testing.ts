 ...
 * as cases any harness can run — `bun:test` for a Host on Bun, the txiki.js
...
export interface HostConformanceOptions {
  host: Host
  /**
   * A fresh, writable, absolute directory on the host; every case works under
   * it.
   */
  root: string
...
export function hostConformanceCases(
  ...
  const processCase = (
    name: string,
    run: (spawn: Host['process']['spawn']) => Promise<void>
  ): HostConformanceCase[] =>
    [{ name, run: () => run(spawn) }]
  ...
  return [
    {
      ...
      run: async () => {
        ok(
          host.defaultCwd.startsWith('/'),
          `defaultCwd is absolute: ${host.defaultCwd}`
        )
        ok(
          Number.isInteger(host.identity.uid)
            && Number.isInteger(host.identity.gid),
          'identity uid/gid are integers'
        )
        equal(typeof host.identity.hostname, 'string', 'identity hostname')
      },
    },
    ...processCase('process: spawn captures stdout and the exit code', async (
      spawn
    ) => {        const result = await outputOf(await spawn({
      command: 'printf',
      args: ['hello\\n'],
      env
    }))
    ...
    ...processCase(
      'process: stdout and stderr are separate; a non-zero exit is reported',
      async (
        spawn
      ) => {        const result = await outputOf(await spawn({
        command: 'sh',
    ...
    ...processCase(
      'process: kill ends a foreground process with SIGTERM',
      async (
        spawn
      ) => {        const handle = await spawn({
        command: 'sleep',
    ...
    ...processCase(
      'process: children receive only the env passed to spawn',
      async (
        spawn
      ) => {        const result = await outputOf(await spawn({
        command: 'printenv',
    ...
    ...processCase(
      'process: cwd is honoured',
      async (spawn) => {        const dir = `${root}/cwd-honoured`
          ...
          equal(result.exit.exitCode, 0, 'exit code')
          ok(
            result.stdout.trimEnd().endsWith('/cwd-honoured'),
            `pwd inside the cwd: ${result.stdout}`
          )
      }
    ),
    ...processCase('process: a missing binary is executable_not_found', async (
      spawn
    ) => {        const result = await outputOf(await spawn({
      command: 'definitely-not-a-host-binary',
      env
    }))
        equal(result.exit.exitCode, null, 'exit code')
        equal(
          result.exit.spawnError?.kind,
          'executable_not_found',
          'spawn error kind'
        )
    }),
    ...processCase(
      'process: a missing cwd is cwd_unusable, not a missing binary',
      async (
        spawn
      ) => {        const result = await outputOf(await spawn({
        command: '/bin/echo',
        args: ['ok'],
        cwd: `${root}/never-created`,
        env
      }))
          equal(result.exit.exitCode, null, 'exit code')
          equal(
    ...
    ...processCase(
      'process: a missing binary with a live cwd is executable_not_found',
      async (
        spawn
    ...
    ...processCase(
      'process: openCwd walks directories, refuses missing ones, snapshots and restores',
      async (
        spawn
          ...
          try {
            await cwd.chdir('sub')
            ok(
              cwd.path.endsWith('/open-cwd/sub'),
              `relative chdir: ${cwd.path}`
            )
            await cwd.chdir('..')
            ...
            ok(
              cwd.path.endsWith('/open-cwd/sub'),
              `absolute chdir: ${cwd.path}`
            )
            snapshot.restore()
            ok(cwd.path.endsWith('/open-cwd'), `restored: ${cwd.path}`)
            equal(
              await codeOf(() => cwd.chdir('missing')),
              'ENOENT',
              'chdir into a missing directory'
            )
          } finally {
            await cwd.close()
          }
          equal(
            await codeOf(() => host.process.openCwd(`${root}/open-cwd-missing`)),
            'ENOENT',
            'openCwd on a missing directory'
          )
      }
    ),
    {
      ...
      run: async () => {
        ...
        equal(
          [stat.isFile, stat.isDirectory, stat.isSymbolicLink, stat.size],
          [true, false, false, 'hello\ntail\n'.length],
          'stat'
        )
        ok(
          stat.mtime instanceof Date && !Number.isNaN(stat.mtime.getTime()),
          'stat mtime is a Date'
        )
        equal(
          (await fs.stat('src', { cwd: dir })).isDirectory,
          true,
          'stat of a directory'
        )
        await fs.rm('src/file.txt', { cwd: dir, force: true })
        equal(
          await fs.exists('src/file.txt', { cwd: dir }),
          false,
          'exists after rm'
        )
        equal(
          await fs.exists('src', { cwd: dir }),
          true,
    ...
    },
    {
      ...
      run: async () => {
        ...
        equal(
          await codeOf(() => fs.rm(`${dir}/absent`)),
          'ENOENT',
          'rm without force on a missing path'
        )
      },
    },
    {
      ...
      run: async () => {
        ...
        equal(
          (await fs.lstat(`${dir}/link`)).isSymbolicLink,
          true,
          'lstat sees the link'
        )
        equal(
          (await fs.stat(`${dir}/link`)).isFile,
          true,
          'stat follows the link'
        )
        ok(
          (await fs.realpath(`${dir}/link`)).endsWith('/links/target.txt'),
    ...
    },
    {
      ...
      run: async () => {
        ...
        await fs.mkdir(dir, { recursive: true })
        equal(
          await codeOf(() => fs.readFile(`${dir}/missing`)),
          'ENOENT',
          'readFile of a missing file'
        )
        equal(
          await codeOf(() => fs.stat(`${dir}/missing`)),
          'ENOENT',
          'stat of a missing file'
        )
        equal(
          await codeOf(() => fs.mkdir(dir)),
          'EEXIST',
          'mkdir of an existing directory'
        )
        equal(
          await codeOf(() => fs.readdir(`${dir}/missing`)),
          'ENOENT',
          'readdir of a missing directory'
        )
      },
    },
    ...processCase(
      'fs and process share one namespace: a file written through fs is read by a process',
      async (
        spawn
          ...
          const wrote = await outputOf(await spawn({
            command: 'sh',
            args: ['-c', 'printf back > written.txt'],
            cwd: dir,
            env
          }))
    ...
    ),
    {
      ...
      run: async () => {
        ...
        ok(isRecord(a), 'object round trip')
        ...
        equal(
          await store.list('conformance'),
          ['conformance/a', 'conformance/b'],
          'list by prefix'
        )
        await store.delete('conformance/a')
        equal(
          await store.readJson('conformance/a'),
          null,
          'deleted key reads null'
        )
        equal(
          await store.list('conformance'),
          ['conformance/b'],
...
export function memoryCommandStorage(): CommandStorage {
  ...
  const bind = (signal?: AbortSignal): CommandStorage => ({
    ...
    updateJson: async <T>(key: string, update: (current: unknown) => T) => {
      signal?.throwIfAborted()
      const next = structuredClone(update(structuredClone(values.get(key) ?? null)))
      values.set(key, next)
