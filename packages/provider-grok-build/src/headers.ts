...
/**
 * Official Grok CLI crate version, used when no local `~/.grok/version.json`
 * exists.
 */
...
export function buildGrokBuildHeaders(
  ...
  request?: Pick<InferenceRequest, 'sessionId'
    | 'requestId'
    | 'modelId'
    | 'turnId'>,
  options?: {
  ...
  headers.set(
    ...
    resolveGrokClientVersion(options?.clientVersion, options?.grokHome)
  )
  headers.set(
    'x-grok-client-identifier',
    nonEmptyString(process.env.GROK_CLIENT_NAME) ?? GROK_CLIENT_IDENTIFIER
  )
  ...
  if (auth.email)
    headers.set('x-email', auth.email)
  if (request?.modelId)
    headers.set('x-grok-model-override', request.modelId)
  if (request?.sessionId) {
    headers.set('x-grok-session-id', request.sessionId)
    headers.set('x-grok-conv-id', request.sessionId)
  }
  if (request?.requestId)
    headers.set('x-grok-req-id', request.requestId)
  if (request?.turnId)
    headers.set('x-grok-turn-idx', request.turnId)
  return headers
}

export function resolveGrokClientVersion(
  explicit?: string,
  grokHome?: string
): string {
