...
test(
  'runGrokDeviceLogin drives the official device-flow contract and assembles a vendor-shaped entry',
  async () => {
    const calls: Array<{
      url: string;
      body: string;
    ...
    const fakeFetch = (async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.endsWith('/user')) {
        return jsonResponse(
          200,
          { userId: 'user_1', firstName: 'G', email: 'g@example.com' }
        )
      }
    ...
    expect(pendings[0]!.verificationUrl)
      .toBe('https://auth.x.ai/activate?user_code=GROK-1234')
    expect(pendings[0]!.userCode).toBe('GROK-1234')

    expect(entryKey)
      .toBe('https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828')
    ...
    expect(deviceParams.get('client_id'))
      .toBe('b1a00492-073a-47ea-816f-4c329264a828')
    ...
    expect(tokenCall[0]!.body).toContain(
...
test(
  'runGrokDeviceLogin seeds team principal from the access token',
  async () => {
    const access = jwt({
      sub: 'user-42',
      principal_type: 'Team',
    ...
    const fakeFetch = (async (input: string | URL | Request) => {
      ...
      if (url.endsWith('/oauth2/token')) {
        return jsonResponse(
          200,
          { access_token: access, refresh_token: 'rt', expires_in: 3600 }
        )
      }
      if (url.endsWith('/user'))
