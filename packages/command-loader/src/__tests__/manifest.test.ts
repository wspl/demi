...
describe('buildManifest', () => {
  ...
  test(
    ...
    async () => {
      ...
      expect(input.properties.from).toMatchObject({
        type: 'string',
      ...
      if (!isManifestGroup(note))
        throw new Error('note')
      const add = note.subcommands[0]!
      if (isManifestGroup(add))
        throw new Error('add')
  ...
  test(
    ...
    async () => {
      ...
      expect(
        rebuilt.map((root) => !isCommandGroup(root) && root.runningHint)
      ).toEqual(
        [
          'rpc hint',
          'runtime hint'
...
describe('treeFromManifest', () => {
  ...
  test('input schemas validate as declared after the round trip', async () => {
    ...
    expect(
      z.object(copy.input!).safeParse({ from: 'a', to: 'b', upper: true })
        .success
    ).toBe(true)
  })
})
