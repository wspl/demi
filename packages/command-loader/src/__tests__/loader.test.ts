...
async function world(withRpc = true) {
  ...
  const run = async (
    ...
    const io: DispatchIO = {
      ...
      stdout: (data) => void out.push(typeof data === 'string'
        ? encodeUtf8(data)
        : data),
      stderr: (data) => void err.push(typeof data === 'string'
        ? encodeUtf8(data)
        : data),
      cwd: dir,
      env,
    ...
    return {
      exit,
      stdout: decodeUtf8(await collectBytes(bytesStream(Buffer.concat(out)))),
      stderr: decodeUtf8(Buffer.concat(err))
    }
  }
  return {
    dir,
...
describe('dispatch', () => {
    ...
    async () => {
      ...
      try {
        ...
        const result = await w.run(
          'scout',
          ['copy', 'a.txt', 'b.txt', '--upper']
        )
        expect(result).toEqual({
          exit: 0,
          stdout: `copied a.txt -> b.txt in ${w.dir}\n`,
          stderr: ''
        })
        expect(
          decodeUtf8(await w.host.fs.readFile('b.txt', { cwd: w.dir }))
        ).toBe('HELLO\n')
      } finally {
        w.dispose()
      }
  ...
  test(
    'an rpc leaf runs its handler through the transport, with stdin and --json',
    async () => {
      const w = await world()
      try {
        ...
        expect(await w.run('scout', ['note', 'add'], 'from stdin')).toEqual({
          exit: 0,
          stdout: '2 notes\n',
          stderr: ''
        })
  ...
  test(
    'a group prints its help; usage errors exit 1 with the reason; an unknown root exits 127',
    async () => {
      const w = await world()
      try {
        ...
        expect(bad.stderr).toBe(
          'scout: Invalid value for "to": Invalid input: expected string, received undefined\n'
        )
        const unknown = await w.run('nope', ['x'])
        expect(unknown.exit).toBe(127)
        expect(unknown.stderr)
          .toBe('nope: not a root command of this manifest\n')
      } finally {
        w.dispose()
      }
    }
  ...
  test(
    'the loader exposes the manifest and the reconstructed roots',
    async () => {
      const w = await world()
      try {
