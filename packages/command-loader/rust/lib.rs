...
pub enum Error {
    ...
    #[error(transparent)]
    Protocol(#[from] demi_command_protocol::ProtocolError),
}

static SCHEMA: LazyLock<jsonschema::Validator> = LazyLock::new(|| {
    let value = serde_json::from_str(include_str!("manifest.schema.json"))
        .expect("generated manifest JSON");
    jsonschema::validator_for(&value).expect("generated manifest schema")
});
...
impl Manifest {
    pub fn parse(value: Value) -> Result<Self, Error> {
        ...
        for (hash, descriptor) in &manifest.packages {
            let checked =
                demi_command_protocol::PackageDescriptor::parse(serde_json::to_value(descriptor)?)?;
            if !ids.insert(&descriptor.id) || checked.digest()? != *hash {
        ...
        struct Body<'a> {
            ...
            packages: &'a BTreeMap<String, demi_command_protocol::PackageDescriptor>,
        }
        let body = Body {
            roots: &manifest.roots,
            packages: &manifest.packages,
        };
        if demi_command_protocol::canonical_digest(&body)? != manifest.hash {
...
mod tests {
    ...
    fn typescript_manifest_help_and_cli_cases_match_rust() {
        let fixture: Value = serde_json::from_str(include_str!("fixtures/cli.json")).unwrap();
