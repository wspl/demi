/**
 * Builds a genuinely large, real long-session fixture (gzipped) next to this
 * file.
 *
 * Plants three secrets, then grows history under DeepSeek V4 Flash until the
 * session
 * has compacted a target number of generations. Prefer refreshing only when the
 * committed fixture is intentionally replaced — verifiers reuse the existing
 * blob.
 *
...
const BUILD_CONTEXT_WINDOW = Number(process.env.COMPACTION_FIXTURE_BUILD_WINDOW
  ?? 100_000)
...
const totalTokens = (): number => Math.round(JSON.stringify(session.transcript()
  .blocks)
  .length / 4)
const boundaries = (): number => session.transcript()
...
log(
  `plant secrets — target ${TARGET_GENERATIONS} generations on deepseek-v4-flash (window=${BUILD_CONTEXT_WINDOW})`
)
await session.send([
  {
    type: 'text',
...
let gens = 0
for (let i = 0; i < files.length && boundaries() < TARGET_GENERATIONS
  && totalTokens() < HARD_TOKEN_CAP; i += 1) {
  try {
    await session.send([{
      type: 'text',
      text: `运行 \`cat ${files[i]}\` 读取这个文件,然后用一句话说明它的职责。`
    }])
  } catch (e) {
  ...
  if (boundaries() > gens) {
    gens = boundaries()
    log(
      `  ── compaction generation ${gens} reached after ${i + 1} files (total≈${totalTokens()} tokens, ${session.transcript().blocks.length} blocks)`
    )
  } else if (i % 3 === 0) {
    log(
      `  read ${i + 1} files — total≈${totalTokens()} tokens, ${boundaries()} generations, ${session.transcript().blocks.length} blocks`
    )
  }
}
...
if (stripped > 0)
  log(
    `  stripped ${stripped} transient error block(s) from the saved transcript`
  )
...
log(
  `   total≈${totalTokens()} tokens  blocks=${blocks.length}  compaction generations=${boundaries()}`
)
log(`   block types: ${[...new Set(blocks.map((b) => b.type))].join(', ')}`)
log(
  `   secrets still present in full transcript: ALPHA${has('ZEBRA-7')} BETA${has('QUARTZ-9')} GAMMA${has('NIMBUS-3')}`
)
