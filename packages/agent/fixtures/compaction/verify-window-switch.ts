/**
 * Window-switch acceptance on the cached large fixture, using DeepSeek V4 Flash
 * only.
 *
 *   STEP 1  small window → large window  ⇒ expect NO compaction
 *   STEP 2  grow replayable context on the large window
 * STEP 3 large window → small window ⇒ expect FORCED compaction by the
 * pre-switch model
...
const SMALL_WINDOW = Number(process.env.COMPACTION_FIXTURE_SMALL_WINDOW
  ?? 8_000)
const LARGE_WINDOW = Number(process.env.COMPACTION_FIXTURE_LARGE_WINDOW
  ?? 400_000)
...
const recallCount = (text: string): number => RX.filter((re) => new RegExp(
  re,
  'i'
).test(text)).length
...
async function send(label: string, text: string): Promise<string> {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      lastErr = e
      log(
        `   (${label} attempt ${attempt + 1} failed: ${String(e).slice(0, 70)})`
      )
    }
  }
  throw lastErr
...
log(
  `loaded: ${fx.blocks.length} blocks, ctx≈${ctx()} replayable tokens, ${gens()} compaction generations`
)
log(
  `windows: small=${SMALL_WINDOW} large=${LARGE_WINDOW} growTarget=${GROW_TARGET}`
)
...
const recall1 = recallCount(await send(
  'step1-recall',
  '只回答暗号值,用「ALPHA=…, BETA=…, GAMMA=…」格式:三个暗号分别是什么?'
))
const step1NoCompact = gens() === g1
log(
  `   compaction on larger-window switch: ${gens() > g1 ? 'YES (unexpected ✗)' : 'no ✓'}   ctx≈${ctx()}, ${gens()} generations`
)
...
while (
  ...
  if (growTurn > 40)
    throw new Error('grow exceeded 40 turns without a compactable window')
}
if (GROW_TARGET > 0) {
...
log(
  `   ctx before switch≈${ctx3} (estimate@small=${smallEstimate}, threshold@0.8=${smallThreshold})`
)
session.updateModel(small.runtime, small.model)
const recall3 = recallCount(await send(
  'step3-recall',
  '只回答暗号值,用「ALPHA=…, BETA=…, GAMMA=…」格式:三个暗号分别是什么?'
))
const step3Forced = gens() > g3
log(
  `   forced compaction by pre-switch model: ${step3Forced ? `YES ✓ (+${gens() - g3} generation, now ${gens()})` : 'NO ✗'}`
)
log(`   recall after shrink-window compaction: ${recall3}/3`)

log('\n── STEP 4: switch back to large — session must keep working')
session.updateModel(large.runtime, large.model)
const recall4 = recallCount(await send(
  'step4-recall',
  '只回答暗号值,用「ALPHA=…, BETA=…, GAMMA=…」格式:三个暗号分别是什么?'
))
...
log(
  `step 3  large→small: forced compaction = ${step3Forced}, recall ${recall3}/3`
)
