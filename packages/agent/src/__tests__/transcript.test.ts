...
test(
  'TranscriptLog appends user turns and provider text/response events',
  () => {
    ...
    transcript.applyProviderEvent(
      model,
      events.response({ inputTokens: 2, outputTokens: 3 })
    )
    ...
    expect(transcript.blocks[1])
      .toMatchObject({ type: 'text', text: 'hi there' })
...
test(
  'TranscriptLog appends steer blocks and replays them in turn order',
  () => {
    const transcript = makeTranscript()
    ...
    const steer = transcript.pushSteer(
      'turn-1',
      model,
      [{ type: 'text', text: 'prefer concise output' }]
    )
...
test(
  'TranscriptLog completes pending tool calls and emits exact tool inference items',
  () => {
    const transcript = makeTranscript()
    ...
    const completed = transcript.completeToolCall(
      ...
      [
        ...
        {
          type: 'image',
          source: { mediaType: 'image/png', data: 'base64-image' }
        },
      ],
      false,
...
test(
  'collectInferenceItems bounds text without splitting surrogate pairs and scrubs lone surrogates',
  () => {
    let id = 0
    ...
    transcript.applyProviderEvent(
      model,
      events.toolCall('tool-1', 'shell_exec', { script: 'run' })
    )
    ...
    transcript.completeToolCall(
      'tool-1',
      [{ type: 'text', text: `abc🙂-\ud83d-🙂xyz` }]
    )

    const result = transcript.collectInferenceItems()
...
test(
  'collectInferenceItems scrubs lone surrogates even when text fits the replay bounds',
  () => {
    const transcript = makeTranscript()
...
test(
  'TranscriptLog completes the pending tool call when tool ids repeat',
  () => {
    const transcript = makeTranscript()

    transcript.applyProviderEvent(
      model,
      events.toolCall('tool-1', 'shell_exec', { script: 'printf first' })
    )
    transcript.completeToolCall('tool-1', [{ type: 'text', text: 'first' }])
    transcript.applyProviderEvent(
      model,
      events.toolCall('tool-1', 'shell_exec', { script: 'printf second' })
    )
...
test(
  'TranscriptLog replays thinking signatures and redacted thinking in provider order',
  () => {
    const transcript = makeTranscript()
    ...
    transcript.pushUserTurn(
      'test-turn',
      model,
      [{ type: 'text', text: 'think through this' }]
    )
    ...
    transcript.applyProviderEvent(
      model,
      { type: 'thinking_delta', text: 'notes' }
    )
    transcript.applyProviderEvent(
      model,
    ...
    expect(transcript.collectInferenceItems()).toEqual([
      {
        type: 'user_message',
        content: [{ type: 'text', text: 'think through this' }]
      },
      {
...
test('TranscriptLog safely stores non-JSON tool inputs', () => {
  ...
  transcript.applyProviderEvent(
    model,
    events.toolCall('tool-1', 'shell_exec', undefined)
  )
  transcript.applyProviderEvent(
    model,
...
test('TranscriptLog removes dangling executing tool calls', () => {
  ...
  transcript.applyProviderEvent(
    model,
    events.toolCall('done', 'shell_exec', { script: 'true' })
  )
  transcript.completeToolCall('done', [{ type: 'text', text: '' }])
  transcript.applyProviderEvent(
    model,
    events.toolCall('dangling', 'shell_exec', { script: 'sleep 1' })
  )
...
test(
  'TranscriptLog inserts compaction boundary and replays from the latest boundary',
  () => {
    const transcript = makeTranscript()
...
test(
  'TranscriptLog snapshot survives JSON roundtrip without changing replay or metadata',
  () => {
    const transcript = makeTranscript()
    ...
    transcript.applyProviderEvent(
      model,
      { type: 'thinking_delta', text: 'private notes' }
    )
    transcript.applyProviderEvent(
      model,
      { type: 'thinking_signature', signature: 'sig-json' }
    )
    transcript.applyProviderEvent(
      model,
      { type: 'redacted_thinking', data: 'redacted-json' }
    )
    transcript.applyProviderEvent(
      model,
      events.toolCall('tool-json', 'read_file', { path: 'src/a.ts' })
    )
    transcript.completeToolCall(
      'tool-json',
      [
    ...
    transcript.applyProviderEvent(
      model,
      events.response({ inputTokens: 10, outputTokens: 2, cacheReadTokens: 3 })
    )
    ...
    transcript.pushSteer(
      'test-turn',
      model,
      [{ type: 'text', text: 'with extra constraint' }]
    )
    transcript.appendExtensionStateSnapshot(
      'todo',
      { items: [{ id: 'T1', text: 'persist me' }] }
    )
    ...
    expect(restored.collectInferenceItems())
      .toEqual(transcript.collectInferenceItems())
    expect(restored.latestExtensionStateSnapshot('todo'))
      .toEqual(transcript.latestExtensionStateSnapshot('todo'))
  }
)

test(
  'TranscriptLog snapshot is insulated from later live block mutations',
  () => {
    const transcript = makeTranscript()

    transcript.applyProviderEvent(
      model,
      events.toolCall('tool-snapshot', 'shell_exec', { script: 'printf hi' })
    )
    const snapshot = transcript.toJSON()
    ...
    if (snapshotTool?.type !== 'tool_call')
      throw new Error('expected tool_call snapshot')
...
test('context estimate anchors on the latest provider-reported usage', () => {
  ...
  transcript.pushUserTurn(
    'turn-2',
    model,
    [{ type: 'text', text: 'y'.repeat(4_000) }]
  )
  expect(transcript.estimateContextTokens()).toBe(1_384 + 1_000)
})

test(
  'an anchor exceeding the context window is discarded as a contract violation',
  () => {
    const transcript = makeTranscript()
    transcript.pushUserTurn(
      'turn-1',
...
test(
  'images and documents contribute to the unanchored context estimate',
  () => {
    const transcript = makeTranscript()
