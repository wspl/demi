...
function runtimeProvider(
  turnsFactory: () => ConstructorParameters<typeof StubProvider>[0]
): Provider {
...
function createStatefulHarness(
  cwd: string,
  hostStates: CounterState[]
): AgentHarness<CounterState> {
...
test(
  'restored sessions share one live state object between harness and session',
  async () => {
    ...
    const turns = (): ConstructorParameters<typeof StubProvider>[0] => [
      [events.toolCall(
        'tool-1',
        'shell_exec',
        { script: 'printf ready', timeoutMs: 1_000 }
      )],
    ...
    const harness: AgentHarness<CounterState> = {
      ...
      lifecycle: (event) => {
        if (event.type === 'before_round_start')
          event.state.count += 1
      },
    }
    ...
    await waitFor(() =>
      client2.transcript().blocks.filter((block) => block.type === 'response')
        .length >= 1,
    )
...
test(
  'opening an owned session id takes it over and closes the previous connection',
  async () => {
    ...
    await waitFor(
      () => first.transcript().blocks.some((block) => block.type === 'response')
    )
    ...
    expect(second.transcript().blocks.map((block) => block.type)).toEqual([
      'user',
      'text',
      'response'
    ])
...
test(
  'a connection reopening after takeover does not disturb the new owner',
  async () => {
