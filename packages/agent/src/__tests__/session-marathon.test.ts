...
test(
  'AgentSession marathon keeps requests and transcript consistent across long-lived actions',
  async () => {
    ...
    const provider = new RecordingProvider([
      (request) => {
        ...
        return [
          events.toolCall('tool-1', 'echo_tool', { value: 'tool step' }),
          events.response()
        ]
      },
      (request) => {
...
test(
  'restored session continues from snapshot without re-executing completed tools',
  async () => {
    const store = new MemorySessionStore<TestState>()
    const firstProvider = new RecordingProvider([
      [
        events.toolCall('tool-1', 'write_once', { path: 'file.txt' }),
        events.response()
      ],
    ...
    const restored = createSession(
      restoredProvider,
      runtime,
      snapshot ? new TranscriptLog(snapshot.transcript.blocks) : undefined
    )
...
test(
  'AgentSession.fromCheckpoint restores state and model context with a fresh idle runtime',
  async () => {
    ...
    snapshot.queue = [{
      ...
      text: 'queued after crash',
      content: text('queued after crash')
    }]
    const restoredProvider = new RecordingProvider([
      (request) => {
        ...
        assertNoOrphanToolItems(request.items)
        return [
          events.toolCall('tool-2', 'write_once', { path: 'second.txt' }),
          events.response()
        ]
      },
      (request) => {
        expect(request.items.filter((item) => item.type === 'tool_result'))
          .toHaveLength(2)
        assertNoOrphanToolItems(request.items)
    ...
    const restoredUser = restored.transcript().blocks.find((block) => {
      if (block.type !== 'user')
        return false
      return block.content.some((item) => item.type === 'text'
        && item.text === 'continue after restore')
    })
    expect(restoredUser).toMatchObject({ type: 'user', id: 'restored-2' })
    expect(restored.transcript().blocks.at(-2)).toMatchObject({
      type: 'text',
      text: 'restored write complete'
    })
...
test(
  'AgentSession.fromCheckpoint completes tool calls left executing by a process death',
  async () => {
    const store = new MemorySessionStore<TestState>()
    const firstProvider = new RecordingProvider([
      [
    ...
    if (!persistedCall || persistedCall.type !== 'tool_call')
      ...
      (request) => {
        ...
        const result = request.items.find(
          (item) => item.type === 'tool_result'
            && item.toolUseId === persistedCall.toolUseId,
        )
        expect(JSON.stringify(result))
          .toContain('Tool call interrupted: write_once')
    ...
    expect(restored.transcript().pendingToolCalls()).toHaveLength(0)
    expect(
      restored.transcript().blocks.find((block) => block.type === 'tool_call')
    ).toMatchObject(
      {
        status: 'error'
      }
...
test(
  'restored session after provider error does not duplicate completed tool results',
  async () => {
    const store = new MemorySessionStore<TestState>()
    const firstProvider = new RecordingProvider([
      [
    ...
    await first.send(text('write file'))
    await expect(first.send(text('after write')))
      .rejects.toThrow('provider failed after write')
    expect(first.state().toolCalls).toBe(1)

    const snapshot = store.snapshots.at(-1)
    expect(snapshot?.transcript.blocks.at(-1)).toMatchObject({
      type: 'error',
      code: 'rate_limit'
    })
    const restoredProvider = new RecordingProvider([
...
function expectRequestMatchesTranscript(
