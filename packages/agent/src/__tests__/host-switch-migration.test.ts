...
test(
  ...
  async () => {
    ...
    const harness: AgentHarness<Record<string, never>> = {
      ...
      systemPrompt: () => 'test',
      preamble: (ctx) => {
        const target = typeof ctx.metadata?.target === 'string'
          ? ctx.metadata.target
          : null
        if (target === null)
          return null
    ...
    const provider = defineProvider({
      ...
      createRuntime: () =>
        new StubProvider([
          ...
          capture([events.toolCall(
            'a-1',
            'shell_exec',
            {
              script: 'mkdir sub && cd sub && printf ready > marker.txt && pwd',
              timeoutMs: 5_000
            }
          )]),
          ...
          capture([events.toolCall(
            'a-2',
            'shell_exec',
            { script: 'pwd && cat marker.txt', timeoutMs: 5_000 }
          )]),
    ...
    const output = shellOutputs
      .filter((event) => event.type === 'shell_output'
        && event.status.status === 'exited')
      .map((event) => (event.type === 'shell_output'
        ? event.status.stdout.delta.trim()
        : ''))
    ...
    const userTexts = (request: InferenceRequest): string[] =>
      request.items
        .filter((
          item
        ): item is Extract<InferenceRequest['items'][number], { type: 'user_message' }> => item.type
          === 'user_message')
        .flatMap((item) => item.content)
        .flatMap((block) => (block.type === 'text' ? [block.text] : []))
    expect(
      userTexts(requests[0])
        .some((text) => text.includes('Execution target switched'))
    ).toBe(false)
    expect(
      userTexts(requests[2]).some(
        (text) => text.includes(`switched from ${pathA} to ${pathB}`)
      )
    ).toBe(true)
    expect(
      userTexts(requests[4]).some(
        (text) => text.includes(`switched from ${pathB} to ${pathA}`)
      )
    ).toBe(true)
    ...
    const userBlocks = blocks.filter((
      block
    ): block is Extract<Block, { type: 'user' }> => block.type === 'user')
    ...
    const textBlocks = blocks.filter((
      block
    ): block is Extract<Block, { type: 'text' }> => block.type === 'text')
