...
test(
  'a turn with tool calls persists at each tool dispatch and the action boundary, not per event',
  async () => {
    ...
    const typesOf = (update: AgentSessionPersistUpdate<Record<string, never>>) =>
      update.changedBlocks.map(({ block }) => (block.type === 'tool_call'
...
test(
  'client resyncs with a snapshot when the patch revision stream has a gap',
  async () => {
    ...
    pair.server.send({
      ...
      revision: 3,
      patches: [{ op: 'add', path: ['blocks', 1], value: blockAt(
        'c',
        'three'
      ) }],
    })
    await waitFor(
      () => receivedFrames.some((frame) => frame.type === 'sync_transcript')
    )
...
test(
  'append_text patches replace the target block instead of mutating it',
  () => {
    ...
    const applied = applyTranscriptPatches(
      [original],
      [{ op: 'append_text', path: ['blocks', 0], delta: ' end' }]
    )
