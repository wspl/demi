...
test(
  'provider request items preserve a stable prefix across ordinary turns',
  async () => {
    ...
    expect(provider.requests[1]?.items.slice(
      0,
      provider.requests[0]?.items.length
    )).toEqual(provider.requests[0]?.items)
    expect(provider.requests[2]?.items.slice(
      0,
      provider.requests[1]?.items.length
    )).toEqual(provider.requests[1]?.items)
...
test(
  'provider request text content is bounded without mutating the transcript audit log',
  async () => {
    ...
    transcript.pushUserTurn(
      'test-turn',
      model,
      [{ type: 'text', text: longReference }],
      longPreamble
    )
    transcript.applyProviderEvent(
      model,
      events.toolCall('tool-1', 'read_file', { path: 'big.txt' })
    )
    transcript.completeToolCall(
      'tool-1',
      [{ type: 'text', text: longToolOutput }]
    )
    ...
    const userBlock = session.transcript()
      .blocks.find((block) => block.type === 'user')
    ...
    const textBlock = session.transcript()
...
test(
  'stable prompt prefix is byte-identical for equivalent histories and changes on cache inputs',
  async () => {
...
test(
  'provider request prefix restabilizes after compaction replaces old history',
  async () => {
    const transcript = makeTranscript()
    ...
    if (!firstPostCompact || !secondPostCompact)
      throw new Error('missing post-compact provider requests')
    ...
    expect(JSON.stringify(secondPostCompact.items.slice(
      0,
      firstPostCompact.items.length
    ))).toBe(
...
test(
  'replay compaction summary reuses the previous turn prefix for provider caches',
  async () => {
    const transcript = makeTranscript()
    transcript.pushUserTurn('test-turn', model, text('old question'))
    transcript.applyProviderEvent(model, events.text('old answer'))
    ...
    if (!turnRequest || !summaryRequest)
      throw new Error('missing provider requests')
    ...
    expect(turnRequest.items.slice(0, replayedItems.length))
      .toEqual(replayedItems)
...
test(
  'provider request is built from effective transcript without internal blocks',
  async () => {
    const transcript = makeTranscript()
    transcript.pushUserTurn('test-turn', model, text('old question'))
    transcript.applyProviderEvent(model, events.text('old answer'))
    ...
    transcript.applyProviderEvent(
      model,
      events.response({ inputTokens: 9, outputTokens: 4, cacheReadTokens: 3 })
    )
    transcript.applyProviderEvent(
      model,
      events.error('provider internal failure', 'context_length_exceeded')
    )
    ...
    const provider = new RecordingProvider([
      (request) => {
        ...
        expect(JSON.stringify(request.items))
          .not.toContain('provider internal failure')
        expect(JSON.stringify(request.items))
          .not.toContain('context_length_exceeded')
...
test(
  'retry and resume provider requests match TranscriptLog.collectInferenceItems',
  async () => {
  ...
  async () => {
    ...
    const provider = new RecordingProvider([
      ...
      (request) => {
        ...
        return [
          events.text('done'),
          events.response({ inputTokens: 1, outputTokens: 1 })
        ]
      },
    ])
    const runtime = createRuntime({
    ...
    const responseBlocks = session.transcript()
...
test(
  'context overflow provider errors are explicit and keep the session recoverable',
  async () => {
    const provider = new RecordingProvider([
    ...
    await expect(session.send(text('too much context')))
...
test(
  'snapshot reconstruction preserves model-visible context exactly',
  async () => {
    const transcript = makeTranscript()
