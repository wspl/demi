...
test(
  'resolveCompactionThreshold uses ratio when absolute tokens are unset',
  () => {
...
test(
  'resolveCompactionThreshold clamps absolute tokens to the context window',
  () => {
...
test(
  'resolveCompactionThreshold disables compaction for non-finite ratio',
  () => {
    expect(
      resolveCompactionThreshold(372_000, Number.POSITIVE_INFINITY, 334_800)
    ).toBe(
