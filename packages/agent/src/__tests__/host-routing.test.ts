...
function routedHarness(
  hosts: Awaited<ReturnType<typeof createHosts>>
...
test(
  'action metadata switches Host while the same Host keeps its shell state',
  async () => {
      ...
      createRuntime: () =>
        new StubProvider([
          [events.toolCall(
            'alice-1',
            'shell_exec',
            { script: 'mkdir nested && cd nested && pwd', timeoutMs: 1_000 }
          )],
          [events.text('done'), events.response()],
          [events.toolCall(
            'bob-1',
            'shell_exec',
            { script: 'pwd', timeoutMs: 1_000 }
          )],
          [events.text('done'), events.response()],
          [events.toolCall(
            'alice-2',
    ...
    await client.open(
      selection,
      hosts.base.defaultCwd,
      globalThis.crypto.randomUUID()
    )

    await client.send(
      [{ type: 'text', text: 'alice first' }],
...
test(
  'a command handle cannot be controlled from another action Host',
  async () => {
    const hosts = await createHosts()
    const provider = defineProvider({
      id: 'stub',
    ...
    const server = new AgentServer({ store: memoryAgentStores(),
      ...
      providers: [provider],
      shell: {
        commandIdFactory: () => (commandIndex++ === 0
          ? commandId
