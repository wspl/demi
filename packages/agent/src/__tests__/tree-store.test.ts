...
test(
  'a later turn saves only its own rows — earlier rows are never rewritten',
  async () => {
    ...
    for (const save of laterSaves) {
      for (const { index } of save.changedBlocks) expect(index)
...
function checkpoint(
  queue: AgentSessionPersistUpdate<unknown>['queue'] = []
): AgentSessionPersistUpdate<unknown> {
  return {
    changedBlocks: [],
    blockCount: 0,
    state: {},
...
test(
  ...
  async () => {
    ...
    expect((await store.children('root')).map((node) => node.id))
      .toEqual(['child'])
    expect((await store.sessionStore('child').load())?.queue).toEqual([brief])
    await store.sessionStore('child').save({
      ...checkpoint(),
      changedBlocks: [{ index: 0, block: userBlock('m1') }],
      blockCount: 1
    })
    const loaded = await store.sessionStore('child').load()
    expect(loaded?.queue).toEqual([])
    expect(loaded?.transcript.blocks.map((block) => block.type))
      .toEqual(['user'])
    await expect(store.createNode(record('child', 'root'), checkpoint()))
  ...
  async () => {
    ...
    for (const id of ['a', 'b', 'c']) {
      ...
      await store.closeNode(
        id,
        { phase: 'completed', closedAt: 2, result: `${id} done`, failure: null }
      )
    }
    expect((await store.children('root')).map((node) => [
      node.closedPhase,
      node.result,
      node.delivered
    ])).toEqual([
...
test(
  'reopen makes an archived node live with the reviving message queued; delete takes the subtree',
  async () => {
    const store = new MemoryAgentStore()
    await store.createNode(record('root', null), checkpoint())
    ...
    await store.closeNode(
      'child',
      { phase: 'error', closedAt: 3, result: null, failure: 'boom' }
    )
    expect(await store.node('child')).toMatchObject({
      closedPhase: 'error',
      failure: 'boom'
    })

    const reviving = { id: 'm2', text: 'again', content: text('again') }
    await store.reopenNode(
      'child',
      { metadata: { round: 1 }, spawnedAt: 9 },
      reviving
    )
    expect(await store.node('child')).toMatchObject({
      closedPhase: null,
    ...
    expect((await store.sessionStore('child').load())?.queue)
      .toEqual([reviving])
