  ...
  for (const part of parts) {
    switch (part) {
      ...
      case 'error':
        log.applyProviderEvent(
          model,
          { type: 'error', message: 'boom', code: 'overloaded' }
        )
        break
      case 'abort':
        log.pushAbort(model)
...
test('a turn whose only trace is the failure unwinds completely', () => {
  expect(findResumePoint(afterUserTurn()))
    .toEqual({ cut: 1, isFullRerun: true })
  expect(findResumePoint(afterUserTurn('error'))).toEqual({
    cut: 1,
    isFullRerun: true
  })
  // Thinking is discardable: nothing keys off it and a rerun simply reasons again.
  expect(findResumePoint(afterUserTurn('thinking', 'error'))).toEqual({
    cut: 1,
    isFullRerun: true
  })
})
...
test(
  'emitted text stops the unwind — a product may already have sent it',
  () => {
    expect(findResumePoint(afterUserTurn('text', 'error'))).toEqual({
      cut: 2,
      isFullRerun: false
    })
  }
...
test('a tool call stops the unwind whether or not it completed', () => {
  expect(findResumePoint(afterUserTurn('toolCall', 'error'))).toEqual({
    cut: 2,
    isFullRerun: false
  })
  ...
  expect(findResumePoint(afterUserTurn('toolCallPending'))).toEqual({
    cut: 2,
    isFullRerun: false
  })
})
...
test(
  'a response stops the unwind — it records a request that did complete',
  () => {
...
test('an abort stops the unwind — it is history the user created', () => {
  expect(findResumePoint(afterUserTurn('abort'))).toEqual({
    cut: 2,
    isFullRerun: false
  })
})
...
test(
  'only the latest turn is in play — an earlier one is settled history',
  () => {
    const log = makeTranscript()
...
test('resume keeps already-emitted text exactly once', async () => {
  ...
  const texts = session
    .transcript()
    .blocks.filter((block): block is Extract<typeof block, { type: 'text' }> => block.type
      === 'text')
...
test(
  'resume drops stale thinking so the rerun does not replay it',
  async () => {
    const provider = new StubProvider([
      [
        { type: 'thinking_start' },
        { type: 'thinking_delta', text: 'first pass reasoning' },
        events.error('backend failed', 'overloaded')
      ],
