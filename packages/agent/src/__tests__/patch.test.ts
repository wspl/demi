...
test(
  'journal patches replicate appended blocks and streaming text deltas',
  () => {
    ...
    expect(drained!.patches.map((patch) => patch.op)).toEqual([
      'add',
      'add',
      'append_text'
    ])
...
test(
  'journal add values are snapshots, unaffected by later mutation of the live block',
  () => {
    const transcript = new TranscriptLog()
...
test(
  'rewind for retry emits a full replace and drops finer-grained history',
  () => {
    const transcript = new TranscriptLog()
    ...
    const replica = applyTranscriptPatches(
      [],
...
test(
  'takePatches returns null when nothing changed and revisions stay monotonic',
  () => {
    const transcript = new TranscriptLog()
