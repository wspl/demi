...
test(
  'WebSocket transports serialize frames as JSON text messages and preserve binary fields',
  async () => {
    ...
    expect(await serverFrame).toEqual({
      type: 'send',
      messageId: 'ws-send-1',
      content: [{ type: 'text', text: 'hello' }],
      metadata: { tenantId: 'tenant-a', sequence: 1n },
    ...
    if (binarySend.type !== 'send')
      throw new Error('expected send frame')
    const sentContent = binarySend.content[0]
    if (sentContent?.type !== 'image' || sentContent.source.type !== 'binary')
    ...
    if (snapshot.type !== 'transcript_reset')
      throw new Error('expected transcript snapshot')
    const block = snapshot.blocks[0]
    if (block?.type !== 'user')
      throw new Error('expected user block')
    const receivedContent = block.content[0]
    if (receivedContent?.type !== 'document')
...
test(
  'WebSocket transports carry AgentClient and AgentServer traffic end to end',
  async () => {
    const [clientSocket, serverSocket] = createSocketPair()
    ...
    const server = new AgentServer({ store: memoryAgentStores(),
      ...
      providers: [runtimeProvider(
        'ws-stub',
        () => new StubProvider([[events.text('over websocket'), events.response()]])
      )],
    })
    ...
    await client.open(
      providerSelectionForText('over websocket'),
      '/workspace',
...
test(
  'WebSocket transports preserve complex AgentClient action convergence',
  async () => {
    const [clientSocket, serverSocket] = createSocketPair()
    ...
    const second = client.send([{
      type: 'text',
      text: 'second ' + 'x'.repeat(20_000)
    }])
    await waitFor(() => seen.some((event) => event.type === 'queue'
      && event.queue.some((message) => message.text.startsWith('second'))))
...
class WebSocketScenarioProvider implements AgentProvider {
  ...
  async *run(request: InferenceRequest): AsyncIterable<ProviderEvent> {
    ...
    if (call === 1) {
      if (!latestUserText(request).startsWith('second'))
        throw new Error('queued send did not reach provider')
