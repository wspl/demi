...
import {
  ...
  shellCommandHandleRequired,
  shellPreviewBudgetTokens,
  toShellToolResult
} from '../tools'

test(
  'standard shell tool schemas do not expose model-controlled output budgets or offsets',
  () => {
...
test(
  'shell tool result exposes output paths and a bounded preview without stdout body sections',
  () => {
...
test(
  'completed short shell_exec hides and releases the command handle',
  async () => {
    ...
    const result = await shellExec.invoke(
      toolContext(),
      { script: 'printf done', timeoutMs: 1 }
    )
    const text = result.output[0]?.type === 'text' ? result.output[0].text : ''
...
test(
  'completed truncated shell_exec keeps the command handle for artifacts',
  async () => {
    const released: string[] = []
    ...
    const result = await shellExec.invoke(
      toolContext(),
      { script: 'printf long', timeoutMs: 1 }
    )
    const text = result.output[0]?.type === 'text' ? result.output[0].text : ''
...
test.each([
...
])('%s evaluates the custom preview budget against the current model', async (
  name,
  input
) => {
  const seen: number[] = []
  ...
  const largerContext = {
    ...
    model: {
      ...context.model,
      model: { ...context.model.model, contextWindow: 1_000_000 }
    },
  }
  const largerResult = await tool.invoke(largerContext, input)
  const largerText = largerResult.output[0]?.type === 'text'
    ? largerResult.output[0].text
    : ''
...
test(
  'shell command handles are required only for running or over-budget output',
  () => {
    expect(shellCommandHandleRequired(runningShellSnapshot(''), 1_000))
      .toBe(true)
    expect(shellCommandHandleRequired(shellSnapshot('short\n'), 1_000))
      .toBe(false)
    expect(
      shellCommandHandleRequired(shellSnapshot('x'.repeat(4_001)), 1_000)
    ).toBe(true)
  }
)

test(
  'AgentServer keeps the custom preview policy across model changes and live session takeover',
  async () => {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      turns.push((request) => {
        const result = [...request.items].reverse()
          .find((item) => item.type === 'tool_result')
        seenResults.push(result?.output[0]?.type === 'text'
    ...
    const selection = (contextWindow: number): ProviderSelection => ({
      providerId: 'stub', model: {
        ...model,
        model: { ...model.model, contextWindow }
      },
    })
    try {
...
const PNG_STREAM = new Uint8Array([
  0x89,
  0x50,
  0x4e,
  0x47,
...
const MP4_STREAM = new Uint8Array([
  0,
  0,
  0,
  0x20,
...
const OPAQUE_STREAM = new Uint8Array([
  0xde,
  0xad,
  0xbe,
  0xef,
...
test(
  'a sniffed binary stream the model accepts is attached as a media block',
  () => {
    const snapshot = {
    ...
    expect(result.output[1]).toEqual({
      ...
      source: {
        mediaType: 'image/png',
        data: Buffer.from(PNG_STREAM).toString('base64')
      },
    })
...
test(
  'a media stream the model does not accept explains why nothing was attached',
  () => {
    const snapshot = {
...
test(
  'unknown binary and truncated streams stay placeholder-only with a reason',
  () => {
    ...
    const opaqueNote = opaque.output[1]?.type === 'text'
      ? opaque.output[1].text
      : ''
    ...
    const truncated = toShellToolResult(
      {
        ...shellSnapshot(
          '<binary stdout: 999 bytes, exceeds the 12-byte output limit>\n'
        ),
        binaryStdout: {
          data: PNG_STREAM,
          truncated: true,
    ...
    const truncNote = truncated.output[1]?.type === 'text'
      ? truncated.output[1].text
      : ''
    expect(truncated.output).toHaveLength(2)
    expect(truncNote).toContain(
      "exceeded the shell's 12-byte binary limit (maxBinaryBytes)"
    )
...
test(
  'media over its modality cap is withheld and points at a smaller version',
  () => {
...
function shellSnapshot(
  output: string
): Extract<ShellCommandStatus, { status: 'exited' }> {
...
function runningShellSnapshot(
  output: string
): Extract<ShellCommandStatus, { status: 'running' }> {
  const bytes = new TextEncoder().encode(output).byteLength
  return {
    status: 'running',
    shellId: 'shell-1',
