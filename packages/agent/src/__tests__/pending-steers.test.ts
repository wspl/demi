...
function pending(id: string): PendingSteer {
  return { id, turnId: 'turn', model, content: [{ type: 'text', text: 'identical guidance' }] }
}
...
test('pending snapshots hide internal wakeups without removing them from the execution queue', () => {
  ...
  expect(queue.snapshot()).toEqual([pending('user')])
  expect(queue.takeForTurn('turn').map((steer) => steer.id)).toEqual(['hidden', 'user'])
  expect(queue.snapshot()).toEqual([])
})

test('AgentClient removes pending steers by id, including when history arrives before the list', () => {
  const { client, receive } = clientHarness()
  const history: Block = { ...pending('first'), type: 'steer', createdAt: '2026-09-08T00:00:00Z' }
  receive({ type: 'transcript_reset', blocks: [history], revision: 1 })
  receive({ type: 'pending_steers', pendingSteers: [pending('first'), pending('second')] })
  ...
  receive({ type: 'transcript_patch', patches: [{ op: 'add', path: ['blocks', 1], value: { ...history, id: 'second' } }], revision: 2 })
...
test('AgentClient validates pending data and owns copies of binary attachments', () => {
  const { client, receive } = clientHarness()
  const steer: PendingSteer = {
    ...pending('binary'),
    content: [{ type: 'image', source: { type: 'binary', mediaType: 'image/png', data: new Uint8Array([1, 2, 3]) } }],
  }
  ...
  expect(() => receive({ type: 'pending_steers', pendingSteers: [{ id: 7 }] } as unknown as ServerFrame)).toThrow()
