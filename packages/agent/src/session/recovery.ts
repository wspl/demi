 ...
 * that reaches all the way back to the user turn, meaning the turn produced
 * nothing
...
/**
 * Whether a block is a leftover of the failed attempt that nobody can have
 * acted on.
 *
 * Everything else has to be assumed acted on. Transcript blocks stream outward
 * as
 * they are produced and products turn them into effects that cannot be recalled
 * —
 * rendering them, posting them to a chat, executing the tool they describe. A
 * tool
 * call counts whatever its status: one still marked executing outlived the
 * process
 * that was running it, so whether its effect landed is unknown, and unknown has
 * to
 * be treated as landed. An abort block is history the user created, not a
 * leftover.
 *
 * Thinking is the interesting case: products display it, but nothing keys off
 * it and
 * a rerun simply reasons again, so it is discardable. A `response` is not — it
 * records a provider request that did complete, and its usage anchors the
 * context
 ...
 * failure and a human asking to continue a dead round ask the same question —
 * how
...
export function findResumePoint(blocks: readonly Block[]): ResumePoint {
  for (let i = blocks.length - 1; i >= 0; i -= 1) {
    ...
    if (block.type === 'user')
      return { cut: i + 1, isFullRerun: true }
    if (!isDiscardableLeftover(block))
      return { cut: i + 1, isFullRerun: false }
  }
