...
const anatomy: [string, string][] = [
  [
    'Shell',
    'One large dialog. The rail sits on the page surface with the account on top and a filter under it, the page on the dialog surface, so it reads like the app itself. Below a phone width the rail becomes a row.'
  ],
  [
    'Page',
  ...
  [
    'MCP',
    'Servers in one list. Status is a dot and a word; an error sits on the status as a tooltip. Tools are tags on a third line, one row, with the rest as +N. Adding opens a dialog.'
  ],
  [
    'Skills',
...
const claudePhases: {
  ...
  },
  {
    ...
    phase: {
      kind: 'failed',
      message: 'That token was revoked. Run the command again for a fresh one.'
    }
  },
]
const codexPhases: {
...
const grokPhases: {
  ...
  },
  {
    ...
    phase: {
      kind: 'done',
      account: 'zan@example.com · SuperGrok'
    }
  },
]
...
const phaseOf = (list: {
  value: string;
  phase: ProviderLoginPhase
}[], value: string) => list.find((p) => p.value === value)!.phase
...
<template>
  <div class="flex flex-col gap-10">
    ...
    <GallerySection
      title="Add device · pairing flow"
      note="One shared flow for a local computer, a remote SSH server, or a headless machine. Start the runner, paste its pairing code, then use the connected device."
    >
    ...
    >
      ...
      <GalleryOverlayWell size="tall">
        <ChangeEmailDialog
          :is-open="true"
          :overlay-store="appOverlayStore"
          :phase="emailPhases.find((p) => p.value === emailPhase)!.phase"
        />
      </GalleryOverlayWell>
    </GallerySection>
    ...
    <GallerySection
      ...
      note="The current password and the new one twice."
    >
      <div class="mb-3"><Segmented
          v-model="passwordPhase"
          :options="passwordPhases"
        /></div>
      <GalleryOverlayWell size="tall">
        <ChangePasswordDialog
          :is-open="true"
          :overlay-store="appOverlayStore"
          :phase="passwordPhases.find((p) => p.value === passwordPhase)!.phase"
        />
      </GalleryOverlayWell>
    </GallerySection>
    ...
    <GallerySection
      ...
      note="A token from the vendor's own CLI: install, run, paste."
    >
      <div class="mb-3"><Segmented
          v-model="claudePhase"
          :options="claudePhases"
        /></div>
      <GalleryOverlayWell size="tall">
        <ProviderLoginDialog
          :is-open="true"
          ...
          :phase="phaseOf(claudePhases, claudePhase)"
        />
      </GalleryOverlayWell>
    </GallerySection>
    ...
    <GallerySection
      title="Sign in · Codex"
      note="A device code confirmed in the browser while the dialog waits."
    >
      <div class="mb-3"><Segmented
          v-model="codexPhase"
          :options="codexPhases"
      ...
      <GalleryOverlayWell size="lg">
        <ProviderLoginDialog
          ...
          :overlay-store="appOverlayStore"
          vendor-name="Codex"
          :phase="phaseOf(codexPhases, codexPhase)"
        />
      </GalleryOverlayWell>
    </GallerySection>

    <GallerySection
      title="Sign in · Grok Build"
      note="A code shown in the browser and pasted back."
    >
      ...
      <GalleryOverlayWell size="lg">
        <ProviderLoginDialog
          :is-open="true"
          :overlay-store="appOverlayStore"
          vendor-name="Grok Build"
          :phase="phaseOf(grokPhases, grokPhase)"
        />
      </GalleryOverlayWell>
    </GallerySection>
  </div>
