...
<template>
  <div :class="fillPane ? 'flex min-h-0 flex-1 flex-col' : 'flex flex-col gap-10'">
    ...
    <template v-if="view === 'panel'">
      ...
      <GallerySection
        title="Work panel"
        note="Change and File are content-sized buttons outside the browser tab strip. They never shrink or scroll with browser tabs; only the browser strip uses the remaining width. The add button uses globe-plus when no browser tabs exist and a regular plus otherwise; it adds browser tabs, using the same content-sized tabs capped at 160px, scrolling and close menus. Browser close menus affect only browser tabs. Change groups its added/removed counts with a 2px gap and shows uncommitted totals and returns to Uncommitted when clicked; file pills still open retained edits there. File uses a Lucide outline icon until a file is selected, then its file-type icon. Browser tabs use a globe. Selecting one shows Back, Forward, Refresh and its own address draft immediately below the strip; a subtle divider separates the address row from page content. Browser and File address bars leave 12px after navigation controls. Browser navigation remains disconnected."
      >
