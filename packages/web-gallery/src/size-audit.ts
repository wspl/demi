...
export function auditControlSizes(root: ParentNode = document.body): SizeFinding[] {
  ...
  for (const card of root.querySelectorAll('.settings-card')) {
    const byKind = {
      input: new Map<number, Element[]>(),
      button: new Map<number, Element[]>()
    }
    for (const el of card.querySelectorAll(CONTROL)) {
      ...
      if (el.tagName !== 'INPUT' && el.closest(':has(> input)'))
        continue
      ...
      if (box.hasAttribute('data-bare'))
        continue
      const h = Math.round(box.getBoundingClientRect().height)
      if (!FAMILIES.includes(h))
        continue
      const seen = el.tagName === 'INPUT' || el.tagName === 'TEXTAREA'
        ? byKind.input
        : byKind.button
      seen.set(h, [...(seen.get(h) ?? []), el])
    }
    for (const seen of Object.values(byKind)) {
      if (seen.size > 1) {
        ...
        findings.push(
          {
            card,
            heights,
            elements: heights.flatMap((h) => seen.get(h) ?? [])
          }
        )
      }
    }
...
export function reportControlSizes(root?: ParentNode): number {
  ...
  if (findings.length) {
    ...
    for (const f of findings) {
      const title = f.card.parentElement?.querySelector('h3')?.textContent?.trim() ??
        '(untitled card)'
      console.warn(
        `  ${title}: ${f.heights.join(' / ')}px`,
        f.elements.map(describe).join(', ')
      )
    }
  }
...
export function installSizeAudit(afterNavigation: (run: () => void) => void): void {
  ...
  if (import.meta.env.DEV)
    afterNavigation(() => window.setTimeout(() => reportControlSizes(), 1500))
}
