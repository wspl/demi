...
<template>
  <Tooltip :content="label" placement="right" :disabled="!collapsed" tag="div">
    <div
      role="button"
      ...
      :class="[
        collapsed ? 'w-7 justify-center' : 'px-2',
        pressed ? 'bg-active text-fg-emphasis' : emphasis ? 'text-fg hover:bg-hover hover:text-fg-emphasis' : 'text-fg-muted hover:bg-hover hover:text-fg',
      ]"
      @click="emit('click')"
    >
      ...
      <template v-if="!collapsed">
        ...
        <span v-if="count !== undefined" class="rounded-full bg-hover px-1.5 text-[11px] leading-4 text-fg-subtle">{{ count }}</span>
