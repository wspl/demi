...
typedef enum {
  AWAITING_RUNNER_READY,
  AWAITING_WATCH_READY,
  WATCH_READY
} WatchPhase;

// One command invocation per process; callbacks share its two connections
// and stdio.
...
static void send_frame(Connection *peer, unsigned type, const void *body,
                       size_t length) {
...
static void input_stream_read(uv_stream_t *stream, ssize_t n,
                              const uv_buf_t *buf) {
  (void)buf;
  if (!n) {
    return;
...
static void pull_input(void) {
  ...
  uv_buf_t buf =
      uv_buf_init((char *)input_state.bytes, sizeof(input_state.bytes));
...
static void write_output(void) {
  uv_buf_t buf =
      uv_buf_init((char *)output_state.bytes + output_state.offset,
                  (unsigned)(output_state.length - output_state.offset));
  uv_stream_t *stream = stdio_stream(output_state.fd);
  if (stream) {
    check(uv_write(&output_state.stream_request, stream, &buf, 1,
                   output_stream_written));
  } else {
    check(uv_fs_write(&loop, &output_state.file_request, output_state.fd,
                      &buf, 1, -1, output_written));
  }
}
...
static void control_connected(uv_connect_t *req, int status) {
  ...
  snprintf(json, sizeof(json),
           "{\"version\":%d,\"id\":\"%s\",\"context\":\"%s\"}",
           DEMI_VERSION, call_id, context);
  send_frame(&control_peer, DEMI_WATCH, json, strlen(json));
}

static void handle_frame(Connection *peer, unsigned type,
                         const unsigned char *body, unsigned length) {
  if (type == DEMI_ERROR) {
  ...
  case DEMI_READY:
    ...
    uv_pipe_connect(&control_peer.connect, &control_peer.pipe, endpoint,
                    control_connected);
    return;
