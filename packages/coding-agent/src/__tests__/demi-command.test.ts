...
test('demi file read returns a text file as text', async () => {
  ...
  const read = await env.exec({
    ...
    script: 'demi file read note.txt'
  })
...
test(
  ...
  async () => {
    ...
    expect(read.stdout.delta)
      .toContain(`<binary stdout: ${png.length} bytes; raw bytes at `)

    // Bytes pipe cleanly into downstream commands.
    const counted = await env.exec({
      shellId: read.shellId,
      script: 'demi file read shot.png | wc -c'
    })
    if (counted.status !== 'exited')
      throw new Error('expected exited result')
...
test(
  'demi --help documents byte-stream reads, and any word stays usable as a file name',
  async () => {
    const { env } = await createDemiEnvironment()
    ...
    const leafHelp = await env.exec({
      ...
      script: 'demi file read --help'
    })
    expect(leafHelp.status).toBe('exited')
    if (leafHelp.status !== 'exited')
      throw new Error('expected exited result')
    ...
    const created = await env.exec({
      shellId: help.shellId,
      script: "demi file create prompt <<'EOF'\nnot help\nEOF"
    })
    expect(created.status).toBe('exited')
    const read = await env.exec({
      shellId: help.shellId,
      script: 'demi file read prompt'
    })
...
test('demi file create writes a new file from heredoc content', async () => {
  ...
  const read = await env.exec({
    shellId: created.shellId,
    script: 'cat src/foo.txt'
  })
  expect(read.stdout.delta).toBe('hello\n')
})

test(
  'demi allows paths outside default cwd when the namespace allows them',
  async () => {
    ...
    if (relative.status !== 'exited')
      throw new Error('expected exited result')
    expect(relative.exitCode).toBe(0)
    await expect(readFile(join(parent, 'relative-outside.txt'), 'utf8'))
      .resolves.toBe('nope\n')
  }
)

test(
  'demi file patch can modify paths outside default cwd when Host.fs allows them',
  async () => {
...
test(
  'demi file edit replaces exact text and fails on ambiguous matches',
  async () => {
    const { env } = await createDemiEnvironment()
...
test(
  'demi file edit uses context only when it disambiguates to one nearest match',
  async () => {
    const { env } = await createDemiEnvironment()
...
test(
  'demi file edit rejects empty old text without modifying the file',
  async () => {
    const { env } = await createDemiEnvironment()
...
test('demi file patch applies a unified diff', async () => {
  ...
  const read = await env.exec({
    shellId: created.shellId,
    script: 'cat patch.txt'
  })
  expect(read.stdout.delta).toBe('one\nthree\n')
})

test(
  'demi file patch accepts unified diff headers with timestamps',
  async () => {
    const { env } = await createDemiEnvironment()
...
test(
  'demi file patch applies multiple files and creates new files',
  async () => {
    const { env } = await createDemiEnvironment()
    ...
    const existing = await env.exec({
      shellId: created.shellId,
      script: 'cat existing.txt'
    })
    expect(existing.stdout.delta).toBe('changed\n')
    const added = await env.exec({
      shellId: created.shellId,
      script: 'cat nested/new.txt'
    })
...
test('demi file patch deletes files with a /dev/null target', async () => {
  ...
  const missing = await env.exec({
    shellId: created.shellId,
    script: 'test -e doomed.txt'
  })
...
test(
  'demi file patch validates all files before writing any changes',
  async () => {
    const { env } = await createDemiEnvironment()
