...
test('todo command state is isolated by agent session id', async () => {
  ...
  const agentA = await runnerShell({
    host,
    commands: createCodingCommandRegistry(),
    agentSessionId: 'agent-a',
    initialEnv: {}
  })
  const agentB = await runnerShell({
    host,
    commands: createCodingCommandRegistry(),
  ...
  const first = await agentA.exec({
    agentSessionId: 'agent-a',
    script: 'demi todo add "First session"'
  })
  const second = await agentB.exec({
    agentSessionId: 'agent-b',
    script: 'demi todo add "Second session"'
  })
...
test(
  ...
  async () => {
    ...
    const other = await runnerShell({
      host,
      commands: createCodingCommandRegistry(),
      agentSessionId: 'other-agent',
      shellIdFactory: () => 'other-shell',
      initialEnv: {}
    })

    const firstShell = await env.exec({
      agentSessionId: 'todo-agent',
      script: 'demi todo add "First shell" --json'
    })
    expect(firstShell.shellId).toBe('todo-recreated-shell-1')
    expect(await env.disposeShell(firstShell.shellId)).toBe(true)
    const secondShell = await env.exec({
      agentSessionId: 'todo-agent',
      script: 'demi todo add "Second shell" --json'
    })
    const otherAgent = await other.exec({
      agentSessionId: 'other-agent',
      script: 'demi todo add "Other agent" --json'
    })
    ...
    const otherList = await other.exec({
      agentSessionId: 'other-agent',
      script: 'demi todo list --json'
    })
...
async function createTodoEnvironment(
