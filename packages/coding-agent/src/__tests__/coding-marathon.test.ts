...
test(
  ...
  async () => {
    ...
    await session.send([{
      type: 'text',
      text: 'Create app file, track tests, then update value.'
    }])
    ...
    const file = await environment.exec({
      shellId: 'coding-shell',
      script: 'cat src/app.ts'
    })
    expect(file.stdout.delta).toBe('export const value = 2\n')
    const todos = await environment.exec({
      agentSessionId: session.id(),
      shellId: 'coding-shell',
      script: 'demi todo list --json'
    })
    expect(JSON.parse(todos.stdout.delta)).toEqual({
      todos: [{ id: 'T1', text: 'Run tests', status: 'pending' }]
    })
  }
)
  ...
  async () => {
    ...
    const provider = new StubProvider([
      ...
      (request: InferenceRequest) => {
        ...
        expect(serialized)
          .toContain('Continue the workflow and close the todo.')
    ...
    const session = new AgentSession(
      ...
      { agentSessionId: 'coding-multiturn-agent' }
    )

    await session.send([{ type: 'text', text: 'Start the workflow.' }])
    await session.send([{
      type: 'text',
      text: 'Continue the workflow and close the todo.'
    }])
...
test(
  ...
  async () => {
    ...
    await session.send([{
      type: 'text',
      text: 'Prepare shell state, then reuse it.'
    }])
...
test(
  ...
  async () => {
    ...
    await session.send([{
      type: 'text',
      text: 'Create a tiny todo module, run its test, fix the failure, and rerun.'
    }])

    const file = await environment.exec({
      shellId: 'coding-fix-shell',
      script: 'cat src/todo.ts'
    })
...
test(
  ...
  async () => {
    ...
    await session.send([{
      type: 'text',
      text: 'Run the interactive long command, feed it input, then stop it.'
    }])
...
test(
  ...
  async () => {
      ...
      ...Array.from({ length: 100 }, () => (request: InferenceRequest) => {
        if (phase === 'after-start') {
          ...
          return [events.toolCall(
            'yield-reader',
            'yield',
            { description: 'Wait before checking reader', durationMs: 1 }
          )]
        }
        ...
        if (phase === 'await-long-ready') {
          ...
          if (result.status === 'running'
            && !result.stdout.includes('long-ready')) {
            poll += 1
            if (poll % 2 === 1) {
              return [
    ...
    await session.send([{
      type: 'text',
      text: 'Exercise every standard shell control tool.'
    }])
    await waitFor(
      () => session?.transcript().blocks.some((block) => block.type === 'text'
        && block.text.includes('full control ok'))
        ?? false,
      () => transcriptSummary(session),
      { timeoutMs: 10_000 },
    )

    const toolNames = session.transcript().blocks.flatMap(
...
function latestShellResult(
  ...
  if (first?.type !== 'text')
    throw new Error('tool result was not text')
...
function requiredField(text: string, name: string): string {
  ...
  if (!match)
    throw new Error(`missing field ${name} in ${text}`)
  return match[1]
}
...
function transcriptSummary(
  session: AgentSession<Record<string, never>> | null
): string {
  if (!session)
  ...
  return session
    ...
    .blocks.map((block) => {
      if (block.type === 'tool_call')
        return `${block.type}:${block.toolName}:${block.status}`
      if (block.type === 'text')
        return `${block.type}:${block.text}`
      if (block.type === 'error')
        return `${block.type}:${block.message}`
      return block.type
    })
    .join(' | ')
}
