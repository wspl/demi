...
/**
 * `demi file edit <path> --old … --new …`: one exact replacement, chosen by
 * occurrence or nearest line.
 */
export default async function edit(
  ctx: CommandContext<EditArgs>
  ...
  let content: string
  try {
    content = new TextDecoder().decode(await ctx.fs.readFile(
      path,
      { cwd: ctx.cwd }
    ))
  } catch (error) {
  ...
  if (!selection.match) {
    await ctx.stderr(
      `${selection.reason} in ${path}: ${formatMatches(matches)}\n`
    )
    return { exitCode: 1 }
  }
...
function chooseMatch(
  ...
  if (context !== undefined) {
    ...
    return tied.length === 1 ? { match: nearest.match, reason: '' } : {
      match: null,
      reason: `Context line ${context} is ambiguous`
    }
  }
  return matches.length === 1 ? { match: matches[0]!, reason: '' } : {
    match: null,
...
function formatMatches(matches: Match[]): string {
  return matches.map(
    (match, index) => `occurrence ${index + 1} at line ${match.line}`
  ).join(', ')
}
