...
/**
 * `demi file create <path>` with the content on stdin: a new file, never an
 * overwrite.
 */
export default async function create(ctx: CommandContext<{
  path: string;
  content: string
  ...
  try {
    await ctx.fs.writeFile(
      ...
      { cwd: ctx.cwd, createParents: true }
    )
  } catch (error) {
    await ctx.stderr(
