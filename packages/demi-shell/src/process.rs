//! `demishell:process`: spawn with pipes, wait, kill, uid/gid, process
//! groups and the tee with a bounded view.
//!
//! Children are spawned with `std::process::Command` and reaped by the
//! shell's own SIGCHLD loop (`waitpid(-1)`), which also reaps adopted
//! orphans when the shell is PID 1. Stdio pipes are handed to tokio and
