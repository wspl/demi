...
//!
//! Three kinds of specifier exist: `demishell:*` built-ins, `/embedded/*`
...
//!
//! `demishell:*` resolves only when the importer is the embedded bundle;
//! that check is what keeps the shell API private to `@demicodes/host-shell`.
...
pub const SCHEME: &str = "demishell:";
        ...
        if let Some(module) = name.strip_prefix(SCHEME) {
            if !base.starts_with(EMBEDDED_PREFIX) {
                return Err(Error::new_resolving_message(
                    base,
                    name,
                    "demishell modules are not available to modules loaded from files",
                ));
            }
            return match module {
