...
import * as net from "demishell:net";
import * as fs from "demishell:fs";
import * as proc from "demishell:process";
import { env, openHandles } from "demishell:runtime";

const enc = (s) => new TextEncoder().encode(s);
const dec = (b) => new TextDecoder().decode(b);
const tmp = `${(env.TMPDIR ?? "/tmp").replace(/\/+$/, "")}/demi-shell-net-${Date.now()}`;
...
const stubDir = env.DEMI_SHELL_CONFORMANCE_DIR;
const ports = (env.DEMI_SHELL_CONFORMANCE_PORTS ?? "").split(",").map(Number);
const bun = env.DEMI_SHELL_CONFORMANCE_BUN;
if (stubDir && bun && ports.length === 3) {
  ...
  const tls = !!env.DEMI_SHELL_CONFORMANCE_CERT;
  let stub;

  test("net: stub starts", async () => {
    stub = await proc.spawn({
      command: bun, args: [`${stubDir}/stub.ts`], cwd: stubDir,
      env: { PATH: env.PATH, HOME: env.HOME ?? "/tmp", DEMI_SHELL_CONFORMANCE_PORTS: ports.join(","), DEMI_SHELL_CONFORMANCE_CERT: env.DEMI_SHELL_CONFORMANCE_CERT ?? "", DEMI_SHELL_CONFORMANCE_KEY: env.DEMI_SHELL_CONFORMANCE_KEY ?? "" },
    });
  ...
  test("net: stub-backed cases skipped (set DEMI_SHELL_CONFORMANCE_DIR/PORTS/BUN)", async () => {
    await fs.rmdir(tmp);
  });
}
