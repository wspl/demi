...
async function writeVendorAuth(
  codexHome: string,
  email: string,
  accountId: string
) {
...
test(
  'codex credentials importDefault, setActive, and resolve via pool',
  async () => {
    ...
    try {
      ...
      expect(activeA.status).toMatchObject({
      ...
      expect(activeB.status).toMatchObject({
        status: 'authenticated',
      ...
      expect(JSON.stringify(await credentials.list()))
        .not.toContain('access_token')

      // active pointer file exists
      const activeRaw = await readFile(
        join(stateDir, 'credentials', 'codex', 'active'),
        'utf8'
      )
...
test(
  'createCodexProvider with custom authStore omits credentials by default',
  async () => {
...
test(
  'beginLogin runs the device-code flow, surfaces pending material, and imports into the pool',
  async () => {
    ...
    try {
      ...
      const loginFetch = (async (
        ...
        if (url.endsWith('/deviceauth/usercode')) {
          return jsonResponse(
            200,
            { device_auth_id: 'dev_1', user_code: 'CODE-0001', interval: '0' }
          )
        }
        ...
        if (url.endsWith('/oauth/token')) {
          return jsonResponse(
            200,
            { id_token: id, access_token: access, refresh_token: 'rt' }
          )
        }
      ...
      expect(pendings[0]!.verificationUrl)
      ...
      expect(list.some((c) => c.id === result.credentialId
        && c.label === 'device@example.com')).toBe(true)
