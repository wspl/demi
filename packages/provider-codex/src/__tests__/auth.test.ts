...
test(
  'FileCodexAuthStore refreshes near-expiry ChatGPT tokens and preserves unknown fields',
  async () => {
    ...
    try {
      ...
      expect(auth.kind === 'chatgpt' ? auth.email : null)
...
test(
  'FileCodexAuthStore resolves API key and reports missing auth without secret leakage',
  async () => {
...
test('JWT helpers parse ChatGPT account claims conservatively', () => {
  ...
  expect(parseJwtExpiration(token)?.toISOString())
    .toBe('2030-03-17T17:46:40.000Z')
  expect(parseChatGptClaims('not-a-jwt')).toEqual({
    accountId: null,
    email: null,
...
test(
  'auth mode resolution and redaction follow official auth.json precedence',
  async () => {
    expect(resolvedAuthMode({ personal_access_token: 'pat' }))
      .toBe('personalAccessToken')
    expect(resolvedAuthMode({ OPENAI_API_KEY: 'sk' })).toBe('apiKey')
    expect(resolvedAuthMode({ tokens: {} })).toBe('chatgpt')
    expect(
      redactCredentialText('Authorization: Bearer abc.def.ghi access_token: secret')
    ...
    expect(await staticStore.status()).toEqual({
      status: 'authenticated',
      accountLabel: 'apiKey'
    })
  }
