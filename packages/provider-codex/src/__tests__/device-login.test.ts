...
function scriptedFetch(
  ...
  return (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.endsWith('/deviceauth/usercode')) {
      return jsonResponse(
        200,
        { device_auth_id: 'dev_auth_1', user_code: 'WXYZ-9876', interval: '0' }
      )
    }
...
test(
  'runCodexDeviceLogin surfaces pending material and assembles vendor-shaped auth',
  async () => {
    const calls: Array<{
      url: string;
      body: string
    ...
    expect(JSON.parse(poll[0]!.body)).toEqual({
      device_auth_id: 'dev_auth_1',
      user_code: 'WXYZ-9876'
    })
  }
)
...
test('runCodexDeviceLogin reports unsupported servers distinctly', async () => {
  ...
  await expect(runCodexDeviceLogin({ fetch: fakeFetch }))
    .rejects.toThrow('Device-code login is not enabled')
})
