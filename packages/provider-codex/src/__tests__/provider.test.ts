...
test(
  ...
  async () => {
    ...
    expect((provider as unknown as { transport?: unknown }).transport)
      .not.toBe(injectedTransport)
  }
)

test(
  'buildCodexHeaders and responsesUrlForAuth follow Codex auth routing',
  () => {
    const headers = buildCodexHeaders(
      chatgptAuth,
      makeRequest(),
      { userAgent: 'demi-test/1.0' }
    )
    ...
    const clamped = buildCodexHeaders(
      chatgptAuth,
      { ...makeRequest(), sessionId: longSessionId },
      { userAgent: 'demi-test/1.0' }
    )
    ...
    expect(responsesUrlForAuth(chatgptAuth))
...
test(
  'CodexProvider streams text, thinking, tool calls, and usage through transport events',
  async () => {
...
test(
  'CodexProvider refreshes auth once after a 401 response and retries the request',
  async () => {
    ...
    expect(transport.requests[0]?.headers.get('Authorization'))
      .toBe('Bearer access-token')
    expect(transport.requests[1]?.headers.get('Authorization'))
...
test(
  'CodexProvider classifies transport timeouts as retryable overloaded errors',
  async () => {
    const transport = new FakeCodexTransport(
      [new Error('Codex SSE response headers timed out after 20000ms')]
    )
    const provider = new CodexProvider({
      authStore: new StaticCodexAuthStore(chatgptAuth),
      transportImpl: transport,
...
test(
  'CodexProvider preserves HTTP failure diagnostics without retrying inside the provider',
  async () => {
...
test(
  'AgentSession retries Codex server errors without replaying the user turn',
  async () => {
    const transport = new FakeCodexTransport([
      [
        {
...
test(
  'AutoCodexResponsesTransport falls back to SSE only before WebSocket has emitted events',
  async () => {
    ...
    const afterStart = new AutoCodexResponsesTransport(
      new YieldThenThrowTransport(
        [{ type: 'response.output_text.delta', delta: 'ws' }],
        new Error('after start')
      ),
      new FakeCodexTransport([[{
        type: 'response.output_text.delta',
        delta: 'sse'
...
test(
  ...
  async () => {
    ...
    expect(events)
      .toEqual([{ type: 'response.output_text.delta', delta: 'ws' }])
    ...
    expect(CapturingWebSocket.instances[0]?.headers['content-type'])
      .toBeUndefined()
  }
)

test(
  'WebSocketCodexResponsesTransport finishes when a response.completed event arrives without a close frame',
  async () => {
    ...
    expect(NonClosingCompletedWebSocket.instances[0]?.closeReasons)
      .toContain('response_done')
  }
)

test(
  'CodexProvider integrates with AgentSession and shell tools for function calls',
  async () => {
    const transport = new FakeCodexTransport([
      [
        {
          ...
          item: {
            ...
            call_id: 'call_1',
            name: 'shell_exec',
            arguments: '{"script":"printf demi-codex","timeoutMs":1000}'
          }
        },
        {
          type: 'response.completed',
    ...
    expect(toolBlock?.type === 'tool_call'
      ? toolBlock.output[0]?.type === 'text' && toolBlock.output[0].text
      : '').toContain('demi-codex')
    expect(transport.requests).toHaveLength(2)
    expect(transport.requests[0]?.body).toMatchObject({
      prompt_cache_key: 'agent-session-1'
    })
    expect(JSON.stringify(transport.requests[1]?.body))
...
test(
  'CodexProvider replays provider-stream steers in a same-turn follow-up before queued sends drain',
  async () => {
    ...
    expect(secondBody.indexOf('first'))
      .toBeLessThan(secondBody.indexOf('steer current'))
...
class CapturingWebSocket {
  ...
  removeEventListener(
    ...
    this.listeners[type] = (this.listeners[type] ?? []).filter(
      (candidate) => candidate !== (listener as (
        event: unknown
      ) => void)
    )
  }
