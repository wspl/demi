/**
 * Error thrown when an operation is canceled via an `AbortSignal` or a session
 * abort.
 */
 ...
 * promise is not canceled; this only stops the caller from awaiting it past the
 * abort.
 */
...
/**
 * Returns the string `code` of an errno-style error (`ENOENT`, `EEXIST`, …), or
 * null.
 */
 ...
 * shape Node's fs errors and txiki.js's native errors share, for failures a
 * Host
...
export function errnoError(
  ...
  return Object.assign(
    new Error(message),
    { name: 'ShellError', code, errno: 0, ...detail }
  )
}
