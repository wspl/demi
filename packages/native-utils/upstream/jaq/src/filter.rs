...
use crate::{read, Error, Runner, Val};
...
pub fn parse_compile(
    ...
    let home = uucore::context::env::home_dir().unwrap_or_default();
    let default = [home.join(".jq")];
    let paths = if paths.is_empty() { &default } else { paths };
    let resolved_paths: Vec<_> = paths.iter().map(uucore::context::resolve).collect();
    let paths = resolved_paths.as_slice();

    let vars: Vec<_> = vars.iter().map(|v| format!("${v}")).collect();
    let arena = Arena::default();
    let loader = Loader::new(jaq_all::defs().chain(defs())).with_std_read(paths);
    //let loader = Loader::new([]).with_std_read(paths);
    let path = uucore::context::resolve(path);
    let modules = loader
