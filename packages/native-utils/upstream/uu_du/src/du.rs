...
use uucore::context::fs::{self, DirEntry, File, Metadata};
use uucore::context::io::{self, BufRead, BufReader, Write, stdout};
#[cfg(not(windows))]
...
impl Stat {
    fn new(
        ...
        Ok(Self {
            ...
            size: if metadata.context_is_dir() { 0 } else { metadata.len() },
            blocks,
            inodes: 1,
            inode: file_info,
            metadata,
    ...
    fn new_from_dirfd(
        ...
        // Create a temporary uucore::context::fs::Metadata by reading the same path
        ...
        Ok(Self {
            ...
            size: if safe_metadata.context_is_dir() {
                0
            } else {
                safe_metadata.len()
            },
...
fn safe_du(
    ...
    if !my_stat.metadata.context_is_dir() {
        return Ok(my_stat);
    }
    ...
    'file_loop: for entry_name in entries {
        ...
        // We can't use uucore::context::fs::Metadata since that requires the full path
...
// Regular traversal using uucore::context::fs
...
fn du_regular(
    ...
    let my_inode = if my_stat.metadata.context_is_dir() {
        my_stat.inode
    } else {
        None
    };
                    ...
                    match Stat::new(&entry_path, Some(&entry), options) {
                        Ok(this_stat) => {
                            ...
                            if is_symlink
                                ...
                                && this_stat.metadata.context_is_dir()
                            ...
                            if this_stat.metadata.context_is_dir() {
                                if options.one_file_system
