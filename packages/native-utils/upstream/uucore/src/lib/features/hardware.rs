...
//!     crate::context_println!("CPU has AVX2 support");
//! }
//!
//! // Check SIMD policy (respects GLIBC_TUNABLES)
//! let policy = SimdPolicy::detect();
//! if policy.has_avx2() {
//!     crate::context_println!("CPU has AVX2 support and it is not disabled by env");
//! }
...
impl SimdPolicy {
    ...
    /// let policy = SimdPolicy::detect();
    /// if policy.allows_simd() {
    ///     crate::context_println!("SIMD is enabled");
    /// } else {
    ///     crate::context_println!("SIMD disabled by: {:?}", policy.disabled_features());
    /// }
    /// ```
    pub fn detect() -> &'static Self {
