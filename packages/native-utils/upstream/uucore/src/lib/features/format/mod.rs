...
pub use spec::Spec;
use {std::error::Error, std::fmt::Display, crate::context::io::Write, crate::context::io::stdout, std::marker::PhantomData, std::ops::ControlFlow, std::ops::Range};
...
pub enum FormatError {
    ...
    IoError(crate::context::io::Error),
    NoMoreArguments,
...
impl From<crate::context::io::Error> for FormatError {
    fn from(value: crate::context::io::Error) -> Self {
        Self::IoError(value)
    }
}
...
fn check_width(width: usize) -> crate::context::io::Result<()> {
    if width > MAX_FORMAT_WIDTH {
        Err(crate::context::io::Error::new(
            crate::context::io::ErrorKind::OutOfMemory,
...
pub trait FormatChar {
    fn write(&self, writer: impl Write) -> crate::context::io::Result<ControlFlow<()>>;
}

impl FormatChar for u8 {
    fn write(&self, mut writer: impl Write) -> crate::context::io::Result<ControlFlow<()>> {
...
impl<F: Formatter<T>, T> Format<F, T> {
    ...
    pub fn fmt(&self, mut w: impl Write, f: T) -> crate::context::io::Result<()> {
