...
use crate::context::fs::read_dir;
use std::hash::Hash;
use crate::context::io::Stdin;
use crate::context::io::{Error, ErrorKind, Result as IOResult};
    ...
    pub fn from_path(path: impl AsRef<Path>, dereference: bool) -> IOResult<Self> {
        #[cfg(any(unix, target_os = "wasi"))]
        {
            let stat = if dereference {
                rustix::fs::stat(crate::context::resolve(&path))
            } else {
                rustix::fs::lstat(crate::context::resolve(&path))
            };
            Ok(Self(stat?))
        }
        #[cfg(windows)]
        {
            use crate::context::fs::OpenOptions;
...
fn resolve_symlink<P: AsRef<Path>>(path: P) -> IOResult<Option<PathBuf>> {
    let result = if fs::symlink_metadata(&path)?.file_type().context_is_symlink() {
...
fn ensure_is_directory(path: &Path) -> IOResult<()> {
    if fs::metadata(path)?.context_is_dir() {
        Ok(())
    } else {
        read_dir(path)?;
        Ok(())
...
///
/// This function is a generalization of [`crate::context::fs::canonicalize`] that
...
///   [`crate::context::fs::canonicalize`], resolving symbolic links and returning
...
pub fn display_permissions(metadata: &fs::Metadata, display_file_type: bool) -> String {
    ...
    if display_file_type {
        let file_type = if metadata.context_is_symlink() {
            'l'
        } else if metadata.context_is_dir() {
            'd'
        } else {
            '-'
        };
...
/// crate::context::fs::create_dir("foo/."); fails in pure Rust
...
pub fn are_files_identical(path1: impl AsRef<Path>, path2: impl AsRef<Path>) -> IOResult<bool> {
    use crate::context::fs::{File, metadata};
    use crate::context::io::{BufReader, ErrorKind, Read};
    ...
    if !metadata1.context_is_file() || !metadata2.context_is_file() {
        return Ok(false);
    }
...
pub fn is_symlink_loop(path: &Path) -> bool {
    ...
    while let (Ok(metadata), Ok(link)) = (
        current_path.context_symlink_metadata(),
        fs::read_link(&current_path),
    ) {
        if !metadata.file_type().context_is_symlink() {
            return false;
        }
...
pub fn is_stdin_directory(stdin: &Stdin) -> bool {
    ...
    #[cfg(windows)]
    {
        ...
        if let Ok(metadata) = fs::metadata(format!("{}", handle as usize)) {
            return metadata.context_is_dir();
        }
        false
    }
...
pub mod sane_blksize {
    ...
    #[cfg(unix)]
    use std::os::unix::fs::MetadataExt;
    use {crate::context::fs::metadata, std::path::Path};
    ...
    pub fn sane_blksize_from_metadata(
        #[cfg(unix)] metadata: &crate::context::fs::Metadata,
        #[cfg(not(unix))] _: &crate::context::fs::Metadata,
    ) -> u64 {
        #[cfg(unix)]
        {
