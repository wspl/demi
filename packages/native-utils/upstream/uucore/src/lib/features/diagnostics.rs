...
use crate::context::io::IsTerminal;
use std::ops::Range;
...
pub fn enabled() -> bool {
    ...
    match MODE.get_or_init(|| Mode::from_env(env::var_os(MODE_VAR).as_deref())) {
        Mode::Always => true,
        Mode::Never => false,
        Mode::Auto => crate::context::io::stderr().is_terminal(),
    }
}
...
impl Snapshot {
    ...
    fn report(
        ...
        let id = crate::util_name();
        let color = env::var_os("NO_COLOR").is_none() && crate::context::io::stderr().is_terminal();
        ...
        if label.is_some() {
            crate::context_eprint!("{id}: {message}\n{body}");
        } else {
            ...
            let body = body
                ...
                .collect::<Vec<_>>()
                .join("\n");
            crate::context_eprintln!("{id}: {message}\n{body}");
        }
        true
    }
}
