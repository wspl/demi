...
use crate::context::process::{Child, Command};
...
pub fn send_signal_to_own_group(signal: usize) -> io::Result<()> {
    ...
    send_signal_to_each(&with_self_last(members, crate::context::process::id()), signal)
}
...
mod tests {
    use super::*;
    use crate::context::process::{Command, Stdio};
    ...
    fn send_signal_to_pid_rejects_invalid_signal_before_any_open() {
        assert_eq!(
            send_signal_to_pid(crate::context::process::id(), 64)
                .unwrap_err()
                .kind(),
