...
#[cfg(any(target_os = "linux", target_os = "android"))]
pub fn copy_fast(
    src: &mut (impl crate::context::io::Read + AsFd),
    dest: &mut impl AsFd,
    ...
    if crate::pipes::splice_unbounded_auto(src, dest)?.is_err() {
        ...
        crate::context::io::copy(src, &mut crate::io::RawWriter(dest))?;
    }
    Ok(())
}

#[cfg(not(any(target_os = "linux", target_os = "android")))]
pub use crate::context::io::copy as copy_fast;
...
mod tests {
    ...
    fn test_copy_fast() {
        ...
        let (mut pipe_read, mut pipe_write) = crate::context::io::pipe().unwrap();
