...
pub fn open_source<P: AsRef<Path>>(path: P, nofollow: bool) -> io::Result<File> {
    ...
    let fd: OwnedFd = open(crate::context::resolve(path), flags, Mode::empty())?;
    Ok(File::from(fd))
}
...
pub fn create_dest_restrictive<P: AsRef<Path>>(path: P, nofollow: bool) -> io::Result<File> {
    ...
    let fd: OwnedFd = open(crate::context::resolve(path), flags, Mode::RUSR.union(Mode::WUSR))?;
    Ok(File::from(fd))
}
...
mod tests {
    use super::*;
    use crate::context::io::{Read, Write};
    ...
    fn create_dest_truncates_but_preserves_existing_mode() {
        ...
        {
            let mut f = crate::context::fs::OpenOptions::new()
                .write(true)
                .create(true)
                .truncate(true)
                .mode(0o644)
        ...
        let mode = crate::context::fs::metadata(&path).unwrap().mode() & 0o777;
        assert_eq!(mode, 0o644);
        assert_eq!(crate::context::fs::metadata(&path).unwrap().len(), 0);
    }
    ...
    fn create_dest_rejects_symlink_with_nofollow() {
        ...
        crate::context::fs::write(&victim, b"do not truncate me").unwrap();
        ...
        assert_eq!(crate::context::fs::read(&victim).unwrap(), b"do not truncate me");
    }
}
