...
pub fn enable_pipe_errors() -> Result<(), Errno> {
    if crate::context::utility_name().is_some() {
        // Builtins receive write errors through their streams; the runner owns
        // job cancellation. Never change another builtin's signal disposition.
        return Ok(());
    }
...
pub fn stdin_was_closed() -> bool {
    if crate::context::utility_name().is_some() { return false; }
...
#[cfg(target_os = "linux")]
pub fn ensure_stdout_not_broken() -> crate::context::io::Result<bool> {
    use nix::{
        poll::{PollFd, PollFlags, PollTimeout, poll},
        sys::stat::{SFlag, fstat},
    };
    use crate::context::io::stdout;
    use std::os::fd::AsFd;
