...
use std::hash::Hash;
use {std::collections::HashMap, std::fmt, std::fmt::Display, std::fmt::Formatter, crate::context::fs, crate::context::io, std::path::PathBuf, std::rc::Rc};
...
impl ProcessInformation {
    ...
    pub fn try_new(value: PathBuf) -> Result<Self, io::Error> {
        ...
        let value = if value.context_is_symlink() {
            fs::read_link(value)?
        } else {
            value
        };
    ...
    pub fn tty(&self) -> Teletype {
        ...
        for dir in result.flatten().filter(|it| it.path().context_is_symlink()) {
...
pub fn walk_process() -> impl Iterator<Item = ProcessInformation> {
    WalkDir::new("/proc/")
        ...
        .into_iter()
        .flatten()
        .filter(|it| it.path().context_is_dir())
...
mod tests {
    use super::*;
    use crate::features::tty::Teletype;
    use {std::collections::HashSet, std::str::FromStr};
    ...
    fn test_pid_entry() {
        ...
        let result = WalkDir::new(format!("/proc/{current_pid}/fd"))
            ...
            .flat_map(|it| it.context_read_link())
