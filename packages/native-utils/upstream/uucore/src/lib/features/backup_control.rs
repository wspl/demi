...
//!         ]);
//!
//!     let backup_mode = match backup_control::determine_backup_mode(crate::context::env::var("VERSION_CONTROL").ok(), &matches) {
//!         Err(e) => {
//!             show!(e);
//!             return;
//!         },
...
use clap::ArgMatches;
use {crate::context::env, std::error::Error, std::ffi::OsStr, std::ffi::OsString, std::fmt::Debug, std::fmt::Display, crate::context::fs, std::path::Path, std::path::PathBuf};
...
pub enum BackupError {
    ...
    /// Currently unused
    BackupImpossible(),
    // BackupFailed(PathBuf, PathBuf, crate::context::io::Error),
}
...
///         ]);
///
///     let backup_mode = backup_control::determine_backup_mode(crate::context::env::var("VERSION_CONTROL").ok(), &matches).unwrap();
...
///         ]);
///
///     let backup_mode = backup_control::determine_backup_mode(crate::context::env::var("VERSION_CONTROL").ok(), &matches);
///
