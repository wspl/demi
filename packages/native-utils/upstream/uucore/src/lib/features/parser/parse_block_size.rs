...
pub fn block_size_from_env(vars: &[&str]) -> BlockSizeEnv {
    for var in vars {
        if let Ok(s) = crate::context::env::var(var) {
...
pub fn default_block_size() -> u64 {
    if crate::context::env::var("POSIXLY_CORRECT").is_ok() {
        512
    } else {
        1024
    }
...
mod tests {
    ...
    fn clear_env_vars(vars: &[&str]) {
        for var in vars {
            unsafe {
                crate::context::env::remove_var(var);
            }
        }
    }
    ...
    fn set_env_var(key: &str, value: &str) {
        unsafe {
            crate::context::env::set_var(key, value);
        }
    }
