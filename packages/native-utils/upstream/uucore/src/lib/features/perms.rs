...
use std::ffi::CString;
use crate::context::fs::Metadata;
use crate::context::io::Error as IOError;
use crate::context::io::Result as IOResult;
...
fn is_root(path: &Path, would_traverse_symlink: bool) -> bool {
    ...
    if !would_traverse_symlink {
        // We cannot check path.context_is_dir() here, as this would resolve symlinks,
    ...
    if let Ok(p) = path.context_canonicalize() {
...
impl ChownExecutor {
    pub fn exec(&self) -> UResult<()> {
        ...
        if let Err(e) = crate::context::io::stdout().flush() {
    ...
    fn traverse<P: AsRef<Path>>(&self, root: P) -> i32 {
        ...
        let ret = if self.matched(meta.uid(), meta.gid()) {
            ...
            #[cfg(target_os = "linux")]
            // We cannot check path.context_is_dir() here, as this would resolve symlinks
            let chown_result = if meta.context_is_dir() {
    ...
    fn safe_dive_into<P: AsRef<Path>>(&self, root: P) -> i32 {
        ...
        if self.traverse_symlinks == TraverseSymlinks::None && root.context_is_symlink() {
            return 0;
        }
    ...
    fn safe_traverse_dir(
        ...
        for entry_name in entries {
            ...
            if meta.context_is_dir() && (follow || !meta.file_type().context_is_symlink()) {
    ...
    fn write_verbose_line(&self, line: &str) -> i32 {
        use crate::context::io::Write;
        i32::from(writeln!(crate::context::io::stdout(), "{line}").is_err())
    }
