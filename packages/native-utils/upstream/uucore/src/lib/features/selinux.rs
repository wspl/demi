...
fn is_ignorable_context_error(error: &(dyn Error + 'static)) -> bool {
    ...
    while let Some(err) = current {
        if let Some(io_error) = err.downcast_ref::<crate::context::io::Error>() {
            return matches!(
...
///     crate::context_eprintln!("Failed to set default context: {err}");
/// }
/// ```
///
...
/// if let Err(err) = result {
///     crate::context_eprintln!("Failed to set context: {err}");
/// }
/// ```
...
fn absolute_for_policy_lookup(path: &Path) -> Cow<'_, Path> {
    ...
    let resolved = match (path.parent(), path.file_name()) {
        (Some(directory), Some(name)) => {
            ...
            crate::context::fs::canonicalize(directory).map(|directory| directory.join(name))
        }
        ...
        // pattern.
        _ => crate::context::fs::canonicalize(path),
    };
...
///     Ok(context) => {
///         if context.is_empty() {
///             crate::context_println!("No SELinux context found for the file");
///         } else {
///             crate::context_println!("SELinux context: {context}");
///         }
///     },
///     Err(SeLinuxError::SELinuxNotEnabled) => crate::context_println!("SELinux is not enabled on this system"),
///     Err(SeLinuxError::FileOpenFailure(e)) => crate::context_println!("Failed to open the file: {e}"),
///     Err(SeLinuxError::ContextRetrievalFailure(e)) => crate::context_println!("Failed to retrieve the security context: {e}"),
///     Err(SeLinuxError::ContextConversionFailure(ctx, e)) => crate::context_println!("Failed to convert context '{ctx}': {e}"),
///     Err(SeLinuxError::ContextSetFailure(ctx, e)) => crate::context_println!("Failed to set context '{ctx}': {e}"),
///     Err(SeLinuxError::OperationNotSupported) => crate::context_println!("Operation not supported"),
/// }
/// ```
...
/// if differ {
///     crate::context_println!("Files have different SELinux contexts");
/// } else {
///     crate::context_println!("Files have the same SELinux context");
/// }
/// ```
...
///     Ok(_) => crate::context_println!("Context preserved successfully (or SELinux is not enabled)"),
///     Err(err) => crate::context_eprintln!("Failed to preserve context: {err}"),
/// }
/// ```
...
pub fn get_getfattr_output(f: &str) -> String {
    use crate::context::process::Command;
    ...
    crate::context_println!("{getfattr_output:?}");
    assert!(
...
mod tests {
    ...
    fn test_absolute_for_policy_lookup() {
        ...
        let directory = crate::context::fs::canonicalize(directory.path()).expect("Failed to canonicalize");
        crate::context::fs::create_dir(directory.join("branch")).expect("Failed to create dir");
        let previous = crate::context::env::current_dir().expect("Failed to read the working directory");
        crate::context::env::set_current_dir(&directory).expect("Failed to change the working directory");
        ...
        crate::context::env::set_current_dir(previous).expect("Failed to restore the working directory");
    }
    ...
    fn test_invalid_context_string_error() {
        ...
        let path = tmpfile.path();
        if !is_selinux_enabled() {
            crate::context_println!("test skipped: Kernel has no support for SElinux context");
            return;
        }
    ...
    fn test_get_selinux_security_context() {
        ...
        crate::context::fs::write(path, b"test content").expect("Failed to write to tempfile");

        let result = get_selinux_security_context(path, false);

        match result {
            Ok(context) => {
                crate::context_println!("Retrieved SELinux context: {context}");
            ...
            Err(SeLinuxError::ContextRetrievalFailure(e)) => {
                ...
                crate::context_println!("Context retrieval failure: {e}");
            }
            Err(SeLinuxError::ContextConversionFailure(ctx, e)) => {
                ...
                assert!(!e.is_empty(), "Error message should not be empty");
                crate::context_println!("Context conversion failure for '{ctx}': {e}");
            }
    ...
    fn test_get_selinux_context_symlink() {
        ...
        crate::context::fs::File::create(&file_path).expect("Failed to create file");
    ...
    fn test_contexts_differ() {
        ...
        crate::context::fs::write(path1, b"content for file 1").expect("Failed to write to first tempfile");
        crate::context::fs::write(path2, b"content for file 2").expect("Failed to write to second tempfile");
    ...
    fn test_preserve_security_context() {
        ...
        crate::context::fs::write(source_path, b"source content").expect("Failed to write to source tempfile");
        crate::context::fs::write(dest_path, b"destination content")
        ...
        if let Err(err) = result {
            match err {
                SeLinuxError::ContextSetFailure(_, _) => {
                    crate::context_println!("Note: Could not set context due to permissions: {err}");
                }
                unexpected => {
