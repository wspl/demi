...
//! handling. This has a couple advantages over manually using [`crate::context::process::exit`]:
...
//!    (i.e. [`crate::context::io::Result`] & `clap::ClapResult`).
...
use {std::cell::Cell, std::error::Error, std::fmt::Display, std::fmt::Formatter, crate::context::io::Write};

thread_local! { static EXIT_CODE: Cell<i32> = const { Cell::new(0) }; }
...
pub fn set_exit_code(code: i32) {
    EXIT_CODE.with(|value| value.set(code));
}
...
/// };
/// use {std::error::Error, std::fmt::Display, std::fmt::Debug, std::path::PathBuf};
///
/// #[derive(Debug)]
/// enum LsError {
...
/// Wrapper type around [`crate::context::io::Error`].
///
...
/// [`FromIo::map_err_context`] method on a [`crate::context::io::Result`] or [`crate::context::io::Error`].
/// ```
/// use uucore::{
///     display::Quotable,
///     error::{FromIo, UResult, UIoError, UError}
/// };
/// use crate::context::fs::File;
/// use std::path::Path;
/// let path = Path::new("test.txt");
///
/// // Manual construction
/// let e: Box<dyn UError> = UIoError::new(
///     crate::context::io::ErrorKind::NotFound,
///     format!("cannot access {}", path.quote())
/// );
/// let res: UResult<()> = Err(e.into());
///
/// // Converting from an `crate::context::io::Error`.
/// let res: UResult<File> = File::open(path).map_err_context(|| format!("cannot access {}", path.quote()));
/// ```
#[derive(Debug)]
pub struct UIoError {
    context: Option<String>,
    inner: crate::context::io::Error,
}

impl UIoError {
    #[allow(clippy::new_ret_no_self)]
    /// Create a new `UIoError` with a given exit code and message.
    pub fn new<S: Into<String>>(kind: crate::context::io::ErrorKind, context: S) -> Box<dyn UError> {
        Box::new(Self {
...
impl Display for UIoError {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        use crate::context::io::ErrorKind::*;
...
///
/// ```
/// use crate::context::io::{Error, ErrorKind};
...
/// ```
pub fn strip_errno(err: &crate::context::io::Error) -> String {
...
/// Enables the conversion from [`crate::context::io::Error`] to [`UError`] and from [`crate::context::io::Result`] to
/// [`UResult`].
pub trait FromIo<T> {
    /// Map the error context of an [`crate::context::io::Error`] or [`crate::context::io::Result`] to a custom error
    fn map_err_context(self, context: impl FnOnce() -> String) -> T;
}

impl FromIo<Box<UIoError>> for crate::context::io::Error {
...
impl<T> FromIo<UResult<T>> for crate::context::io::Result<T> {
    fn map_err_context(self, context: impl FnOnce() -> String) -> UResult<T> {
        self.map_err(|e| e.map_err_context(context) as Box<dyn UError>)
    }
}

impl FromIo<Box<UIoError>> for crate::context::io::ErrorKind {
    fn map_err_context(self, context: impl FnOnce() -> String) -> Box<UIoError> {
        Box::new(UIoError {
            context: Some(context()),
            inner: crate::context::io::Error::new(self, ""),
        })
    }
}

impl From<crate::context::io::Error> for UIoError {
    fn from(f: crate::context::io::Error) -> Self {
        Self {
            context: None,
            inner: f,
        }
...
impl From<crate::context::io::Error> for Box<dyn UError> {
    fn from(f: crate::context::io::Error) -> Self {
...
/// crate::context_println!("{}", uio_result.unwrap_err());
/// ```
#[cfg(unix)]
impl<T> FromIo<UResult<T>> for Result<T, nix::Error> {
    fn map_err_context(self, context: impl FnOnce() -> String) -> UResult<T> {
        self.map_err(|e| {
            Box::new(UIoError {
                context: Some(context()),
                inner: crate::context::io::Error::from_raw_os_error(e as i32),
            }) as Box<dyn UError>
        })
    }
}
...
impl<T> FromIo<UResult<T>> for nix::Error {
    fn map_err_context(self, context: impl FnOnce() -> String) -> UResult<T> {
        Err(Box::new(UIoError {
            context: Some(context()),
            inner: crate::context::io::Error::from_raw_os_error(self as i32),
        }) as Box<dyn UError>)
    }
}
...
impl From<nix::Error> for UIoError {
    fn from(f: nix::Error) -> Self {
        Self {
            context: None,
            inner: crate::context::io::Error::from_raw_os_error(f as i32),
        }
    }
}
...
/// [`UIoError`]. It takes:
///
/// - An instance of [`crate::context::io::Error`]
...
///
/// let io_err = crate::context::io::Error::new(
///     crate::context::io::ErrorKind::PermissionDenied, "fix me please!"
/// );
///
...
///
/// // prints "fix me please!: Permission denied"
/// crate::context_println!("{uio_err}");
/// // prints "Error code: 2: Permission denied"
/// crate::context_println!("{other_uio_err}");
/// ```
///
...
/// [`crate::context::io::Error`] is appended to whatever error message is defined in
/// addition (as secondary argument).
///
/// If you want to show only the error message for the [`crate::context::io::ErrorKind`]
...
impl Display for ClapErrorWrapper {
    fn fmt(&self, _f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        ...
        if let Err(print_fail) = {
            let rendered = self.error.render().to_string();
            if self.error.use_stderr() {
                crate::context::io::stderr().write_all(rendered.as_bytes())
            } else {
                crate::context::io::stdout().write_all(rendered.as_bytes())
            }
        } {
            ...
            let _ = writeln!(crate::context::io::stderr(), "{}: {print_fail}", crate::util_name());
