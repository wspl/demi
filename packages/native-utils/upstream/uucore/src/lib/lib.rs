...
use context::io::{BufRead, BufReader};
use std::iter;
#[cfg(unix)]
                ...
                .unwrap_or_else(|err| {
                    match err {
                        uucore::locale::LocalizationError::ParseResource {
                            error: err_msg,
                            snippet,
                        } => crate::context_eprintln!("Localization parse error at {snippet}: {err_msg:?}"),
                        other => crate::context_eprintln!("Could not init the localization system: {other}"),
                    }
                    context::process::exit(99)
                });
...
macro_rules! bin {
    ...
    ($util:ident) => {
        ::uucore::bin_inner! {$util, {
            ...
            if let Err(e) = context::io::stdout().flush() {
                crate::context_eprintln!("Error flushing stdout: {e}");
            }
        }}
    };
}
...
pub fn localized_help_template(util_name: &str) -> clap::builder::StyledStr {
    use context::io::IsTerminal;

    // Determine if colors should be enabled - same logic as configure_localized_command
    let colors_enabled = if context::env::var("NO_COLOR").is_ok() {
        false
    } else if context::env::var("CLICOLOR_FORCE").is_ok() || context::env::var("FORCE_COLOR").is_ok() {
        true
    } else {
        IsTerminal::is_terminal(&context::io::stdout())
            && context::env::var("TERM").unwrap_or_default() != "dumb"
    };
...
#[cfg(not(windows))]
static ARGV: LazyLock<Vec<OsString>> = LazyLock::new(|| context::env::args_os().collect());
...
pub fn util_name() -> &'static str {
    context::utility_name().unwrap_or_else(|| &UTIL_NAME)
}
...
pub fn execution_phrase() -> &'static str {
    if let Some(name) = context::utility_name() { return name; }
    &EXECUTION_PHRASE
}
...
pub fn read_yes() -> bool {
    ...
    context::io::stdin().read_line(&mut s).is_ok() && matches!(s.chars().next(), Some('y' | 'Y'))
}
...
pub fn read_byte_lines<R: context::io::Read>(
    mut buf_reader: BufReader<R>,
) -> impl Iterator<Item = context::io::Result<Vec<u8>>> {
    iter::from_fn(move || {
...
pub fn read_os_string_lines<R: context::io::Read>(
    buf_reader: BufReader<R>,
) -> impl Iterator<Item = context::io::Result<OsString>> {
...
macro_rules! prompt_yes(
    ($($args:tt)+) => ({
        use std::io::Write;
        $crate::context_eprint!("{}: ", uucore::util_name());
        $crate::context_eprint!($($args)+);
        $crate::context_eprint!(" ");
        let res = $crate::context::io::stderr().flush().map_err(|err| {
