...
fn find_mount_point<P: AsRef<Path>>(path: P) -> io::Result<PathBuf> {
    let mut current = path.as_ref().context_canonicalize()?;
    let current_dev = uucore::context::fs::metadata(&current)?.dev();

    while let Some(parent) = current.parent().filter(|p| !p.as_os_str().is_empty()) {
        let parent_dev = uucore::context::fs::metadata(&parent)?.dev();
    ...
    P: AsRef<Path>,
{
    ...
    let canonical_path = path.context_canonicalize().map_err(|_| FsError::InvalidPath)?;
