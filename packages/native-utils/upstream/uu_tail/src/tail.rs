...
use uucore::context::io::{self, BufReader, BufWriter, ErrorKind, Read, Seek, SeekFrom, Write, stdin, stdout};
        ...
        match open_result {
            Ok(mut file) => {
                ...
                if !settings.presume_input_pipe
                    ...
                    && (!st.context_is_file() || st.len() > blksize_limit)
                {
