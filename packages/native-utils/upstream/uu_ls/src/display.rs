...
use {std::borrow::Cow, std::iter};
use {std::ffi::OsStr, std::ffi::OsString, std::fmt::Write as FmtWrite, uucore::context::fs, uucore::context::fs::DirEntry, uucore::context::fs::FileType, uucore::context::fs::Metadata, uucore::context::io::BufWriter, uucore::context::io::Stdout, uucore::context::io::Write};
...
fn display_item_name(
    ...
    if is_long_symlink {
        ...
        match path.path().context_read_link() {
            Ok(target_path) => {
                name.push(" -> ");
                        ...
                        Ok(resolved_target) => {
                            ...
                            let target_display = if let Some(style_manager) = style_manager {
                                let md = match target_data.metadata() {
                                    ...
                                    None => {
                                        target_data.p_buf.context_symlink_metadata().ok().map(Cow::Owned)
                                    }
                                };
        ...
        let leading_char = {
            if let Some(ft) = item.file_type() {
                if ft.is_char_device() {
                    ...
                    'b'
                } else if ft.context_is_symlink() {
                    'l'
                } else if ft.context_is_dir() {
                    'd'
                } else {
                    '-'
                }
...
fn classify_file(path: &PathData) -> Option<char> {
    ...
    if file_type.context_is_dir() {
        Some('/')
    } else if file_type.context_is_symlink() {
        Some('@')
    } else {
        #[cfg(unix)]
        {
...
fn write_os_str<W: Write>(writer: &mut W, string: &OsStr) -> uucore::context::io::Result<()> {
