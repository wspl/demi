...
use {std::borrow::Cow, std::ffi::OsStr, std::ffi::OsString, uucore::context::io, uucore::context::io::IsTerminal, uucore::context::io::Write as _, uucore::context::io::stdout, std::num::IntErrorKind};
    ...
    match parse_block_size::block_size_from_env(&["LS_BLOCK_SIZE", "BLOCK_SIZE"]) {
        ...
        parse_block_size::BlockSizeEnv::NotSet => {
            ...
            match parse_block_size::block_size_from_env(&["BLOCKSIZE"]) {
                ...
                parse_block_size::BlockSizeEnv::NotSet => {
                    if uucore::context::env::var_os("POSIXLY_CORRECT").is_some() && !opt_kb {
...
fn is_color_compatible_term() -> bool {
    let term = uucore::context::env::var_os("TERM");
    let colorterm = uucore::context::env::var_os("COLORTERM");
...
fn extract_quoting_style(
    ...
    if let Some(style) = opt_quoting_style {
        ...
        if let Ok(style) = uucore::context::env::var("QUOTING_STYLE") {
            ...
            let _ = writeln!(
                io::stderr(),
                "{}",
                translate!("ls-invalid-quoting-style", "program" => uucore::context::env::args().next().unwrap_or_else(|| "ls".to_string()), "style" => style)
            );
        }
...
fn parse_width(width_match: Option<&String>) -> Result<u16, LsError> {
    ...
    let ret = match width_match {
        ...
        None => match uucore::context::env::var_os("COLUMNS") {
...
fn parse_time_style(options: &clap::ArgMatches) -> Result<(String, Option<String>), LsError> {
    ...
    if let Some(field) = options
        ...
        .map(Cow::from)
        .or_else(|| uucore::context::env::var("TIME_STYLE").ok().map(Cow::from))
    {
                ...
                > options.indices_of(options::TIME_STYLE).unwrap().next_back()
        {
            ...
            let field = if let Some(field) = field.strip_prefix("posix-") {
                ...
                if uucore::context::env::var_os("LC_TIME").as_deref() == Some(OsStr::new("POSIX"))
                    || uucore::context::env::var_os("LC_ALL").as_deref() == Some(OsStr::new("POSIX"))
                {
