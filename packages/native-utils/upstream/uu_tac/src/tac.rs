...
use uucore::context::io::{BufWriter, Read, Write, stdin, stdout};
use {uucore::context::fs::File, std::path::Path};
...
///
/// This function writes each line in `data` to [`uucore::context::io::Stdout`] in
/// reverse.
///
/// # Errors
///
/// If there is a problem writing to `stdout`, then this function
/// returns [`uucore::context::io::Error`].
fn buffer_tac_regex(
    data: &[u8],
...
/// `"/abc/def"`.
fn buffer_tac(data: &[u8], before: bool, separator: &OsStr) -> uucore::context::io::Result<()> {
    let out = stdout();
    let mut out = BufWriter::new(out.lock());
        ...
        let data: &[u8] = if filename == "-" {
            ...
            if uucore::signals::stdin_was_closed() {
                let e: Box<dyn UError> = TacError::ReadError(
                    OsString::from("-"),
                    uucore::context::io::Error::from_raw_os_error(libc::EBADF),
                )
                .into();
                show!(e);
                set_exit_code(1);
...
fn buffer_stdin() -> uucore::context::io::Result<StdinData> {
