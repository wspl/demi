...
pub enum TouchError {
    ...
    ReferenceFileInaccessible(PathBuf, uucore::context::io::Error),
...
fn to_uioerror(err: &uucore::context::io::Error) -> UIoError {
    let copy = if let Some(code) = err.raw_os_error() {
        uucore::context::io::Error::from_raw_os_error(code)
    } else {
        uucore::context::io::Error::from(err.kind())
    };
    UIoError::from(copy)
}
