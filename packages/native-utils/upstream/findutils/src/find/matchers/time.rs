...
impl NewerOptionType {
    ...
    fn get_file_time(self, metadata: &Metadata) -> uucore::context::io::Result<SystemTime> {
        match self {
...
/// Provide access to the *change* timestamp, since uucore::context::fs::Metadata doesn't expose it.
pub trait ChangeTime {
    /// Returns the time of the last change to the metadata.
    fn changed(&self) -> uucore::context::io::Result<SystemTime>;
}
...
impl ChangeTime for Metadata {
    fn changed(&self) -> uucore::context::io::Result<SystemTime> {
        ...
        Err(uucore::context::io::Error::from(uucore::context::io::ErrorKind::Unsupported))
    }
}
