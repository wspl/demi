...
pub(crate) fn parse(args: Vec<OsString>) -> ParseResult<HiArgs> {
...
fn parse_low(args: Vec<OsString>) -> ParseResult<LowArgs> {
    let parser = Parser::new();
    let mut low = LowArgs::default();
    if let Err(err) = parser.parse(args.iter().skip(1).cloned(), &mut low) {
    ...
    if config_args.is_empty() {
        crate::debug!("no extra arguments found from configuration file");
        return ParseResult::Ok(low);
    }
...
fn set_log_levels(low: &LowArgs) {
    ...
    match low.logging {
        Some(LoggingMode::Trace) => crate::messages::set_level(2),
        Some(LoggingMode::Debug) => crate::messages::set_level(1),
        None => crate::messages::set_level(0),
    }
}
