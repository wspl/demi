...
pub(crate) struct State {
    messages: AtomicBool,
    ignore_messages: AtomicBool,
    errored: AtomicBool,
    level: AtomicU8,
}
thread_local! { static CURRENT: RefCell<Arc<State>> = RefCell::new(Arc::new(State::default())); }
pub(crate) fn snapshot() -> Arc<State> {
    CURRENT.with(|state| state.borrow().clone())
}
pub(crate) fn with<T>(state: Arc<State>, f: impl FnOnce() -> T) -> T {
    struct Restore(Option<Arc<State>>);
    impl Drop for Restore {
        fn drop(&mut self) {
            CURRENT.with(|state| {
                state.replace(self.0.take().unwrap());
            });
        }
    }
    let _restore =
        Restore(Some(CURRENT.with(|current| current.replace(state))));
    f()
}
pub(crate) fn set_level(level: u8) {
    snapshot().level.store(level, Ordering::Relaxed);
}
pub(crate) fn level() -> u8 {
    snapshot().level.load(Ordering::Relaxed)
}

#[macro_export]
macro_rules! debug { ($($arg:tt)*) => { if crate::messages::level() >= 1 { uucore::context_eprintln!($($arg)*); } }; }
#[macro_export]
macro_rules! trace { ($($arg:tt)*) => { if crate::messages::level() >= 2 { uucore::context_eprintln!($($arg)*); } }; }
    ...
    ($($tt:tt)*) => {{
        {
            ...
            let stdout = uucore::context::io::stdout().lock();
            let mut stderr = uucore::context::io::stderr().lock();
            ...
            if let Err(err) = write!(stderr, "rg: ") {
                if err.kind() == std::io::ErrorKind::BrokenPipe {
                    uucore::context::exit(0);
                } else {
                    uucore::context::exit(2);
                }
            }
...
pub(crate) fn messages() -> bool {
    snapshot().messages.load(Ordering::Relaxed)
}
...
pub(crate) fn set_errored() {
    snapshot().errored.store(true, Ordering::Relaxed);
}
