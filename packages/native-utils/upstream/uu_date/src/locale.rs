...
mod tests {
    cfg_langinfo! {
        ...
        fn test_c_locale_format() {
            ...
            let original_lc_all = uucore::context::env::var_os("LC_ALL");
            let original_lc_time = uucore::context::env::var_os("LC_TIME");
            let original_lang = uucore::context::env::var_os("LANG");
            ...
            unsafe {
                // Set C locale
                uucore::context::env::set_var("LC_ALL", "C");
                uucore::context::env::remove_var("LC_TIME");
                uucore::context::env::remove_var("LANG");
            }
            ...
            unsafe {
                if let Some(val) = original_lc_all {
                    uucore::context::env::set_var("LC_ALL", val);
                } else {
                    uucore::context::env::remove_var("LC_ALL");
                }
                if let Some(val) = original_lc_time {
                    uucore::context::env::set_var("LC_TIME", val);
                } else {
                    uucore::context::env::remove_var("LC_TIME");
                }
                if let Some(val) = original_lang {
                    uucore::context::env::set_var("LANG", val);
                } else {
                    uucore::context::env::remove_var("LANG");
                }
            }
