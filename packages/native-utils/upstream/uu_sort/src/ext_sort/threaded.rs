...
use uucore::context::io::{Read, Write, stderr};
...
pub fn ext_sort(
    ...
    if let Some(ref prog) = settings.compress_prog {
        ...
        match uucore::context::process::Command::new(prog)
            .stdin(uucore::context::process::Stdio::null())
            .stdout(uucore::context::process::Stdio::null())
            .stderr(uucore::context::process::Stdio::null())
            .spawn()
        {
            Ok(mut child) => {
