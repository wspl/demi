...
use uucore::context::io::{Write, stdout};
...
pub fn mkdir(path: &Path, config: &Config) -> UResult<()> {
    ...
    // uucore::context::fs::create_dir("foo/."); fails in pure Rust
...
fn create_dir(path: &Path, is_parent: bool, config: &Config) -> UResult<()> {
    let path_exists = path.context_exists();
...
/// Create with restrictive permissions, then apply the invocation's exact mode.
/// No utility may mutate the shared process umask.
#[cfg(unix)]
fn create_dir_with_mode(
    ...
    let mode = mode & !(shaped_umask.bits() as u32);
    uucore::context::fs::DirBuilder::new().mode(mode).create(path)?;
    uucore::context::fs::set_permissions(path, uucore::context::fs::Permissions::from_mode(mode))
}

#[cfg(not(unix))]
fn create_dir_with_mode(path: &Path, _mode: u32, _shaped_umask: u32) -> uucore::context::io::Result<()> {
    uucore::context::fs::create_dir(path)
}
        ...
        Ok(()) => {
            ...
            if config.set_security_context {
                uucore::smack::set_smack_label_and_cleanup(path, config.context, |p| {
                    uucore::context::fs::remove_dir(p)
                })?;
            }
            Ok(())
        }

        Err(_) if config.recursive && path.context_is_dir() => {
