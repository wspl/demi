...
use uucore::context::fs::{read_dir, remove_dir};
        ...
        if let Err(error) = remove(path, opts) {
            ...
            #[cfg(unix)]
            {
                ...
                fn points_to_directory(path: &Path) -> io::Result<bool> {
                    Ok(uucore::context::fs::metadata(&path)?.file_type().context_is_dir())
                }

                let mut bytes = path.as_os_str().as_bytes();
                if error.raw_os_error() == Some(libc::ENOTDIR) && bytes.ends_with(b"/") {
                    // Strip the trailing slash or .context_symlink_metadata() will follow the symlink
                    bytes = strip_trailing_slashes_from_path(bytes);
                    let no_slash: &Path = OsStr::from_bytes(bytes).as_ref();
                    if no_slash.context_is_symlink() && points_to_directory(no_slash).unwrap_or(true) {
                        show_error!(
                            "{}",
