    ...
    let result = std::thread::spawn(move || {
        uucore::context::with(context, || {
            ...
            if matches!(name, "diff" | "cmp") && args.iter().any(|arg| arg == "--help") {
                ...
                if name == "diff" {
                    uucore::context_println!("Compare files. -u unified, -c context, -y side by side, -q brief, -s identical, -e ed script.");
                } else {
                    uucore::context_println!("Compare files byte by byte. -s silent, -l list differences, -n LIMIT, -i SKIP.");
                }
                return Ok(0);
            }
            let code = match name {
