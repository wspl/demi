...
// reads (`crates/runner/src/init/linux.rs`).
...
export interface GuestBootArgs {
  ...
  dns: string[]
  /**
   * The home image was just made from a directory: the guest chowns it to its
   * user on this boot.
   */
  firstBoot: boolean
}
...
/**
 * Firecracker's recommended kernel parameters: serial console, exit on panic,
 * no PCI scan, the runner as init.
 */
...
export function bootArgs(args: GuestBootArgs): string {
  for (const [name, value] of [
    ...
    if (/\s|"/.test(value!))
      throw new Error(
        `the ${name} cannot ride the kernel command line: ${value}`
      )
  }
  const parts = [
    KERNEL_ARGS,
  ...
  if (args.dns.length > 0)
    parts.push(`demi.dns=${args.dns.join(',')}`)
  if (args.firstBoot)
    parts.push('demi.firstboot=1')
  return parts.join(' ')
}
