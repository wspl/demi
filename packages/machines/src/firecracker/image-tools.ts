...
/**
 * The backing file enlarged to `bytes` (never shrunk here); the guest grows the
 * filesystem into it.
 */
export async function growImage(
  ...
  if ((await stat(imagePath)).size >= bytes)
    return
  await truncate(imagePath, bytes)
}

/**
 * Empty persistent overlay volume, containing upper/work directories created by
 * guest init.
 */
