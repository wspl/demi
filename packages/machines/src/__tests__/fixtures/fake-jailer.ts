...
async function runVm(
  ...
  await writeFile(
    join(root, `${basename(executable)}.pid`),
    String(process.pid)
  )
  await writeFile(
    join(config.directory, 'vm.json'),
  ...
  const server = createServer(connection => {
    connection.once('data', data => {
      if (data.toString() !== 'exit')
        throw new Error('unexpected fake VM request')
      connection.end()
...
if (process.argv[2] === '--fake-vm') {
  ...
  await writeFile(
    join(config.directory, 'launcher.json'),
    JSON.stringify({ pid: process.pid, args })
  )
  if (config.mode === 'fail')
    process.exit(23)
  if (config.mode === 'no-pid')
    process.exit(0)

  const flag = (name: string): string => {
    const index = args.indexOf(name)
    if (index < 0 || args[index + 1] === undefined)
      throw new Error(`missing fake jailer argument ${name}`)
    return args[index + 1]!
  }
