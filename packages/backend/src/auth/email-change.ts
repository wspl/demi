...
/**
 * Deployment supplies mail delivery; tests capture messages without sending
 * email.
 */
...
export class EmailChanges {
  ...
  async start(userId: string, email: string, password: string) {
    if (!this.sender)
      return { code: 'mail_unavailable' } as const
    ...
    if (await this.control.findUserByEmail(email))
    ...
    if (!issued)
      return { code: 'too_many_attempts' } as const
    try {
  ...
  confirm(userId: string, id: string, code: string) {
    return this.control.confirmEmailChallenge(
      userId,
      id,
