...
const renameHostBodySchema = z.object(
...
export function conversationRoutes(options: {
  ...
  const own = async (c: Context<AuthEnv>) => {
    ...
    return conversation &&
      conversation.userId === c.get('user').id
      ? conversation
      : null
  }
  ...
  app.get('/', async (c) => {
    ...
    const conversations = await control.listConversations(
      c.get('user').id,
      { archived }
    )
    return c.json(
  ...
  app.post('/batch', async (c) => {
    const parsed = z.strictObject({ items: z.array(z.strictObject({
        id: z.string().min(1),
        patch: conversationPatchSchema
      })).min(1).max(100) }).safeParse(await c.req.json().catch(() => null))
    if (!parsed.success)
      return c.json({
        code: 'invalid_body',
  ...
  app.patch('/:id', async (c) => {
    ...
    return c.json({
      ...result,
      ...(single ? { code: single.code, message: single.message } : {})
  ...
  app.post('/:id/read', async (c) => {
    ...
    const parsed = z.strictObject({ revision: z.number().int().nonnegative() })
    ...
    if (parsed.data.revision > conversationStores.summary(conversation.id).revision)
      return c.json({
  ...
  app.post('/:id/hosts', async (c) => {
    ...
    const parsed = attachBodySchema.safeParse(
      await c.req.json().catch(() => null)
    )
    if (!parsed.success)
      return c.json({
  ...
  app.patch('/:id/hosts/:deviceId', async (c) => {
    ...
    switch (await control.renameAttachedHost(
      conversation.id,
    ...
    try {
      const path = await withHost(conversation.id, async host => {
        await host.fs.writeFile(
          name,
          bytes,
          { cwd: host.defaultCwd, createParents: true }
        )
