...
/** Optional built SPA assets. API misses remain JSON errors; deep HTML navigation falls back to the entry page. */
export function webAssetRoutes(directory: string | undefined): Hono {
  const app = new Hono()
  if (!directory) return app
  ...
  app.get('*', async (c, next) => {
    ...
    if (file) return file
    if (!extname(c.req.path) && c.req.header('Accept')?.includes('text/html')) return index(c, next)
    return next()
  })
  return app
}
