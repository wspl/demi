...
/**
 * A workspace on one of the user's devices at a path, or a Cloud one: a host
 * provisioned for it, the path its home.
 */
...
export function workspaceRoutes(options: {
  ...
  app.get(
    '/',
    async (c) => c.json(
      { workspaces: await control.listWorkspaces(c.get('user').id) }
    )
  )
  ...
  app.post('/', async (c) => {
    ...
    if (!parsed.success) {
      return c.json({
        code: 'invalid_body',
        message: 'Expected { deviceId, path, name } or { cloud: true, name }'
      }, 400)
    }
    ...
    const workspace = await control.createWorkspace(
      { userId: c.get('user').id, ...parsed.data }
    )
  ...
  app.patch('/:id', async (c) => {
    ...
    const parsed = patchWorkspaceBodySchema.safeParse(
      await c.req.json().catch(() => null)
    )
    if (!parsed.success)
      return c.json({
  ...
  app.delete('/:id', async (c) => {
    ...
    if (inUse > 0) {
      return c.json({
        ...
        message: `${inUse} conversation(s) still target this workspace`
      }, 409)
    }
