...
export class ChangeStore {
  ...
  async fork(sourceId: string, destinationId: string, blocks: readonly Block[]): Promise<void> {
    ...
    for (const block of blocks) {
      ...
      // A view the shell contract does not describe is another tool's.
      const view = shellToolViewSchema.safeParse(block.view)
      if (!view.success || !view.data.files) {
        continue
      }
      const call = { commandId: view.data.commandId, files: view.data.files }
