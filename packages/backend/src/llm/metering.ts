...
export interface MeterOptions {
  ...
  /**
   * Enforcement at the inference entry: throw to refuse the request (e.g. rate
   * limit).
   */
...
export function meterRuntime(
  ...
  if (runtime.dispose)
    metered.dispose = () => runtime.dispose?.()
  return metered
}
    ...
    async *[Symbol.asyncIterator]() {
      for await (const event of run) {
        if (event.type === 'response')
          observe(event.usage, { modelId: request.modelId })
        yield event
      }
    },
  }
  if (run.steer)
    metered.steer = (input) => run.steer?.(input)
  return metered
}
