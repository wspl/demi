...
/**
 * Inspects target admission without waking Cloud or starting a provider
 * process.
 */
export async function executionAvailable(
  ...
  return device?.kind === 'managed'
    ? cloudConfigured
    : registry.deviceOnline(target.deviceId)
}
...
export function modelAvailability(
  ...
  if (provider.requiresProcessCapableHost && !execution)
    return {
      available: false,
      reason: 'execution',
      message: 'The selected execution environment is unavailable'
    }
