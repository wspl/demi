...
/**
 * Polls the running command with shell_status every `everyMs` until it exits,
 * then says `text`.
 */
function pollThenSay(
  driverId: string,
  text: string,
  everyMs = 150
): TurnScript {
  ...
  const poll: TurnScript = (request: InferenceRequest) => {
    if (!lastResult(request).includes('status: running'))
      return model.say(text)
...
describe.each<Target>(['cloud', 'runner:alpha'])(
  ...
  (target) => {
    test('a command polled to its end with shell_status', async () => {
      ...
      for (const middle of turn.received.slice(1, -1))
        expect(middle).toContain('status: running')
    ...
    test('stdin fed with shell_write reaches the command', async () => {
      ...
      const turn = await driver.turn({
        model: [
          ...
          (request) => model.tool('t2', 'shell_write', {
            commandId: handleIn(request),
            stdin: 'hello there\n'
          }),
  ...
  test('a background job outlives its command', async () => {
    ...
    const turn = await driver.turn({
      model: [
        model.shell(
          't1',
          '(sleep 0.5; echo done > bg.txt) > /dev/null 2>&1 & echo started'
        ),
        model.say('backgrounded')
      ],
    })
    expect(turn.received[0]).toContain('started')
    expect(turn.received[0]).toContain('status: exited')
    await waitFor(
      () => existsSync(driver.filePath('bg.txt')),
      undefined,
      { timeoutMs: 5_000 }
    )
    const next = await driver.turn({ model: [
        model.shell('t2', 'cat bg.txt'),
        model.say('seen')
      ] })
