    ...
    test('create, read, edit, list', async () => {
      ...
      const read = await driver.turn({
        model: [
          model.shell(
            't2',
            'pwd && demi file read notes.md | grep -n a | sort -r'
          ),
          model.say('read')
        ],
      })
      ...
      expect(edited.received[0])
        .toContain('Edited notes.md\nalpha\ndelta\ngamma')
      expect(await driver.readFile('src/notes.md'))
        .toBe('alpha\ndelta\ngamma\n')

      const listed = await driver.turn({
        model: [
          model.shell('t4', 'ls && demi host current'),
          model.say('listed')
        ],
      })
