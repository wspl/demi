...
rm removed.txt`), model.say('Created the files.')] })
      const first = retainedEdits(conversation.transcript().find(block => block.type === 'tool_call')?.view)
...
PATCH`), model.say('Patched both files.')] })
      ...
      const second = retainedEdits(calls.at(-1)?.view)
...
/** The stored shell view's command and the files it retained; a call that retained none is a test failure. */
function retainedEdits(view: unknown) {
  const parsed = shellToolViewSchema.parse(view)
  if (!parsed.files) {
    throw new Error(`Command ${parsed.commandId} retained no files`)
  }
  return { commandId: parsed.commandId, files: parsed.files }
}
