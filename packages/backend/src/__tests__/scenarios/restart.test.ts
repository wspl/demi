...
test(
  'R1 — backend restart, idle: the runner reconnects, the conversation resumes on it, the ledger carries over',
  async () => {
    const driver = await world.conversation('runner:alpha')
    const before = await driver.turn({ model: [
        model.shell('t1', 'echo -n kept > kept.txt && cat kept.txt'),
        model.say('remember me')
      ] })
    expect(before.received[0]).toContain('kept')
    const blocksBefore = driver.transcript().map((block) => block.id)
    const usageBefore = await world.api<{ totals: Array<{ requests: number }> }>(
      '/api/usage'
    )
    ...
    await driver.attach()
    await waitFor(
      () => driver.transcript().length === blocksBefore.length,
      () => `${driver.transcript().length} blocks`,
      { timeoutMs: 10_000 }
    )
    expect(driver.transcript().map((block) => block.id)).toEqual(blocksBefore)

    const after = await driver.turn({ model: [
        model.shell('t2', 'cat kept.txt && demi host current'),
        model.say('and again')
      ] })
    expect(after.received[0]).toContain('kept')
    expect(after.received[0]).toContain('on device "alpha"')
    const usageAfter = await world.api<{ totals: Array<{ requests: number }> }>(
      '/api/usage'
    )
    const sum = (usage: { totals: Array<{ requests: number }> }) => usage.totals.reduce(
      (total, group) => total + group.requests,
      0
    )
...
test(
  'R2 — backend restart mid-turn: the transcript has no dangling tool call; the next turn executes',
  async () => {
    const driver = await world.conversation('runner:alpha')
    await driver.turn({ model: [model.say('warm')] })
    const started = world.frames.length
    const { begin } = driver.startTurn({ model: [
        model.shell('t1', 'sleep 5; echo late', 10_000),
        model.say('never')
      ] })
    await waitFor(
      () => world.frames.slice(started)
        .some((frame) => frame.message.type === 'job_start'),
      undefined,
      { timeoutMs: 5_000 }
    )
    await delay(100)
    ...
    const next = await driver.turn({ model: [
        model.shell('t2', 'echo after restart'),
        model.say('recovered')
      ] })
...
test(
  'R3 — runner death mid-command is a tool error; the returned runner serves the next turn; files survive',
  async () => {
    const driver = await world.conversation('runner:alpha')
    await driver.turn({ model: [
        model.shell('t1', 'echo -n before > before.txt'),
        model.say('written')
      ] })

    const started = world.frames.length
    const { begin, done } = driver.startTurn({ model: [
        model.shell('t2', 'sleep 5; echo late', 10_000),
        model.say('the runner is gone')
      ] })
    await waitFor(
      () => world.frames.slice(started)
        .some((frame) => frame.message.type === 'job_start'),
    ...
    const back = await driver.turn({ model: [
        model.shell('t3', 'cat before.txt'),
        model.say('back')
      ] })
...
test(
  'R4 — cloud persistence: files, todos and the ledger survive a backend restart',
  async () => {
    ...
    const after = await driver.turn({ model: [
        model.shell('t2', 'cat notes.md && demi todo list'),
        model.say('found')
      ] })
