  ...
  (target) => {
    test('a turn survives its client', async () => {
      ...
      const { begin } = driver.startTurn({
        model: [
          model.shell('t1', 'echo -n survived > proof.txt && cat proof.txt'),
          model.slowSay('the complete slow answer arrived intact', 600)
        ],
      })
      await delay(150)
      await driver.detach()
      await driver.attach()
      await waitFor(
        () => driver.lastText() === 'the complete slow answer arrived intact',
        () => driver.lastText(),
        { timeoutMs: 10_000 }
      )
      ...
      const types = driver.transcript()
        .filter(
        block => !(block.type === 'user' &&
          block.preamble?.startsWith('[Execution context '))
      )
