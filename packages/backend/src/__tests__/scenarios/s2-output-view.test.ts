...
/**
 * Text past the runner's head+tail view and the model's preview, within the
 * cloud capture limit.
 */
...
describe.each<Target>(['cloud', 'runner:alpha'])(
  ...
  (target) => {
    test('a stream past the view budget', async () => {
      ...
      const status = turn.shell.find(
        (event) => event.status.status === 'exited'
      )?.status
    ...
    test('a binary final stream', async () => {
      ...
      expect(received).toContain(
        `<binary stdout: ${PNG_BYTES.length} bytes${expected(target).binaryPlaceholder}`
      )
      // The model does not accept png here, so nothing is attached and the note says where the bytes are.
      expect(received).toContain(
        'Binary stdout is image/png, which this model does not accept natively'
      )
    ...
    test('a non-zero exit with stderr', async () => {
      ...
      const turn = await driver.turn({ model: [
          model.shell('t1', 'cat nope.txt; echo after'),
          model.say('seen')
        ] })
      const received = turn.received[0]!
      expect(received).toContain('exitCode: 0')
      ...
      const failed = await driver.turn({ model: [
          model.shell('t2', 'cat nope.txt'),
          model.say('seen')
        ] })
