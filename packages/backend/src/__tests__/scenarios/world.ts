...
export interface WorldOptions {
  ...
  runners?: string[]
  /**
   * A fixed port, for the restart file: a runner must find the restarted
   * process.
   */
  port?: number
  /**
   * Reuse a data directory: the restart file reopens its world over the
   * previous one.
   */
  dataDir?: string
  /**
  ...
  publicUrl?: string
  /**
   * The backend's liveness ping; on by default it is what carries `pong.jobs`,
   * which the idle rule reads. Default 0 (off).
   */
  pingIntervalMs?: number
  /**
   * The provider-request rate limit; a scenario with many short turns raises
   * it.
   */
...
export interface Device {
  ...
  workspaceId: string
  /**
   * Jobs that were running when the runner was killed: they never report an
   * exit.
   */
  lost: number
}
...
/**
 * What a runner job's output frames may carry: the head of each stream; the
 * tail rides the exit frame.
 */
...
export class World {
  ...
  static async create(options: WorldOptions = {}): Promise<World> {
    ...
    for (const name of options.runners ?? [])
      await world.pair(name)
    return world
  }

  private static openBackend(
    dataDir: string,
  ...
  device(name: string): Device {
    ...
    if (!device)
      throw new Error(`no runner named ${name}`)
    return device
  }
  ...
  async pair(name: string): Promise<Device> {
    ...
    const { device } = await this.api<{ device: { id: string } }>(
      '/api/devices/claim',
      { code: runner.codes[0] }
    )
    await waitFor(
    ...
    const { workspace } = await this.api<{ workspace: { id: string } }>(
      '/api/workspaces',
      {
        deviceId: device.id,
        path: home,
  ...
  /**
   * Stops a runner's process; `returnRunner` starts it again over the same
   * state.
   */
  ...
  async returnRunner(name: string): Promise<void> {
    ...
    await waitFor(
      () => device.runner.statuses.includes('online'),
      () => device.runner.log.join('\n'),
      { timeoutMs: 15_000 }
    )
  }

  /**
   * Closes the backend without the invariants and reopens it over the same data
   * directory.
   */
  async restartBackend(): Promise<void> {
    if (this.options.port === undefined)
      throw new Error('restartBackend needs a world with a fixed port')
    for (const driver of this.drivers)
      await driver.detach()
    const seen = new Map([...this.devices.values()].map((device) => [
      device.name,
      device.runner.statuses.length
    ]))
    await this.backend.close()
    this.backend = await World.openBackend(
      this.dataDir,
      this.options,
      this.model,
      this.frames
    ...
    for (const device of this.devices.values()) {
      ...
      await waitFor(
        () => device.runner.statuses.slice(from).includes('online'),
        () => device.runner.log.join('\n'),
        { timeoutMs: 15_000 }
      )
    }
  ...
  async cloudFile(
    ...
    if (!(fake instanceof FakeProvisioner))
      throw new Error('Use runner assertions for real VM files')
    const guest = [...fake.guests.values()][0]
    if (!guest)
      return null
    return readFile(join(guest.homeDir, 'sessions', conversationId, relative), 'utf8')
      .catch(() => null)
  }
  ...
  private async checkInvariants(): Promise<void> {
    ...
    for (const driver of this.drivers) {
      ...
      expect(
        cold.blocks.map((block) => block.id),
        `cold transcript of ${driver.label}`
      )
        .toEqual(live)
    }
    // Runner sockets carried the view only; every job exited; every pipe end reported.
    for (const device of this.devices.values()) {
      const of = (direction: 'in' | 'out') => this.frames.filter(
        (f) => f.deviceId === device.deviceId && f.direction === direction
      )
        .map((f) => f.message)
      ...
      for (const message of of('in')) {
        if (message.type === 'job_output')
          jobBytes.set(
            message.jobId,
            (jobBytes.get(message.jobId) ?? 0) + message.bytes.byteLength
          )
      }
      for (const [jobId, bytes] of jobBytes)
        expect(bytes, `job ${jobId} output on ${device.name}'s socket`)
          .toBeLessThanOrEqual(JOB_FRAMES_BYTES)
      const count = (direction: 'in' | 'out', ...types: string[]) => of(direction)
        .filter((m) => types.includes(m.type)).length
      expect(
        count('in', 'job_exit') + device.lost,
        `jobs exited on ${device.name}`
      )
    ...
    expect(
      usage.totals.reduce((sum, group) => sum + group.requests, 0),
      'ledger rows'
    )
      .toBe(this.model.answered)
  }
}
