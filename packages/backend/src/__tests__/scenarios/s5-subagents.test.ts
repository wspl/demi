...
describe.each<Target>(['cloud', 'runner:alpha'])(
  ...
  (target) => {
    test(
      'an explore child works on the parent\'s files like any child; a default child writes',
      async () => {
        ...
        const lifecycle = driver.events.filter((event) => event.type === 'subagent')
...
test(
  'a cross-host command preserves the child node storage scope',
  async () => {
    ...
    const parent = await driver.turn({ model: [
    ...
    const child = world.model.requests.filter(
      request => request.sessionId !== driver.id && request.items.some(
        item => item.type === 'tool_result' &&
          item.toolUseId === 'scope-child-list'
      )
    )
      .at(-1)
