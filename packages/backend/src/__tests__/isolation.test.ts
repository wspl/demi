...
async function createUser(
  ...
  const response = await backend.session.fetch('/api/users', json({
    email,
    password: `${email.split('@')[0]}-pass-1`,
    role: 'user'
  }))
  if (response.status !== 201)
    throw new Error(`create ${email}: HTTP ${response.status}`)
...
test(
  "the matrix: another user's objects answer 404 on every route, to users and admins alike; revoke, then re-claim",
  async () => {
    ...
    const { device } = await must<{ device: { id: string } }>(
      await alice.fetch('/api/devices/claim', json({ code: runner.codes[0] })),
      201
    )
    const { workspace } = await must<{ workspace: { id: string } }>(
      await alice.fetch(
        '/api/workspaces',
        json({ deviceId: device.id, path: home, name: 'proj' })
      ),
      201
    )
    ...
    const { attachment } = await must<{ attachment: {
        id: string;
        sha256: string
      } }>(
    ...
    expect(
      await streamOpens(alice, `/api/conversations/${conversation.id}/stream`)
    )
      .toBe(true)
    ...
    const denied: Array<[string, string, unknown?]> = [
      ...
      [
        'PATCH',
        `/api/conversations/${conversation.id}/hosts/${device.id}`,
        { name: 'taken' }
      ],
      ...
      [
        'POST',
        '/api/workspaces',
        { deviceId: device.id, path: home, name: 'squat' }
      ],
    ]
    for (const actor of [bob, backend.session]) {
      for (const [method, path, body] of denied) {
        ...
        expect(response.status, `${actor.user.email} ${method} ${path}`)
          .toBe(404)
      }
      expect(
        await streamOpens(actor, `/api/conversations/${conversation.id}/stream`),
        `${actor.user.email} stream`
      )
        .toBe(false)
      const lists = await Promise.all([
        ...
        '/api/devices',
        '/api/workspaces'
      ].map(async (path) => Object.values(
        await must<Record<string, unknown[]>>(await actor.fetch(path), 200)
      )[0]))
      expect(lists.map((list) => list?.length ??
        0), `${actor.user.email} lists`)
        .toEqual(actor === bob ? [1, 0, 0] : [0, 0, 0])
    }
    // Alice still has everything.
    expect(
      (await must<{ hosts: unknown[] }>(
        await alice.fetch(`/api/conversations/${conversation.id}/hosts`),
        200
      )).hosts
    )
      .toHaveLength(1)
    expect(
      (await must<{ devices: unknown[] }>(
        await alice.fetch('/api/devices'),
        200
      )).devices
    )
      .toHaveLength(1)
    expect(
      new Uint8Array(
        await (await alice.fetch(`/api/blobs/${attachment.sha256}`)).arrayBuffer()
      )
    )
      .toEqual(PNG_BYTES)
    ...
    expect(
      new Uint8Array(
        await (await bob.fetch(`/api/blobs/${attachment.sha256}`)).arrayBuffer()
      )
    )
      .toEqual(PNG_BYTES)
    ...
    await must(
      await alice.fetch(`/api/workspaces/${workspace.id}`, { method: 'DELETE' }),
      204
    )
    await must(
      await alice.fetch(`/api/devices/${device.id}`, { method: 'DELETE' }),
      204
    )
    expect(
      (await must<{ hosts: unknown[] }>(
      ...
      .toHaveLength(0)
    await waitFor(
      () => runner.statuses.includes('rejected'),
      undefined,
      { timeoutMs: 5_000 }
    )
    await runner.stop()
    ...
    await waitFor(
      () => again.codes.length > 0,
      () => again.log.join('\n'),
      { timeoutMs: 10_000 }
    )
    expect(
      (await alice.fetch(`/api/devices/${device.id}`, { method: 'DELETE' })).status
    )
      .toBe(404)
    const reclaimed = await must<{ device: { id: string } }>(
      await bob.fetch('/api/devices/claim', json({ code: again.codes[0] })),
      201
    )
    expect(reclaimed.device.id).not.toBe(device.id)
    expect(
      (await must<{ devices: Array<{ id: string }> }>(
        await bob.fetch('/api/devices'),
        200
      )).devices.map((entry) => entry.id)
    )
      .toEqual([reclaimed.device.id])
    expect(
      (await must<{ devices: unknown[] }>(
        await alice.fetch('/api/devices'),
        200
    ...
    expect(
      (await alice.fetch(
        `/api/devices/${reclaimed.device.id}`,
        { method: 'DELETE' }
      )).status
    )
      .toBe(404)
