...
test(
  'pairing: claim, reconnect with the device token, revoke refuses the reconnect',
  async () => {
    ...
    const { device } = (await claimed.json()) as { device: {
        id: string;
        name: string;
        online: boolean
      } }
    ...
    expect(listed.devices)
      .toEqual([expect.objectContaining({ id: device.id, online: true })])
    ...
    await api(
      backend,
      `/api/devices/${device.id}/fs?path=${encodeURIComponent(runnerDir)}`
    )
    ).json()) as {
      path: string;
      home: string;
    ...
    for (let tries = 0; tries < 100 && !offline; tries += 1) {
      const rows = (await (await api(backend, '/api/devices')).json()) as {
    ...
    await waitFor(
      () => restarted.statuses.includes('online'),
      undefined,
      { timeoutMs: 5_000 }
    )
    ...
    expect(restarted.details.some((detail) => detail.includes('revoked')))
      .toBe(true)
...
test(
  'claim codes expire and rotate on the waiting socket; the stale code is dead',
  async () => {
...
test(
  'a malformed runner frame closes the socket; a bad device token is rejected',
  async () => {
    ...
    await new Promise<void>(
      (resolve) => garbage.addEventListener(
        'open',
        () => resolve(),
        { once: true }
      )
    ...
    await new Promise<void>(
      (resolve) => badToken.addEventListener(
        'open',
        () => resolve(),
        { once: true }
      )
    ...
    badToken.send(
      wire.encode({
        ...
        runner: {
          ...
          platform: 'test',
          version: '0',
          identity: { uid: 0, gid: 0, hostname: 'x', homeDir: '/' }
        },
      }),
    )
    expect(wire.decodeBackendToRunner(new Uint8Array(await reply))).toMatchObject(
      {
        type: 'hello_error',
        code: 'unknown_device',
...
test(
  'M4 acceptance: a session executes on the claimed device; disconnect is a tool error; reconnect resumes',
  async () => {
    ...
    new StubProvider([
      ...
      [events.toolCall(
          't3',
          'shell_exec',
          { script: 'cat made.txt', timeoutMs: 10_000 }
        )],
      ...
      [events.toolCall('t4', 'shell_exec', {
          script: 'demi todo add "run the suite" && demi todo list && demi todo list --json',
          timeoutMs: 10_000
        })],
    ...
    const { provider: stubProviderId } = (await providerResponse.json()) as {
      provider: { id: string }
    }
    ...
    expect(
      shellEvents.filter((event) => event.status.status === 'exited').at(-1)?.status.stdout.delta
    )
      .toBe('hello')
    ...
    await waitFor(
      () => existsSync(join(runnerDir, 'started.marker')),
      undefined,
      { timeoutMs: 10_000 }
    )
    await runner.stop()
    ...
    await waitFor(
      () => revived.statuses.includes('online'),
      undefined,
      { timeoutMs: 5_000 }
    )
...
test(
  'a device token holds one live connection: the newcomer is refused and retries once the first is gone',
  async () => {
    ...
    await waitFor(
      () => refusals.some((line) => line.includes('already_connected')),
      undefined,
      { timeoutMs: 5_000 }
    )
...
async function deviceOnline(
  ...
  const { devices } = (await (await api(backend, '/api/devices')).json()) as { devices: Array<{
      id: string;
      online: boolean
    }> }
