...
chain(
  'claude-code on a runner: vault token in the CLI env, native wire to a mock upstream',
  async () => {
    ...
    const upstream = Bun.serve({
      ...
      fetch: async (request) => {
        ...
        return Response.json({ error: {
            ...
            message: `mock upstream: no ${request.method} ${path}`
          } }, { status: 404 })
      },
    })
