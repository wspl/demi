...
test(
  'malformed content and provider frames fail before mutation; later frames still arrive',
  async () => {
    ...
    for (const frame of [
      ...
      { type: 'send', messageId: 'bad', content: [null] },
      {
        ...
        content: [{
            type: 'text',
            text: 'must not become the title'
          }]
      },
    ])
    ...
    expect(
      f.replies.every(
        (frame) => frame.type === 'error' && frame.code === 'invalid_frame'
      )
    )
      .toBe(true)
    expect((await f.control.getConversation(f.conversation.id))?.title)
      .toBe('New conversation')
  }
)

test(
  'open and set_provider record only a provider the user may name',
  async () => {
    const f = await fixture()
    ...
    expect(
      f.replies.every(
        (frame) => frame.type === 'error' && frame.code === 'provider_not_found'
      )
    )
      .toBe(true)
    expect(await f.control.getConversation(f.conversation.id))
      .toMatchObject({ providerId: 'mine', modelId: 'm' })
  }
)
...
test(
  ...
  async () => {
    ...
    const f = await fixture(undefined, (control) => new Proxy(control, {
      get(target, key) {
        if (key === 'defaultConversationTitle')
...
test(
  'an outbound blob write failure is reported without dropping later frames',
  async () => {
    const f = await fixture({
      put: async () => {
    ...
    expect(f.replies[0])
      .toMatchObject({ type: 'error', code: 'frame_send_failed' })
...
test(
  'frame admission stays held until asynchronous handling finishes, including failure',
  async () => {
    const f = await fixture()
