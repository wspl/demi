...
test('M6 acceptance: virtual→real switch with context block and migration pipe, then real→virtual and release', async () => {
  ...
  const stubRuntime = () =>
    new StubProvider([
      ...
      [events.toolCall('t1', 'shell_exec', { script: 'printf secret > notes.txt && demi host', timeoutMs: 10_000 })],
      [events.text('one'), events.response()],
      // Turn 2 (after switch to the device workspace): pull the virtual tree over the prev pipe.
      [events.toolCall('t2', 'shell_exec', { script: 'demi host prev shell -- tar c -C /workspace . | tar x && cat notes.txt && demi host', timeoutMs: 20_000 })],
