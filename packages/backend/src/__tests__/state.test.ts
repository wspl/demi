...
test(
  'state revalidation changes after settings or conversation mutations and excludes secrets',
  async () => {
    const backend = await openBackend({
      dataDir: await mkdtemp(join(tmpdir(), 'demi-state-')),
      port: 0
    })
    try {
      ...
      await backend.session.fetch(
        ...
        json({ appearance: { theme: 'dark' } }, 'PATCH')
      )
      const updated = await backend.session.fetch(
        '/api/state',
        { headers: { 'If-None-Match': etag } }
      )
      ...
      await backend.session.fetch('/api/providers', json({
        providerType: 'openai',
        label: 'API',
...
test(
  'optional SPA hosting serves assets and deep navigation while preserving API errors',
  async () => {
    ...
    await writeFile(
      ...
      '<html>fixture page</html>'
    )
    await writeFile(
      join(webDirectory, 'main.js'),
    ...
    try {
      expect(
        await (await fetch(
          `${backend.url}/conversation/example`,
          { headers: { Accept: 'text/html' } }
        )).text()
      )
        .toContain('fixture page')
      expect(await (await fetch(`${backend.url}/main.js`)).text())
