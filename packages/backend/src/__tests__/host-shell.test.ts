...
function lastExited(
  shellEvents: Extract<ClientSessionEvent, { type: 'shell_output' }>[]
) {
  const status = shellEvents.filter((event) => event.status.status === 'exited')
    .at(-1)?.status
...
async function pairDevice(backend: TestBackend, name: string) {
  ...
  const created = await json(backend, '/api/workspaces', {
    deviceId: device.id,
    path: home,
    name: `${name} workspace`
  })
...
test(
  'host shell --host: the job runs on the named host with the caller\'s pipes attached both ways, nothing bulk crosses the sockets; the directory carries',
  async () => {
          ...
          defineProvider({
            ...
            new StubProvider([
              [events.toolCall(
                  't1',
                  'shell_exec',
                  { script: scripts[0]!, timeoutMs: 30_000 }
                )],
              [events.text('one'), events.response()],
              [events.toolCall(
                  't2',
                  'shell_exec',
                  { script: scripts[1]!, timeoutMs: 30_000 }
                )],
              [events.text('two'), events.response()],
              [events.toolCall(
                  't3',
                  'shell_exec',
                  { script: scripts[2]!, timeoutMs: 10_000 }
                )],
              [events.text('three'), events.response()],
              [events.toolCall(
                  't4',
                  'shell_exec',
                  { script: scripts[3]!, timeoutMs: 30_000 }
                )],
              [events.text('four'), events.response()],
              [events.toolCall(
                  't5',
                  'shell_exec',
                  { script: scripts[4]!, timeoutMs: 30_000 }
                )],
    ...
    const provider = (await (await json(backend, '/api/providers', {
      providerType: 'stub',
      label: 'Stub',
      apiKey: 'k'
    ...
    expect(
      (await json(
        ...
        { target: { kind: 'workspace', workspaceId: a.workspaceId } },
        'PATCH'
      )).status
    )
      .toBe(200)
    expect(
      (await json(
        backend,
        `/api/conversations/${conversation.id}`,
        { target: { kind: 'workspace', workspaceId: b.workspaceId } },
        'PATCH'
      )).status
    )
      .toBe(200)
    ...
    const { client, shellEvents } = await openClient(
      backend,
      conversation.id,
      selection
    )
    ...
    expect(copied?.stdout.delta)
      .toContain(`beta  ${b.deviceId}  online  ${b.home}  (main)`)
    expect(copied?.stdout.delta)
      .toContain(`alpha  ${a.deviceId}  online  ${a.home}  (attached)`)
    ...
    const audit = (from: number) => {
      ...
      const of = (deviceId: string, direction: 'in' | 'out') => turn.filter(
        ...
        .map((f) => f.message)
      const types = (deviceId: string, direction: 'in' | 'out') => new Set(
        of(deviceId, direction).map((m) => m.type)
      )
      return { of, types }
    }
    {
      ...
      expect(of(b.deviceId, 'out').some((m) => m.type === 'rpc_output'))
        .toBe(false)
      expect(
        of(a.deviceId, 'in').reduce(
          (total, m) => total + (m.type === 'job_output'
            ? m.bytes.byteLength
            : 0),
          0
        )
      ...
      expect(
        of(a.deviceId, 'in')
          .filter((m) => m.type === 'pipe_done')
          .every((m) => m.type === 'pipe_done' && m.ok)
      )
        .toBe(true)
    }
    ...
    expect(
      readFileSync(join(a.home, 'push.bin'))
        .equals(payload.subarray(0, 250_000))
    )
      .toBe(true)
    // `tar x` prints nothing, so alpha sends no view frames; its stdin end reports once the body is in.
    await waitFor(
      () => frames.slice(beforePush).some(
        (f) => f.deviceId === a.deviceId && f.message.type === 'pipe_done'
      )
    )
    {
      ...
      expect(pipes?.type === 'rpc_pipes' &&
        pipes.stdin !== undefined).toBe(true)
      expect(
        of(b.deviceId, 'in')
          .filter((m) => m.type === 'rpc_call')
          .every((m) => m.type === 'rpc_call' && m.stdin)
      )
        .toBe(true)
      const started = of(a.deviceId, 'out').find((m) => m.type === 'job_start')
      expect(
        started?.type === 'job_start' &&
          started.stdin?.id === (pipes?.type === 'rpc_pipes'
            ? pipes.stdin?.id
            : undefined)
      )
        .toBe(true)
    ...
    expect(peeked?.stdout.delta.replace(/\s+/g, ' ').trim()).toStartWith(
      `00 1f 3e 5d 7c ${payload.length} notes.bin host: machine "Cloud"`
    )
...
test(
  'host shell streams stderr before stdout ends, forwards shell_write, and cancels the far job from device and cloud callers',
  async () => {
    ...
    const backend = await openBackend({
      ...
      runner: {
        pingIntervalMs: 0,
        trace: (deviceId, direction, message) => {
          frames.push({ deviceId, direction, message })
        }
      },
      providerTypes: { stub: {
          credential: 'api_key',
      ...
      for (const caller of [
        ...
        expect(
          (await json(
            ...
            `/api/conversations/${conversation.id}`,
            { target: onDevice
                ? { kind: 'workspace', workspaceId: b.workspaceId }
                : { kind: 'cloud' } },
            'PATCH'
          )).status
        )
        ...
        const { client, shellEvents } = await openClient(
          backend,
          conversation.id,
          selectionFor(provider.provider.id)
        )
        const start = frames.length
        try {
          await client.send([{
              type: 'text',
              text: 'start the interactive command'
            }])
          const running = shellEvents.find(
            (event) => event.status.status === 'running'
          )!.status
          ...
          const targetJob = since().find(
            (frame) => frame.deviceId === a.deviceId &&
              frame.message.type === 'job_start'
          )!.message
