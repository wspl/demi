...
test(
  'session provider clones retain independent state and steering while refreshing edited credentials',
  async () => {
              ...
              return {
                run: () => createProviderRun((async function* () {
                  ...
                  yield events.response()
                    ...
                    steers.push({ runtimeId, text: input.id })
                  } }),
    ...
    const resolve = createSessionProviderResolver({
      ...
      processHost: async () => {
        throw new Error('HTTP providers must not resolve a machine')
      },
    })
    ...
    try {
      ...
      expect(steers).toEqual([
        { runtimeId: 1, text: 'original' },
        { runtimeId: 2, text: 'clone' }
      ])
      await runtime.dispose?.()
      expect(await read(clone)).toBe('old:3')
      expect(disposed).toEqual([1])
      await vault.update(entry.id, { config: {
          kind: 'api_key',
          providerType: 'stateful',
          apiKey: 'new'
        } })
      ...
      expect(await read(clone)).toBe('new:1')
      expect(selections.at(-1)?.model).toMatchObject({
        model: { id: 'next-model' },
...
test(
  'a delayed old vault read cannot poison the cache after a provider edit',
  async () => {
    const db = openSqliteDatabase(':memory:')
    migrate(db, CONTROL_MIGRATIONS)
    const control = new LocalControlService(db)
    ...
    try {
      ...
      expect((await assembly.providerFor(entry.id))?.provider)
        .toBe(current?.provider)
    } finally {
      release.resolve()
      db.close()
    }
