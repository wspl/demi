...
test(
  'settings patches merge fields across devices, survive restart, and remain private to the user',
  async () => {
    ...
    try {
      ...
      expect(
        (await device.fetch(path, json({ appearance: { fontSize: 100 } }))).status
      )
        .toBe(400)
      expect(
      ...
      await backend.session.fetch('/api/users', {
        ...json({
      ...
      expect(await (await other.fetch(path)).json())
        .toEqual({ preferences: { appearance: {}, shortcuts: {} } })
      expect(
