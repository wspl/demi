...
test(
  ...
  async () => {
    ...
    expect(master.cookie)
      .toMatch(new RegExp(`^${SESSION_COOKIE}=[A-Za-z0-9_-]{40,}$`))
    expect(await (await fetch(`${backend.url}/api/setup`)).json())
      .toEqual({ needed: false })
    const again = await fetch(`${backend.url}/api/setup`, json({
...
test(
  'login, lockout after five failures, logout ends the session',
  async () => {
    ...
    const wrong = await fetch(
      ...
      json({ email: MASTER.email, password: 'nope-nope' })
    )
...
test(
  'the limiter forgets a name after the lock window, so sprayed emails do not accumulate',
  () => {
    let now = 1_000_000
    const limiter = new LoginLimiter(
      { lockAfter: 3, lockMs: 1_000, now: () => now }
    )
...
test(
  'a user changes their own password with the current one in hand',
  async () => {
    ...
    expect(
      (await me.fetch(
        '/api/auth/password',
        json({ current: 'wrong-wrong', next: 'second-pass-2' }, 'PUT')
      )).status
    )
      .toBe(401)
    expect(
      (await me.fetch(
        '/api/auth/password',
        json({ current: MASTER.password, next: 'short' }, 'PUT')
      )).status
    )
      .toBe(400)
    expect((await me.fetch('/api/auth/password', json({
      current: MASTER.password,
      next: 'second-pass-2'
    }, 'PUT'))).status).toBe(204)

    expect((await fetch(`${backend.url}/api/auth/login`, json(MASTER))).status)
      .toBe(401)
...
test(
  'the session slides: a request near the end renews it, silence past the end expires it',
  async () => {
    ...
    const backend = await openBackend({
      dataDir,
      port: 0,
      auth: { ttlMs: 1000, renewBelowMs: 500, now: () => now }
    })
    const me = backend.session
...
test(
  'email identity is validated and case insensitive; nickname changes persist separately',
  async () => {
    ...
    try {
      ...
      expect(
        (await fetch(
          `${backend.url}/api/auth/login`,
          json({ email: 'invalid', password: MASTER.password })
        )).status
      )
        .toBe(400)
      const updated = await signedIn.fetch(
        '/api/auth/me',
        json({ nickname: '  New name  ' }, 'PATCH')
      )
      expect(await updated.json())
        .toMatchObject({ user: { email: MASTER.email, nickname: 'New name' } })
      expect(await (await backend.session.fetch('/api/auth/me')).json())
        .toMatchObject({ user: { nickname: 'New name' } })
      expect(
        (await signedIn.fetch(
          '/api/auth/me',
          json({ nickname: ' ', role: 'admin' }, 'PATCH')
        )).status
      )
        .toBe(400)
      expect((await signedIn.fetch('/api/auth/email', json({
...
test(
  'email changes require a delivered, unexpired, single-use code and survive restart',
  async () => {
    ...
    const options = {
      ...
      port: 0,
      auth: { now: () => now },
      accountMail: { sendVerification: async (message: typeof delivered[number]) => {
    ...
    try {
      const me = backend.session
      expect((await me.fetch('/api/auth/email', json({
        email: 'next@example.test',
        password: 'wrong-password'
      }))).status).toBe(401)
      const started = await me.fetch('/api/auth/email', json({
        email: 'NEXT@example.test',
        password: MASTER.password
      }))
      expect(started.status).toBe(202)
      const { challenge } = await started.json() as { challenge: {
          id: string;
          email: string
        } }
      ...
      expect(
        (await me.fetch(
          ...
          json({ id: challenge.id, code: 'abcdef' })
        )).status
      )
        .toBe(400)
      await backend.close()
      backend = await openBackend(options)
      const confirmed = await backend.session.fetch(
        '/api/auth/email/confirm',
        json({ id: challenge.id, code: delivered[0]!.code })
      )
      expect(confirmed.status).toBe(200)
      expect(await confirmed.json())
        .toMatchObject({ user: { email: 'next@example.test' } })
      expect(
        ...
        .toBe(400)
      expect(
        (await fetch(`${backend.url}/api/auth/login`, json(MASTER))).status
      )
        .toBe(401)
      expect(
        (await login(backend, 'next@example.test', MASTER.password)).user.id
      )
        .toBe(me.user.id)
      now += 60_000
      const expired = await backend.session.fetch('/api/auth/email', json({
        email: 'later@example.test',
        password: MASTER.password
      }))
      const next = await expired.json() as { challenge: { id: string } }
      now += 10 * 60_000
      expect((await backend.session.fetch('/api/auth/email/confirm', json({
...
test(
  'email delivery failure allows retry; five wrong codes and password changes invalidate challenges',
  async () => {
    ...
    try {
      const start = () => backend.session.fetch('/api/auth/email', json({
      ...
      expect(
        (await backend.session.fetch(
          '/api/auth/email/confirm',
          json({ id: challenge.id, code })
        )).status
      )
        .toBe(400)
      now += 60_000
      const next = await (await start()).json() as { challenge: { id: string } }
      await backend.session.fetch('/api/auth/password', json({
        current: MASTER.password,
        next: 'new-password-2'
      }, 'PUT'))
      expect(
        (await backend.session.fetch(
          '/api/auth/email/confirm',
          json({ id: next.challenge.id, code })
        )).status
      )
        .toBe(400)
    } finally {
