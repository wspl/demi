...
test(
  'instance secret: generated once with 0600, stable across loads, corrupt file is loud',
  () => {
    ...
    expect(Buffer.from(second).toString('hex'))
    ...
    expect(() => loadOrCreateInstanceSecret(corruptDir))
      .toThrow('Corrupt instance secret')
  }
)

test(
  'crypto: round trip, unique ciphertexts, wrong key and tampering fail loudly',
  () => {
    ...
    expect(
      () => decryptJson(
        secret,
        `${v}:${iv}:${tag}:${flipped.toString('base64')}`
      )
    ).toThrow()
  }
)
...
test(
  ...
  async () => {
    ...
    expect(created.config.kind === 'api_key' && created.config.apiKey)
      .toBe('sk-secret')
    ...
    }>('SELECT config, provider_type FROM providers WHERE id = ?', [created.id])
    ...
    expect(
      (await vault.list({ ownerUserId: user.id })).map(
...
test(
  'subscription uniqueness belongs to the scope and family; API-key entries remain repeatable',
  async () => {
    const db = openSqliteDatabase(':memory:')
    migrate(db, CONTROL_MIGRATIONS)
    const control = new LocalControlService(db)
      ...
      for (const ownerUserId of [null, user.id]) {
        const options = {
          ...
          config: {
            ...
            providerType: 'mock-subscription'
          }
        }
