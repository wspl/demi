...
test('a process provider reuses its process on one target and replaces it after a workspace switch', async () => {
  ...
  try {
    ...
    expect((await api(backend, path, patch({ target: { kind: 'workspace', workspaceId: workspaces[0]!.id } }))).status).toBe(200)
    ...
    expect((await api(backend, path, patch({ target: { kind: 'workspace', workspaceId: workspaces[1]!.id } }))).status).toBe(200)
