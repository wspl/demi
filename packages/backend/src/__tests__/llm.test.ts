...
function stubBackendOptions(
  dataDir: string,
  turns: number
): Parameters<typeof openBackend>[0] {
  return {
    dataDir,
    port: 0,
    providerTypes: {
...
test(
  'providers: create/list redact key material, unknown types rejected, delete unresolves',
  async () => {
    ...
    const unknown = await api(
      backend,
      '/api/providers',
      post({ providerType: 'nope', label: 'x', apiKey: 'k' })
    )
    ...
    const created = await api<{ provider: Record<string, unknown> }>(
      ...
      post({
        ...
        models: [
          manualModel('custom-1'),
          manualModel('custom-2')
        ]
      }),
    )
    ...
    const listed = await api<{ providers: Array<Record<string, unknown>> }>(
      backend,
      '/api/providers'
    )
    ...
    const models = await api<{ providers: Array<{
        providerId: string;
        models: Array<{
          id: string;
          providerId: string
    ...
    expect(models.body.providers[0]?.models.map((model) => model.id))
    ...
    expect(
      (await api<{ providers: unknown[] }>(backend, '/api/providers')).body.providers
    )
      .toHaveLength(0)
    expect(
      (await api<{ providers: unknown[] }>(backend, '/api/models')).body.providers
    )
      .toHaveLength(0)
...
test(
  'metering: every provider request lands in the ledger; /api/usage aggregates',
  async () => {
...
test(
  'enforcement: the provider request rate limit refuses at the inference entry',
  async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'demi-m5-limit-'))
    const backend = await openBackend({
      ...stubBackendOptions(dataDir, 5),
      usage: { providerRequestsPerMinute: 1 }
    })
    const created = await api<{ provider: { id: string } }>(
      backend,
      '/api/providers',
...
test(
  'subscription login: concurrent completions publish one provider with its vault pool',
  async () => {
    ...
    const competing = await api<{ login: { id: string } }>(
      backend,
      '/api/providers/subscription-login',
      post({ providerType: 'stub-sub', label: 'Competing' })
    )
    expect(competing.status).toBe(202)

    await waitFor(() => false, undefined, { timeoutMs: 30, intervalMs: 10 })
      .catch(() => {})
    const pending = await api<{ login: {
        status: string;
        verificationUrl: string;
        userCode: string
      } }>(backend, `/api/providers/subscription-login/${loginId}`,)
    ...
    await waitFor(() => {
      void Promise.all(
        [loginId, competing.body.login.id].map(
          (id) => api<{ login: (typeof states)[number] }>(backend,
          `/api/providers/subscription-login/${id}`
          )
        )
      ).then(
        (polled) => {
      ...
      return states.length === 2 &&
        states.every((state) => state.status !== 'pending')
    }, undefined, { timeoutMs: 5_000 })

    expect(states.filter((state) => state.status === 'completed'))
      .toHaveLength(1)
    expect(states.find((state) => state.status === 'failed')?.message)
      .toContain('already has a stub-sub subscription')
    ...
    const catalog = await api<{ subscriptions: Array<{
        providerType: string;
        configured: boolean
      }> }>(backend, '/api/providers/catalog')
    expect(catalog.body.subscriptions)
      .toContainEqual({ providerType: 'stub-sub', configured: true })
    const again = await api<{ code: string }>(
      backend,
      '/api/providers/subscription-login',
      post({ providerType: 'stub-sub' })
    )
    expect(again.status).toBe(409)
    expect(again.body.code).toBe('provider_exists')
    // A subscription entry takes a new label and nothing else.
    const relabel = await api<{ provider: { label: string } }>(
      backend,
      `/api/providers/${providerId}`,
      patch({ label: 'Work' })
    )
...
test(
  'the vendor catalog: entries from models.dev vendors carry the family and a live model list; custom endpoints name theirs',
  async () => {
    ...
    const catalog = await api<{ vendors: Array<Record<string, unknown>> }>(
      backend,
    ...
    expect((await api<{ code: string }>(backend, '/api/providers', post({
      vendorId: 'amazon-bedrock',
      label: 'x',
      apiKey: 'k'
    }))).body.code).toBe('unknown_vendor')
    ...
    const providerId = created.body.provider.id as string
    type Catalog = { providers: Array<{
        providerId: string;
        models: Array<Record<string, unknown>>
      }> }
    const live = (await api<Catalog>(backend, '/api/models')).body.providers.find(
      (p) => p.providerId === providerId
    )!
    expect(live.models.map((m) => m.id))
    ...
    const typed = await api<{ provider: Record<string, unknown> }>(
      backend,
      `/api/providers/${providerId}`,
      patch({
        label: 'DS',
        models: [manualModel('deepseek-v4')],
        baseUrl: 'https://proxy.example/v1',
        apiKey: 'sk-2'
      })
    )
    expect(typed.body.provider).toEqual(expect.objectContaining({
      label: 'DS',
      models: [manualModel('deepseek-v4')],
      baseUrl: 'https://proxy.example/v1',
      vendorId: 'deepseek'
    ...
    const typedCatalog = (await api<Catalog>(backend, '/api/models')).body.providers.find(
      (p) => p.providerId === providerId
    )!
    expect(typedCatalog.models.map((m) => m.id)).toEqual(['deepseek-v4'])
    await api(backend, `/api/providers/${providerId}`, patch({ models: null }))
    const liveAgain = (await api<Catalog>(backend, '/api/models')).body.providers.find(
      (p) => p.providerId === providerId
    )!
    expect(liveAgain.models.map((m) => m.id))
      .toEqual(['deepseek-v4', 'deepseek-v4-flash'])
    ...
    const custom = await api<{ provider: Record<string, unknown> }>(
      ...
      post({
        ...
        models: [manualModel('internal-70b')]
      }),
    )
    expect(custom.status).toBe(201)
    expect(custom.body.provider).toEqual(expect.objectContaining({
      providerType: 'openai',
      wireApi: 'chat-completions',
      vendorId: null,
      models: [manualModel('internal-70b')]
    ...
    expect((await api<{ code: string }>(backend, '/api/providers', post({
      ...
      label: 'x',
      apiKey: 'k'
    }))).body.code).toBe('subscription_only')
...
test(
  'a process-capable provider gets a session-scoped instance carrying the target spawn',
  async () => {
    ...
    const claimed = await api<{ device: { id: string } }>(
      backend,
      '/api/devices/claim',
      post({ code: codes[0] })
    )
    ...
    await control.setConversationWorkspace(
    ...
    const socket = backend.session.socket(
      `/api/conversations/${conversation.body.conversation.id}/stream`
    )
...
test(
  'provider edits apply after the active request, unchanged requests retain state, and deletion blocks inference',
  async () => {
              ...
              const makeRuntime = (cursor = 0): AgentProvider => ({
                async *run() {
                  if (config.kind !== 'api_key')
                    throw new Error('Expected API-key fixture')
    ...
    try {
      ...
      expect(calls).toEqual([
        { key: 'old-key', request: 1 },
        { key: 'new-key', request: 1 },
        { key: 'new-key', request: 2 }
      ])
      ...
      await client.send([{ type: 'text', text: 'after deletion' }])
        .catch(() => {})
      expect(errors.some((message) => message.includes('no longer available')))
        .toBe(true)
      expect(calls).toHaveLength(3)
      const usage = await api<{ totals: Array<{ requests: number }> }>(
        backend,
        '/api/usage'
      )
...
test(
  'a DeepSeek vendor tool continuation replays reasoning to the compatible endpoint',
  async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'demi-deepseek-replay-'))
    const requests: Array<{ messages: Array<{
        role: string;
        ...
        if (requests.length > 1) {
          ...
          if (assistant?.reasoning_content !== 'Read the current directory.') {
            return Response.json(
              { error: { message: 'Missing reasoning_content in assistant tool-call message' } },
              { status: 400 }
            )
          }
        }
        const delta = requests.length === 1
          ? {
            ...
            tool_calls: [{
                ...
                function: {
                  name: 'shell_exec',
                  arguments: JSON.stringify({ script: 'pwd', timeoutMs: 1000 })
                }
              }]
          }
          : { content: 'done' }
    ...
    try {
      ...
      expect(
        requests[1]?.messages.find((message) => message.tool_calls?.length)?.reasoning_content
      )
        .toBe('Read the current directory.')
    } finally {
      await client?.close()
      await backend.close()
      upstream.stop(true)
