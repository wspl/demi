...
test(
  'accounts: master creates admins and users, admins create users only, nobody outranks the master',
  async () => {
    ...
    expect((await createUser(master, 'bob@example.test', 'user')).body.code)
      .toBe('email_taken')
    expect((await master.fetch('/api/users', json({
    ...
    const listed = (await (await alice.fetch('/api/users')).json()) as { users: Array<{
        email: string;
        role: string
      }> }
    expect(listed.users.map((entry) => `${entry.email}:${entry.role}`)).toEqual(
      [
        'master@example.test:master',
        'alice@example.test:admin',
        'bob@example.test:user'
      ]
    )
    expect((await bob.fetch('/api/users')).status).toBe(403)
    expect((await createUser(bob, 'carol@example.test', 'user')).status)
      .toBe(403)

    // An admin creates users, not admins.
    expect((await createUser(alice, 'carol@example.test', 'user')).status)
      .toBe(201)
    expect((await createUser(alice, 'dave@example.test', 'admin')).body.code)
      .toBe('forbidden')
    ...
    expect(
      (await alice.fetch(
        `/api/users/${bobId}`,
        json({ password: 'bob-pass-2' }, 'PATCH')
      )).status
    )
      .toBe(204)
    expect((await login(backend, 'bob@example.test', 'bob-pass-2')).user.id)
      .toBe(bobId)
    expect(
      (await alice.fetch(
        `/api/users/${admin.body.user!.id}`,
        json({ password: 'alice-pass-2' }, 'PATCH')
      )).status
    )
      .toBe(403)
    expect(
      (await alice.fetch(
        `/api/users/${master.user.id}`,
        json({ password: 'master-pass-2' }, 'PATCH')
      )).status
    )
      .toBe(403)
    expect(
      ...
      .toBe(204)
    expect(
      (await login(backend, 'alice@example.test', 'alice-pass-2')).user.role
    )
      .toBe('admin')
    expect(
      (await master.fetch(
        '/api/users/nobody',
        json({ password: 'whatever-1' }, 'PATCH')
      )).status
    )
      .toBe(404)
...
test('the instance mode is read back as configured', async () => {
  for (const mode of ['shared', 'isolated'] as const) {
    ...
    expect(await (await backend.session.fetch('/api/settings')).json())
      .toEqual({ mode })
