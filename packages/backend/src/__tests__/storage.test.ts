...
test('conversation migrations create the data-plane tables', () => {
  ...
  const tables = db
    .all<{ name: string }>("SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name")
    .map((row) => row.name)
  for (const table of ['nodes', 'blocks', 'host_store'])
    expect(tables).toContain(table)
  db.close()
})

test(
  'DbHostStore round-trips portable JSON and lists by literal prefix',
  async () => {
    const db = openSqliteDatabase(':memory:')
    migrate(db, CONVERSATION_MIGRATIONS)
...
test('ControlService conversation CRUD and ordering', async () => {
  ...
  expect(
    await control.createMaster(
      { email: 'again@example.test', passwordHash: '!' }
    )
  ).toBeNull()
  ...
  expect((await control.listConversations(user.id)).map((record) => record.id))
    .toEqual([second.id])
  expect(
    (await control.listConversations(user.id, { archived: true })).map(
      (record) => record.id
    )
  )
    .toEqual([first.id])
...
test('ControlService device and workspace records', async () => {
  ...
  const other = await control.createDevice({
    userId: user.id,
    name: 'desktop',
    platform: 'linux',
    tokenHash: 'hash-2'
  })
  expect((await control.listDevices(user.id)).map((row) => row.id))
    .toEqual([device.id, other.id])
  ...
  expect((await control.getConversation(conversation.id))?.target)
    .toEqual({ kind: 'workspace', workspaceId: workspace.id })
...
test(
  'the tree store writes node and block rows, media leaves the database, the cold read is the root',
  async () => {
    const root = mkdtempSync(join(tmpdir(), 'demi-conv-'))
    const blobs = new DirBlobStore(join(root, 'blobs'))
    const stores = new ConversationStores(
    ...
    const rows = db.all<{
      idx: number;
      block_json: string
    }>('SELECT idx, block_json FROM blocks WHERE node_id = ? ORDER BY idx', ['conv-1'])
    ...
    if (user?.type !== 'user')
      throw new Error('expected user block')
    ...
    expect(stores.transcriptBlocks('conv-1').map((block) => block.id))
      .toEqual(['u1'])
...
test(
  'the tree store keeps the tree contract: the brief in the create, delivery by the parent save, reopen, cascade',
  async () => {
    const root = mkdtempSync(join(tmpdir(), 'demi-conv-tree-'))
    const stores = new ConversationStores(
      join(root, 'conversations'),
      () => new DirBlobStore(join(root, 'blobs'))
    )
    ...
    await tree.createNode({
      ...
      description: 'first',
      metadata: { round: 1 }
    }, { ...emptyCheckpoint(), queue: [brief] })
    await tree.createNode(
      { ...rootRecord('b', 'c'), spawnedAt: 2 },
      emptyCheckpoint()
    )
    await tree.createNode(rootRecord('a1', 'a'), emptyCheckpoint())
    expect((await tree.children('c')).map((node) => [
      node.id,
      node.description,
      node.metadata
    ])).toEqual([
    ...
    await tree.closeNode('b', {
      phase: 'error',
      closedAt: 4,
      result: null,
      failure: 'boom'
    })
    expect(await tree.node('a')).toMatchObject({
      closedPhase: 'completed',
      result: 'a done',
      delivered: false
    })
    expect(await tree.node('b')).toMatchObject({
      closedPhase: 'error',
      failure: 'boom',
      delivered: false
    })
    ...
    await tree.reopenNode(
      'a',
      { metadata: { round: 2 }, spawnedAt: 9 },
      reviving
    )
    expect(await tree.node('a')).toMatchObject({
      closedPhase: null,
...
test(
  'conversation handles are an LRU: a cold read holds one only until others are touched, data survives eviction',
  async () => {
...
test('expired web sessions are swept when a session opens', async () => {
  ...
  expect(db.get<{ n: number }>('SELECT COUNT(*) AS n FROM web_sessions')?.n)
    .toBe(1)
