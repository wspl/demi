...
// Run the shipped extension with controlled browser APIs, without starting Chrome.
for (const scenario of ['stopped', 'silent', 'aborted', 'disconnected']) {
  test(`the ${scenario} capture leaves replacement to the Host`, async () => {
    const source = await readFile(new URL('../../../../crates/demi-commands/src/browser/capture/offscreen.js', import.meta.url), 'utf8')
