...
test('slots: a /30 each, host .1 and guest .2, taken and given back', () => {
  ...
  expect(slotOf(options, 0)).toEqual({
    index: 0,
    tap: 'demi0',
    guestAddress: '172.16.0.2/30',
    gateway: '172.16.0.1',
    mac: '06:fc:00:00:00:00'
  })
  expect(slotOf(options, 1)).toMatchObject({
    tap: 'demi1',
    guestAddress: '172.16.0.6/30',
    gateway: '172.16.0.5'
  })
  expect(slotOf(options, 64)).toMatchObject({
    guestAddress: '172.16.1.2/30',
    gateway: '172.16.1.1',
    mac: '06:fc:00:00:00:40'
  })
  expect(() => slotOf({ ...options, subnet: '10.0.0.0/30' }, 1))
...
test('the kernel command line the guest init reads', () => {
  ...
  const line = bootArgs({
    backendUrl: 'http://172.16.0.1:3271',
    deviceToken: 'tok',
    slot,
    dns: ['1.1.1.1'],
    firstBoot: true
  ...
  expect(line).toBe(
    'console=ttyS0 reboot=k panic=1 pci=off init=/demi-runner demi.backend=http://172.16.0.1:3271 demi.token=tok demi.ip=172.16.0.22/30 demi.gw=172.16.0.21 demi.dns=1.1.1.1 demi.firstboot=1'
  )
  expect(bootArgs({
    backendUrl: 'http://b',
    deviceToken: 't',
...
test(
  'configuration from the environment: absent, direct, jailer, and the errors',
  () => {
    expect(firecrackerConfigFromEnv({}, '/data')).toBeNull()
    const base = {
      [MANAGED_ENV.firecracker]: '/opt/fc/firecracker',
      [MANAGED_ENV.kernel]: '/opt/fc/vmlinux',
      [MANAGED_ENV.rootfs]: '/opt/fc/rootfs.ext4'
    }
    const direct = firecrackerConfigFromEnv(base, '/data')!
    expect(direct.launch).toEqual({ mode: 'direct' })
    expect(direct).toMatchObject({
      vcpus: DEFAULTS.vcpus,
      memMib: DEFAULTS.memMib,
      homeMib: DEFAULTS.homeMib,
      subnet: DEFAULTS.subnet,
      slots: DEFAULTS.slots,
      dns: DEFAULTS.dns,
      runDir: '/data/firecracker',
      imagesDir: '/data/machines'
    })
    const jailer = firecrackerConfigFromEnv({
      ...base,
      [MANAGED_ENV.launch]: 'jailer',
      [MANAGED_ENV.jailer]: '/opt/fc/jailer',
      [MANAGED_ENV.helper]: '/usr/local/libexec/demi/firecracker-jailer.sh',
      [MANAGED_ENV.uidBase]: '30000',
      [MANAGED_ENV.slots]: '16',
      [MANAGED_ENV.dns]: '9.9.9.9'
    }, '/data')!
    expect(jailer.launch).toEqual({
      mode: 'jailer',
      jailer: '/opt/fc/jailer',
      helper: '/usr/local/libexec/demi/firecracker-jailer.sh',
    ...
    expect(
      () => firecrackerConfigFromEnv(
        { [MANAGED_ENV.firecracker]: '/fc' },
        '/data'
      )
    )
      .toThrow(MANAGED_ENV.kernel)
    expect(
      () => firecrackerConfigFromEnv(
        { ...base, [MANAGED_ENV.launch]: 'jailer' },
        '/data'
      )
    )
      .toThrow(MANAGED_ENV.jailer)
    expect(
      () => firecrackerConfigFromEnv(
        { ...base, [MANAGED_ENV.vcpus]: 'two' },
        '/data'
      )
    )
      .toThrow(MANAGED_ENV.vcpus)
    expect(
      () => firecrackerConfigFromEnv(
        { ...base, [MANAGED_ENV.launch]: 'podman' },
        '/data'
      )
    )
...
async function fixture() {
  ...
  const config = firecrackerConfigFromEnv({
    DEMI_MANAGED_FIRECRACKER: '/missing-firecracker',
    DEMI_MANAGED_KERNEL: kernel,
    DEMI_MANAGED_ROOTFS: rootfs
  }, directory)!
  const tools: ImageTools = {
    makeHomeImage: async (_home, image) => {
      await writeFile(image, 'empty-home')
    },
    makeSystemImage: async image => {
      await writeFile(image, 'empty-system')
    },
    growImage: async () => {},
  }
  ...
  const provisioner = new FirecrackerProvisioner(
    config,
    { tools, processes, store }
  )
  return { config, provisioner, store, alive, killed }
}

test(
  'reconciliation kills orphan VMs before publishing both working disks',
  async () => {
    ...
    expect(await readFile(join(copied, 'system.ext4'), 'utf8'))
      .toBe('installed-package')
    expect(await readFile(join(copied, 'home.ext4'), 'utf8'))
      .toBe('project-files')
...
test(
  'reset replaces only system, pins the base, and replays an operation id without changing generation',
  async () => {
    ...
    expect(await readFile(join(stage, 'system.ext4'), 'utf8'))
      .toBe('empty-system')
    await provisioner.reset('device-a', 'reset-2', base)
    expect(await store.read('device-a')).toEqual(reset)
    expect(
      await readFile(join(config.imagesDir, 'bases', base, 'rootfs'), 'utf8')
    )
      .toBe('root-v1')
  }
)
...
test(
  'a partial disk publication cannot replace the last complete generation',
  async () => {
    const { config, provisioner, store } = await fixture()
    await provisioner.reconcile()
    await provisioner.reset(
      'device-a',
      'initial',
      await provisioner.currentBaseVersion()
    )
    const before = (await store.read('device-a'))!
    ...
    await expect(
      store.publish('device-a', { ...before, generation: 'partial' }, stage)
    ).rejects.toThrow()
...
test(
  'published checkpoints retain only the current and previous complete generations',
  async () => {
    const { config, provisioner, store } = await fixture()
    await provisioner.reconcile()
    await provisioner.reset(
    ...
    expect(
      await stat(join(config.imagesDir, 'device-a', 'generations', state.generation))
        .then(() => true, () => false)
    )
      .toBe(false)
    const previous = join(config.runDir, 'previous-readback')
    await store.copy('device-a', { ...state, generation: 'second' }, previous)
    expect(await readFile(join(previous, 'home.ext4'), 'utf8'))
      .toBe('empty-home')
...
test(
  'failed JSON serialization preserves the published file and removes its temporary file',
  async () => {
