...
/**
 * The provider a user may name — the instance's in shared mode, their own in
 * isolated mode — or null.
 */
export async function visibleProvider(
  vault: Pick<ProviderVault, 'get'>,
  mode: InstanceMode,
  userId: string,
  providerId: string
  ...
  return provider &&
    provider.ownerUserId === providerOwner(mode, userId)
    ? provider
    : null
}

/**
 * Whether a stored provider's owner column agrees with the mode the instance
 * runs in.
 */
export function ownerFitsMode(
  mode: InstanceMode,
