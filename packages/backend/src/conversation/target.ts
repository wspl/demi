...
import type { ControlService, ConversationTargetPointer, ExecutionTarget } from '../storage/control'
import type { ConversationStores } from '../storage/conversation-store'
import { cloudSessionDirectory, resolveExecutionTarget } from './execution-target'
...
export type SwitchTargetResult = { outcome: 'switched' | 'noop' | 'conversation_not_found' | 'workspace_not_found' | 'device_not_found' | 'archived' | 'turn_in_flight' | 'conflict' }
...
export class ConversationTargets {
  ...
  async withHost<T>(id: string, operation: (host: Host) => Promise<T>, signal?: AbortSignal): Promise<T> {
  ...
  async resolve(id: string): Promise<ExecutionTarget> {
    ...
    return resolveExecutionTarget(this.deps.control, this.deps.registry, conversation)
  }
  ...
  async hostFor(id: string): Promise<Host> {
    ...
    if (!device || device.userId !== conversation.userId) throw new Error('Device not owned by this user')
    if (device.kind === 'managed') {
      if (!managedHosts) throw new Error('Cloud is not configured')
      await managedHosts.ensureRunning(device)
    }
    const path = target.kind === 'cloud' ? cloudSessionDirectory(id, registry.deviceIdentity(device.id)?.homeDir) : target.path
    const host = registry.hostFor({ deviceId: device.id, path }, id, stores.hostStore(id))
    if (target.kind === 'cloud') await host.fs.mkdir(path, { recursive: true })
    return host
  }

  async switch(id: string, to: ConversationTargetPointer): Promise<SwitchTargetResult> {
    const { control } = this.deps
    const conversation = await control.getConversation(id)
    if (!conversation) return { outcome: 'conversation_not_found' }
    if (conversation.archived) return { outcome: 'archived' }
    if (JSON.stringify(conversation.target) === JSON.stringify(to)) return { outcome: 'noop' }
    if (to.kind === 'workspace') {
      const workspace = await control.getWorkspace(to.workspaceId)
      if (!workspace || workspace.userId !== conversation.userId) return { outcome: 'workspace_not_found' }
    }
    if (to.kind === 'device') {
      const device = await control.getDevice(to.deviceId)
      if (!device || device.userId !== conversation.userId || device.kind !== 'user') return { outcome: 'device_not_found' }
    }
    ...
    try {
      ...
      const won = await control.switchConversationTarget(id, conversation.target, to, { from, to: destination }, {
