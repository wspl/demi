...
export async function resolveRemoteFileRefs(
  ...
  const references = content.map(block => remoteFileRefSchema.safeParse(block))
  ...
  return content.map(block => {
    ...
    return {
      type: 'text',
      text: `[Remote file: ${JSON.stringify(device.name)}; deviceId=${ref.deviceId}; path=${JSON.stringify(ref.path)}]\nRead this device's current file when needed: ${command}`
    }
  })
}
