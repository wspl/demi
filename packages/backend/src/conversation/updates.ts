...
interface FieldResult {
  ...
  code?: string;
  message?: string;
  httpStatus?: 404 | 409 | 500
}

/**
 * Applies independent fields with explicit outcomes. Archive and target changes
 * share the existing admission gates.
 */
export class ConversationUpdates {
  ...
  async apply(userId: string, id: string, patch: ConversationPatch) {
    ...
    const apply = async (
      field: string,
      operation: () => Promise<void | Omit<FieldResult, 'field'>>
    ) => {
      try {
    ...
    if (patch.archived !== undefined)
      await apply('archived', () => this.archive(id, patch.archived!))
    for (const field of ['title', 'pinned', 'model', 'target'] as const) {
      const present = field === 'model'
      ...
      await apply(field, async () => {
        if (field === 'target') {
          ...
          if (result.outcome === 'switched' || result.outcome === 'noop')
            return
          return refused(
            result.outcome,
            `Target switch refused: ${result.outcome}`,
            result.outcome.endsWith('_not_found') ? 404 : 409
          )
        }
        ...
        try {
          ...
          const providerId = patch.providerId === undefined
            ? current.providerId
            : patch.providerId
          const modelId = patch.modelId === undefined
            ? current.modelId
            : patch.modelId
          if (providerId &&
          ...
          if ((providerId === null) !== (modelId === null))
            return refused(
              'invalid_model',
              'Provider and model must be selected or cleared together'
            )
  ...
  private async archive(
    id: string,
    archived: boolean
  ): Promise<void | Omit<FieldResult, 'field'>> {
