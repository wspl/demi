...
export class ProviderRateLimiter {
  ...
  constructor(
    private readonly requestsPerMinute: number = DEFAULT_PROVIDER_REQUESTS_PER_MINUTE
  ) {}

  /**
   * Records one request; throws a `rate_limited`-coded error when over the
   * ceiling.
   */
  take(userId: string): void {
    ...
    if (window.length >= this.requestsPerMinute) {
      ...
      throw Object.assign(
        new Error(
          `Provider request rate limit reached (${this.requestsPerMinute}/min)`
        ),
        {
          code: 'rate_limited',
        }
