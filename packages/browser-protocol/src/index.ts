...
const tab = z.string().min(1).max(128).describe('Browser tab ID returned by open or tabs')
...
export const browserTargetShape = {
  ref: locator.optional().describe('A node reference returned by inspect or find'),
  ...
  label: locator.optional().describe('Accessible name of a form control'),
  ...
  within: locator.optional().describe('Container node reference'),
}
...
const navigated = z.strictObject({ tab: z.string(), url: z.string(), title: z.string() })
...
export const browserOperations = {
  open: { input: z.strictObject({ url: locator, timeout }), result: info },
  ...
  goto: { input: z.strictObject({ tab, url: locator, timeout }), result: navigated },
  back: { input: z.strictObject({ tab, timeout }), result: navigated },
  forward: { input: z.strictObject({ tab, timeout }), result: navigated },
  reload: { input: z.strictObject({ tab, timeout }), result: navigated },
  ...
  read: { input: z.strictObject({ ...targeted, property: z.enum(['text', 'html', 'value', 'attribute']), attribute: locator.optional(), all: z.boolean().optional() }), result: z.union([z.strictObject({ value: z.unknown() }), z.strictObject({ values: z.array(z.unknown()), truncated: z.boolean() })]) },
  ...
  wait: { input: z.strictObject({ ...targeted, url: locator.optional(), state: z.enum(['visible', 'hidden', 'attached', 'detached', 'enabled']).optional() }), result: z.strictObject({ matched: z.boolean(), url: z.string(), ref: z.string().optional() }) },
