...
# the series, with extra.config merged in. Usage: kernel/build.sh
# <aarch64|x86_64>
set -euo pipefail
arch="${1:?arch}"
...
cd "linux-$version"
curl -fsSL \
  "https://raw.githubusercontent.com/firecracker-microvm/firecracker/$fc_tag/resources/guest_configs/microvm-kernel-ci-$arch-6.1.config" \
  -o .config
