...
test(
  'Anthropic API provider resolves endpoint and API key from env vars',
  async () => {
    await withEnv(
      {
      ...
      async () => {
        ...
        expect(requests[0]?.url)
          .toBe('https://anthropic-gateway.example/v1/messages')
...
test(
  'a reused Anthropic runtime uses the output limit of each request after a model switch',
  async () => {
    const requests: CapturedRequest[] = []
    const provider = createAnthropicApiProvider({
      apiKey: () => 'test-key',
...
test(
  'Anthropic API provider explicit baseUrl and apiKey take precedence over env vars',
  async () => {
    await withEnv(
      {
      ...
      async () => {
        ...
        const runtime = await providerRuntime(
          provider,
          selection('anthropic-gateway', 'claude-gateway')
        )

        await collect(runtime.run(request({ modelId: 'claude-gateway' })))

        expect(requests[0]?.url)
          .toBe('https://explicit-gateway.example/anthropic/v1/messages')
...
test(
  'Anthropic API model catalog mirrors Claude Code defaults and explicit models replace it',
  async () => {
...
test(
  'Anthropic API request body groups user/tool_result and assistant/tool_use turns',
  () => {
...
test(
  'Anthropic API stream maps thinking, text, tool use, and usage',
  async () => {
...
async function* eventsFromData(values: Array<{
  event: string;
...
test(
  'effort and adaptive thinking configs map onto budget tokens instead of being dropped',
  () => {
    const base = request({
    ...
    const effort = buildAnthropicMessagesBody(
      { ...base, thinking: { type: 'effort', effort: 'high', summary: null } },
      undefined
    )
    expect(effort.thinking).toEqual({
      type: 'enabled',
      budget_tokens: 32_000 - 1_024
    })

    const adaptive = buildAnthropicMessagesBody(
      { ...base, thinking: { type: 'adaptive', effort: 'low' } },
      undefined
    )
    ...
    const disabled = buildAnthropicMessagesBody(
...
test(
  'thinking budgets clamp below max_tokens and max_tokens defaults are agent-sized',
  () => {
    const base = request({
      items: [{ type: 'user_message', content: [{ type: 'text', text: 'hi' }] }]
    })
    ...
    expect(clamped.thinking).toEqual({
      type: 'enabled',
      budget_tokens: 8_192 - 1_024
    })
